...
function showdetails(approval_id){
	...
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalagendaso/showDetails']); ?>',
		type   : 'POST',
...
function confirm(id,type){
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalagendaso/confirm']); ?>',
		type   : 'POST',
		data   : {approval_id:id},
		success: function (data) {
			if(data){
				if(type == 'approve'){
					...
					$(".modals-place-2").load('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalagendaso/rejectReason','id'=>'']); ?>'+id, function() {
				...
				openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalagendaso/notAllowed','id'=>'']); ?>'+id,'modal-global-info');
			}
		},
		error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
	});
...
function approve(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalagendaso/approveConfirm','id'=>'']); ?>'+id,'modal-global-confirm');
}
</script>