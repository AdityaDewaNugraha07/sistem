...
function showdetails(approval_id){
	...
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalproforma/showDetails']); ?>',
		type   : 'POST',
...
function confirm(id,type){
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalproforma/confirm']); ?>',
		type   : 'POST',
		data   : {approval_id:id},
		success: function (data) {
			if(data){
				...
				openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalproforma/notAllowed','id'=>'']); ?>'+id,'modal-global-info');
			}
		},
		error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
	});
...
function approve(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalproforma/approveConfirm','id'=>'']); ?>'+id,'modal-global-confirm');
}
function reject(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalproforma/rejectConfirm','id'=>'']); ?>'+id,'modal-global-confirm');
}
</script>