...
function showdetails(approval_id){
	...
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalhasilso/showDetails']); ?>',
		type   : 'POST',
...
function confirm(id,type){
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalhasilso/confirm']); ?>',
		type   : 'POST',
		data   : {approval_id:id},
		success: function (data) {
			if(data){
				if(type == 'approve'){
					...
					$(".modals-place-2").load('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalhasilso/rejectReason','id'=>'']); ?>'+id, function() {
				...
				openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalhasilso/notAllowed','id'=>'']); ?>'+id,'modal-global-info');
			}
		},
		error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
	});
...
function approve(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalhasilso/approveConfirm','id'=>'']); ?>'+id,'modal-global-confirm');
}
function setHasilConfirm(){
	var stockopname_agenda_id = "<?= $modReff->stockopname_agenda_id ?>";
    var jenis_produk = <?= json_encode(json_decode($modReff->jenis_produk)); ?>;
    $("#hasil-confirm").html("");
    $("#hasil-confirm").addClass("animation-loading");
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/gudang/stockopname/setHasil']); ?>',
        type   : 'POST',
        data   : {stockopname_agenda_id:stockopname_agenda_id,jenis_produk:jenis_produk,summaryonly:true},
        success: function (data) {
			if(data.hasil){
