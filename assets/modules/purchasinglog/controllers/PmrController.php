                                ...
                                if(is_numeric($i)){
                                    foreach($details as $ii => $detail){
                                        if((is_array($detail))&&($ii!='total')){
                                            ...
                                            $modDetail->panjang = ($model->jenis_log=="LS")?$ii:"0";
...
//				exit;
        ...
        if(\Yii::$app->request->isAjax){
            ...
            if($jenis_log=="LA"){
                if($tujuan=="INDUSTRI"){
                    ...
                    $data['approver_2_html'] = \yii\bootstrap\Html::tag('option', \app\models\MPegawai::findOne($data['approver_2'])->pegawai_nama,['value'=>$data['approver_2']]);
                    $data['approver_2_label'] = "Approver 2<br><span style='font-size:1.1rem;'><b>Dirut</b></span>";
                    $data['approver_3'] = \app\components\Params::DEFAULT_PEGAWAI_ID_JENNY_CHANDRA;
                    $data['approver_3_html'] = \yii\bootstrap\Html::tag('option', \app\models\MPegawai::findOne($data['approver_3'])->pegawai_nama,['value'=>$data['approver_3']]);
                    $data['approver_3_label'] = "Approver 3<br><span style='font-size:1.1rem;'><b>Owner</b></span>";
                }
		...
		if(\Yii::$app->request->isAjax){
            ...
            if(!empty($pmr_id)){
                $modDetail = \app\models\TPmrDetail::find()
								...
								->where(['pmr_id'=>$pmr_id])->all();
            }
            $data['html'] = '';
            if(count($modDetail)>0){
                foreach($modDetail as $i => $detail){
                    $data['html'] .= $this->renderPartial('_item',['model'=>$detail,'i'=>$i,'edit'=>$edit,'jenis_log'=>$jenis_log]);
                }
            }
            return $this->asJson($data);
        }
    ...
    public function actionSetDetail(){
        if(\Yii::$app->request->isAjax){
            $jenis_log = Yii::$app->request->post('jenis_log');
            $data['html'] = "";
            $data['html'] .= $this->renderPartial('_table',['jenis_log'=>$jenis_log]);
            return $this->asJson($data);
        }
    }
