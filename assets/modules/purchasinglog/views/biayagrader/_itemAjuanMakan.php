<tr style="">
    <td style="vertical-align: middle; text-align: center; padding: 2px;" class="td-kecil">
		...
		<?= yii\bootstrap\Html::activeHiddenInput($model, '[ii]ajuanmakan_grader_id',['style'=>'width:50px;']); ?>
		<?= $model->kode ?>
    </td>
    <td style="vertical-align: middle; text-align: center; padding: 2px;" class="td-kecil">
		<?= \app\components\DeltaFormatter::formatDateTimeForUser2($model->periode_awal)." sd ".\app\components\DeltaFormatter::formatDateTimeForUser2($model->periode_akhir) ?>
    </td>
	<td style="vertical-align: middle; text-align: center; padding: 2px;" class="td-kecil">
	...
	<td class="text-align-center" style="padding: 2px;">
		<a class="btn btn-xs btn-outline blue-hoki" id="btn-info" onclick="detailAjuanMakan(<?= $model->ajuanmakan_grader_id ?>);"><i class="fa fa-info-circle"></i></a>
		<?php if(empty($model->voucher_pengeluaran_id)){ ?>
			<a class="btn btn-xs red" id="btn-delete" onclick="deleteAjuanMakan(<?= $model->ajuanmakan_grader_id ?>);" style="margin-left: -7px"><i class="fa fa-trash-o"></i></a>
		<?php } ?>
