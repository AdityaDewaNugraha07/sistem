	...
	<table class="table tracking table-striped table-bordered table-hover">
		<thead>
			<tr style="background-color: #F1F4F7; ">
				...
				<th style="text-align: center;"><?= Yii::t('app', 'Destination'); ?></th>
		...
		<tbody>
			...
			if( Yii::$app->user->identity->user_group_id == app\components\Params::USER_GROUP_ID_SUPER_USER ||
				...
				if(\app\models\MapTrackingpembelianChecklist::checkTrack($model->bpb_kode)){
			...
			<tr style="<?= $highlight; ?>">
				...
				<td><?= $model->bpb_kode; ?></td>
				<td style="text-align: center;"><?= app\components\DeltaFormatter::formatDateTimeForUser2($model->bpb_tgl_keluar) ?></td>
				<td style="text-align: center;"><?= !empty($model->departement_id)?$model->departement->departement_nama:" - "; ?></td>
				<td style="text-align: center;"><?= $model->bpb_status; ?></td>
				<td style="text-align: center;">
					<a class="btn btn-xs blue-hoki btn-outline" href="javascript:void(0)" onclick="infoBPB(<?= $model->bpb_id ?>)">
