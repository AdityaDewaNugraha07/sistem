...
class PobhpController extends DeltaBaseController
{
	...
	public function actionIndex(){
		...
		$modDetailSpp = \app\models\TSppDetail::find()->all();
        ...
        if( Yii::$app->request->post('TSpo')){
            ...
            try {
                ...
                $success_4 = true; // insert map_spp_detail_reff
                $success_5 = true; // update map_penawaran_bhp
				$assigned = [];
				$spo_total_all = 0;
					...
					foreach($_POST['TSpoDetail'] as $i => $detail){
						...
						if(!empty($assigned[$peg_id])){
							if($_POST['TSpo']['spo_is_pkp']==1){
								$assigned[$peg_id]['spo_ppn_nominal'] += $detail['subtotal'] * 0.1;
								$assigned[$peg_id]['spo_total'] += $detail['subtotal'] + ($detail['subtotal'] * 0.1);
							}else{
								...
								if( (isset($_POST['TSpoDetail'])) && (count($_POST['TSpoDetail'])>0) ){
									foreach($_POST['TSpoDetail'] as $i => $detail){
										if($peg == $this->getAssigned($detail['spod_harga'])['pegawai_id']){
											...
											$sql3 = "SELECT * FROM t_spp_detail WHERE suplier_id = ".$model->suplier_id." AND bhp_id = ".$detail['bhp_id']."  ";
...
//															print_r($mapPenawaran->validate());
		...
		return $this->render('index',['model'=>$model,'modDetail'=>$modDetail,'modDetailSpp'=>$modDetailSpp]);
	}
	...
	public function actionDaftarSpo(){
        if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='table-spo'){
				...
				$param['column'] = [$param['table'].'.spo_id','spo_kode',['col_name'=>'spo_tanggal','formatter'=>'formatDateForUser2'],'suplier_nm','spo_total','approve_status',$param['table'].'.terima_bhp_id','terimabhp_kode',$param['table'].'.cancel_transaksi_id','assigned.pegawai_nama','assigned.pegawai_nama','m_default_value.name_en as mata_uang'];
            ...
            if(!empty($suplier_id)){
				...
				if(count($mods)>0){
					foreach($mods as $i => $detail){
						...
						$sql2 = "SELECT bhp_id, sum(spod_qty), string_agg(spod_keterangan, ', ') AS keterangan FROM t_spo_detail 
								JOIN t_spo ON t_spo.spo_id = t_spo_detail.spo_id 
								WHERE t_spo.suplier_id = ".$suplier_id." AND t_spo.cancel_transaksi_id IS NULL AND bhp_id = ".$detail['bhp_id']." 
								GROUP BY bhp_id 
								ORDER BY bhp_id";
						$mods2 = \Yii::$app->db->createCommand($sql2)->queryOne();
						if(count($mods2)>0){
							$total_spp = $detail['sum'];
							$total_spo = $mods2['sum'];							
							if($total_spo < $total_spp){
								$mods3 = \Yii::$app->db->createCommand("SELECT SUM(sppd_qty) AS spp_closed FROM t_spp_detail WHERE t_spp_detail.suplier_id = ".$suplier_id." AND bhp_id = ".$detail['bhp_id']." AND status_closed IS NOT NULL")->queryOne();
								$findsppd_id = \Yii::$app->db->createCommand("SELECT sppd_id AS findsppd_id FROM t_spp_detail WHERE t_spp_detail.suplier_id = ".$suplier_id." AND bhp_id = ".$detail['bhp_id']." AND status_closed IS NOT NULL")->queryAll();
								$qty_closed = 0; 
								$spo_closed = 0;
								if( !empty($mods3['spp_closed']) ){
//									$total_spo = $mods3['spp_closed']; 
									$spdd_id_closed = [];
									foreach($findsppd_id as $isppd_id => $sppd_id){
										$spdd_id_closed[] = $sppd_id['findsppd_id'];
									}
									if(count($spdd_id_closed)>0){
										$spdd_id_closed = implode(",", $spdd_id_closed);
										$spo_closed = \Yii::$app->db->createCommand("SELECT SUM(spod_qty) FROM map_spp_detail_reff JOIN t_spo_detail ON map_spp_detail_reff.reff_detail_id = t_spo_detail.spod_id WHERE map_spp_detail_reff.sppd_id IN (".$spdd_id_closed.")")->queryOne();
										$spo_closed = !empty($spo_closed) ? $spo_closed['sum']:0;
										$spo_closed = ($spo_closed > $mods3['spp_closed']) ? $mods3['spp_closed'] : $spo_closed;
									}
									$qty_closed = $mods3['spp_closed'] - $spo_closed;
								}
								$qty = ($total_spp-$qty_closed)-$total_spo;
							}
						}else{
							$qty = $total_spp;
						}
						if($qty != 0){
							$sppterkait = \app\models\TSpoDetail::tarikSppToSpo($suplier_id,$detail['bhp_id']);
							$ket = "";
							$sppd_id = "";
							if(!empty($sppterkait['ket'])){
								$ket = implode(', ', $sppterkait['ket']);
								$detail['keterangan'] = $ket;
							}
							if(!empty($sppterkait['sppd_id'])){
								$sppd_id = implode(', ', $sppterkait['sppd_id']);
								$modSppDetail = \app\models\TSppDetail::find()
												->where(['bhp_id'=>$detail['bhp_id'],'suplier_id'=>$suplier_id])
												->andWhere("sppd_id in (".$sppd_id.") AND status_closed IS NULL")
												->one();
								if(count($modSppDetail)>0){
									$data['sppdetail'] = $modSppDetail->attributes;
									$data['spp'] = $modSppDetail->spp->attributes;
									if(!empty($modSppDetail->sppd_id)){
										...
										if(count($mapPenawaran)>0){
											foreach($mapPenawaran as $itwr => $penawaran){
							...
							$data['detail'] = $detail;
							$data['html'] .= $this->createItemDetail($modSpoDetail,$detail,$qty,$data['spp'],$data['sppdetail']);
						}
					}
				}
				$data['supplier'] = $modSupplier->attributes;
            }
			if(count($data['penawaran'] )>0){
	...
	public function createItemDetail($modSpoDetail,$detail,$qty,$modSpp=null,$modSppDetail=null){
		...
		$modSpoDetail->spod_qty = $qty;
		$modSpoDetail->sppd_qty = $qty;
		$modSpoDetail->spod_keterangan = $detail['keterangan'];
