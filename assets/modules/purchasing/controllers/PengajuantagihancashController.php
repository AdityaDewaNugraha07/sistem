	...
	public function actionPickPanelTBP(){
        if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='table-dt'){
				...
				$param['where'] = $param['table'].".cancel_transaksi_id IS NULL AND spl_id IS NOT NULL AND terima_bhp_id NOT IN ( SELECT terima_bhp_id FROM t_pengajuan_tagihan WHERE cancel_transaksi_id IS NULL AND status != 'DITOLAK') AND EXTRACT(year FROM tglterima) >= '2019'";
