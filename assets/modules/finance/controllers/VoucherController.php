...
//				exit;
	...
	public function actionEditItem(){
        if(\Yii::$app->request->isAjax){
            ...
            $modDetails = \app\models\TVoucherPengeluarandetail::find()->where(['voucher_pengeluaran_id'=>$voucher_pengeluaran_id])->all();
			$data['item'] = '';
			if(count($modDetails)>0){
				foreach($modDetails as $i => $detail){
					$detail->jumlah = \app\components\DeltaFormatter::formatNumberForUserFloat($detail->jumlah);
	...
	public function actionSetDropdownSupplier(){
        if(\Yii::$app->request->isAjax){
			...
			case "Top-up Kas Kecil":
				$sql = "SELECT * FROM t_ppk where voucher_pengeluaran_id IS NULL AND tipe = 'Kas Kecil'";
				$mod = Yii::$app->db->createCommand($sql)->queryAll();
			...
			case "Open Voucher":
				$sql = "SELECT *,departement_nama FROM t_open_voucher 
						...
						WHERE cancel_transaksi_id IS NULL AND cara_bayar = 'Transfer Bank' AND voucher_pengeluaran_id IS NULL AND status_approve = 'APPROVED'";
				$mod = Yii::$app->db->createCommand($sql)->queryAll();
			...
			switch ($type){
				case "Pembelian BHP":
					if(!empty($supplier_id)){
						...
						if(count($modTerima)>0){
							$data['htmlterima'] = $this->renderPartial('_detailTerima',['modDetail'=>$modTerima,'voucher_pengeluaran_id'=>$voucher_pengeluaran_id]);
						}
						if(count($modDps)>0){
							$data['htmldp'] = $this->renderPartial('_detailDp',['modDps'=>$modDps,'voucher_pengeluaran_id'=>$voucher_pengeluaran_id,'pakaidp'=>true]);
						}
						if(count($modSPO)>0){
				...
				case "Pembayaran DP BHP":
					if(!empty($supplier_id)){
						...
						$data['htmldp'] = $this->renderPartial('_detailDp',['modDps'=>$modDps,'voucher_pengeluaran_id'=>$voucher_pengeluaran_id]);
					}
					break;
				case "Top-up Kas Kecil":
					if(!empty($ppk_id)){
					...
					if(!empty($gkk_id)){
						...
						if(!empty($modGkk)){
							...
							if(!empty($modGkk->tbp_reff)){
								...
								if(count($modTBPs)>0){
									$data['htmlterima'] = $this->renderPartial('_detailTerima',['modDetail'=>$modTBPs,'voucher_pengeluaran_id'=>$voucher_pengeluaran_id]);
								}
							}
				...
				case "Open Voucher":
					if(!empty($open_voucher_id)){
						...
						if(!empty($modOpenVoucher)){
							...
							$data['htmlovk'] = $this->renderPartial('_detailOvk',['model'=>$modOpenVoucher,'modOpenVoucherDetail'=>$modOpenVoucherDetail]);
						}
                        if(!empty($voucher_pengeluaran_id)){
							$data['voucher'] = \app\models\TVoucherPengeluaran::findOne($voucher_pengeluaran_id)->attributes;
                        }
	...
	public function actionGetDetailTerima(){
		if(\Yii::$app->request->isAjax){
			...
			foreach($modDetail as $i => $detail){
				if($detail->ppn_peritem != 0){
					$ppn_item += $detail->ppn_peritem;
				}
				if($detail->pph_peritem != 0){
					$pph_item += $detail->pph_peritem;
				}
                ...
                if(!empty($modRetur)){
                    $hargaRetur += $modRetur->total_kembali;
                    $html .= "<tr style=''>
			...
			$html .= "<tr style=''>
							...
							<td style='font-size: 1.2rem; text-align:right;' ><b>".
								\app\components\DeltaFormatter::formatNumberForUser($pph_item)
							."</b></td>
						</tr>";
            if($total_pbbkb>0){
                $html .= "<tr style=''>
            ...
            $html .= "<tr style=''>
							...
							<td style='font-size: 1.2rem; text-align:right;' ><b>".\app\components\DeltaFormatter::formatNumberForUser($total + $ppn_item - $hargaRetur - $pph_item + $total_pbbkb)."</b></td>
						</tr>";
			$html .= "</table>";
			$data['html'] = $html;
			return $this->asJson($data);
	...
	public function actionDaftarAfterSave(){
        if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='modal-aftersave'){
				...
				$param['column'] = [$param['table'].'.voucher_pengeluaran_id',
									$param['table'].'.tipe',
									$param['table'].'.kode',
								   ['col_name'=>$param['table'].'.tanggal_bayar','formatter'=>'formatDateForUser2'],
									'm_suplier.suplier_nm',
								   ['col_name'=>'total_nominal','formatter'=>'formatNumberForUser'],
				...
				$param['join']= ['LEFT JOIN m_suplier ON m_suplier.suplier_id = '.$param['table'].'.suplier_id 
...
//					exit;
	...
	public function actionDetailBbk(){
		...
		if(\Yii::$app->request->isAjax){
			...
			$modDetail = \app\models\TVoucherPengeluarandetail::find()->where(['voucher_pengeluaran_id'=>$_GET['id']])->all();
			$paramprint['judul'] = Yii::t('app', 'BUKTI BANK KELUAR');
			return $this->renderAjax('detailBbk',['model'=>$model,'modDetail'=>$modDetail,'paramprint'=>$paramprint]);
        }
	}
	...
	public function actionPrintBbk(){
		$this->layout = '@views/layouts/metronic/print';
		$model = \app\models\TVoucherPengeluaran::findOne($_GET['id']);
//		$modJurnals2 = \app\models\TAcctJurnal::find()->where(['reff_no'=>$model->kode])->andWhere('debet != 0')->one();
//		$model->akun_debit = $modJurnals2->acct_id;
		$modDetail = \app\models\TVoucherPengeluarandetail::find()->where(['voucher_pengeluaran_id'=>$_GET['id']])->all();
        ...
        if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='table-open-voucher'){
				...
				$param['column'] = ['t_open_voucher.open_voucher_id',
                                    ...
                                    'approval1.status AS approval1_status',
                                    'approval1.updated_at AS approval1_at',
                                    'pegawai2.pegawai_nama AS approver_2',
                                    'approval2.status AS approval2_status',
                                    'approval2.updated_at AS approval2_at',
                                    't_open_voucher.keterangan',
                                    't_open_voucher.status_bayar', // 16
				...
				$param['join']= ['JOIN m_departement ON m_departement.departement_id = t_open_voucher.departement_id 
                                  ...
                                  JOIN t_approval AS approval1 ON approval1.reff_no = t_open_voucher.kode AND approval1.assigned_to = t_open_voucher.approver_1
                                  LEFT JOIN t_approval AS approval2 ON approval2.reff_no = t_open_voucher.kode AND approval2.assigned_to = t_open_voucher.approver_2
