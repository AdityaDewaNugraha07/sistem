		...
		<tbody>
			...
			if(count($modDetail)){
				...
				foreach($modDetail as $i => $detail){
					$sql = "SELECT * FROM t_terima_bhp_detail JOIN m_brg_bhp ON m_brg_bhp.bhp_id = t_terima_bhp_detail.bhp_id WHERE terima_bhp_id = ".$detail->terima_bhp_id;
					...
					if(count($detailterimas)>0){
						foreach($detailterimas as $ii => $detailterima){
                            ...
                            $totalterimadetail += $detailterima['terimabhpd_harga'] * $detailterima['terimabhpd_qty'] - $totalretur;
                            $totalpphdetail += $detailterima['pph_peritem'];
						}
					}
				...
				?>
					<tr>
						...
						<td style="font-size: 1.2rem; padding: 5px; text-align: right;">
							<?= \yii\bootstrap\Html::hiddenInput('TTerimaBhp[ii][pph]',$totalpphdetail,[]); ?>
							<span class="pull-left"><?= $matauang ?></span>
							<span class="pull-right"><?= \app\components\DeltaFormatter::formatNumberForUser($totalpphdetail); ?></span>
						</td>
