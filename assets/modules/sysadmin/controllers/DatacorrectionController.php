...
class DatacorrectionController extends DeltaBaseController
{
	...
	public function actionIndex(){
		...
		if(isset($_GET['pengajuan_manipulasi_id'])){
            $model = \app\models\TPengajuanManipulasi::findOne($_GET['pengajuan_manipulasi_id']);
            $model->tanggal = \app\components\DeltaFormatter::formatDateTimeForUser2($model->tanggal);
            $modPegawai = \app\models\MPegawai::findOne( \app\models\MUser::findOne($model->created_by)->pegawai_id );
		...
		if( Yii::$app->request->post('TPengajuanManipulasi') ){
            ...
            try {
                ...
                $modNota = \app\models\TNotaPenjualan::findOne(['kode'=>$model->reff_no]);
                if($model->tipe == "KOREKSI HARGA JUAL"){
                    $modOp = \app\models\TOpKo::findOne($modNota->op_ko_id);
                    ...
                    foreach($_POST['TNotaPenjualanDetail'] as $ii => $posDet){
                        $modNotaDet = \app\models\TNotaPenjualanDetail::findOne($posDet['nota_penjualan_detail_id']);
                        $opdet = \app\models\TOpKoDetail::findOne(['op_ko_id'=>$modOp->op_ko_id,'produk_id'=>$modNotaDet->produk_id]);
                    ...
                    $model->datadetail1 = \yii\helpers\Json::encode($datadetail1);
                    ...
                    $modTempo = \app\models\TTempobayarKo::findOne($modNota->op_ko_id);
                    $modPiutang = new \app\models\TPiutangPenjualan();
                    $modPiutang->load(\Yii::$app->request->post());
                    $modPiutang->kode = \app\components\DeltaGenerator::kodePPC();
					...
					$modPiutang->custtop_top = !empty($modTempo)?$modTempo:0;
                    ...
                    if(!empty($_POST['TPiutangAlertDetail'])){
                        foreach($_POST['TPiutangAlertDetail'] as $i => $detail){
                            $modAlertDetail = \app\models\TPiutangAlertDetail::findOne($detail['piutang_alert_detail_id']);
                            $modAlert = \app\models\TPiutangAlert::findOne($modAlertDetail->piutang_alert_id);
...
//                exit;
    ...
    public function actionGetReff(){
        if(\Yii::$app->request->isAjax){
            ...
            if(!empty($pengajuan_manipulasi_id)){
                $model = \app\models\TPengajuanManipulasi::findOne($pengajuan_manipulasi_id);
            }else{
                $model = new \app\models\TPengajuanManipulasi();
            }
            $data['html'] = '';
    ...
    public function actionOpenlogjasa(){
		if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='modal-open-logjasa'){
				$param['table']= \app\models\TPiutangAlert::tableName();
				$param['pk']= $param['table'].".". \app\models\TPiutangAlert::primaryKey()[0];
    ...
    function actionPickNota(){
		if(\Yii::$app->request->isAjax){
            ...
            $data['html'] = ""; $data['pmr_id']=""; $modReff = "";
            if(!empty($tipe)){
                if($tipe == "KOREKSI HARGA JUAL" || $tipe == "KOREKSI NOPOL MOBIL" || $tipe == "POTONGAN PIUTANG"){
                    if(!empty($kode)){
                        $modNota = \app\models\TNotaPenjualan::findOne(['kode'=>$kode]);
                        $modCust = \app\models\MCustomer::findOne(['cust_id'=>$modNota->cust_id]);
                        $data['cust'] = $modCust->attributes;
                    }
                }
                if($tipe == "KOREKSI HARGA JUAL"){
                    $data['approver1'] = \app\components\Params::DEFAULT_PEGAWAI_ID_IWAN_SULISTYO;
                    $data['approver2'] = \app\components\Params::DEFAULT_PEGAWAI_ID_EKO_NOWO;
                    $data['approver3'] = \app\components\Params::DEFAULT_PEGAWAI_ID_AGUS_SOEWITO;
                    $data['approver1_display'] = \app\models\MPegawai::findOne( \app\components\Params::DEFAULT_PEGAWAI_ID_IWAN_SULISTYO )->pegawai_nama;
                    $data['approver2_display'] = \app\models\MPegawai::findOne( \app\components\Params::DEFAULT_PEGAWAI_ID_EKO_NOWO )->pegawai_nama;
                    $data['approver3_display'] = \app\models\MPegawai::findOne( \app\components\Params::DEFAULT_PEGAWAI_ID_AGUS_SOEWITO )->pegawai_nama;
                }
                if($tipe == "KOREKSI NOPOL MOBIL"){
                    if(!empty($model->datadetail1)){
                        $datadetail1 = \yii\helpers\Json::decode($model->datadetail1);
                    }
                    $data['approver1'] = \app\components\Params::DEFAULT_PEGAWAI_ID_IWAN_SULISTYO;
                    $data['approver1_display'] = \app\models\MPegawai::findOne( \app\components\Params::DEFAULT_PEGAWAI_ID_IWAN_SULISTYO )->pegawai_nama;
                ...
                if($tipe == "POTONGAN PIUTANG"){
                    ...
                    $modReff->tagihan = !empty($datadetail1)? number_format( $datadetail1['new']['t_piutang_penjualan']['tagihan'] ) : number_format($modNota->total_bayar - $terbayar);
                ...
                if($tipe == "KOREKSI PIUTANG LOG & JASA"){
                    ...
                    $data['approver2_display'] = \app\models\MPegawai::findOne( \app\components\Params::DEFAULT_PEGAWAI_ID_EKO_NOWO )->pegawai_nama;
                    $data['approver3_display'] = \app\models\MPegawai::findOne( \app\components\Params::DEFAULT_PEGAWAI_ID_AGUS_SOEWITO )->pegawai_nama;
                    $modReff = \app\models\TPiutangAlert::findOne(['piutang_nomor_nota'=>$kode]);
                }
            }
    ...
    function actionGetItemNotaPenjualan(){
        if(\Yii::$app->request->isAjax){
            ...
            $model = \app\models\TPengajuanManipulasi::findOne($id);   
            $modNota = \app\models\TNotaPenjualan::findOne(['kode'=>$model->reff_no]);
    ...
    public function actionDaftarAfterSave(){
		if(\Yii::$app->request->isAjax){
			...
			if(\Yii::$app->request->get('dt')=='modal-aftersave'){
				$param['table']= \app\models\TPengajuanManipulasi::tableName();
				$param['pk']= $param['table'].".".\app\models\TPengajuanManipulasi::primaryKey()[0];
