...
class PegawaiController extends DeltaBaseController
{
	...
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			...
			$param['column'] = ['pegawai_id','pegawai_nama','pegawai_jk','departement_nama','jabatan_nama',$param['table'].'.active'];
	...
	public function actionPrintout(){
		...
		$pegawai_id = Yii::$app->request->get('id');
		$model = \app\models\MPegawai::find()->where('pegawai_id IN('.$pegawai_id.')')
											 ->select("*")
											 ->join("JOIN", "m_departement", "m_departement.departement_id = m_pegawai.departement_id")
											 ->join("LEFT JOIN", "m_jabatan", "m_jabatan.jabatan_id = m_pegawai.jabatan_id")
											 ->all();
		$paramprint['judul'] = Yii::t('app', 'Master Pegawai');
		if($caraprint == 'PRINT'){
