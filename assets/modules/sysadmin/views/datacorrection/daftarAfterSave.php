...
function dtAfterSave(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/sysadmin/datacorrection/daftarAfterSave') ?>',data:{dt: 'modal-aftersave'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
...
function lihatDetail(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/sysadmin/datacorrection/index','pengajuan_manipulasi_id'=>'']); ?>'+id);
}
function edit(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/sysadmin/datacorrection/index','pengajuan_manipulasi_id'=>'']); ?>'+id+'&edit=1');
}
