...
function dtMaster(){
    var dt_table =  $('#table-master').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/sysadmin/pegawai/index') ?>',data:{dt: 'table-master'} },
        order: [
            [0, 'DESC']
        ],
        columnDefs: [
...
function printout(caraprint){
	window.open("<?= yii\helpers\Url::toRoute('/sysadmin/pegawai/printout') ?>?id="+$('#showing_id').val()+"&caraprint="+caraprint,"",'location=_new, width=1200px, scrollbars=yes');
}
</script>