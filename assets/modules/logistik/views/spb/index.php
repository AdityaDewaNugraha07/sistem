        ...
        <div class="portlet light bordered">
            <div class="portlet-body">
                ...
                <div class="row" style="margin-bottom: 10px; display: <?= (count($spb_exist)>0)?'block':'none'; ?>">
                    <div class="col-md-12">
                        <a class="btn blue btn-sm btn-outline pull-right" onclick="daftarSpb(<?= Yii::$app->user->identity->pegawai->departement_id ?>)"><i class="fa fa-list"></i> <?= Yii::t('app', 'SPB Yang Telah Dibuat'); ?></a>
                            ...
                            <div class="portlet-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <?= $form->field($model, 'spb_diminta')->dropDownList(\app\models\MPegawai::getOptionListByDept( !empty($model->spb_diminta)?$model->spbDiminta->departement_id:null ),['class'=>'form-control select2','prompt'=>'']); ?>
                                        ...
                                        <?php
										if(isset($_GET['spb_id'])){
											echo $form->field($model, 'spb_disetujui')->dropDownList(\app\models\MPegawai::getOptionList(),['class'=>'form-control select2','prompt'=>'','disabled'=>'disabled']);
										}else{
											echo $form->field($model, 'spb_disetujui')->dropDownList(\app\models\MPegawai::getOptionMenyetujuiSPB( null ),['class'=>'form-control select2','prompt'=>'']);
										}
										if(isset($_GET['spb_id'])){
											echo $form->field($model, 'spb_mengetahui')->dropDownList(\app\models\MPegawai::getOptionList(),['class'=>'form-control select2','prompt'=>'','disabled'=>'disabled']);
										}else{
											echo $form->field($model, 'spb_mengetahui')->dropDownList(\app\models\MPegawai::getOptionListAtasan(),['class'=>'form-control select2','prompt'=>'']);
										}
										?>
										<?php if(isset($_GET['spb_id'])){ ?>
										<div class="form-group">
...
function getAllItem(spb_id,editable=false){
    $.ajax({
		...
		type   : 'POST',
		data   : {spb_id:spb_id,editable:editable},
		success: function (data) {
			if(editable){
                $('#table-detail tbody').html(data.html);
                reordertable('#table-detail');
                $('#table-detail tbody tr').each(function(i){
					var initialValue = { id: data.value[i].bhp_id, text: data.value[i].bhp_nama };
					$(this).find('select[name*="[bhp_id]"]').select2({
						allowClear: !0,
						placeholder: 'Ketik nama item',
						width: null,
