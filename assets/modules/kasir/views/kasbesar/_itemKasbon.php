...
<tr style="">
    ...
    <td style="vertical-align: middle;" class="td-kecil">
		<div class="input-group input-sm date date-picker bs-datetime" style="padding:3px;">
            <?= \yii\bootstrap\Html::activeTextInput($model, '[ii]tanggal',['class'=>'form-control','style'=>'width:75px; font-size:1.2rem; padding:3px;','readonly'=>'readonly','placeholder'=>'Tgl Kas Bon']); ?>
	...
	<td style="vertical-align: middle; text-align: center;" class="td-kecil">
		<?= \yii\bootstrap\Html::activeCheckbox($model, "[ii]is_kasbonkaskecil",['class'=>'','label'=>'']) ?>
    </td>
	<td style="vertical-align: middle; text-align: center;" class="td-kecil">
		<?php
		$ret = "";
			...
			if(!empty($model->bkk_id)){
				...
				if(!empty($model->bkk->voucher_pengeluaran_id)){
					...
					if($mod->status_bayar == "PAID"){
						echo "<br>";
						$ret = '<a class="btn btn-sm green-seagreen btn-outline" style="font-size:1rem; padding: 3px;" onclick="terimauangganti('.$model->kas_bon_id.')"><i class="fa fa-download"></i> Terima Uang</a>';
					}
				}
			}else{
				$ret = '<a class="btn btn-sm blue btn-outline" target="BLANK" style="font-size:1rem; padding: 3px;" href="'. \yii\helpers\Url::toRoute(['/kasir/bkk/index','kas_bon_id'=>$model->kas_bon_id]) .'"><i class="fa fa-share"></i> Buat BKK </a>';
//				$ret = '';
			}
			if($model->is_kasbonkaskecil == TRUE){
				if($model->terimakasbon_kk == TRUE){
					$ret = '<a class="btn btn-sm green-seagreen btn-outline" style="font-size:1rem; padding: 3px;" onclick="terimauangkaskecil('.$model->kas_bon_id.')"><i class="fa fa-download"></i> Terima Uang</a>';
				}else{
					$ret = '<span style="font-size:1rem;" class="font-yellow-gold">Belum Diterima</span>';
				}
			}
		}
		?>
