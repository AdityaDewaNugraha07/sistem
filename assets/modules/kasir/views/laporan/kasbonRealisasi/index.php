				...
				<table class="table table-striped table-bordered table-hover" id="table-laporan">
					<thead>
						<tr>
							...
							<th style="line-height: 1"><?= Yii::t('app', 'Tanggal<br>Realiasasi'); ?></th>
...
<?php $this->registerJs("
	...
	setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Kasbon Kas Kecil'))."');
...
function dtLaporan(){
    var dt_table =  $('#table-laporan').dataTable({
		pageLength: 100,
        ajax: { 
			url: '<?= \yii\helpers\Url::toRoute('/kasir/laporan/kasbonRealisasi') ?>',
			data:{
				dt: 'table-laporan',
...
function printout(caraPrint){
	window.open("<?= yii\helpers\Url::toRoute('/kasir/laporan/kasbonRealisasiPrint') ?>?"+$('#form-search-laporan').serialize()+"&caraprint="+caraPrint,"",'location=_new, width=1200px, scrollbars=yes');
}
