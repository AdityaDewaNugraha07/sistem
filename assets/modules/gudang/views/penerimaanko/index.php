            ...
            <div class="portlet-body">
                <ul class="nav nav-tabs">
					<li class="active <?= $hiddenTab ?>">
						<a href="<?= \yii\helpers\Url::toRoute("/gudang/penerimaanko/index") ?>"> <?= Yii::t('app', 'Penerimaan Reguler'); ?> </a>
					</li>
					<li class="<?= $hiddenTab ?>">
						<a href="<?= \yii\helpers\Url::toRoute("/gudang/penerimaanko/terimarepacking") ?>"> <?= Yii::t('app', 'Penerimaan Hasil Mutasi Produksi'); ?> </a>
					</li>
                    <li class="">
                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                ...
                                <span class="pull-right">
									<!--<a class="btn blue btn-sm btn-outline pull-right" onclick="daftarAfterSave()"><i class="fa fa-list"></i> <?= Yii::t('app', 'Riwayat Penerimaan'); ?></a>-->
								</span>
                            </div>
                            <div class="portlet-body">
								<div class="row">
                                    <div class="col-md-6">
										...
										<?php } ?>
										<?php // echo $form->field($model, 'produk_id')->dropDownList(\app\models\MBrgProduk::getOptionList() ,['class'=>'form-control select2','prompt'=>'','onchange'=>'setProduk(); generateNomorProduksi();'])->label("Kode Produk"); ?>
									...
									<div class="col-md-6">
										...
										<?= $form->field($model, 'qty_kecil')->textInput(['class'=>'form-control input-small float','onblur'=>'setMeterKubik();'])->label("Qty Penerimaan"); ?>
								...
								<div class="row">
                                    <div class="col-md-5" style="margin-bottom: 20px; ">
                                        <h4><?= Yii::t('app', 'Data Penerimaan'); ?></h4>
                                    </div>
									<div class="col-md-7">
...
//															{error}</div>'])->textInput(['disabled'=>'disabled'])->label("Tanggal Penerimaan"); 
											echo $form->field($model, 'tanggal')->textInput(['disabled'=>'disabled','class'=>'form-control input-small']);
										?>
										...
										<?= $form->field($model, 'gudang_id')->dropDownList( \app\models\MGudang::getOptionList() ,['class'=>'form-control select2','prompt'=>'']); ?>
                        ...
                        <div class="form-actions pull-right">
                            <div class="col-md-12 right">
								...
								<?php echo \yii\helpers\Html::button( Yii::t('app', 'Print QR Code'),['id'=>'btn-print','class'=>'btn blue btn-outline ciptana-spin-btn','onclick'=>'printKartuBarang('.(isset($_GET['tbko_id'])?$_GET['tbko_id']:'').');','disabled'=>true]); ?>
...
<?php $this->registerJs(" 
	...
	$('select[name*=\"[produk_id]\"]').select2({
		...
		ajax: {
			url: '".\yii\helpers\Url::toRoute('/ppic/produk/findProduk')."',
			dataType: 'json',
			delay: 250,
...
function addItem(){
	...
	var produk_id = $("#<?= \yii\bootstrap\Html::getInputId($model, 'produk_id') ?>").val();
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/gudang/penerimaanko/addItem']); ?>',
        type   : 'POST',
...
function daftarAfterSave(){
    openModal('<?= \yii\helpers\Url::toRoute(['/gudang/penerimaanko/daftarAfterSave']) ?>','modal-aftersave');
}
function createMasterProduk(){
	openModal('<?= \yii\helpers\Url::toRoute('/ppic/produk/create') ?>','modal-produk-create',null,'setDropdownProduk();');
}
function setInduk(){
...
function printKartuBarang(id){
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/gudang/penerimaanko/counterPrint']); ?>',
        type   : 'POST',
        data   : {id:id},
        success: function (data) {
            if(data.status == "print"){
                ...
                openModal('<?= \yii\helpers\Url::toRoute('/gudang/penerimaanko/pengajuancetakulang') ?>?id='+id+'&statuspengajuan='+data.status,'modal-transaksi','65%');
            }
        },
        error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
    });
...
function generateNomorProduksi(){
	...
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/gudang/penerimaanko/generateNomorProduksi']); ?>',
        type   : 'POST',
...
function masterProduk(){
	var url = '<?= \yii\helpers\Url::toRoute(['/ppic/produk/masterOnModal','disableAction'=>'1']); ?>';
	$(".modals-place-3-min").load(url, function() {
		$("#modal-master-produk .modal-dialog").css('width','90%');
