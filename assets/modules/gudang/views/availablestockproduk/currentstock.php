...
<div class="modal fade" id="modal-currentstock" tabindex="-1" role="basic" aria-hidden="true">
...
<?php
if(empty($url)){
    $url = \yii\helpers\Url::toRoute(['/gudang/availablestockproduk/currentStock']);
}
?>
<script>
function dtProduk(){
    var dt_table =  $('#table-produk').dataTable({
        ajax: { url: '<?= $url ?>',data:{dt: 'table-produk'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
