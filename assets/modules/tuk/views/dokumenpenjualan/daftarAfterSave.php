...
function dtAfterSave(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/tuk/dokumenpenjualan/daftarAfterSave') ?>',data:{dt: 'modal-aftersave'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
...
function lihatDetail(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/tuk/dokumenpenjualan/index','dokumen_penjualan_id'=>'']); ?>'+id);
}
function edit(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/tuk/dokumenpenjualan/index','dokumen_penjualan_id'=>'']); ?>'+id+'&edit=1');
}
</script>