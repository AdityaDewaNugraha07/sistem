	...
	public function actionDaftarAfterSave(){
		if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='modal-aftersave'){
				...
				$param['join']= ['JOIN t_packinglist ON t_packinglist.packinglist_id = '.$param['table'].'.packinglist_id
...
//									$detail->kubikasi_realisasi += $keluar->kubikasi;
	...
	public function actionPrintSPM(){
		...
		if($caraprint == 'PRINT'){
			return $this->render('printSPM',['model'=>$model,'paramprint'=>$paramprint,'modDetail'=>$modDetail,'modPackinglist'=>$modPackinglist,'modContainer'=>$modContainer]);
		}else if($caraprint == 'PDF'){
			$pdf = Yii::$app->pdf;
			$pdf->options = ['title' => $paramprint['judul']];
			$pdf->filename = $paramprint['judul'].'.pdf';
			$pdf->methods['SetHeader'] = ['Generated By: '.Yii::$app->user->getIdentity()->userProfile->fullname.'||Generated At: ' . date('d/m/Y H:i:s')];
			$pdf->content = $this->render('printSPM',['model'=>$model,'paramprint'=>$paramprint,'modDetail'=>$modDetail,'modPackinglist'=>$modPackinglist,'modContainer'=>$modContainer]);
			return $pdf->render();
