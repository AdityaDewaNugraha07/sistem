...
class InvoiceController extends DeltaBaseController
{
	...
	public function actionIndex(){
		...
		if(isset($_GET['invoice_id'])){
			$model = \app\models\TInvoice::findOne($_GET['invoice_id']);
        ...
        if( Yii::$app->request->post('TInvoice')){
            ...
            try {
				...
				$model->total_bayar = $_POST['TInvoice']['total_bayar'] - $_POST['TInvoice']['total_potongan'];
                ...
                if($model->validate()){
                    if($model->save()){
						...
						if((isset($_GET['edit'])) && (isset($_GET['invoice_id']))){
							$modDetail = \app\models\TInvoiceDetail::find()->where(['invoice_id'=>$_GET['invoice_id']])->all();
							if(count($modDetail)>0){
								\app\models\TInvoiceDetail::deleteAll(['invoice_id'=>$_GET['invoice_id']]);
							}
						}
						
						foreach($_POST['TInvoiceDetail'] as $i => $detail){
							$modDetail = new \app\models\TInvoiceDetail();
...
//							$modDetail->kubikasi = $detail['kubikasi_display'];
			...
			if(!empty($packinglist_id)){
				...
				$modPackinglistCont = Yii::$app->db
									->createCommand("SELECT container_no, container_kode, seal_no 
													 ...
													 GROUP BY container_no, container_kode, seal_no")->queryAll();
				$modCust = \app\models\MCustomer::findOne($modPackinglist->cust_id);
	...
	function actionGetItems(){
		if(\Yii::$app->request->isAjax){
            $packinglist_id = Yii::$app->request->post('packinglist_id'); $data = [];
			if(!empty($packinglist_id)){
				$model = new \app\models\TInvoice();
				$modPackinglist = \app\models\TPackinglist::findOne($packinglist_id);
				$modCust = \app\models\MCustomer::findOne($modPackinglist->cust_id);
				...
				if($modPackinglist->bundle_partition == true){
					$sql = "SELECT packinglist_id, grade, jenis_kayu, glue, profil_kayu, kondisi_kayu, max(bundles_no) AS bundles, SUM(pcs) AS pcs, SUM(volume) AS volume, SUM( ROUND(volume::numeric,4) ) AS volume_display
							FROM t_packinglist_container
							WHERE packinglist_id = {$packinglist_id} 
							GROUP by 1,2,3,4,5,6
							ORDER by 1,2 ASC";
				}else{
					$sql = "SELECT packinglist_id, grade, jenis_kayu, glue, profil_kayu, kondisi_kayu, thick, thick_unit, length, length_unit, width, width_unit, count(packinglist_id) AS bundles, SUM(pcs) AS pcs, SUM(volume) AS volume, SUM( ROUND(volume::numeric,4) ) AS volume_display
							FROM t_packinglist_container
							WHERE packinglist_id = {$packinglist_id}
				...
				if(count($modContainer)>0){
					foreach($modContainer as $i => $container){
						...
						if($modPackinglist->bundle_partition===TRUE){
							$searchbund = \Yii::$app->db->createCommand("SELECT container_no, bundles_no FROM t_packinglist_container 
																		WHERE packinglist_id = ".$packinglist_id." 
																				".(!empty($condition['grade'])?"AND grade='{$condition['grade']}'":"")."
																				".(!empty($condition['grade'])?"AND jenis_kayu='{$condition['jenis_kayu']}'":"")."
						...
						if(!empty($modProduk)){
							...
							$modProduk = \app\models\MBrgProduk::createNewByPackinglist($condition);
							$modDetail->produk_id = $modProduk['produk_id'];
							// end create
						}
	...
	function actionGetItemsById(){
		if(\Yii::$app->request->isAjax){
            ...
            $data = [];
            if(!empty($id)){
				$modDetail = \app\models\TInvoiceDetail::find()->where(['invoice_id'=>$id])->all();
				if($modPackinglist->bundle_partition===FALSE && empty($edit) && $diff_size_diff_price=='false'){
					$modDetail = \app\models\TInvoiceDetail::find()
				...
				if($modPackinglist->bundle_partition===FALSE && empty($edit) && $grouping_qty=='true'){
					...
					foreach($modContainer as $i => $container){
						...
						$modProduk = \app\models\MBrgProduk::findOne($condition);
						$modDetail[$i] = \app\models\TInvoiceDetail::findOne(['invoice_id'=>$id,'produk_id'=>$modProduk->produk_id]);
            ...
            if(count($modDetail)>0){
                foreach($modDetail as $i => $detail){
					if(($modPackinglist->bundle_partition===FALSE && empty($edit) && $diff_size_diff_price=='false') || ($modPackinglist->bundle_partition===FALSE && empty($edit) && $grouping_qty=='true')){
...
//            exit;
	...
	public function actionDaftarAfterSave(){
		if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='modal-aftersave'){
				$param['table']= \app\models\TInvoice::tableName();
				$param['pk']= $param['table'].".". \app\models\TInvoice::primaryKey()[0];
	...
	public function actionSetDropdownPackinglist(){
		if(\Yii::$app->request->isAjax){
            ...
            $data['html'] = [];
			$arraymap = \app\models\TPackinglist::getOptionListInvoiceBaru($status_inv);
	...
	public function actionPrint(){
		...
		if(!empty($_GET['id'])){
			$model = \app\models\TInvoice::findOne($_GET['id']);
			$modOpEx = \app\models\TOpExport::findOne($model->op_export_id);
			$modPackinglist = \app\models\TPackinglist::findOne($model->packinglist_id);
			...
			if($modPackinglist->bundle_partition===FALSE && $model->diff_size_diff_price==FALSE){
				$modDetails = \app\models\TInvoiceDetail::find()
				...
				foreach($modContainer as $i => $container){
					$condition = self::getMasterProduk($modPackinglist->jenis_produk, $container, $model->packinglist_id, $modPackinglist->bundle_partition);
					$modProduk = \app\models\MBrgProduk::findOne($condition);
					$modDetails[$i] = \app\models\TInvoiceDetail::findOne(['invoice_id'=>$_GET['id'],'produk_id'=>$modProduk->produk_id]);
				...
				$modDetails = \app\models\TInvoiceDetail::find()->where(['invoice_id'=>$_GET['id']])->orderBy("invoice_detail_id ASC")->all();
		...
		if(\Yii::$app->request->isAjax){
            ...
            if( Yii::$app->request->post('deleteRecord')){
                ...
                try {
                    $success_1 = false;
                    $success_2 = false;
					if(\app\models\TInvoiceDetail::deleteAll("invoice_id = ".$id)){
						$success_2 = true;
					}else{
						$success_2 = false;
					}
	...
	public function actionUpdateDiffSizeDiffPrice(){
		if(\Yii::$app->request->isAjax){
			...
			$model= \app\models\TInvoice::findOne($invoice_id);
            $data = false;
    ...
    public function actionKonfirmsipeb($invoice_id){
		if(\Yii::$app->request->isAjax){
            ...
            $model->peb_barangekspor_bruto = 0; $model->peb_barangekspor_netto = 0; 
