...
class GradeController extends DeltaBaseController
{
	
	public $defaultAction = 'index';
	
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-grade'){
			$param['table']= \app\models\MGrade::tableName();
			$param['pk']= \app\models\MGrade::primaryKey()[0];
			$param['column'] = ['grade_id','jenis_produk','kode','nama','othername',$param['table'].'.active'];
	...
	public function actionCreate(){
		if(\Yii::$app->request->isAjax){
			$model = new \app\models\MGrade();
			$model->active = true;
			if( Yii::$app->request->post('MGrade')){
    ...
    public function actionInfo($id){
		if(\Yii::$app->request->isAjax){
			$model = \app\models\MGrade::findOne($id);
			...
			if( Yii::$app->request->post('MGrade')){
                ...
                try {
                    ...
                    if ($success_1) {
                        $transaction->commit();
                        $data['status'] = true;
                        $data['message'] = Yii::t('app', 'Data Grade Berhasil Diupdate');
                    } else {
                        $transaction->rollback();
                        $data['status'] = false;
                        (!isset($data['message']) ? $data['message'] = Yii::t('app', \app\components\Params::DEFAULT_FAILED_TRANSACTION_MESSAGE) : '');
...
//                    }else{
                        if($model->delete()){
                            ...
                            $data['message'] = Yii::t('app', 'Data Grade Gagal dihapus');
                        }
                        if ($success_1) {
                            $transaction->commit();
                            $data['status'] = true;
                            $data['message'] = Yii::t('app', '<i class="icon-check"></i> Data Grade Berhasil Dihapus');
                        } else {
                            $transaction->rollback();
                            $data['status'] = false;
                            (!isset($data['message']) ? $data['message'] = Yii::t('app', \app\components\Params::DEFAULT_FAILED_TRANSACTION_MESSAGE) : '');
