                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    ...
                                    <span class="caption-subject hijau bold"><?= Yii::t('app', 'Master Bahan Kayu'); ?></span>
                                </div>
                                <div class="tools">
                            ...
                            <div class="portlet-body">
                                <table class="table table-striped table-bordered table-hover" id="table-kayu">
                                    <thead>
                                        <tr>
                                            <th></th>
                                            <th style="width: 150px;"><?= Yii::t('app', 'Kelompok Kayu') ?></th>
...
<script>
function dtKayu(){
    var dt_table =  $('#table-kayu').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/ppic/kayu/index') ?>',data:{dt: 'table-kayu'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
...
function create(){
	openModal('<?= \yii\helpers\Url::toRoute('/ppic/kayu/create') ?>','modal-kayu-create');
}
function info(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/ppic/kayu/info','id'=>'']) ?>'+id,'modal-kayu-info');
}
...
function updateStatus(id){
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/kayu/updateStatus','id'=>'']); ?>'+id,
        type   : 'POST',
        data   : {},
        success: function (data) {
			if(data){
                $('#modal-kayu-info').modal('hide');
				$('#table-kayu').dataTable().fnClearTable();
			}
        },
