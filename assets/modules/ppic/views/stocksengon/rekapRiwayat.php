<table class="table table-striped table-bordered table-hover" id="table-detail-riwayat" style="width: 100%;">
    ...
    <tbody>
        <?php
        $sql = "SELECT tgl_transaksi, substring(reff_no,0,13) AS reff_no, status, lokasi, keterangan FROM h_persediaan_log WHERE tgl_transaksi BETWEEN '".$model->tgl_awal."' AND '".$model->tgl_akhir."' GROUP BY 1,2,3,4,5 ORDER BY tgl_transaksi ASC, reff_no ASC, status DESC";
        ...
        <tr style="" class="">
            ...
            $stocks = Yii::$app->db->createCommand("SELECT SUM(fisik_pcs) AS pcs, SUM(fisik_volume) AS m3 FROM h_persediaan_log WHERE tgl_transaksi BETWEEN '".$model->tgl_awal."' AND '".$model->tgl_akhir."' AND reff_no ILIKE '%".$mod['reff_no']."%' AND lokasi = '".$mod['lokasi']."' AND status='".$mod['status']."'")->queryOne();
            ?>
