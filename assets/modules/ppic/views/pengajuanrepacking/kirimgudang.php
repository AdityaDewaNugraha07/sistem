...
$this->title = 'Permintaan Barang Jadi';
app\assets\DatepickerAsset::register($this);
app\assets\Select2Asset::register($this);
app\assets\InputMaskAsset::register($this);
?>
<!-- BEGIN PAGE TITLE-->
<h1 class="page-title"> Permintaan Barang Jadi <small>( Tarik Barang Dari Gudang Ke Produksi )</small></h1>
<!-- END PAGE TITLE-->
<!-- END PAGE HEADER-->
        ...
        <div class="portlet light bordered">
            <div class="portlet-body">
				...
				<div class="row" style="margin-bottom: 10px; margin-top: 20px;">
					<div class="col-md-6">
						<?= $form->field($model, 'hasil_dari')->dropDownList(["Repacking"=>"Repacking","Regrade"=>"Regrade","Restamp"=>"Restamp","Restamp"=>"Restamp","Proses Produksi"=>"Proses Produksi"],['class'=>'form-control','prompt'=>'','onchange'=>'setHasildari();'])->label("Hasil Dari"); ?>
					</div>
					<div class="col-md-6">
                                    ...
                                    <div class="col-md-12">
                                        <div class="table-scrollable">
                                            <table id="table-detail-paletasal" class="table table-striped table-bordered table-advance table-hover" style="width: 100%">
                                                ...
                                                <tfoot>
                                                    <tr>
                                                        <td colspan="2"><a class="btn btn-xs grey" id="add-paletasal"><i class="fa fa-plus"></i> Add</a></td>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                        </div>
...
?>
<?php $this->registerJs(" 
    setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Permintaan Barang Jadi'))."');
    $pagemode
	formconfig();
...
function paletlama(ele){
	var url = '<?= \yii\helpers\Url::toRoute(['/ppic/pengajuanrepacking/paletditerima']); ?>';
...
function pickPalet(nomor_produksi){
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/pengajuanrepacking/pickpalet']); ?>',
        type   : 'POST',
...
function getItemsRandomByPk(id){
    ...
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/pengajuanrepacking/getItemsRandomByPk']); ?>',
        type   : 'POST',
        data   : {id:id},
        success: function (data) {
            if(data.html){
...
function daftarAfterSave(){
    openModal('<?= \yii\helpers\Url::toRoute(['/ppic/pengajuanrepacking/DaftarAfterSavePengiriman']) ?>','modal-aftersave');
}
...
function getPaletAsal(id){
    ...
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/pengajuanrepacking/getPaletAsal']); ?>',
        type   : 'POST',
        data   : {id:id},
        success: function (data) {
            if(data.html){
