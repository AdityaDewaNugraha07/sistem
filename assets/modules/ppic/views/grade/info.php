...
?>
<div class="modal fade draggable-modal" id="modal-grade-info" tabindex="-1" role="basic" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="btn btn-icon-only btn-default fa fa-close" data-dismiss="modal" aria-hidden="true" style="float: right; padding-top: 2px; padding-bottom: 2px;"></button>
                <h4 class="modal-title"><?= Yii::t('app', 'Detail Informasi Grade'); ?></h4>
            </div>
            ...
            <div class="modal-footer" style='text-align:center'>
                ...
                if($model->active==true){
                    echo yii\helpers\Html::button(Yii::t('app', 'Non-Active kan Data ini!'),['class'=>'btn red-flamingo btn-outline','onclick'=>"updateStatus(".$model->grade_id.")"]);
                }else{
                    echo yii\helpers\Html::button(Yii::t('app', 'Active kan Data ini!'),['class'=>'btn green-seagreen btn-outline','onclick'=>"updateStatus(".$model->grade_id.")"]);
                }
				if(empty($disableAction)){
                    echo yii\helpers\Html::button(Yii::t('app', 'Delete'),['class'=>'btn red btn-outline pull-right','onclick'=>"openModal('".\yii\helpers\Url::toRoute(['/ppic/grade/delete','id'=>$model->grade_id,'tableid'=>'table-grade'])."','modal-delete-record')"]);
                    echo yii\helpers\Html::button(Yii::t('app', 'Edit'),['class'=>'btn blue btn-outline pull-right','onclick'=>"openModal('".\yii\helpers\Url::toRoute(['/ppic/grade/edit','id'=>$model->grade_id])."','modal-grade-edit')"]);
				}
				?>
            </div>
        </div>
