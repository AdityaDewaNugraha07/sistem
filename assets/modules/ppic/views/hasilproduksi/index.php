...
function setProduk(){
    ...
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/hasilproduksi/getProduk']); ?>',
        type   : 'POST',
...
function addItem(){
	...
	var produk_id = $("#<?= \yii\bootstrap\Html::getInputId($model, 'produk_id') ?>").val();
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/hasilproduksi/addItem']); ?>',
        type   : 'POST',
...
function getItemsByPk(id){
    ...
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/hasilproduksi/getItemsByPk']); ?>',
        type   : 'POST',
        data   : {id:id},
        success: function (data) {
            if(data.html){
...
function daftarAfterSave(){
    openModal('<?= \yii\helpers\Url::toRoute(['/ppic/hasilproduksi/daftarAfterSave']) ?>','modal-aftersave');
}
...
function printKartuBarang(id){
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/hasilproduksi/counterPrint']); ?>',
        type   : 'POST',
        data   : {id:id},
        success: function (data) {
            if(data.status == "print"){
                ...
                openModal('<?= \yii\helpers\Url::toRoute('/ppic/hasilproduksi/pengajuancetakulang') ?>?id='+id+'&statuspengajuan='+data.status,'modal-transaksi','65%');
            }
        },
        error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
    });
...
function generateNomorProduksi(){
	...
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/hasilproduksi/generateNomorProduksi']); ?>',
        type   : 'POST',
