...
class ApprovalhasilsoController extends DeltaBaseController
{
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			...
			$param['column'] = ['approval_id',
								...
								't_stockopname_hasil.keterangan', //7
								'assigned_nama', //8
								'approved_by_nama', //9
								'level', //10
			...
			$param['where'] = "(view_approval.reff_no ILIKE '%HSO%' and view_approval.status = 'Not Confirmed')";
			$param['join'] = "left join t_stockopname_hasil on view_approval.reff_no = t_stockopname_hasil.kode
                              JOIN m_pegawai AS pegawai1 ON pegawai1.pegawai_id = t_stockopname_hasil.by_prepared
                              JOIN m_pegawai AS pegawai2 ON pegawai2.pegawai_id = t_stockopname_hasil.by_gmopr
                              JOIN m_pegawai AS pegawai3 ON pegawai3.pegawai_id = t_stockopname_hasil.by_dirut";
					...
					if(!empty($model)){
						...
						if($model->validate()){
							if($model->save()){
                                ...
                                if($model->level == 2){
                                    $modHasil = \app\models\TStockopnameHasil::findOne(['kode'=>$model->reff_no]);
...
//                    exit;
	...
	public function actionConfirm(){
		if(\Yii::$app->request->isAjax){
			...
			$modAselole = \app\models\TStockopnameHasil::findOne(['kode'=>$modApprove->reff_no]);
