...
class ApprovalhargaprodukController extends DeltaBaseController
{
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			...
			$param['join'] = "left join m_harga_produk on m_harga_produk.kode = view_approval.reff_no 
								left join m_brg_produk on m_brg_produk.produk_id = m_harga_produk.produk_id 
								";
			$param['group'] = "group by approval_id, reff_no, tanggal_berkas, assigned_nama, approved_by_nama, level, view_approval.status, view_approval.created_at, view_approval.created_at, m_brg_produk.produk_group";
			$param['having'] = "having reff_no ILIKE '%PRP%' ";
	...
	public function actionInfo($id){
		if(\Yii::$app->request->isAjax){
			...
			$model_m_harga_produk = \app\models\MHargaProduk::find()->where(["kode"=>$model->reff_no])->one();
				$harga_tanggal_penetapan = $model_m_harga_produk->harga_tanggal_penetapan;
				$created_by = $model_m_harga_produk->created_by;
				...
				$created_at = $model_m_harga_produk->created_at;
			$model_m_brg_produk = \app\models\MBrgProduk::find()->where(["produk_id"=>$model_m_harga_produk->produk_id])->one();
			$produk_group = $model_m_brg_produk->produk_group;
			return $this->renderAjax('info',['model'=>$model, 'produk_group'=>$produk_group,'harga_tanggal_penetapan'=>$harga_tanggal_penetapan,'created_by'=>$created_by,'created_at'=>$created_at]);
		}
	}
	...
	public function actionRejectReason($id){
		if(\Yii::$app->request->isAjax){
			$model = \app\models\TApproval::findOne($id);
			$modelReff = \app\models\MHargaProduk::findOne(['kode'=>$model->reff_no]);
			...
			if( Yii::$app->request->post('MHargaProduk')){
				...
				try {
					$success_1 = false;
					$success_2 = false;
					if(!empty($model) && !empty($_POST['MHargaProduk']['reject_reason'])){
						$model->approved_by = Yii::$app->user->identity->pegawai_id;
						...
						if($model->validate()){
							if($model->save()){
								...
								$arrPost = ['status'=>"REJECTED",
                                                    ...
                                                    'at'=>date('Y-m-d H:i:s'),
                                                    'reason'=>$_POST['MHargaProduk']['reject_reason'],
                                                    'kode'=>trim($model->reff_no)
													];

								if(!empty($modelReff->reject_reason)){
									//select * from json_array_elements_text('["foo", "bar"]')
									$sql = "select json_array_length(reject_reason::JSON) from m_harga_produk where trim(kode) = '".$model->reff_no."' ";
									...
									if ($jumlah_json < 4) {
										...
										$sql_empty_json = "update m_harga_produk set reject_reason = NULL where trim(kode) = '".$model->reff_no."' ";
                                ...
                                $approver4 = \app\models\TApproval::findOne(['reff_no'=>$model->reff_no, 'assigned_to'=>59]);
                                $final_status = $approver4->status;

	                            if ($model->level == 4 && $final_status == 'REJECTED') {
	                                $sqlUpdate =  " UPDATE m_harga_produk
	...
	public function actionConfirm(){
		if(\Yii::$app->request->isAjax){
			...
			$modPRP = \app\models\MHargaProduk::findOne(['kode'=>$modApprove->reff_no]);
			$checkApprovals = Yii::$app->db->createCommand("SELECT * FROM t_approval WHERE reff_no = '".$modPRP->kode."' AND level < ".$modApprove->level." AND status != 'APPROVED' ")->queryAll();
			$data = true;
		...
		if(\Yii::$app->request->isAjax){
			...
			if( Yii::$app->request->post('MHargaProduk')){
				...
				try {
					$success_1 = false;
					$success_2 = false;
					if(!empty($model) && !empty($_POST['MHargaProduk']['approve_reason'])){
						$model->approved_by = Yii::$app->user->identity->pegawai_id;
						...
						if($model->validate()){
							if($model->save()){
								...
								$arrPost = ['status'=>"APPROVED",
											'by'=> $model->assigned_to,
											'at'=>date('Y-m-d H:i:s'),
											'reason'=>$_POST['MHargaProduk']['approve_reason'],
											'kode'=>$model->reff_no
											];
								
								if(!empty($modelReff->approve_reason)){
									$sql = "select json_array_length(approve_reason::JSON) from m_harga_produk where trim(kode) = '".$model->reff_no."' ";
									$jumlah_json = Yii::$app->db->createCommand($sql)->queryScalar();
									...
									if ($jumlah_json < 4) {
										...
										$sql_empty_json = "update m_harga_produk set approve_reason = NULL where trim(kode) = '".$model->reff_no."' ";
										$query = Yii::$app->db->createCommand($sql_empty_json)->execute();
	                            ...
	                            if ($model->level == 4 && $final_status == 'APPROVED') {
	                                $sqlUpdate =  " UPDATE m_harga_produk
					...
					if ($success_1 && $success_2) {
						...
						//(!isset($data['message']) ? $data['message'] = Yii::t('app', \app\components\Params::DEFAULT_FAILED_TRANSACTION_MESSAGE) : '');
						//(isset($data['message_validate']) ? $data['message'] = null : '');
						$data['message'] = "<br>sukses 1 = ".$success_1."<br>sukses 2 = ".$success_2."<br>sukses 3 = ".$success_3;
					}
				} catch (\yii\db\Exception $ex) {
					$transaction->rollback();
					$data['message'] = $ex;
