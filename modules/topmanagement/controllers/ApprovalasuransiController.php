...
class ApprovalasuransiController extends DeltaBaseController
{
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			...
			$param['where'] = "(reff_no ILIKE '%ASR%' and view_approval.status = 'Not Confirmed')";
			$param['join'] = "left join t_asuransi on view_approval.reff_no = t_asuransi.kode";
    ...
    public function actionShowDetails(){
        if(\Yii::$app->request->isAjax){
			...
			$modReff = \app\models\TAsuransi::findOne(['kode'=>$model->reff_no]);
	...
	public function actionApproveReason($id){
		if(\Yii::$app->request->isAjax){
			$model = \app\models\TApproval::findOne($id);
			$modelReff = \app\models\TAsuransi::findOne(['kode'=>$model->reff_no]);
			if( Yii::$app->request->post('TAsuransi')){
				$transaction = \Yii::$app->db->beginTransaction();
				try {
					$success_1 = false;
					$success_2 = false;
					if(!empty($model) && !empty($_POST['TAsuransi']['approve_reason'])){
						$model->approved_by = Yii::$app->user->identity->pegawai_id;

						// ambil user_id untuk update t_asuransi
						$user_id = Yii::$app->user->identity->user_id;

						$model->tanggal_approve = date('Y-m-d');
						
						// tanggal jam menit detik hari ini untuk update t_asuransi
						...
						if($model->validate()){
							if($model->save()){
								...
								$arrPost = ['status'=>"APPROVED",
											...
											'at'=>date('Y-m-d H:i:s'),
											'reason'=>$_POST['TAsuransi']['approve_reason']
											];
                                ...
                                $sqlUpdate =  " UPDATE t_asuransi
                                                SET approve_reason = '$modelReff->approve_reason', status_approval = '".$status_approval."'
		...
		if(\Yii::$app->request->isAjax){
			...
			if( Yii::$app->request->post('TAsuransi')){
				...
				try {
					$success_1 = false;
					$success_2 = false;
					if(!empty($model) && !empty($_POST['TAsuransi']['reject_reason'])){
						$model->approved_by = Yii::$app->user->identity->pegawai_id;
						...
						if($model->validate()){
							if($model->save()){
								...
								$arrPost = ['status'=>"APPROVED",
											'by'=> $model->assigned_to,
											'at'=>date('Y-m-d H:i:s'),
											'reason'=>$_POST['TAsuransi']['reject_reason']
											];
                                ...
                                $sqlUpdate =  " UPDATE t_asuransi
                                                SET reject_reason = '$modelReff->reject_reason', status_approval = '".$status_approval."'
                                                WHERE kode = '".$model->reff_no."' ";
                                $success_2 = Yii::$app->db->createCommand($sqlUpdate)->execute();
	...
	public function actionConfirm(){
		if(\Yii::$app->request->isAjax){
			...
			$modHO = \app\models\TAsuransi::findOne(['kode'=>$modApprove->reff_no]);
