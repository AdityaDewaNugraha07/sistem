...
class ApprovalhargalogController extends DeltaBaseController
{
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			...
			$param['join'] = "left join m_harga_log on m_harga_log.kode = view_approval.reff_no 
								left join m_brg_log on m_brg_log.log_id = m_harga_log.log_id 
								";
			...
			$param['having'] = "having reff_no ILIKE '%LOG%' ";
	...
	public function actionInfo($id){
        ...
        $model_m_harga_log      = \app\models\MHargaLog::find()->where(["kode"=>$model->reff_no])->one();
        $harga_tanggal_penetapan= $model_m_harga_log->harga_tanggal_penetapan;
        $created_by             = $model_m_harga_log->created_by;
        ...
        $created_at             = $model_m_harga_log->created_at;
	...
	public function actionRejectReason($id){
		if(\Yii::$app->request->isAjax){
			$model = \app\models\TApproval::findOne($id);
			$modelReff = \app\models\MHargaLog::findOne(['kode'=>$model->reff_no]);
			...
			if( Yii::$app->request->post('MHargaLog')){
				...
				try {
					$success_1 = false;
					$success_2 = false;
					if(!empty($model) && !empty($_POST['MHargaLog']['reject_reason'])){
						$model->approved_by = Yii::$app->user->identity->pegawai_id;
						...
						if($model->validate()){
							if($model->save()){
								...
								$arrPost = ['status'=>"REJECTED",
                                            ...
                                            'at'=>date('Y-m-d H:i:s'),
                                            'reason'=>$_POST['MHargaLog']['reject_reason'],
                                            'kode'=>trim($model->reff_no)
                                            ];

								if(!empty($modelReff->reject_reason)){
									//select * from json_array_elements_text('["foo", "bar"]')
									$sql = "select json_array_length(reject_reason::JSON) from m_harga_log where trim(kode) = '".$model->reff_no."' ";
									...
									if ($jumlah_json < 4) {
										...
										$sql_empty_json = "update m_harga_log set reject_reason = NULL where trim(kode) = '".$model->reff_no."' ";
	...
	public function actionConfirm(){
		if(\Yii::$app->request->isAjax){
			...
			$modLOG = \app\models\MHargaLog::findOne(['kode'=>$modApprove->reff_no]);
			$checkApprovals = Yii::$app->db->createCommand("SELECT * FROM t_approval WHERE reff_no = '".$modLOG->kode."' AND level < ".$modApprove->level." AND status != 'APPROVED' ")->queryAll();
			$data = true;
		...
		if(\Yii::$app->request->isAjax){
			...
			if( Yii::$app->request->post('MHargaLog')){
				...
				try {
					$success_1 = false;
					$success_2 = false;
					if(!empty($model) && !empty($_POST['MHargaLog']['approve_reason'])){
						$model->approved_by = Yii::$app->user->identity->pegawai_id;
						...
						if($model->validate()){
							if($model->save()){
								...
								$arrPost = ['status'=>"APPROVED",
											'by'=> $model->assigned_to,
											'at'=>date('Y-m-d H:i:s'),
											'reason'=>$_POST['MHargaLog']['approve_reason'],
											'kode'=>$model->reff_no
											];
								
								if(!empty($modelReff->approve_reason)){
									$sql = "select json_array_length(approve_reason::JSON) from m_harga_log where trim(kode) = '".$model->reff_no."' ";
									$jumlah_json = Yii::$app->db->createCommand($sql)->queryScalar();
									...
									if ($jumlah_json < 4) {
										...
										$sql_empty_json = "update m_harga_log set approve_reason = NULL where trim(kode) = '".$model->reff_no."' ";
										$query = Yii::$app->db->createCommand($sql_empty_json)->execute();
