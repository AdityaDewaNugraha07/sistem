...
class ApprovalposengonController extends DeltaBaseController
{
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			...
			$param['column'] = ['approval_id',
								't_posengon_rencana.kode', 					//1
								'tanggal_berkas', 							//2
								"CONCAT( TO_CHAR(tanggal_dibutuhkan_awal :: DATE, 'dd/mm/yyyy'),' sd ',TO_CHAR(tanggal_dibutuhkan_akhir :: DATE, 'dd/mm/yyyy')) as dibutuhkan", //3
								"CONCAT( TO_CHAR(tanggal_pengiriman_awal :: DATE, 'dd/mm/yyyy'),' sd ',TO_CHAR(tanggal_pengiriman_akhir :: DATE, 'dd/mm/yyyy')) as dibutuhkan", //4
								'm_pegawai.pegawai_nama AS dibuat_oleh', 	//5
								'assigned_nama', 							//6
								'approved_by_nama', 						//7
								'level', 									//8
								$param['table'].'.status',          //9
                                                                'substring(t_posengon_rencana.kode,1,3) as jenis_kode']; 		//10		
			$param['where'] = "(substring(view_approval.reff_no,1,3) IN ('RBS','RBJ') and view_approval.status = 'Not Confirmed')";
			$param['join'] = "JOIN t_posengon_rencana on t_posengon_rencana.kode = view_approval.reff_no
                              JOIN t_pmr on t_pmr.posengon_rencana_id = t_posengon_rencana.posengon_rencana_id
                              JOIN m_user ON m_user.user_id = t_posengon_rencana.created_by
			...
			$param['order'] = "t_posengon_rencana.created_at DESC, level DESC";
	...
	public function actionInfo($id){
		if(\Yii::$app->request->isAjax){
			...
			$modReff = \app\models\TPosengonRencana::findOne(['kode'=>$model->reff_no]);
            $modDetail = \app\models\TPosengon::find()->where(['posengon_rencana_id'=>$modReff->posengon_rencana_id])->all();
            $modPmr = \app\models\TPmr::findOne(['posengon_rencana_id'=>$modReff->posengon_rencana_id]);
			return $this->renderAjax('info',['model'=>$model,'modReff'=>$modReff,'modDetail'=>$modDetail,'modPmr'=>$modPmr]);
		}
	}
...
//                    exit;
					if ($success_1) {
						...
						// AUTO CLOSE t_pmr on t_posengon_rencana approved REQUEST RACHMAN 2020-05-13
						$modPMR =  \app\models\TPmr::findOne(['posengon_rencana_id'=>$modReff->posengon_rencana_id]);
						$pmr_id = $modPMR->pmr_id;
						$jenis_log = $modPMR->jenis_log;
						if ($jenis_log = 'LS') {
							$sql_update = "update t_pmr set status = 'CLOSE' where pmr_id = ".$pmr_id."";
...
//                    exit;
	...
	public function actionConfirm(){
		if(\Yii::$app->request->isAjax){
			...
			$modAselole = \app\models\TPosengonRencana::findOne(['kode'=>$modApprove->reff_no]);
