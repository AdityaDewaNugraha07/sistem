...
class ApprovalagendasoController extends DeltaBaseController
{
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			...
			$param['column'] = ['approval_id',
								...
								'tanggal_berkas', //2
								'pegawai1.pegawai_nama AS pegawai1', //3
								'pegawai2.pegawai_nama AS pegawai2', //4
								'pegawai3.pegawai_nama AS pegawai3', //5
								'pegawai4.pegawai_nama AS pegawai4', //6
								't_stockopname_agenda.keterangan', //7
								'assigned_nama', //8
								'approved_by_nama', //9
								'level', //10
			...
			$param['where'] = "(view_approval.reff_no ILIKE '%ASO%' and view_approval.status = 'Not Confirmed')";
			$param['join'] = "left join t_stockopname_agenda on view_approval.reff_no = t_stockopname_agenda.kode
                              JOIN m_pegawai AS pegawai1 ON pegawai1.pegawai_id = t_stockopname_agenda.penanggungjawab
                              JOIN m_pegawai AS pegawai2 ON pegawai2.pegawai_id = t_stockopname_agenda.by_kadivacc
                              JOIN m_pegawai AS pegawai3 ON pegawai3.pegawai_id = t_stockopname_agenda.by_kanitgud
                              JOIN m_pegawai AS pegawai4 ON pegawai4.pegawai_id = t_stockopname_agenda.by_kadivmkt";
    ...
    public function actionShowDetails(){
        if(\Yii::$app->request->isAjax){
			...
			$modReff = \app\models\TStockopnameAgenda::findOne(['kode'=>$model->reff_no]);
...
//                    exit;
	...
	public function actionConfirm(){
		if(\Yii::$app->request->isAjax){
			...
			$modAselole = \app\models\TStockopnameAgenda::findOne(['kode'=>$modApprove->reff_no]);
