...
class ApprovalketerlambataninputalertController extends DeltaBaseController
{
    public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
            ...
            $param['column'] = ['approval_id',	
                                    't_piutang_alert.piutang_nomor_nota',
                                    ...
                                    $param['table'].'.created_at'];
            $param['where'] = "(substring(parameter1::TEXT,1,3) ='PAL')  and view_approval.status = 'Not Confirmed'";
            $param['join'] = "left join t_piutang_alert on view_approval.reff_no = t_piutang_alert.piutang_nomor_nota
                                                    left join m_customer on m_customer.cust_id = t_piutang_alert.customer_id";
    ...
    public function actionConfirm(){
        if(\Yii::$app->request->isAjax){
            ...
            $modHO = \app\models\TPiutangAlert::findOne(['piutang_nomor_nota'=>$modApprove->reff_no] );
    ...
    public function actionApproveReason($id){
        if(\Yii::$app->request->isAjax){
                $model = \app\models\TApproval::findOne($id);
                $modelReff = \app\models\TPiutangAlert::findOne(['piutang_nomor_nota'=>$model->reff_no]);
                if( Yii::$app->request->post('TPiutangAlert')){
                        $transaction = \Yii::$app->db->beginTransaction();
                        try {
                                $success_1 = false;
                                $success_2 = false;
                                $success_3 = false;
                                if(!empty($model) && !empty($_POST['TPiutangAlert']['approve_reason'])){
                                        $model->approved_by = Yii::$app->user->identity->pegawai_id;

                                        // ambil user_id untuk update t_piutang_alert
                                        ...
                                        if($model->validate()){
                                                if($model->save()){
                                                        ...
                                                        $arrPost = ['status'=>"APPROVED",
                                                                                ...
                                                                                'at'=>date('Y-m-d H:i:s'),
                                                                                'reason'=>$_POST['TPiutangAlert']['approve_reason']
                                                                                ];
                                                        ...
                                                        $sql_json = "select approve_reason from t_piutang_alert where piutang_nomor_nota = '".$model->reff_no."' ";
                                                        $jsons = Yii::$app->db->createCommand($sql_json)->queryScalar();

                                                        if (empty($jsons)) {
                                                                $sqlUpdate =  " UPDATE t_piutang_alert
                                                                                                SET approve_reason = '$modelReff->approve_reason'
                                                                                                WHERE piutang_nomor_nota = '".$model->reff_no."' ";
    ...
    public function actionRejectReason($id){
            if(\Yii::$app->request->isAjax){
                    $model = \app\models\TApproval::findOne($id);
                    $modelReff = \app\models\TPiutangAlert::findOne(['kode'=>$model->reff_no]);
                    if( Yii::$app->request->post('TPiutangAlert')){
                            $transaction = \Yii::$app->db->beginTransaction();
                            try {
                                    $success_1 = false;
                                    $success_2 = false;
                                    if(!empty($model) && !empty($_POST['TPiutangAlert']['reject_reason'])){
                                            $model->approved_by = Yii::$app->user->identity->pegawai_id;
                                            $model->tanggal_approve = date('Y-m-d');
                                            ...
                                            if($model->validate()) {
                                                ...
                                                'by'=> $model->assigned_to,
                                                'at'=>date('Y-m-d H:i:s'),
                                                'reason'=>$_POST['TPiutangAlert']['reject_reason']
                                                                                                    ];
