	...
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			...
			$param['column'] = ['approval_id',
								...
								$param['table'].'.created_at'];
			$param['where'] = "(reff_no ILIKE '%PBL%' and view_approval.status = 'Not Confirmed')";
			...
			if( Yii::$app->request->post('TPengajuanPembelianlog')){
				...
				try {
					...
					if(!empty($model) && !empty($_POST['TPengajuanPembelianlog']['approve_reason'])){
						...
						if($model->validate()){
							if($model->save()){
                                ...
                                // set disik approval owner cuy
                                if ($model->assigned_to == 123) {
                                    $modelReff->status = 'APPROVED';
                                } else {
                                    $modelReff->status = 'Not Confirmed';
                                }
                                // eo set disik approval owner cuy
					...
					if ($success_1 && $success_2) {
			            ...
			            if($model->level == 2){
			                $modPPL = \app\models\TPengajuanPembelianlog::findOne(['kode'=>$model->reff_no]);
			                //$send_mail = \Yii::$app->runAction("/sendmail/approvalpengajuanpembelianlog",['pengajuan_pembelianlog_id'=>$modPPL->pengajuan_pembelianlog_id]);
			            }
					} else {
						$transaction->rollback();
						$data['status'] = false;
