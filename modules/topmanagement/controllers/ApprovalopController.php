...
class ApprovalopController extends DeltaBaseController
{
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			...
			$param['where'] = "(substring(reff_no::TEXT,2,2) ='OP') and view_approval.status = 'Not Confirmed'";
			$param['join'] = "left join t_op_ko on view_approval.reff_no = t_op_ko.kode 
								left join m_customer on m_customer.cust_id = t_op_ko.cust_id";
	...
	public function actionVopConfirmed(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			$param['table'] = \app\models\ViewApproval::tableName();
			$param['pk'] = "approval_id";
			$param['column'] = ['approval_id',															// 0
	...
	public function actionInfo($id){
		...
		if(\Yii::$app->request->isAjax){
			...
			$ext = $attch->file_ext;
			return $this->renderAjax('image',['attch'=>$attch, 'ext'=>$ext]);
		}
	}
	...
	public function actionConfirm(){
		if(\Yii::$app->request->isAjax){
			...
			$modHO = \app\models\TOpKo::findOne(['kode'=>$modApprove->reff_no]);
            ...
            // hitung ulang cek approval buat status_approval di table t_op_ko, bikin dibawah jangan ganggu yang atas xD
            $xxx = 1;
			if (count($checkApprovals) > 0){
				foreach($checkApprovals as $i => $check){
			...
			$lvl = Yii::$app->db->createCommand("select max(level) as lvl from t_approval where reff_no = '{$modHO->kode}'")->queryOne();
			if($xxx < $lvl['lvl']){
				$sql_status_approval = "update t_op_ko set status_approval = 'Not Confirmed' where kode = '".$modApprove->reff_no."' ";
			} else {
				$sql_status_approval = "update t_op_ko set status_approval = 'APPROVED' where kode = '".$modApprove->reff_no."' ";
			}
    ...
    public function actionApproveReason($id){
		if(\Yii::$app->request->isAjax){
			$model = \app\models\TApproval::findOne($id);
			$modelReff = \app\models\TOpKo::findOne(['kode'=>$model->reff_no]);
			if( Yii::$app->request->post('TOpKo')){
				...
				try {
					...
					$success_2 = false;
					$success_3 = false;
					if(!empty($model) && !empty($_POST['TOpKo']['approve_reason'])){
						$model->approved_by = Yii::$app->user->identity->pegawai_id;

						// ambil user_id untuk update t_op_ko
						...
						if($model->validate()){
							if($model->save()){
								...
								$arrPost = ['status'=>"APPROVED",
											...
											'at'=>date('Y-m-d H:i:s'),
											'reason'=>$_POST['TOpKo']['approve_reason']
											];
								...
								$sql_json = "select approve_reason from t_op_ko where kode = '".$model->reff_no."' ";
								$jsons = Yii::$app->db->createCommand($sql_json)->queryScalar();

								if (empty($jsons)) {
									$sqlUpdate =  " UPDATE t_op_ko
		...
		if(\Yii::$app->request->isAjax){
			...
			if( Yii::$app->request->post('TOpKo')){
				...
				try {
					$success_1 = false;
					$success_2 = false;
					if(!empty($model) && !empty($_POST['TOpKo']['reject_reason'])){
						$model->approved_by = Yii::$app->user->identity->pegawai_id;
						$model->tanggal_approve = date('Y-m-d');
						$model->status = \app\models\TApproval::STATUS_REJECTED;
						if($model->validate()) {
							if($model->save()) {
								...
								$arrPost = ['status'=>"REJECTED",
                                                    'by'=> $model->assigned_to,
                                                    'at'=>date('Y-m-d H:i:s'),
                                                    'reason'=>$_POST['TOpKo']['reject_reason']
													];
