    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
				...
				<h4 class="modal-title" style="font-weight: bold;"><?= Yii::t('app', 'Approval Harga Log ').'<b>'.\app\components\DeltaGlobalClass::getBerkasNamaByBerkasKode($model->reff_no).'</b>'; ?> </h4>
			</div>
...
function confirm(id,type){
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalhargalog/confirm']); ?>',
		type   : 'POST',
		data   : {approval_id:id},
		success: function (data) {
			if (data) {
				if (type == 'approve') {
					$(".modals-place-2").load('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalhargalog/approveReason','id'=>'']); ?>'+id, function() {
						...
						$(".modals-place-2").load('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalhargalog/rejectReason','id'=>'']); ?>'+id, function() {
						$("#modal-transaksi").modal('show');
						$("#modal-transaksi").on('hidden.bs.modal', function () {
							$("#modal-transaksi").hide();
							$("#modal-transaksi").remove();
				...
				openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalhargalog/notAllowed','id'=>'']); ?>'+id,'modal-global-info');
			}
		},
...
function approve(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalhargalog/approveConfirm','id'=>'']); ?>'+id,'modal-global-confirm');
}
</script>
