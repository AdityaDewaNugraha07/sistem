			...
			<div class="portlet box blue-hoki bordered">
				...
				<div class="portlet-body" style="background-color: #d9e2f0" >
					<div class="row"  style="margin-top: -15px;">
                        ...
                        <?php } ?>
                        <?php
                        if(!empty($modReff->approver4)){
                        $approver4 = Yii::$app->db->createCommand("SELECT * FROM t_approval WHERE reff_no = '{$model->reff_no}' AND assigned_to = ".$modReff->approver4)->queryOne();
                        ?>
						<div class="col-md-3">
							<table style="width: 100%; font-size: 1.1rem;">
								<tr>
									<td style="width: 30%; vertical-align: top; padding: 3px;">Approver 4</td>
									<td style="font-weight: 800; line-height: 0.9; padding: 5px;">: 
										<?= \app\models\MPegawai::findOne($modReff->approver4)->pegawai_nama; ?>
										<span style="font-weight: 500; font-size: 1rem;">
											<?php if($approver4['status']==\app\models\TApproval::STATUS_APPROVED){
												echo " <br>&nbsp; <span class='font-green-seagreen'> ".\app\models\TApproval::STATUS_APPROVED." at ".
														\app\components\DeltaFormatter::formatDateTimeForUser2($approver4['updated_at'])."</span>";
												if(count($reasons)>0){
													foreach($reasons as $i => $reason){
														if($reason['by']==$approver4['assigned_to']){
															echo " <br>&nbsp; <span class='font-green-seagreen'> Reason : <i>".$reason['reason']."</i></span>";
														}
													}
												}
											}else if($approver4['status']==\app\models\TApproval::STATUS_REJECTED){
												echo " <br>&nbsp; <span class='font-red-flamingo'> ".\app\models\TApproval::STATUS_REJECTED." at ".
														\app\components\DeltaFormatter::formatDateTimeForUser2($approver4['updated_at'])."</span>";
												if(count($reasons)>0){
													foreach($reasons as $i => $reason){
                    ...
                    <?php 
                        if( $modReff->tipe == "KOREKSI HARGA JUAL" || $modReff->tipe == "KOREKSI NOPOL MOBIL"|| $modReff->tipe == "KOREKSI ALAMAT BONGKAR" || $modReff->tipe == "POTONGAN PIUTANG" ){ 
                                ...
                                <table class="table table-striped table-bordered table-advance table-hover" id="table-detail">
                                    ...
                                    <tbody>
                                        ...
                                        foreach($new_data_detail as $i => $detail){
                                            ...
                                            if($new_data['jenis_produk'] == 'Limbah') {
                                                $produkNama = app\models\MBrgLimbah::findOne($detail['produk_id'])->limbah_nama;
                                            }else {
                                                $produkNama = app\models\MBrgProduk::findOne($detail['produk_id'])->produk_nama;
                                            }
                                            ...
                                            echo    "<td class='text-align-left'>". $produkNama ."</td>";
                        ...
                        <div class="row"  style="margin-top: -15px;">
                            ...
                            <div class="col-md-4">
                                <table style="width: 100%">
                                    <thead>
                                        <?php
                                        if($modReff->tanggal > '2021-05-03'){
                                        ?>
                                        <tr>
                                            <th>Supir Lama</th>
                                            <td class="font-red-flamingo"><u><?= $modDetail['supir_old']; ?></u></td>
                                            <td>==></td>
                                            <th>Supir Baru</th>
                                            <td class="font-green-seagreen"><u><?= $modDetail['supir_new']; ?></u></td>
                                        </tr>
                                        <?php
                                        }
                                        ?>
                    ...
                    <?php }else if( $modReff->tipe == "KOREKSI ALAMAT BONGKAR" ){ ?>
                        <br><br>
                        <div class="row"  style="margin-top: -15px;">
                            <div class="col-md-4"></div>
                            <div class="col-md-4">
                    ...
                    $sql = "SELECT *, (termin_tagihan - termin_terbayar) AS sisa_bayar 
                            ...
                            WHERE piutang_alert_id = {$modAlert->piutang_alert_id} ORDER BY termin ASC";
