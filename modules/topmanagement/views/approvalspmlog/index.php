            ...
            <div class="portlet-body">
                <ul class="nav nav-tabs">
                    <li class="<?php echo $status1;?>">
						<a href="<?= yii\helpers\Url::toRoute("/topmanagement/approvalspmlog/index"); ?>"> <?= Yii::t('app', 'Not Confirmed'); ?> </a>
                    </li>
					<li class="<?php echo $status2;?>">
						<a href="<?= \yii\helpers\Url::toRoute("/topmanagement/approvalspmlog/indexConfirmed") ?>"> <?= Yii::t('app', 'Confirmed'); ?> </a>
					</li>
                </ul>
                <div class="row">
                    <div class="col-md-12">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="table-master">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Assigned To') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Approved By') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Status') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Detail') ?></th>
                                        </tr>
                                    </thead>
                                </table>
                            </div>
...
<?php $this->registerJs(" dtMaster(); 
setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Approve SPM Log'))."');", 
yii\web\View::POS_READY); 
?>
<?php $url = \yii\helpers\Url::toRoute('/topmanagement/approvalspmlog/'.$this->context->action->id); ?>
<script>
function dtMaster(){
    var dt_table =  $('#table-master').dataTable({
        ...
        columnDefs: [
			...
			{	targets: 2, class: "text-align-center",
                render: function (data, type, full, meta) {
                    return formatDateForUser(full[2]);
                }
            },
			...
			{	targets: 4, class: "text-align-left"},
...
function info(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalspmlog/info','id'=>'']) ?>'+id,'modal-master-info','95%'," $('#table-master').dataTable().fnClearTable(); ");
}
