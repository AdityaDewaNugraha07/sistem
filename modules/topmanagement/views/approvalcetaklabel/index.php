...
$this->title = 'Approval Cetak Ulang Label Produk';
            ...
            <div class="portlet-body">
                <ul class="nav nav-tabs">
                    <li class="<?php echo $status1;?>">
						<a href="<?= yii\helpers\Url::toRoute("/topmanagement/approvalcetaklabel/index"); ?>"> <?= Yii::t('app', 'Not Confirmed'); ?> </a>
                    </li>
					<li class="<?php echo $status2;?>">
						<a href="<?= \yii\helpers\Url::toRoute("/topmanagement/approvalcetaklabel/indexConfirmed") ?>"> <?= Yii::t('app', 'Confirmed'); ?> </a>
					</li>
                </ul>                
                <div class="row">
                    <div class="col-md-12">
                            ...
                            <div class="portlet-body">
                                <table class="table table-striped table-bordered table-hover" id="table-master">
                                    <thead>
                                        <tr>
                                            <th></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Kode<br>Permintaan') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Tanggal') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Kode Hasil<br>Produksi/Terima') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Kode Barang Jadi') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Priority') ?></th>
...
<?php $this->registerJs(" dtMaster(); setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Approve Cetak Label'))."');", yii\web\View::POS_READY); ?>
<?php $url = \yii\helpers\Url::toRoute('/topmanagement/approvalcetaklabel/'.$this->context->action->id); ?>
<script>
function dtMaster(){
...
function info(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalcetaklabel/info','id'=>'']) ?>'+id,'modal-master-info','95%'," $('#table-master').dataTable().fnClearTable(); ");
}
</script>