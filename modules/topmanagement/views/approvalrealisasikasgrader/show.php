...
$model = \app\models\TRealisasidinasGrader::find()->where(['kode'=>$modApp->reff_no])->one();
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    if(!empty($modRDG->approve_reason)){
                        $modApproveReason = \yii\helpers\Json::decode($modRDG->approve_reason);
                    ...
                    if(!empty($modRDG->reject_reason)){
                        $modApproveReason = \yii\helpers\Json::decode($modRDG->reject_reason);
                        foreach($modApproveReason as $kolom => $value){
                            $approver1 = $model->assigned_to;
                            if($value['by'] == $approver1){
                                $approver = Yii::$app->db->createCommand("select pegawai_nama from m_pegawai where pegawai_id = ".$approver1." ")->queryScalar();
