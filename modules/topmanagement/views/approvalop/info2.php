...
function confirm(id,type){
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalop/confirm']); ?>',
		type   : 'POST',
		data   : {approval_id:id},
		success: function (data) {
			if(data){
				if(type == 'approve'){
					$(".modals-place-2").load('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalop/approveReason','id'=>'']); ?>'+id, function() {
					...
					$(".modals-place-2").load('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalop/rejectReason','id'=>'']); ?>'+id, function() {
						$("#modal-transaksi").modal('show');
						$("#modal-transaksi").on('hidden.bs.modal', function () {
							$("#modal-transaksi").hide();
							$("#modal-transaksi").remove();
				...
				openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalop/notAllowed','id'=>'']); ?>'+id,'modal-global-info');
			}
		},
		error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
	});
