...
$this->title = 'Approval Pengajuan Perubahan Jenis Kayu ';
            ...
            <div class="portlet-body">
                <ul class="nav nav-tabs">
                    <li class="<?php echo $status1;?>">
						<a href="<?= yii\helpers\Url::toRoute("/topmanagement/approvalperubahanjenis/index"); ?>"> <?= Yii::t('app', 'Not Confirmed'); ?> </a>
                    </li>
					<li class="<?php echo $status2;?>">
						<a href="<?= \yii\helpers\Url::toRoute("/topmanagement/approvalperubahanjenis/indexConfirmed") ?>"> <?= Yii::t('app', 'Confirmed'); ?> </a>
					</li>
                </ul>
                <div class="row">
                    <div class="col-md-12">
                            ...
                            <div class="portlet-body">
                                <table class="table table-striped table-bordered table-hover" id="table-master">
                                    <thead>
                                        <tr>
                                            <th></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Kode<br>Pengajuan') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Tanggal<br>Pengajuan') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Peruntukan') ?></th>
...
<?php $this->registerJs(" dtMaster(); setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Approval Pengajuan Perubahan Jenis Kayu'))."');", yii\web\View::POS_READY); ?>
<?php $url = \yii\helpers\Url::toRoute('/topmanagement/approvalperubahanjenis/'.$this->context->action->id); ?>
<script>
function dtMaster(){
...
function info(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvalperubahanjenis/info','id'=>'']) ?>'+id,'modal-master-info','90%'," $('#table-master').dataTable().fnClearTable(); ");
}
</script>