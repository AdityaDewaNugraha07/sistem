...
$this->title = 'Approval Adjustment Log';
            ...
            <div class="portlet-body">
                <ul class="nav nav-tabs">
                    <li class="<?php echo $status1;?>">
						<a href="<?= yii\helpers\Url::toRoute("/topmanagement/approvaladjustmentlog/index"); ?>"> <?= Yii::t('app', 'Not Confirmed'); ?> </a>
                    </li>
					<li class="<?php echo $status2;?>">
						<a href="<?= \yii\helpers\Url::toRoute("/topmanagement/approvaladjustmentlog/indexConfirmed") ?>"> <?= Yii::t('app', 'Confirmed'); ?> </a>
					</li>
                </ul>
                <div class="row">
                    <div class="col-md-12">
                            ...
                            <div class="portlet-body">
                                <table class="table table-striped table-bordered table-hover" id="table-master">
                                    <thead>
                                        <tr>
                                            <th></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Kode<br>Adjustment') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Tanggal<br>Berkas') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Kode Loglist') ?></th>
                                            <th style="line-height: 1;"><?= Yii::t('app', 'Kode SPK Shipping') ?></th>
...
<?php $this->registerJs(" dtMaster(); setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Approve Adjustment Log'))."');", yii\web\View::POS_READY); ?>
<?php $url = \yii\helpers\Url::toRoute('/topmanagement/approvaladjustmentlog/'.$this->context->action->id); ?>
<script>
function dtMaster(){
    var dt_table =  $('#table-master').dataTable({
        ...
        columnDefs: [
			...
			{	targets: 7, class: "td-kecil text-center"},
            {	targets: 8, class: "td-kecil text-align-center",
                render: function ( data, type, full, meta ) {
                    var ret = ' - ';
                    if(data=='APPROVED'){
                        ret = '<span class="label label-success" style="font-size: 11px;">'+data+'</span>';
                    }else if(data=='REJECTED' || data=='ABORTED'){
                        ret = '<span class="label label-danger" style="font-size: 11px;">'+data+'</span>';
                    }else if(data=='Not Confirmed'){
                        ret = '<span class="label label-default" style="font-size: 11px;">'+data+'</span>';
                    }
                    return ret;
                }
            },
...
function info(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/topmanagement/approvaladjustmentlog/info','id'=>'']) ?>'+id,'modal-master-info','95%'," $('#table-master').dataTable().fnClearTable(); ");
}
