                            ...
                            <div class="table-scrollable">
                                ...
                                for ($i = 0; $i < ($modReff->total_container); $i++) {
                                    if ($modReff->bundle_partition === true) { ?>
                                        ...
                                        $column = "";
                                        $thick_span = 'rowspan="2"';
                                        $width_span = 'rowspan="2"';
                                        $length_span = 'rowspan="2"';
                                        ...
                                        ?>
                                        <table class="table table-striped table-bordered table-advance table-hover table-contrainer"
                                               id="table-detail">
                                            <thead>
                                            <tr>
                                                ...
                                                <?= $column ?>
                                                <th <?= $thick_span ?> style="width: 80px; line-height: 1;"
                                                                       class="kolom-pcs">
                                                    Thick<br>(<?= $params_thick_unit ?>)
                                                </th>
                                                <th <?= $width_span ?> style="width: 80px; line-height: 1;"
                                                                       class="kolom-pcs">
                                                    Width<br>(<?= $params_width_unit ?>)
                                                </th>
                                                <th <?= $length_span ?> style="width: 80px; line-height: 1;"
                                                                        class="kolom-pcs">
                                                    Length<br>(<?= $params_length_unit ?>)
                                                </th>
                                                ...
                                                foreach ($modDetails as $v => $detail) {
                                                    ...
                                                    foreach ($params_thick as $vv => $thick) {
                                                        if (count($params_thick) > 1) {
                                                            ...
                                                            echo "<td style='text-align:right;'>" . DeltaFormatter::formatNumberForUserFloat($hmm['pcs']) . "</td>";
                                                            $subtot_pcs += $hmm['pcs'];
                                                            $subtot_volume += $hmm['volume'];
                                                        } else {
                                                            echo "<td style='text-align:right;' class='font-purple'>" . DeltaFormatter::formatNumberForUserFloat($thick) . "</td>";
                                                        }
                                                    }
                                                    foreach ($params_width as $vv => $width) {
                                                        if (count($params_width) > 1) {
                                                            ...
                                                            echo "<td style='text-align:right;' class='font-purple'>" . DeltaFormatter::formatNumberForUserFloat($width) . "</td>";
                                                        }
                                                    }
                                                    foreach ($params_length as $vv => $length) {
                                                        if (count($params_length) > 1) {
                                                            ...
                                                            echo "<td style='text-align:right;' class='font-purple'>" . DeltaFormatter::formatNumberForUserFloat($length) . "</td>";
                                                        }
                                                    }
                                                    echo "<td style='text-align:right;'>" . DeltaFormatter::formatNumberForUserFloat($subtot_pcs) . "</td>";
                                        ...
                                        <table class="table table-striped table-bordered table-advance table-hover table-contrainer"
                                            ...
                                            if (count($modDetail) > 0) {
                                                ...
                                                foreach ($modDetail as $ii => $detail) {
                                                    ...
                                                    echo "<td style='text-align:right;'>" . DeltaFormatter::formatNumberForUserFloat($detail['thick']) . "</td>";
                                                    echo "<td style='text-align:right;'>" . DeltaFormatter::formatNumberForUserFloat($detail['width']) . "</td>";
                                                    echo "<td style='text-align:right;'>" . DeltaFormatter::formatNumberForUserFloat($detail['length']) . "</td>";
                                                    echo "<td style='text-align:right;'>" . DeltaFormatter::formatNumberForUserFloat($detail['pcs']) . "</td>";
                                            ...
                                            <tr>
                                                ...
                                                <td style="text-align: right;"><?= DeltaFormatter::formatNumberForUserFloat($tot_pcs) ?></td>
...
<div class="modal-footer" style="text-align: center">
    ...
    <div id="place-button">
        ...
        $tampil = true;
        if($model->level !== 1) {
            $prev_approve = TApproval::findOne(['reff_no' => $model->reff_no, 'level' => $model->level - 1])->status;
            if($prev_approve === 'Not Confirmed') {
                $tampil = false;
            }
        }
        ...
        <?php else: ?>
            ...
            <p style="color: red; font-style: italic">*Proforma ini belum dapat di konfirmasi, karena orang yang <?= $model->level === 2 ? 'memeriksa' : 'menyetujui' ?> belum melakukan konfirmasi.</p>
            <?php endif ?>
        <?php endif ?>
    </div>
</div>
