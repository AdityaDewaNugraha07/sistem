...
//                    }
    ...
    public function actionSalesPrint(){
        ...
        if ($search != "") {
            $andWhere = " and (sales_kode ilike '%".$search."%' or sales_nm ilike '%".$search."%' or sales_phone ilike '%".$search."%' or active ilike '%".$search."%' )";
        } else {
            $andWhere = '';
        }
        
        $sql = "select * from m_sales where 1=1 ".$andWhere." ";
        $model = Yii::$app->db->createCommand($sql)->queryAll();
        $caraprint = Yii::$app->request->get('caraprint');
        $paramprint['judul'] = "Laporan Sales";
		if ($caraprint == 'PRINT') {
			return $this->render('/sales/print',['model'=>$model,'paramprint'=>$paramprint, 'search'=>$search]);
		} else if($caraprint == 'PDF') {
			$pdf = Yii::$app->pdf;
			$pdf->options = ['title' => $paramprint['judul']];
			$pdf->filename = $paramprint['judul'].'.pdf';
			$pdf->methods['SetHeader'] = ['Generated By: '.Yii::$app->user->getIdentity()->userProfile->fullname.'||Generated At: ' . date('d/m/Y H:i:s')];
			$pdf->content = $this->render('/sales/print',['model'=>$model,'paramprint'=>$paramprint]);
			return $pdf->render();
