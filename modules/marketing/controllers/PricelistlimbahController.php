...
class PricelistlimbahController extends DeltaBaseController
{
	...
	public function actionIndex(){
        ...
        if( Yii::$app->request->post('MHargaLimbah')){
            ...
            try {
                ...
                $success_3  = true;
                // var_dump($_POST);die;
                $kode       = trim(\app\components\DeltaGenerator::kodeHargaLimbah());
                $model->load(\Yii::$app->request->post());

                $cek_tanggal_terpakai = MHargaLimbah::find()->where([
                    'harga_tanggal_penetapan' => \app\components\DeltaFormatter::formatDateTimeForDb($_POST['harga_tanggal_penetapan'])
                ])->count();
                $cek_not_confirmed  = MHargaLimbah::find()->where([
                ...
                if($cek_tanggal_terpakai > 0) {
                    if($cek_not_confirmed > 0) {
                        foreach($_POST['MHargaLimbah'] as $i => $detailupdate) {
                            $modHarga   = MHargaLimbah::find()
                            ...
                            ->andWhere(['limbah_id' => $detailupdate['limbah_id']])
                            ->one();
                            $modHarga->harga_enduser = $detailupdate['harga_enduser'];
                            $modHarga->save();
                        } 
                    }else {
                    ...
                    foreach($_POST['MHargaLimbah'] as $i => $detailpost){
                        $modCurrentHarga = MHargaLimbah::find()->where([
                            'limbah_id'=>$detailpost['limbah_id'],
                            'harga_tanggal_penetapan'=>\app\components\DeltaFormatter::formatDateTimeForDb($_POST['harga_tanggal_penetapan'])
                        ])->one();
    ...
    public function actionGetContent(){
        if(\Yii::$app->request->isAjax){
            ...
            if($tipe == 'input') {
                $tanggal_terakhir   = MHargaLimbah::find()
            ...
            // if($tanggal != NULL) {
                ...
                //                     ->all();
                $MBrgLimbah = \app\models\MBrgLimbah::find()
                        ->where(['active' => true])
                        ->orderBy(['seq' => SORT_ASC])
                        ->all();
                foreach($MBrgLimbah as $i => $m) {
                    $MHargaLimbah               = $m->getHargaLimbah()
                                                ->andWhere(['harga_tanggal_penetapan' => \app\components\DeltaFormatter::formatDateTimeForDb($tanggal)])
                                                ->one();
                    $model = new MHargaLimbah();
                    $model->limbah_id           = $m->limbah_id;
                    $model->limbah_kode         = $m->limbah_kode;
                    $model->limbah_nama         = $m->limbah_nama;
                    $model->limbah_satuan_jual  = $m->limbah_satuan_jual;
                    $model->limbah_satuan_muat  = $m->limbah_satuan_muat;
                    $model->harga_enduser       = isset($MHargaLimbah->harga_enduser) ? $MHargaLimbah->harga_enduser : 0;
                    $model->harga_keterangan    = isset($MHargaLimbah->harga_keterangan) ? $MHargaLimbah->harga_keterangan : "";
                    $data['html'] .= $this->renderPartial('_content', compact('model', 'i', 'tipe'));
                }
            // }else {
		...
		if(\Yii::$app->request->isAjax){
            if( Yii::$app->request->post('deleteRecord')){
                ...
                try {
                    $success_1 = false;
                    $success_2 = false;
                    $limbah = MHargaLimbah::findOne(['harga_tanggal_penetapan' => $id]);
                    $delete = MHargaLimbah::deleteAll(['and', 'harga_tanggal_penetapan = :tanggal'], [':tanggal' => $id ]);
                    if($delete) $success_1 = true;

                    $approval = TApproval::deleteAll(['reff_no' => $limbah->kode]);
    ...
    public function actionGraf($id){
		if(\Yii::$app->request->isAjax){
            $MBrgLimbah     = \app\models\MBrgLimbah::findOne($id);
            $limbah_id      = $MBrgLimbah->limbah_id;
            $MHargaLimbah   = MHargaLimbah::find()->where(['limbah_id' => $limbah_id])->orderBy('harga_tanggal_penetapan')->all();
            return $this->renderAjax('_graf',['MBrgLimbah'=>$MBrgLimbah,'MHargaLimbah'=>$MHargaLimbah]);
        }
    }
