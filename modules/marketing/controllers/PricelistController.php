...
class PricelistController extends DeltaBaseController
{
    ...
    public function actionIndex(){
        ...
        $modHarga = new MHargaProduk();
        
        $xxx = MHargaProduk::find()->where(['kode' => $modHarga->kode])->orderBy(['harga_id' => SORT_DESC])->one();
    ...
    public function actionCreate(){
        ...
        $model->produk_group = $_GET['jp'] ?: 'Platform';
        ...
        return $this->render('create', compact('model', 'modHarga'));
    }
        ...
        if(Yii::$app->request->isAjax){
            ...
            if( Yii::$app->request->post('deleteRecord')){
                ...
                try {
                    ...
                    $produklist = [];
                    // m_brg_produk
                    $modProduk = MBrgProduk::find()->where(['produk_group'=>$jp])->all();
                    foreach($modProduk as $i => $produk){
                        $produklist[] = $produk->produk_id;
                    }
                    $delete = MHargaProduk::deleteAll(['and', 'harga_tanggal_penetapan = :tanggal', ['in', 'produk_id', $produklist]], [':tanggal' => $tp ]);
                    if($delete){
                        $success_1 = true;
                    }else{
	...
	public function actionHistory(){
        if(Yii::$app->request->get('dt')=='table-history'){
			$param['table']= MHargaProduk::tableName();
			$param['pk']= MHargaProduk::primaryKey()[0];
    ...
    public function actionGraf($id){
		if(Yii::$app->request->isAjax){
            $MBrgProduk = MBrgProduk::findOne($id);
            $produk_id = $MBrgProduk->produk_id;
            $MHargaProduk = MHargaProduk::find()->where(['produk_id' => $produk_id])->all();
