...
class JasaController extends DeltaBaseController
{
	...
	public function actionIndex(){
        if(\Yii::$app->request->get('dt')=='table-master'){
			...
			$param['column'] = ['produk_jasa_id','jenis','kode','nama','satuan','keterangan',$param['table'].'.active'];
	...
	public function actionCreate(){
		if(\Yii::$app->request->isAjax){
			$model = new \app\models\MProdukJasa();
			$model->active = true;
			if( Yii::$app->request->post('MProdukJasa')){
    ...
    public function actionInfo($id){
		if(\Yii::$app->request->isAjax){
			$model = \app\models\MProdukJasa::findOne($id);
			...
			if( Yii::$app->request->post('MProdukJasa')){
                ...
                try {
                    ...
                    if ($success_1) {
                        $transaction->commit();
                        $data['status'] = true;
                        $data['message'] = Yii::t('app', 'Data Jasa Berhasil Diupdate');
                    } else {
                        $transaction->rollback();
                        $data['status'] = false;
                        (!isset($data['message']) ? $data['message'] = Yii::t('app', \app\components\Params::DEFAULT_FAILED_TRANSACTION_MESSAGE) : '');
                ...
                try {
                    $success_1 = false;
                        if($model->delete()){
                            ...
                            $data['message'] = Yii::t('app', 'Data Jasa Gagal dihapus');
                        }
                        if ($success_1) {
                            $transaction->commit();
                            $data['status'] = true;
                            $data['message'] = Yii::t('app', '<i class="icon-check"></i> Data Jasa Berhasil Dihapus');
                        } else {
                            $transaction->rollback();
                            $data['status'] = false;
                            (!isset($data['message']) ? $data['message'] = Yii::t('app', \app\components\Params::DEFAULT_FAILED_TRANSACTION_MESSAGE) : '');
