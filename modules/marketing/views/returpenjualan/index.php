...
function setMeterKubik(ele){
	$(ele).each(function(i, e) {
		var p = unformatNumber( $(e).parents('tr').find('input[name*="[produk_p]"]').val() );
		var l = unformatNumber( $(e).parents('tr').find('input[name*="[produk_l]"]').val() );
		var t = unformatNumber( $(e).parents('tr').find('input[name*="[produk_t]"]').val() );
		var sat_p = $(e).parents('tr').find('input[name*="[produk_p_satuan]"]').val();
		var sat_l = $(e).parents('tr').find('input[name*="[produk_l_satuan]"]').val();
		var sat_t = $(e).parents('tr').find('input[name*="[produk_t_satuan]"]').val();
		var qty = unformatNumber( $(e).parents('tr').find('input[name*="[qty_kecil]"]').val() );
		var sat_p_m = 0;
		var sat_l_m = 0;
		var sat_t_m = 0;
		...
		setTimeout(function() {
			if( $(e).parents('tr').find("input[name*='[is_random]']").val() != 1 ){
				//$(e).parents('tr').find('input[name*="[kubikasi]"]').val( result );
							$(e).parents('tr').find('input[name*="[kubikasi]"]').val( formatNumberFixed4(result_display) );//perubahan pertanggal 19/05/2021 pembulatan 4 digit
				$(e).parents('tr').find('input[name*="[kubikasi_display]"]').val( formatNumberFixed4(result_display) );
				subTotal();
			}
		}, 300);	
	})
...
function subTotal(){
	...
	$("#table-detail tbody tr").each(function(){
		...
		var kubikasi = unformatNumber( $(this).find('input[name*="[kubikasi]"]').val() ); //perubahan pertanggal 19/05/2021 pembulatan 4 digit
                //var kubikasi = unformatNumber( $(this).find('input[name*="[kubikasi_display]"]').val() ); 
		var subtotal = 0;
		...
		$(this).find('input[name*="[subtotal]"]').val( formatNumberForUser(Math.round(subtotal)) ); //perubahan pertanggal 19/05/2021 pembulatan 4 digit
	});
	total();
}
...
function total(){
	...
	$("#<?= yii\bootstrap\Html::getInputId($model, "total_retur") ?>").val( formatNumberForUser(Math.round(total_retur)) ); // perubahan per tanggal 19/05/2022
}
