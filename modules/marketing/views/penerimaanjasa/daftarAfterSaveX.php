...
$sql_user_group_id = "select user_group_id from m_user where user_id = ".$user_id."";
$user_group_id = Yii::$app->db->createCommand($sql_user_group_id)->queryScalar();
?>
...
function dtAfterSaveX(){
    var dt_table =  $('#table-aftersavex').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/marketing/penerimaanjasa/daftarAfterSaveX') ?>',data:{dt: 'modal-aftersavex'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
