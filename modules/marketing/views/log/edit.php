...
?>
<div class="modal fade draggable-modal" id="modal-log-edit" tabindex="-1" role="basic" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="btn btn-icon-only btn-default fa fa-close" data-dismiss="modal" aria-hidden="true" style="float: right; padding-top: 2px; padding-bottom: 2px;"></button>
                <h4 class="modal-title"><?= Yii::t('app', 'Edit Data Log'); ?></h4>
            </div>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
                        <?= $form->field($model, 'log_kelompok')->dropDownList(\app\models\MKayu::getOptionListGroupKayu(),['class'=>'form-control bs-select', 'onchange'=>'setLogKelompok()']) ?>
                        <div class="place-kayu" style="display: none;">
                            <?= $form->field($model, 'kayu_id')->dropDownList([],['class'=>'form-control', 'onchange'=>'setKodeLog();setNamaLog();']) ?>
                        </div>
                        ...
                        <?= $form->field($model, 'log_kode')->textInput(); ?>
                        <?= $form->field($model, 'log_nama',['template'=>'{label}<div class="col-md-8">{input} {error}</div>'])->textInput(); ?>
                    </div>
                    <div class="col-md-6">
                        <?= $form->field($model, 'log_keterangan')->textarea(); ?>
                        <?php 
                                ...
                                <div class="col-md-8">
                                    <div class="fileinput fileinput-new" data-provides="fileinput">
                                        <div class="fileinput-new thumbnail">
                                            <img src="'.(!empty($model->log_gambar)?
                                            \yii\helpers\Url::base().'/uploads/gud/log/'.$model->log_gambar :
...
function setDropdownKayu(callback=null){
	var log_kelompok = $("#<?= yii\bootstrap\Html::getInputId($model, "log_kelompok") ?>").val();
    var kayuIdDropdown = $("#<?= \yii\bootstrap\Html::getInputId($model, 'kayu_id') ?>");
    var kayuIdValue = '<?= $model->kayu_id ?>';
	kayuIdDropdown.html("");
	$.ajax({
		...
		success: function (data) {
			if(data.html){
                kayuIdDropdown.html(data.html);
                kayuIdDropdown.val(kayuIdValue);
                kayuIdDropdown.trigger('change');
...
function save() {
	var $form = $('#form-log-edit');
	if (formrequiredvalidate($form)) {
		if(validateData()){
            submitformajax($form,"$(\'#modal-log-edit\').modal(\'hide\'); $(\'#table-log\').dataTable().fnClearTable();")
...
function validateData(){
    var has_error = 0;
    var range_awal = unformatNumber($('#form-log-edit').find('input[name*="[range_awal]"]').val());
    var range_akhir = unformatNumber($('#form-log-edit').find('input[name*="[range_akhir]"]').val());
    var log_kode = $('#form-log-edit').find('input[name*="[log_kode]"]').val();

    if(range_awal <= 0){
        has_error = has_error + 1;
        $('#form-log-edit').find('input[name*="[range_awal]"]').addClass('error-tb-detail');
    } else {
        $('#form-log-edit').find('input[name*="[range_awal]"]').removeClass('error-tb-detail');
    }

    if(range_akhir <= 0){
        has_error = has_error + 1;
        $('#form-log-edit').find('input[name*="[range_akhir]"]').addClass('error-tb-detail');
    } else {
        $('#form-log-edit').find('input[name*="[range_akhir]"]').removeClass('error-tb-detail');
    }

    if(!log_kode){
        has_error = has_error + 1;
        $('#form-log-edit').find('input[name*="[log_kode]"]').addClass('error-tb-detail');
    } else {
        $('#form-log-edit').find('input[name*="[log_kode]"]').removeClass('error-tb-detail');
    }
