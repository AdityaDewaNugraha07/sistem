						...
						<td colspan="3" style="padding: 0px;">
							<table style="width: 100%" id="table-detail">
								...
								<tr style="border-bottom: solid 1px #000;">
                                    <?php if( $model->opKo->jenis_produk == "Limbah" || $model->opKo->jenis_produk == "Log" ){ ?>
									...
									<tr>
										<td style="padding: 2px 5px; border-right: 1px solid black;">
											...
											if($model->opKo->jenis_produk == "Limbah") {
												$produk = $modDetail[$i]->limbah->limbah_kode." - (".$modDetail[$i]->limbah->limbah_produk_jenis.") ".$modDetail[$i]->limbah->limbah_nama;
											} else if ($model->opKo->jenis_produk == "JasaKD" || $model->opKo->jenis_produk == "JasaGesek" || $model->opKo->jenis_produk == "JasaMoulding"){
												$produk = $modDetail[$i]->produkJasa->nama;
											} else if ($model->opKo->jenis_produk == "Log"){
                                    			$modKayu = app\models\MKayu::findOne($modDetail[$i]->log->kayu_id);
												$produk = $modDetail[$i]->log->log_kelompok . '-' . $modKayu->kayu_nama . ' (' . $modDetail[$i]->log->range_awal .' cm - ' . $modDetail[$i]->log->range_akhir .' cm)';
											} else {
												$produk = $modDetail[$i]->produk->produk_nama;
											}
											echo $produk;
											?>
										</td>
										...
										<td style="padding: 2px 5px; border-right: solid 1px #000;">
											<span style="float: right">
												<?= ($model->opKo->jenis_produk=="Limbah")? ( ($modDetail[$i]->satuan_kecil=="Rit")?$modDetail[$i]->satuan_besar:"" ) : ($model->opKo->jenis_produk=="Log")?number_format($modDetail[$i]->kubikasi,2):number_format($modDetail[$i]->kubikasi,4); ?>
											</span> 
										</td>
										<td style="padding: 2px 5px; border-right: solid 1px #000;">
