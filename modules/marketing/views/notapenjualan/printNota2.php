    ...
    <tr>
        <td colspan="3" style="padding: 0px;">
            <table style="width: 100%" id="table-detail">
                ...
                <?php for ($i = 0; $i < $max; $i++) {
                    if (count($modDetail) >= ($i + 1)) {
                        ...
                        if ($modDetail[$i]->notaPenjualan->jenis_produk == "Log"){
                            ...
                            $range_akhir = $modBrgLog['range_akhir'] >= 200?' UP': ' - '.$modBrgLog['range_akhir'] .'cm';
                            $logNama = $modKayu['group_kayu'] .'-'.$modKayu['kayu_nama'].'<br>('. $modBrgLog['range_awal'] .'cm'.$range_akhir.')';
                            // jika log fsc100 tampilkan nama ilmiah
							if($modBrgLog && stripos($modBrgLog->log_nama, 'FSC100') !== false){
                                $logNama = $modKayu['group_kayu'] .'-'.$modKayu['kayu_nama'].'<br><b>FSC</b>100%<br>('. $modBrgLog['range_awal'] .'cm - ' . $modBrgLog['range_akhir'] .'cm)';
                ...
                <?php if ($model->jenis_produk == "Log"){ ?>
                    <td colspan="13" class="text-align-right" style="padding: 5px; border-right: solid 1px #000;"><b>Total
                            Bayar</b> &nbsp;
                    </td>
                <?php } else { ?>
                    <td colspan="5" class="text-align-right" style="padding: 5px; border-right: solid 1px #000;"><b>Total
                            Bayar</b> &nbsp;
                    </td>
                <?php } ?>
                    <td class="text-align-right" style="padding: 5px;"><b>
