                            ...
                            <div class="portlet-body">
                                <div class="row">
									...
									<div class="col-md-6">
                                        ...
                                        <div id="place-plafond" style="display: <?= (!isset($_GET['po_ko_id'])?"none": (($model->sistem_bayar!="Tempo")?"none":"") ) ?>;">
											<?php 
											if(isset($_GET['po_ko_id'])){
												$model = TPoKo::findOne($_GET['po_ko_id']);
												if($model->data_piutang){
													$data_piutang = \yii\helpers\Json::decode($model->data_piutang);
													$model->maks_plafon = app\components\DeltaFormatter::formatNumberForUserFloat($data_piutang[0]['maks_plafon']);
													$model->sisa_piutang = app\components\DeltaFormatter::formatNumberForUserFloat($data_piutang[0]['piutang_aktif']);
													$model->op_aktif = app\components\DeltaFormatter::formatNumberForUserFloat($data_piutang[0]['op_aktif']);
													...
													$model->sisa_plafon = $is_negative ? '-' . $sisa_plafon : $sisa_plafon;
												}
											}
											?>
											<div class="form-group" style="margin-bottom: 5px;">
												...
												<div class="col-md-5">
													<?php echo \yii\bootstrap\Html::activeTextInput($model, "maks_plafon",['class'=>'form-control float','disabled'=>true]);?>
												</div>
											</div>
											<div class="form-group" style="margin-bottom: 5px;">
												<label class="col-md-5 control-label"><?= Yii::t('app', 'Piutang Aktif'); ?></label>
												<div class="col-md-5">
													<?php echo \yii\bootstrap\Html::activeTextInput($model, "sisa_piutang",['class'=>'form-control float','disabled'=>true]) ?>
												</div>
											</div>
											<div class="form-group" style="margin-bottom: 5px;">
												<label class="col-md-5 control-label"><?= Yii::t('app', 'OP Aktif'); ?></label>
												<div class="col-md-5">
													<?php echo \yii\bootstrap\Html::activeTextInput($model, "op_aktif",['class'=>'form-control float','disabled'=>true]) ?>
												</div>
											</div>
											<div class="form-group" style="margin-bottom: 5px;">
												<label class="col-md-5 control-label"><?= Yii::t('app', 'Sisa Plafon'); ?></label>
												<div class="col-md-5">
													<?php echo \yii\bootstrap\Html::activeTextInput($model, "sisa_plafon",['class'=>'form-control float','disabled'=>true,'style'=>'font-weight:800']) ?>
												</div>
											</div>
										</div>
										...
										if(isset($_GET['po_ko_id'])){
											$model = TPoKo::findOne($_GET['po_ko_id']);
											$model->tanggal_bayarmax= app\components\DeltaFormatter::formatDateTimeForUser2($model->tanggal_bayarmax);  
										}
										echo $form->field($model, 'tanggal_bayarmax',[
																	'template'=>'{label}<div class="col-md-7"><div class="input-group input-medium date date-picker bs-datetime">{input} <span class="input-group-addon">
																	<button class="btn default" type="button" style="margin-left: 0px;"><i class="fa fa-calendar"></i></button></span></div> 
																	{error}</div>'])->textInput(['readonly'=>'readonly']); ?>
                                ...
                                <div class="row" id="detail-order" style="margin-top: -20px; margin-bottom: -20px;">
                                    <div class="col-md-12">
										...
										<span style="color: red;">
											**Jika produk alias <b>dicentang</b> maka nama produk alias akan diteruskan dari <b>PO sampai INVOICE</b>.
											Jika produk alias <b>tidak dicentang</b> maka nama produk alias hanya akan tampil di <b>PO saja</b>.
										</span>
                                    </div>
                                    <div class="col-md-12">
											...
											<table class="table table-striped table-bordered table-advance table-hover" style="width: 90%" id="table-detail">
												<thead>
													<tr>
														...
														<th style="width: 100px; line-height: 0.9; padding: 5px; font-size: 1.3rem;"><?= Yii::t('app', 'Range<br>Diameter'); ?></th>
                                                        <th style="width: 150px; line-height: 0.9; padding: 5px; font-size: 1.3rem;"><?= Yii::t('app', 'Diameter<br>Alias'); ?></th>
														<th style="width: 50px; line-height: 0.9; padding: 5px; font-size: 1.3rem;"><?= Yii::t('app', 'FSC'); ?></th>
														...
														<th style="width: 150px; line-height: 0.9; padding: 5px; font-size: 1.3rem;"><?= Yii::t('app', 'Subtotal'); ?></th>
												...
												<tfoot>
													<tr>
														...
														<?php } else { ?>
															...
															<td></td>
															<td style="vertical-align: middle; text-align: right;">
																<span id="place-total_harga">0</span>
															</td>
														<?php } ?>
													</tr>
												</tfoot>
...
function setTopHari(){
	...
	var id = '<?= isset($_GET['po_ko_id'])?$_GET['po_ko_id']:''; ?>';
    $("#<?= yii\bootstrap\Html::getInputId($model, "maks_plafon") ?>").val('0');
	$("#<?= yii\bootstrap\Html::getInputId($model, "op_aktif") ?>").val('0');
	$("#<?= yii\bootstrap\Html::getInputId($model, "sisa_piutang") ?>").val('0');
	$("#<?= yii\bootstrap\Html::getInputId($model, "sisa_plafon") ?>").val('0');

	if(cust_id){
		$.ajax({
			url    : '<?= \yii\helpers\Url::toRoute(['/marketing/purchaseorder/setTopHari']); ?>',
			type   : 'POST',
			data   : {cust_id:cust_id, jns_produk:jns_produk, id:id},
			success: function (data) {
				if(data.cust){
					if(data.top_hari){
					...
					$('#<?= \yii\bootstrap\Html::getInputId($model, "maks_plafon") ?>').val(formatNumberForUser(data.maks_plafon));
					$('#<?= \yii\bootstrap\Html::getInputId($model, "sisa_piutang") ?>').val(formatNumberForUser(data.sisa_piutang));
					$('#<?= \yii\bootstrap\Html::getInputId($model, "op_aktif") ?>').val(formatNumberForUser(data.op_aktif));
					$('#<?= \yii\bootstrap\Html::getInputId($model, "sisa_plafon") ?>').val(formatNumberForUser(data.sisa_plafon));
				}
			},
			error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
		});
...
function hitungTotal(){
	var komposisi = 0; var kubikasi = 0; var total_harga = 0;
	$('#table-detail tbody > tr').each(function(){
	...
	<?php } else{ ?>
		...
		$('#place-total_harga').text(formatNumberForUser(total_harga));
	<?php } ?>
...
function afterSave(id){
	...
	<?php if(isset($_GET['edit'])){ ?>
		...
		$('#<?= \yii\bootstrap\Html::getInputId($model, "tanggal_bayarmax") ?>').prop("disabled", false);
...
function setDDProdukIdAlias(ele){
	...
	var range_diameter = $(ele).closest('tr').find('select[name*="[range_diameter]"]').val();
	var fsc = $(ele).closest('tr').find('input[name*="[fsc]"]').is(":checked");

	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/marketing/purchaseorder/setDDProdukIdAlias']); ?>',
        type   : 'POST',
        data   : {id: id, jns_produk:jns_produk, range_diameter:range_diameter, fsc:fsc},
        success: function (data){ 
			if(data.html){
                ...
                $(ele).find('.select2-selection').css('font-size','1.2rem');
                $(ele).find('.select2-selection').css('padding-left','5px');
                $(ele).find(".tooltips").tooltip({ delay: 50 });
                
                if(data.produk_ids){
                    var produk_ids = data.produk_ids.join(',');
                    var produk_id_split = produk_ids.split(',');
                    $(ele).closest('tr').find('select[name*="[produk_id_alias]"]').val( produk_id_split ).trigger('change');
			}
			}
        },
        error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
...
function reordertables(obj_table){
    ...
    $(obj_table + ' > tbody > tr').each(function(){
        ...
        $(this).find('input, select, textarea').each(function(){
            ...
            if (isArray) name = name.slice(0, -2); // hapus []

            var nameClean = name.replace(/]/g, "");
            var parts = nameClean.split("[");

            if (parts.length >= 3) {
                parts[1] = row; // ganti index
                var newName = parts[0] + "[" + parts[1] + "][" + parts[2] + "]";
                if (parts.length === 4) {
                    newName += "[" + parts[3] + "]";
                }
                if (isArray) newName += "[]";
                ...
                var newId = parts[0] + "_" + parts[1] + "_" + parts[2];
                if (parts.length === 4) {
                    newId += "_" + parts[3];
                }
                $(this).attr("id", newId);
...
function setDiameterAlias(ele){
	var range_diameter = $(ele).val();
	$(ele).closest('tr').find('input[name*="[diameter_alias]"]').val(range_diameter);
	
	if(range_diameter){
		$(ele).closest('tr').find('select[name*="[produk_id_alias]"]').prop("disabled", false);
	} else {
		$(ele).closest('tr').find('select[name*="[produk_id_alias]"]').prop("disabled", true);
	}
