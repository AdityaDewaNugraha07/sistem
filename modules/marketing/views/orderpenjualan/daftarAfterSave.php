...
function dtAfterSave(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/marketing/orderpenjualan/daftarAfterSave') ?>',data:{dt: 'modal-aftersave'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
			...
			{	targets: 8,  searchable:false,
				render: function ( data, type, full, meta ) {
                    ...
                    dua = full[11].search("(2)");
                    tiga = full[11].search("(3)");
					return ret;
                }
			},
			...
			{ targets: 11,
				render: function ( data, type, full, meta ) {
                    ...
                    if (full[13] == "APPROVED") {
                        ret = '<span class="label label-success" style="font-size:1.1rem;">'+full[13]+'</span>';
                    } else if (full[13] == "REJECTED") {
                        ret = '<span class="label label-danger" style="font-size:1.1rem;">'+full[13]+'</span>';
                    } else if (full[13] == "Not Confirmed") {
                        ret = '<span class="label label-default" style="font-size:1.1rem;">'+full[13]+'</span>';
                    } else {
                        ret = '';
                    }
					return ret;
			...
			{ targets: 12,
				render: function ( data, type, full, meta ) {
                    ...
                    if (full[14] == "APPROVED") {
                        ret = '<span class="label label-success" style="font-size:1.1rem;">'+full[14]+'</span>';
                    } else if (full[14] == "REJECTED") {
                        ret = '<span class="label label-danger" style="font-size:1.1rem;">'+full[14]+'</span>';
                    } else if (full[14] == "Not Confirmed") {
                        ret = '<span class="label label-default" style="font-size:1.1rem;">'+full[14]+'</span>';
                    } else {
                        ret = '';
                    }                    
                    return ret;
			...
			{ targets: 13,
				render: function ( data, type, full, meta ) {
                    ...
                    if (full[15] == "APPROVED") {
                        ret = '<span class="label label-success" style="font-size:1.1rem;">'+full[15]+'</span>';
                    } else if (full[15] == "REJECTED") {
                        ret = '<span class="label label-danger" style="font-size:1.1rem;">'+full[15]+'</span>';
                    } else if (full[15] == "Not Confirmed") {
                        ret = '<span class="label label-default" style="font-size:1.1rem;">'+full[15]+'</span>';
                    } else {
                        ret = '';
                    }                    
					return ret;
