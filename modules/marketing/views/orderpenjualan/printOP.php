    ...
    if($model->jenis_produk !== 'Log'){
        ...
        foreach($modDetail as $i => $detail){
			...
			$modAlias = yii::$app->db->createCommand("SELECT t_po_ko_detail.po_ko_id, t_po_ko_detail.alias
                                                    ...
                                                    LEFT JOIN m_brg_log ON m_brg_log.log_id = t_op_ko_detail.produk_id  
                                                    WHERE t_op_ko_detail.op_ko_detail_id={$detail['op_ko_detail_id']} AND
			...
			if(empty($modAlias)){
				...
				if ($modAlias['alias'] == true) {
				    ...
				    $modPo = Yii::$app->db->createCommand("
										SELECT t_po_ko_detail.alias, t_po_ko_detail.produk_alias, diameter_alias, SUM(t_op_ko_detail.kubikasi) AS kubikasi, 
                                        harga_jual,t_op_ko_detail.keterangan
                                        FROM t_op_ko_detail 
                                        JOIN t_op_ko ON t_op_ko.op_ko_id = t_op_ko_detail.op_ko_id
                                        JOIN t_po_ko_detail ON t_po_ko_detail.po_ko_id = t_op_ko.po_ko_id 
                                                AND t_po_ko_detail.alias = true 	
                                                AND t_op_ko_detail.produk_id = ANY(string_to_array(t_po_ko_detail.produk_id_alias, ',')::int[])
                                        WHERE t_op_ko.op_ko_id = $model->op_ko_id
                                        GROUP BY t_po_ko_detail.alias, t_po_ko_detail.produk_alias, diameter_alias, harga_jual, t_op_ko_detail.keterangan
								")->queryAll();
        ...
        foreach ($produkData as $data) {?>
            <tr>
				<td colspan="4" style="padding: 2px 5px; vertical-align: top;" class="l">
                    <?= $data['produk']; ?>
                </td>
                <td colspan="2" style="padding: 2px 5px; text-align: center; vertical-align: top;" class="l"></td>
                <td colspan="2" style="padding: 2px 5px;  vertical-align: top;" class="l">
                    <span style="float: right;">M3</span>
                </td>
                <td colspan="3" style="padding: 2px 5px;  vertical-align: top;" class="l">
                    <span style="float: right">
                        <?= \app\components\DeltaFormatter::formatNumberForUserFloat($data['kubikasi']); ?>
                    </span>
                </td>
                <td style="padding: 2px 5px;  vertical-align: top;" class="l r">
                    <span style="float: left">Rp. </span>
                    <span style="float: right"><?= \app\components\DeltaFormatter::formatNumberForUserFloat($data['harga_jual']) ?></span>
                </td>
                    <td style="padding: 2px 5px;  vertical-align: top;" class="r">
