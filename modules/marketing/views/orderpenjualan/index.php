                            ...
                            <div class="portlet-body">
                                <div class="row">
                                    <div class="col-md-5">
										...
										<?= $form->field($model, 'jenis_produk')->dropDownList(\app\models\MDefaultValue::getOptionList('jenis-produk'),['onchange'=>'openPoFamily();']); ?>
										
										<?php if(!isset($_GET['op_ko_id']) || isset($_GET['edit'])){ ?>
										<div class="form-group" id="place-tarik_po"> <!-- style="visibility: hidden" -->
											<label class="col-md-5 control-label">Kode PO Customer</label>
											<div class="col-md-7" style="padding-bottom: 5px;">
												<span class="input-group-btn" style="width: 90%">
												<?= \yii\bootstrap\Html::activeDropDownList($model, 'po_ko_id', \app\models\TPoKo::getOptionList(),['class'=>'form-control select2', 'prompt'=>'','onchange'=>'setTarikPO();']); ?>
												</span>
												<span class="input-group-btn" style="width: 10%">
													<a class="btn btn-icon-only btn-default tooltips" onclick="pickPO();" data-original-title="Lihat Kode PO Customer" style="margin-left: 3px; border-radius: 4px;">
										...
										<?php } else { 
											if($model->jenis_produk == 'Log'){ ?>
												<?= \yii\bootstrap\Html::activeHiddenInput($model, "po_ko_id") ?>
												<?= $form->field($model, 'kode_po_ko')->textInput(['readonly'=>true])->label('Kode PO Customer'); ?>
										<?php }} ?>
										...
										<div id="po_family" style="display: none;">
											<?= $form->field($model, 'po')->textInput(); ?>
									...
									<div class="col-md-6">
										...
										<div class="form-group" style="margin-bottom: 5px;">
											...
											<div class="col-md-7">
												...
												<span class="input-group-btn" style="width: 25%">
													<a class="btn btn-icon-only btn-default tooltips" onclick="pickCustomer();"  id="btn-icon-cust" data-original-title="Lihat Master Customer" style="margin-left: 3px; border-radius: 4px;"><i class="fa fa-list"></i></a>
												</span>
											</div>
										</div>
										<?php }else{ ?>
										...
										<?php } ?>
                                        <?= $form->field($model, 'cust_alamat')->textarea(['style'=>'width:90%', 'readonly' => 'readonly']); ?>
										...
										<div id="place-plafond" style="display: <?= (!isset($_GET['op_ko_id'])?"none": (($model->sistem_bayar!="Tempo")?"none":"") ) ?>;">
											...
											<div class="form-group" style="margin-bottom: 5px;">
												...
												<div class="col-md-5">
													<?php 
													if(isset($_GET['op_ko_id'])){
														$tempo = TTempobayarKo::findOne(['op_ko_id'=>$_GET['op_ko_id']]);
														if($tempo){
															$modTempo->op_aktif = \app\components\DeltaFormatter::formatNumberForUserFloat($tempo->op_aktif);
														}
													}
													?>
										...
										<div id="place-tarikdata" style="display: none;">
											<label class="col-md-5 control-label" for="topko-tarik_data">OP Peruntukan</label>
											<div class="col-md-5">
												<select id="topko-tarik_data" class="form-control" name="TOpKo[tarik_data]" style="width:100%;" onchange="setTarikData();">
                                    ...
                                    <div class="form-group">
                                        <label class="col-md-2 control-label">Status Approval</label>
                                        <div class="col-md-8">
                                            <?php
                                            if (isset($_GET['op_ko_id'])) {
                                                ...
                                                if(count($approves)>0){
                                                    foreach($approves as $i => $approve){
                                                        ...
                                                        echo \yii\helpers\Html::button( Yii::t('app', 'by : '.$approval.'<br>reason : '.$approve['reason'].'<br>tanggal : '.\app\components\DeltaFormatter::formatDateTimeForUser2($approve['at'])),['class'=>'btn green btn-outline ciptana-spin-btn pull-left text-left', 'style' => 'text-align: left; font-size: 10px;margin-top: 10px; margin-right: 10px;']);
                                                    }
                                                }
                                ...
                                <?php
                                // DETAIL ORDER AWAL
                                // JIKA $model->jenis_produk adalah JasaKD, JasaGesek, atau JasaMoulding maka bandingkan dengan detail order awal
                                // 
                                if ($beda > 0) {
                                ?>                                
                                <hr>
                                            ...
                                            <table class="table table-striped table-bordered table-advance table-hover" style="width: 90%;" id="table-detail-awal">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th rowspan="2" style="width: 50px; line-height: 0.9; padding: 5px; font-size: 1.3rem;"></th>
													</tr>
													<tr>
														...
														<th class="place-satuan-log" style="font-size: 1.2rem; line-height: 0.9; width: 50px; display: none;"><?= Yii::t('app', '-'); ?></th>
                                                        <th class="place-satuan-log" style="font-size: 1.2rem; line-height: 0.9; width: 130px; display: none;"><?= Yii::t('app', 'Satuan<br>Beli'); ?></th>
                                                        <th class="place-satuan-log" style="font-size: 1.2rem; line-height: 0.9; width: 70px; display: none;"><?= Yii::t('app', 'M<sup>3</sup>'); ?></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <?php
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="td-kecil text-left"><?php echo $produk_nama;?></td>
                                                        <td class="td-kecil text-center"><?php echo $qty_besar;?></td>
                                                        <td class="td-kecil text-right"><?php echo $qty_kecil;?> (pcs)</td>
                                                        <td class="td-kecil text-right"><?php echo $kubikasi;?></td>
                                                        <td class="td-kecil"></td>
                                                        <td class="td-kecil text-right"><?php echo $harga_jual;?></td>
                                                        <td class="td-kecil text-right"><?php echo $subtotal;?></td>
                                                        <td class="td-kecil"></td>
                                                    </tr>
                                                    <?php
                                                        $i++;
                                ...
                                <div class="row" id="detail-order" style="margin-top: -20px; margin-bottom: -20px;">
									...
									<div class="col-md-6" style="margin-top: -15px; margin-bottom: 10px; float:right;" id='log-tarik-data'>
									<?php if(!isset($_GET['op_ko_id'])){ ?>
										<?php echo $form->field($model, 'terima_logalam_id')->dropDownList([],['class'=>'form-control select2', 'id' => 'dropdown-logalam-id', 'prompt'=>'Tarik Data Log', 'onchange'=>'setItemLogPelabuhan()'])->label(''); ?>
									<?php } ?>
									</div>
                                ...
                                ?>
                                <hr>
                                <div class="row" id="place-table-terima-jasa" style="margin-top: -10px; margin-bottom: -10px;">
                                    <div class="col-md-12">
											...
											<table class="table table-striped table-bordered table-advance table-hover" style="width: 90%" id="table-terima-jasa">
												<thead>
													<tr>
														...
														<th rowspan="2" style="width: 120px; line-height: 0.9; padding: 5px; font-size: 1.2rem; width: 50px;"><?= Yii::t('app', 'No. Palet'); ?></th>
														...
														<th rowspan="2" style="width: 60px; line-height: 0.9; font-size: 1.2rem; text-align: right;"><?= Yii::t('app', 'Ket'); ?></th>
														<th rowspan="2" style="width: 35px; line-height: 0.9; font-size: 1.2rem;"></th>
													</tr>
                                                    <tr>
                                                        <th style="width: 80px; line-height: 0.9; font-size: 1.2rem;"><?= "T" ?></th>
                                                        <th style="width: 80px; line-height: 0.9; font-size: 1.2rem;"><?= "L" ?></th>
                                                        <th style="width: 80px; line-height: 0.9; font-size: 1.2rem;"><?= "P" ?></th>
		                        ...
		                        <div class="form-actions pull-right col-md-12 row">
		                            <div class="col-md-12 right">
										<div class="col-md-6">
											...
											// if(!isset($_GET['op_ko_id'])){
											// 	$label_status = "Verify Status";
											// } else {
											// 	if($model->status == null){
											// 		$label_status = "Status";
											// 	} else {
											// 		$label_status = "Status : ".$model->status;
											// 	}
											// }
											?>
											<style>
...
<?php $this->registerJs(" 
	...
	$('#form-hapus').submit( function(e){
		...
		$.ajax({
			url    : '".\yii\helpers\Url::toRoute(['/marketing/orderpenjualan/hapusfile'])."',
			type   : 'POST',
			data   : formData,
			success: function (data) {
				if(data){
					$(data).each(function(){
						$('#loading').show();
						$('#xxx').load('".\yii\helpers\Url::toRoute(['/marketing/orderpenjualan/xxx'])." div#yyy', {}, function () {
							if ($('#beres').length) {
	...
	openPoFamily();
	$('#log-tarik-data').css('display','none');
	// $('select[name*=\"[terima_logalam_id]\"]').select2({
	// 	allowClear: !0,
	...
	$('select[name*=\"[po_ko_id]\"]').select2({
		allowClear: !0,
		placeholder: 'Ketik Kode PO Customer',
...
function index(){
	...
	$.ajax({
		url: '<?php \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/index']) ?>',
		method: 'GET',
		data: { cust_id: cust_id },
		success: function(data) {
			
			if(cust_id != null){
				$('#dropdown-logalam-id').prop("disabled",false);
			}else{
				$('#dropdown-logalam-id').prop("disabled",true);
			}
		},
		function (jqXHR) { getdefaultajaxerrorresponse(jqXHR);
		}
	});
	if(tarik_data == "Pelabuhan"){
		ambilDataOptions(cust_id);
	}
}

function ambilDataOptions(cust_id) {
    $.ajax({
        url: '<?= \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/getLogPelabuhanOptions']) ?>',
        method: 'GET',
        data: { cust_id: cust_id },
		dataType: 'json',
        success: function(data) {
            // var $dropdown = $('#<?php //echo \yii\bootstrap\Html::getInputId($model, "terima_logalam_id") ?>');
			var $dropdown = $('#dropdown-logalam-id');
            $dropdown.empty(); // Clear existing options
            $dropdown.append($('<option/>').val('').text('Tarik Data Log'));
...
//	if (jp == "JasaKD" || jp == "JasaGesek" || jp == "JasaMoulding" || jp == "Plywood" || jp == "Platform" || jp == "Lamineboard" || jp == "Sawntimber" || jp == "Veneer") {
	...
	if(jp == "Log"){
		...
		$('#place-tarik_po').css('display','none');
		$("#<?= \yii\bootstrap\Html::getInputId($model, "po_ko_id") ?>").val('').trigger('change');
		$('#<?= \yii\bootstrap\Html::getInputId($model, "cust_id") ?>').prop("disabled", false);
		<?php if(!isset($_GET['op_ko_id'])){ ?>
			$("#<?= \yii\bootstrap\Html::getInputId($model, "sales_id") ?>").val('').trigger('change');
			$('#<?= \yii\bootstrap\Html::getInputId($model, "cara_bayar") ?>').val('Transfer Bank');
		<?php } ?>
		
		$('#btn-icon-cust').css('pointer-events', '');
		<?php if(!isset($_GET['op_ko_id'])){ ?> 
		$("#place-attch").empty();
		for (var i = 1; i <= 6; i++) {
			var place_attach = `
				<div class="col-md-2">
					<div class="fileinput fileinput-new" data-provides="fileinput">
						<div class="fileinput-new thumbnail">
							<img src="<?= Yii::$app->view->theme->baseUrl ?>/cis/img/no-image.png" alt="" />
						</div>
						<div class="fileinput-preview fileinput-exists thumbnail"></div>
						<div>
							<span class="btn btn-xs blue-hoki btn-outline btn-file">
			...
			`;
			$("#place-attch").append(place_attach);
		}
		<?php } ?>
	}	
}
...
function setCustomer(){
	var cust_id = $('#<?= yii\bootstrap\Html::getInputId($model, "cust_id") ?>').val();
	var tarik_data = $("#<?= yii\bootstrap\Html::getInputId($model, "tarik_data") ?>").val("Pabrik");
	$.ajax({
...
function setVerify(){
	<?php if(!isset($_GET['op_ko_id'])){ ?>
		var terima_logalam_id = $('#dropdown-logalam-id').val();
	<?php } else { ?>
	...
	if(sistem_bayar == "Tempo"){
		...
		// console.log(parseInt(maks_top_hari *1));
	...
	$('#table-detail > tbody > tr').each(function(){
		var harga_jual = unformatNumber( $(this).find('input[name*="[harga_jual]"]').val() );
		var harga_jual_lama = unformatNumber( $(this).find('input[name*="[harga_jual_lama]"]').val() );
		// var harga_jual_lama = unformatNumber( $(this).find('.harga_jual_lama').val() );
		...
		<?php } ?>
		<?php if ( (isset($_GET['edit'])) || (!isset($_GET['op_ko_id']))){ ?> // edit || index - awal
			...
			if (harga_jual < harga_jual_lama) {
				$(id).val('low3'); //kebijakan hasil meeting per tanggal 25 september 2021 jika penjualan dibawah harga approval sampai direksi untuk semua produk 
	...
	//			}
			...
			// console.log('oke if');
		<?php } else { ?>
			var val = $(this).find('input[name*="[status_harga]"]').val();

			total_i++;
			if(val) {
				list_harga_status.push(val.split(' ')[0]);
			}
			// console.log('oke else');
			// console.log('val '+val);
		<?php } ?>

		// console.log('harga lama '+id);
		// console.log('harga jual '+terima_logalam_id);
	});
	...
	*/
	...
	var jenis_produk = $('#topko-jenis_produk').val();
	// console.log(set_low_price);
    if ( jenis_produk == 'JasaKD' || jenis_produk == 'JasaGesek' || jenis_produk == 'JasaMoulding') {
        if (over_plafond != '' || over_top != '') {
            $("#place-verifystatus").html("<span class='font-yellow-gold'>Status : "+over_plafond+''+over_top+"</span>");
            $("#<?= \yii\bootstrap\Html::getInputId($model, "status") ?>").val("Status : "+over_plafond+''+over_top);
        } else {
            $("#place-verifystatus").html("<span class='font-yellow-gold'>Status</span>");
            $("#<?= \yii\bootstrap\Html::getInputId($model, "status") ?>").val('');
        }
    } else if(jenis_produk == 'Log'){
		$("#place-verifystatus").html("<span class='font-yellow-gold'>Status</span>");
        $("#<?= \yii\bootstrap\Html::getInputId($model, "status") ?>").val('');
	}
	else {
...
function masterProduk(ele){
	...
	var jenis_produk = $('#<?= yii\bootstrap\Html::getInputId($model, 'jenis_produk') ?>').val();
	var url = '<?= \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/produkInStock3','disableAction'=>'']); ?>1&tr_seq='+tr_seq+"&jenis_produk="+jenis_produk;
    $(".modals-place-3-min").load(url, function() {
...
function pickProduk(produk_id, tr_seq, data, stock_pcs, stock_kubik, stock_palet, harga_enduser, kode){
	...
	$.ajax({
        ...
        success: function (data) {
			if(data){
				if(jns_produk == "Limbah" || jns_produk == "JasaKD"  || jns_produk == "JasaGesek" || jns_produk == "JasaMoulding" || jns_produk == "Log" || jns_produk == data.produk_group){
					var already = [];
					$('#table-detail > tbody > tr').each(function(){
						var produk_id = $(this).find('select[name*="[produk_id]"]');
						if( produk_id.val() ){
...
function setVal(){
	...
	if(cust_id){
		$.ajax({
			...
			type   : 'POST',
			data   : {cust_id:cust_id,jns_produk:jns_produk,op_ko_id:"<?= isset($_GET['op_ko_id'])?$_GET['op_ko_id']:"" ?>",po_ko_id:po_ko_id},
			success: function (data) {
				if(data.cust){
					if(data.top_hari){
...
function addItem(){
    ...
    $.ajax({
        ...
        type   : 'POST',
        data   : {jns_produk:jns_produk, po_ko_id:po_ko_id},
        success: function (data) {
            if(data.item){
...
function setItem(ele,produk_id=null){
	var jns_produk = $('#<?php echo yii\bootstrap\Html::getInputId($model, "jenis_produk") ?>').val();
	var cust_id = $('#<?php echo yii\bootstrap\Html::getInputId($model, "cust_id") ?>').val();
	var po_ko_id = $('#<?php echo yii\bootstrap\Html::getInputId($model, "po_ko_id") ?>').val();
	if(!produk_id){
		produk_id = $(ele).val();
	}
	
    $.ajax({
        url    : '<?php echo \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/setItem']); ?>',
        type   : 'POST',
        data   : {produk_id:produk_id,jns_produk:jns_produk,po_ko_id:po_ko_id},
        success: function (data) {
            ...
            if(data.produk){
				...
				if(jns_produk == 'Log'){ 
					...
					$(ele).parents('tr').find('input[name*="[satuan_besar]"]').val("M3");
					$(ele).parents('tr').find('input[name*="[qty_besar]"]').val("0");
					$(ele).parents('tr').find('input[name*="[kubikasi]"]').attr('disabled','disabled');
					$(ele).parents('tr').find('input[name*="[maks_kubikasi]"]').val(data.maks_kubikasi);
				}
				...
				if(cust_id){
                    ...
                    if(data.harga_enduser == 0){
						if(jns_produk == 'Log'){
							alert('Perhatian Pricelist Produk '+data.produk.log_nama+' Setup Harga Untuk Periode ini Masih NOL (0), Silahkan Lakukan Setup Harga Terlebih Dahulu');
							//$(ele).parents('tr').find('input[name*="[harga_jual]"]').removeAttr('disabled');
							$(ele).parents('tr').find('input[name*="[harga_jual]"]').attr('disabled','disabled'); //.removeAttr('disabled'); //
							$(ele).parents('tr').find('input[name*="[harga_jual_lama]"]').attr('disabled','disabled');
						} else {
							alert('Perhatian Pricelist Produk '+data.produk.produk_nama+' Setup Harga Untuk Periode ini Masih NOL (0), Silahkan Lakukan Setup Harga Terlebih Dahulu');
							//$(ele).parents('tr').find('input[name*="[harga_jual]"]').removeAttr('disabled');
							$(ele).parents('tr').find('input[name*="[harga_jual]"]').attr('disabled','disabled'); //.removeAttr('disabled'); //
							$(ele).parents('tr').find('input[name*="[harga_jual_lama]"]').attr('disabled','disabled');
						}
				...
				if(data.availablestock){
					...
					var b = a.toFixed(4);
					if(jns_produk == 'Log'){
						$(ele).parents('tr').find('#place-availablestock').html(data.availablestock.stock+" pcs");
					} else {
						$(ele).parents('tr').find('#place-availablestock').html(data.availablestock.qty_kecil+"("+data.availablestock.in_qty_kecil_satuan+")<br>"+b+" pcs");
					}
				}
				...
				if(data.availablestock){
					...
					if(jns_produk == 'Log'){
						$(ele).parents('tr').find('#place-availablestock').html(data.availablestock.stock+" pcs");
						console.log('avail '+data.availablestock);
					} else {
...
function setMeterKubik(ele){
    ...
    result = sat_p_m * sat_l_m * sat_t_m * qty;
        //result = (Math.round( result * 100000 ) / 100000 ).toPrecision(5).toString();
        setTimeout(function() {
            if( $(ele).parents('tr').find("input[name*='[is_random]']").val() != 1 ){
                //$(ele).parents('tr').find('input[name*="[kubikasi]"]').val( result );
                $(ele).parents('tr').find('input[name*="[kubikasi]"]').val( formatNumberFixed4(result) ); //ini adalah pembulatan 4 digit - cek pemblatan di custom.js
                //$(ele).parents('tr').find('input[name*="[kubikasi]"]').val( formatNumberForUser(result) );
                //$(ele).parents('tr').find('input[name*="[kubikasi]"]').val( formatNumberFixed4(formatNumberForUser(result)) );
//                $(ele).parents('tr').find('input[name*="[kubikasi]"]').val( formatNumberForUser4Digit(formatNumberForUser(result)) );
                total();
            }
    }, 300);
}
...
function setQtyByKubikasi(ele){
    ...
    result = Math.round(kubikasi / (sat_p_m * sat_l_m * sat_t_m));
    setTimeout(function() {
//        $(ele).parents('tr').find('input[name*="[qty_kecil]"]').val( formatNumberFixed4(formatNumberForUser(result)) );
		//$(ele).parents('tr').find('input[name*="[qty_kecil]"]').val( formatNumberForUser(result) );
        //$(ele).parents('tr').find('input[name*="[qty_kecil]"]').val( formatNumberFixed4(result) );
        $(ele).parents('tr').find('input[name*="[qty_kecil]"]').val( result );
		total();
    }, 300);
}
...
function afterSave(id){
	...
	<?php if(isset($_GET['edit'])){ ?>
		...
		$('#<?= \yii\bootstrap\Html::getInputId($model, "tarik_data") ?>').prop("disabled", true);
		$('#<?= \yii\bootstrap\Html::getInputId($model, "po_ko_id") ?>').prop("disabled", false);
		$('#btn-save').prop('disabled',false);
		$('#btn-print').prop('disabled',true);

		if($('#<?= yii\bootstrap\Html::getInputId($model, "jenis_produk") ?>').val() == 'Log'){
			$('#<?= \yii\bootstrap\Html::getInputId($modTempo, "top_hari") ?>').prop("disabled", true);
		}
	<?php } ?>
	<?php if(isset($_GET['op_ko_id'])){ ?>
		<?php if(!empty($model->terima_logalam_id)){ ?>
			$('#<?= \yii\bootstrap\Html::getInputId($model, "tarik_data") ?>').val("Pelabuhan");
			$('#btn-add-item').hide();
			$('#place-loglist').css('display', '');
			setItemLogPelabuhan();
		<?php } else { ?>
			$('#<?= \yii\bootstrap\Html::getInputId($model, "tarik_data") ?>').val("Pabrik");
...
function getItems(op_ko_id,edit=null){
    ...
    $.ajax({
		...
		type   : 'POST',
		data   : {op_ko_id:op_ko_id,edit:edit,po_ko_id:po_ko_id},
		success: function (data) {
			if(data.html){
				...
				if(edit){ // edit load item process
					...
					$('#table-detail tbody tr').each(function(){
						...
						if(jns_produk == 'Log'){
							$(this).find("input[name*='[qty_besar]']").prop("disabled",false);
							$(this).find("input[name*='[harga_jual]']").prop("disabled",true);
						}
					});
					if(jns_produk == 'Log'){
						$("#<?= \yii\bootstrap\Html::getInputId($model, "sistem_bayar") ?>").prop("disabled", true);
						$("#<?= \yii\bootstrap\Html::getInputId($model, "cara_bayar") ?>").prop("disabled", true);
					}
					reordertable('#table-detail');
				}
			}
...
function daftarAfterSaveX(){
    openModal('<?= \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/daftarAfterSaveX']) ?>','modal-aftersavex','90%');
}
...
function hapus_file(attachment_id) {
	...
	if (x) {
		$( "#files" ).load("<?= \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/hapusfile?attachment_id=']);?>attachment_id", {attachment_id:attachment_id});
    	return true;
	} else {
    	return false;
	}
...
function masterLog(ele){
	...
	var po_ko_id = $('#<?= yii\bootstrap\Html::getInputId($model, 'po_ko_id') ?>').val();
	if(po_ko_id){
		var url = '<?= \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/openlog','disableAction'=>'']); ?>1&tr_seq='+tr_seq+"&jenis_produk="+jenis_produk+"&po_ko_id="+po_ko_id;
		$(".modals-place-3-min").load(url, function() {
		...
		cisAlert("Pilih Kode PO Customer terlebih dahulu!");
	}
}

function setVolume(ele){
	var kubikasi = $(ele).parents('tr').find('.qty_besar').val();
	$(ele).parents('tr').find('.kubikasi').val(kubikasi);
	total();
	// console.log(kubikasi);
}
...
function setTarikData(){
	var tarik_data = $("#<?= yii\bootstrap\Html::getInputId($model, "tarik_data") ?>").val();
	var terima_logalam_id = $('#dropdown-logalam-id');
	var initialSelect2Config =  {
									allowClear: true,
									placeholder: 'Tarik Data Log',
									width: '100%'
								};
	if(tarik_data == 'Pabrik'){
		...
		$("#log-tarik-data").css('display','');
		$('#place-loglist').css("display", "");
		$('#table-detail-loglist > tbody').empty();
		$('#table-detail > tbody').empty();
		$('#btn-add-item').css('display','none');
		// biar balik ke kondisi awal
		terima_logalam_id.val(null); 
        terima_logalam_id.select2(initialSelect2Config);
...
function setItemLogPelabuhan(){
	...
	<?php } ?>
    var po_ko_id = $("#<?= yii\bootstrap\Html::getInputId($model, "po_ko_id") ?>").val();
	// console.log(terima_logalam_id);
    
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/setItemLogPelabuhan']); ?>',
        type   : 'POST',
        data   : {terima_logalam_id:terima_logalam_id, po_ko_id:po_ko_id},
        success: function (response) {
            ...
            if(response.status == 'success'){
					...
					var newRow = `
						<tr>
							<td style="vertical-align: middle; text-align: center;">
								...
								<input type="hidden" class="form-control fisik_pcs" style="width:100%; font-size:1.2rem; padding:5px;" value="1" disabled>
								<input type="hidden" class="form-control produk_id" style="width:100%; font-size:1.2rem; padding:5px;" value="`+data.produk_id+`" disabled>
								<input type="hidden" class="form-control log_kode" style="width:100%; font-size:1.2rem; padding:5px;" value="`+data.log_kode+`" disabled>
								<input type="hidden" class="form-control harga_jual" style="width:100%; font-size:1.2rem; padding:5px;" value="`+data.harga_jual+`" disabled>
							</td>
							<td class="td-kecil"><input type="text" class="form-control no_barcode" style="width:100%; font-size:1.2rem; padding:5px;" value="`+data.no_barcode+`" disabled></td>
							...
							<td class="td-kecil"><input type="text" class="form-control no_lap" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.no_lap+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control no_grade" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.no_grade+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control no_btg" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.no_btg+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control float panjang" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.panjang+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control float diameter_ujung1" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.diameter_ujung1+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control float diameter_ujung2" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.diameter_ujung2+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control float diameter_pangkal1" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.diameter_pangkal1+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control float diameter_pangkal2" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.diameter_pangkal2+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control float diameter_rata" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.diameter_rata+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control float cacat_panjang" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.cacat_panjang+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control float cacat_gb" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.cacat_gb+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control float cacat_gr" style="width:100%; font-size:1.2rem; padding:5px; text-align: center;" value="`+data.cacat_gr+`" disabled></td>
							<td class="td-kecil"><input type="text" class="form-control float fisik_volume" style="width:100%; font-size:1.2rem; padding:5px;" value="`+data.volume+`" disabled></td>
						</tr>
					`;
				...
				// cisAlert("Terdapat produk yang tidak ada di master data!");
				cisAlert("Terdapat produk yang tidak sesuai dengan PO!");
			}
        },
...
function fillOP() {
    ...
    $('#table-detail-loglist > tbody > tr').each(function() {
        var produk_id = $(this).find('.produk_id').val();
		var log_kode = $(this).find('.log_kode').val();
        var volume = unformatNumber($(this).find('.fisik_volume').val());
		var qty_kecil = unformatNumber($(this).find('.fisik_pcs').val());
		var qty_besar = unformatNumber($(this).find('.fisik_volume').val());
		var harga_jual = unformatNumber($(this).find('.harga_jual').val());

		var key = produk_id + '|' + log_kode + '|' + harga_jual;

        if (produkVolumeMap[key]) {
            produkVolumeMap[key].volume += volume;
			produkVolumeMap[key].qty_kecil += qty_kecil;
			produkVolumeMap[key].qty_besar += qty_besar;
        } else {
            produkVolumeMap[key] = {
                produk_id: produk_id,
                log_kode: log_kode,
	...
	var index = 0;
    $.each(produkVolumeMap, function(key, data) {
		var no_urut = index + 1;
        var newRow = `
            <tr>
                <td style="vertical-align: middle; text-align: center;" class="td-kecil">
					<span class="no_urut">`+no_urut+`</span>
					<input type="hidden" class="form-control" name="TOpKoDetail[`+index+`][op_ko_detail_id] style="width:100%; font-size:1.2rem; padding:5px;" value="" disabled>
				</td>
				<td style="vertical-align: middle;" id="item-detail" class="td-kecil">
					<input type="hidden" class="form-control produk_id" name="TOpKoDetail[`+index+`][produk_id] style="width:100%; font-size:1.2rem; padding:5px;" value="`+data.produk_id+`" disabled>
					<input type="text" class="form-control log_kode" style="width:100%; font-size:1.2rem; padding:5px;" value="`+data.log_kode+`" disabled>
				</td>
                <td style="vertical-align: middle;" class="td-kecil">
					<input type="hidden" class="form-control qty_kecil" name="TOpKoDetail[`+index+`][qty_kecil] style="width:100%; font-size:1.2rem; padding:5px;" value="`+data.qty_kecil+`" disabled>
					<input type="hidden" class="form-control satuan_kecil" name="TOpKoDetail[`+index+`][satuan_kecil] style="width:100%; font-size:1.2rem; padding:5px;" value="Pcs" disabled>
				</td>
				<td style="vertical-align: middle;" class="td-kecil">
					<span class="input-group-btn" style="width: 50%">
						<input type="text" class="form-control qty_besar text-right td-kecil" name="TOpKoDetail[`+index+`][qty_besar] style="width:100%; font-size:1.2rem; padding:5px;" value="`+data.qty_besar.toFixed(2)+`" disabled>
					</span>
					<span class="input-group-btn" style="width: 50%">
						<input type="text" class="form-control satuan_besar td-kecil" name="TOpKoDetail[`+index+`][satuan_besar] style="width:100%; font-size:1.2rem; padding:5px;" value="M3" disabled>
					</span>
				</td>
				<td style="vertical-align: middle; text-align: right;" class="td-kecil;">
					<input type="text" class="form-control kubikasi text-right td-kecil" name="TOpKoDetail[`+index+`][kubikasi] style="width:100%; font-size:1.2rem; padding:5px;" value="`+data.volume.toFixed(2)+`" disabled>
				</td>
				<td style="vertical-align: middle; font-size: 1rem !important;" class="td-kecil text-align-center"></td>
				<td style="vertical-align: middle;" class="td-kecil">
					<input type="text" class="form-control harga_jualx text-right td-kecil" name="TOpKoDetail[`+index+`][harga_jual] style="width:100%; font-size:1.2rem; padding:5px;" value="`+ formatNumberForUser(data.harga_jual) +`" onblur="setVerify(); total();" disabled>
					<input type="hidden" class="form-control harga_hpp" name="TOpKoDetail[`+index+`][harga_hpp] style="width:100%; font-size:1.2rem; padding:5px;" value="0" disabled>
					<input type="hidden" class="form-control harga_jual_lama text-right" name="TOpKoDetail[`+index+`][harga_jual_lama] style="width:100%; font-size:1.2rem; padding:5px;" value="`+ data.harga_jual +`" disabled>
					<input type="hidden" class="form-control TOpKoDetail_`+index+`_status_harga" name="TOpKoDetail[`+index+`][status_harga] style="width:100%; font-size:1.2rem; padding:5px;" value="" disabled>
				</td>
				<td style="vertical-align: middle;" class="td-kecil">
					<input type="text" class="form-control subtotal text-right td-kecil" name="TOpKoDetail[`+index+`][subtotal] style="width:100%; font-size:1.2rem; padding:5px;" value="" onblur="total();" disabled>
				</td>
				<td style="vertical-align: middle;" class="td-kecil"><center>-</center></td>
            </tr>
        `;
	...
	total();
	//formatNumberForUser(data.harga_jual)
	// console.log($('#table-detail > tbody').find('.TOpKoDetail_0_status_harga').val());
    // console.log(produkVolumeMap);
}

function pickPO(){
	var url = '<?= \yii\helpers\Url::toRoute(['/marketing/purchaseorder/daftarPO']); ?>';
	$(".modals-place-3-min").load(url, function() {
		$("#modal-master .modal-dialog").css('width','90%');
		$("#modal-master").modal('show');
		$("#modal-master").on('hidden.bs.modal', function () {});
...
function pickedPO(id,par){
    par = par.replace("xyz","'");
	$("#modal-master").find('button.fa-close').trigger('click');
	$("#<?= yii\bootstrap\Html::getInputId($model, "po_ko_id") ?>").empty().append('<option value="'+id+'">'+par+'</option>').val(id);
	setTarikPO();
	// resetTableDetail();
}
...
function setTarikPO(){
    ...
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/setTarikPO']); ?>',
        type   : 'POST',
        data   : {po_ko_id:po_ko_id},
        success: function (data) {
			// console.log(data.customer);
			var cust_nama = (!data.customer.cust_pr_nama)?data.customer.cust_an_nama:data.customer.cust_pr_nama;
			var cust_alamat = (!data.customer.cust_pr_alamat)?data.customer.cust_an_alamat:data.customer.cust_pr_alamat;
			var sales_nama = data.sales.sales_nm;

			$("#<?= yii\bootstrap\Html::getInputId($model, "cust_id") ?>").empty().append('<option value="'+data.cust_id+'">'+ cust_nama +'</option>').val(data.cust_id);
			$("#<?= yii\bootstrap\Html::getInputId($model, "cust_alamat") ?>").val(cust_alamat);
			$("#<?= yii\bootstrap\Html::getInputId($model, "alamat_bongkar") ?>").val(data.alamat_bongkar);
			$("#<?= yii\bootstrap\Html::getInputId($model, "provinsi_bongkar") ?>").val(data.provinsi_bongkar);
			$("#<?= yii\bootstrap\Html::getInputId($model, "sistem_bayar") ?>").val(data.sistem_bayar).trigger('change');
			$("#<?= yii\bootstrap\Html::getInputId($modTempo, "top_hari") ?>").val(data.top_hari);
			$("#<?= yii\bootstrap\Html::getInputId($model, "cara_bayar") ?>").val(data.cara_bayar);
			$("#<?= yii\bootstrap\Html::getInputId($model, 'syarat_jual') ?> input[value='" + data.syarat_jual + "']").prop('checked', true);
			$("#<?= yii\bootstrap\Html::getInputId($model, "sales_id") ?>").val(data.sales_id).trigger('change');
			$("#<?= yii\bootstrap\Html::getInputId($model, "tanggal_kirim") ?>").val(formatDateForUser(data.tanggal_kirim));

			$('#<?= \yii\bootstrap\Html::getInputId($model, "cust_id") ?>').prop("disabled", true);
			$('#btn-icon-cust').css('pointer-events', 'none');
			$("#<?= \yii\bootstrap\Html::getInputId($modTempo, "top_hari") ?>").prop("disabled", true);
			$("#<?= \yii\bootstrap\Html::getInputId($model, "sistem_bayar") ?>").prop("disabled", true);
			$("#<?= \yii\bootstrap\Html::getInputId($model, "cara_bayar") ?>").prop("disabled", true);

			setPreviewPO(data.kode);
			resetTableDetail();
		}
	});
}

function setPreviewPO(kode){
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/setPreviewPO']); ?>',
        type   : 'POST',
        data   : {kode:kode},
        success: function (data) {
			placeAttach = $("#place-attch");
            placeAttach.empty();
			jml_file = 0;
			...
			if (data.length > 0) {
                $.each(data, function(index, attachment) {
                    ...
                    if (['jpg', 'jpeg', 'bmp', 'png', 'gif', 'tiff'].includes(attachment.file_ext)) {
                        attachmentHTML = `
                            <div class="col-md-2">
                                <div class="fileinput fileinput-new" data-provides="fileinput">
                                    <div class="fileinput-new thumbnail" style="width: 55px;">
                                        <a class="btn btn-xs blue-hoki btn-outline tooltips" href="javascript:void(0)" onclick="showFile(${attachment.attachment_id}, 'ya')">
                                            <img src="${attachment.file_url}" alt=""/>
                                        </a>
										<input type="text" class="tattachment-file" id="file${index+1}" name="TAttachment[file${index+1}]" value='${attachment.attachment_id}' style="display: none;">
                                    </div>
                                    <div class="fileinput-preview fileinput-exists thumbnail"></div>
                                </div>
                            </div>
                    ...
                    placeAttach.append(attachmentHTML);
					jml_file++;
                });
            }

			for (let i = jml_file + 1; i <= 6; i++) {
                placeAttach.append(`
                    <input type="text" class="tattachment-file" id="file${i}" name="TAttachment[file${i}]" style="display: none;">
                `);
            }
		}
	});
...
function showFile(id, a){
	var url = '<?= \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/showFile','id'=>'']) ?>'+id+'&dataindex='+a;
	$(".modals-place-2").load(url, function() {
...
function validatingKubikasi(callback){
	var id = $("#<?= yii\bootstrap\Html::getInputId($model, "po_ko_id") ?>").val();
	var produks = [];
	var edit = '<?= isset($_GET['edit'])?$_GET['edit']:''; ?>';
	var op_id = '<?= isset($_GET['op_ko_id'])?$_GET['op_ko_id']:''; ?>';

	$('#table-detail tbody > tr').each(function(){
		var produk_id = $(this).find('select[name*="[produk_id]"]').val();
		var kubikasi = $(this).find('input[name*="[kubikasi]"]').val();
		if (produk_id) {
			produks.push({
				produk_id: produk_id,
				kubikasi: kubikasi
	...
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/marketing/orderpenjualan/validatingKubikasi']); ?>',
		type   : 'POST',
		data   : {produks:produks, id:id, edit:edit,op_id:op_id},
		success: function (data) {
			var isValid = true;
			...
			Object.values(data.post).forEach(function(postItem) {
				...
				var op = data.op[id] ? parseFloat(data.op[id].kubikasi) : 0;
