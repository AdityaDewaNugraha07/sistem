		...
		<td colspan="3" style="padding: 0px;">
			<table style="width: 100%" id="table-detail">
				<tr style="border-bottom: solid 1px #000; border-top: solid 1px #000;">
					<th style="width: 110px; text-align: center;  padding: 10px; border-right: solid 1px #000;"><?= Yii::t('app', 'Kode Nota') ?></th>
					...
					<th style="width: 50px; text-align: center;  padding: 10px; border-right: solid 1px #000; line-height: 1;"><?= Yii::t('app', 'Total<br>Palet'); ?></th>
				...
				<?php for($i=0;$i<$max;$i++){
					if( count($modDetail) >= ($i+1) ){
						if($modDetail[$i]['jenis_produk'] == 'Log'){
							$total_besar += 0;
						} else {
							$total_besar += $modDetail[$i]['palet'];
						}
						$total_kecil += $modDetail[$i]['pcs'];
						$total_kubikasi += $modDetail[$i]['kubikasi'];
						$total_bayar += $modDetail[$i]['total_bayar'];
				?>
					<tr>
						...
						<td style="text-align: center; padding: 3px; border-right: solid 1px #000;"><?= ($modDetail[$i]['jenis_produk'] == 'Log')?'0':\app\components\DeltaFormatter::formatNumberForUserFloat($modDetail[$i]['palet']); ?></td>
						...
						<td style="text-align: right; padding: 3px;">
							...
							<?php } ?>
							<span class="pull-right"><?= \app\components\DeltaFormatter::formatNumberForUserFloat($modDetail[$i]['total_bayar']); ?></span>
						</td>
					</tr>
				<?php }else{ ?>
					<tr>
				...
				<tr style="border-top: solid 1px #000;" >
					<th colspan="3" style="padding: 5px; text-align: right; border-right: solid 1px #000;">Total &nbsp;</th>
					<td style="padding: 5px; text-align: center; border-right: solid 1px #000;"><b><?= app\components\DeltaFormatter::formatNumberForUserFloat($total_besar) ?></b></td>
					...
					<td style="padding: 5px; text-align: right;"><b>
						...
						<?php } ?>
							<span class="pull-right"><?= \app\components\DeltaFormatter::formatNumberForUserFloat($total_bayar); ?></span>
					</b></td>
				</tr>
			</table>
		</td>
