        ...
        <div class="portlet light bordered">
            <div class="portlet-body">
                ...
                <ul class="nav nav-tabs">
                    ...
                    <li class="active">
                        <a href="<?= yii\helpers\Url::toRoute("/marketing/pricelistlog/index"); ?>"> <?= Yii::t('app', 'Price List'); ?> </a>
                    </li>
                </ul>
                <div class="row">
                    <div class="col-md-12">
                                        ...
                                        <table class="table table-striped table-bordered table-hover" id="table-pricelist">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width: 120px;"><?= Yii::t('app', 'Kode Log') ?></th>
                                                    <th style="width: 300px;"><?= Yii::t('app', 'Nama Log') ?></th>
                                                    <th style="width: 160px;"><?= Yii::t('app', 'Range Dimensi') ?></th>
...
<?php $this->registerJs("
setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Log'))."');
setTglDropdown();
formconfig();
setGetLoad();
...
function setTglDropdown() {
    $.ajax({
        url: '<?= \yii\helpers\Url::toRoute(['/marketing/pricelistlog/setTglDropdown']); ?>',
        type: "POST",
...
function getContent(tipe,tgl=null){
    ...
    $.ajax({
        url    : '<?php echo \yii\helpers\Url::toRoute(['/marketing/pricelistlog/getContent']); ?>',
        type   : 'POST',
        ...
        success: function (data) {
            ...
            $("#status-approval").load(`<?= \yii\helpers\Url::toRoute(['/marketing/pricelistlog/statusApproval']); ?>?tp=${tgl}&kode=${kode}`);
        },
...
function setPrice(tgl_penetapan=null){
    ...
    if(tgl_penetapan){
        $('#table-pricelist tbody tr').each(function(index){
            var log_id = $(this).find('input[name*="log_id"]').val();
            var tr = $(this);
            tr.addClass('animation-loading');
            $.ajax({
                url    : '<?= \yii\helpers\Url::toRoute(['/marketing/pricelistlog/setPrice']); ?>',
                type   : 'POST',
                data   : {log_id:log_id,tgl_penetapan:tgl_penetapan},
                success: function (data) {
...
function hapusPricelist(){
    ...
    openModal('<?= \yii\helpers\Url::toRoute(['/marketing/pricelistlog/delete']) ?>?id='+tp,'modal-delete-record',null,'setTimeout(function(){ location.reload(); },1000);');
}
