		...
		<td colspan="3" style="padding: 0px;">
			<table style="width: 100%" id="table-detail">
					...
					<?php if($model->opKo->jenis_produk=="Log"){ ?>
						<tr>
							<td style="padding: 2px 5px; border-right: 1px solid black;">
								<?php
									$modLog = app\models\MBrgLog::findOne($modDetail[$i]->produk_id);
							...
							<td style="padding: 2px 5px; border-right: 1px solid black;"><?= '<center>'.$modDetail[$i]->log->range_awal." - ".$modDetail[$i]->log->range_akhir .'</center>'; ?></td>
							<td style="padding: 2px 5px; border-right: solid 1px #000;"></td>
							<td style="padding: 2px 5px; border-right: solid 1px #000;">
								<span>
									<?php //echo \app\components\DeltaFormatter::formatNumberForUserFloat($modDetail[$i]->qty_besar) ?> 
									<center><i><?= $modDetail[$i]->satuan_besar ?></i></center>
								</span>
							</td>
							<td style="padding: 2px 5px; border-right: solid 1px #000; line-height: 1">
								<span style="float: right">
									<?php echo (!empty($modDetail[$i]->kubikasi))?number_format($modDetail[$i]->kubikasi,2):"";?>
								</span>
							</td>
							<td style="padding: 2px 5px; border-right: solid 1px #000;">
								<span style="float: right">
									...
									<i><?php echo (!empty($modDetail[$i]->qty_kecil_realisasi))?"(".$modDetail[$i]->satuan_kecil_realisasi.")":"" ?></i>
								</span>
							</td>
							<td style="padding: 2px 5px; border-right: solid 1px #000;">
								<span>
									...
									<i><?php echo (!empty($modDetail[$i]->qty_besar_realisasi))?'<center>'.$modDetail[$i]->satuan_besar.'</center>':"" ?></i>
								</span>
							</td>
							<td style="padding: 2px 5px; border-right: solid 1px #000;">
								<span style="float: right">
									<?php echo (!empty($modDetail[$i]->kubikasi_realisasi))?number_format($modDetail[$i]->kubikasi_realisasi,2):"";?>
								</span> 
							</td>
				...
				<tr style="border-top: solid 1px #000;" >
					...
					<?php } ?>
					<td class="text-align-center" style="padding: 5px; border-right: solid 1px #000;"><b><?= ($model->opKo->jenis_produk=="Limbah" || $model->opKo->jenis_produk=="Log")? "" : \app\components\DeltaFormatter::formatNumberForUserFloat($total_besar) ?></b></td>
						...
						<b>
							<?php 
								if ($model->opKo->jenis_produk=="Limbah"){
									...
									echo "";
									// echo number_format($total_besar)."<i>(".$modDetail[0]->satuan_besar .")</i>";
								} else {
					...
					<td class="text-align-right" style="padding: 5px; border-right: solid 1px #000;">
						...
						<?php 
								if ($model->opKo->jenis_produk=="Limbah"){
									...
									echo number_format($total_kubik,2);
								} else {
						...
						<b>
							<?php 
								if ($model->opKo->jenis_produk=="Limbah"){
									...
									echo number_format($total_kecil_realisasi)."<i>(".$modDetail[0]->satuan_kecil_realisasi .")</i>";
								} else {
						...
						<b>
							<?php 
								if ($model->opKo->jenis_produk=="Limbah"){
									...
									echo "";
									// echo number_format($total_besar_realisasi, 2)."<i>(".$modDetail[0]->satuan_besar_realisasi .")</i>";
								} else {
						...
						<b>
							<?php 
								if ($model->opKo->jenis_produk=="Limbah"){
									...
									echo number_format($total_kubik_realisasi,2);
								} else {
							...
							<td colspan="13" style=" border-top: solid 1px ;">
								<table style="width: 100%; font-size: 1.1rem; text-align: center; border: solid 1px #000; border-bottom: solid 1px #000; border-left: solid 1px transparent;" border="1">
									...
									foreach($sql as $i => $data){
										...
										// $persediaan = \app\models\HPersediaanLog::findOne(['no_barcode'=>$data['no_barcode'], 'reff_no'=>$model->kode]);
										// if($persediaan){
										// 	foreach($persediaan as $a => $sedia){
										// 		$logs = Yii::$app->db->createCommand(
										// 					"SELECT * FROM m_brg_log WHERE kayu_id = {$persediaan->kayu_id} AND {$persediaan->fisik_diameter} 
										// 					BETWEEN range_awal AND range_akhir"
										// 				)->queryAll();
										// 		$kayu = \app\models\MKayu::findOne(['kayu_id'=>$persediaan->kayu_id]);
										// 	}
										// }
									?>
										<tr style="height: 0.4cm;  border-right: solid 1px transparent; border-bottom: solid 1px transparent;">
												...
												<td style="width: 1.2cm;"><?= $log['range_awal'] .'cm - '. $log['range_akhir'] .'cm'; ?></td>
