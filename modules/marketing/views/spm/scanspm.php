...
$this->title = 'Scan Pemuatan Gudang';
                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-body">
								<div class="row" style="text-align: center;">
									<div class="col-md-12">
										<?= \yii\bootstrap\Html::activeDropDownList($model, 'spm_ko_id', \app\models\TSpmKo::getOptionListScanPemuatan(),['class'=>'form-control select2','prompt'=>'','onchange'=>'setScanner();','style'=>'width:100%;']); ?>
									</div>
								</div>
								<br>
								<div class="row" style="text-align: center;">
...
//		}
	...
	reading();
	setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Scan Pemuatan'))."',7);
	setScanner();
...
function getItemsScanned(id){
    $.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/marketing/spm/getItemsScanned']); ?>',
		type   : 'POST',
		data   : {id:id},
...
function total(){
	...
	$("#table-detail-produklist tbody tr").each(function(){
		total_besar += unformatNumber( $(this).find('input[name*="[qty_besar]"]').val() );
        ...
        total_kubikasi += formatNumberFixed4($(this).find('input[name*="[kubikasi]"]').val(),4) * 1;
	});
	$(".place-totalbesar").html( formatNumberForUser(total_besar) );
...
/*function pick(prod_number){
	var spm_ko_id = $('#<?= \yii\bootstrap\Html::getInputId($model, "spm_ko_id") ?>').val();
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/marketing/spm/saveNomorProduksi']); ?>',
        type   : 'POST',
        data   : {spm_ko_id:spm_ko_id,prod_number:prod_number},
        success: function (data) {
			if(data.msg){
				cisAlert(data.msg);
			}
...
function pick(nomor_produksi){
	var spm_ko_id = $('#<?= \yii\bootstrap\Html::getInputId($model, "spm_ko_id") ?>').val();
    var nomor_produksi = nomor_produksi;
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/marketing/spm/showDetail']); ?>',
        type   : 'POST',
        data   : {spm_ko_id:spm_ko_id, nomor_produksi:nomor_produksi},
        success: function (data) {            
			if(data){
                if (data['msg'] == "Data ok") {
                    openModal('<?= \yii\helpers\Url::toRoute(['/marketing/spm/review','spm_ko_id'=>'']) ?>'+spm_ko_id+'&nomor_produksi='+nomor_produksi,'modal-review','300px');
                } else {
                    //openModal('<?= \yii\helpers\Url::toRoute(['/marketing/spm/view','nomor_produksi'=>'']) ?>'+datas+'&gudang_id='+gudang_id,'modal-review','300px');
                    cisAlert(data['msg']);
                }
            }
        },
...
function hapusItem(ele){
	var spm_ko_id = $("#<?= \yii\bootstrap\Html::getInputId($model, "spm_ko_id") ?>").val();
	var produk_keluar_id = $(ele).parents("tr").find("input[name*='[produk_keluar_id]']").val();
    openModal('<?= \yii\helpers\Url::toRoute(['/marketing/spm/deleteNomorProduksi','id'=>'']) ?>'+produk_keluar_id,'modal-delete-record');
}
</script>