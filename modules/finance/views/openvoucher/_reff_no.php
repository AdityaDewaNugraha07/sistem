...
<?php } else if($tipe == "PEMBAYARAN ASURANSI LOG SHIPPING") {?>
    <div class="form-group" style="margin-bottom: 10px;">
        <label class="col-md-4 control-label">Kode Asuransi</label>
        <div class="col-md-8">
            <span class="input-group-btn" style="width: 100%">
                <?= \yii\bootstrap\Html::activeTextInput($model, 'reff_no',['class'=>'form-control','prompt'=>'','oninput'=>'alert()','disabled'=>true]); ?>
            </span>
            <span class="input-group-btn" style="width: 25%">
                <a class="btn btn-icon-only btn-default tooltips" onclick="openAsuransi();" data-original-title="Cari Kode Pengajuan Asuransi" style="margin-left: 3px; border-radius: 4px;"><i class="fa fa-search"></i></a>
            </span>
        </div>
    </div>
<?php } ?>