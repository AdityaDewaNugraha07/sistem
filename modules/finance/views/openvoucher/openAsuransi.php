...
function dtAfterSave(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/finance/openvoucher/openAsuransi') ?>',data:{dt: 'modal-aftersave'} },
        order: [
            [0, 'desc'],
        ],
        columnDefs: [
            ...
            { 	targets: 1, class:'text-align-center',
                render: function ( data, type, full, meta ) {
                    ret =  "<a onclick='pick(\""+data+"\",\""+full[3]+"\")' class='btn btn-xs btn-icon-only btn-default tooltips' data-original-title='Pick' style='width: 25px; height: 25px;'><i class='fa fa-plus-circle'></i></a>";
                    return ret;
                }
            },
            { 	targets: 2,
                render: function ( data, type, full, meta ) {
					...
					return '<center>'+full[1]+'<br>'+date+'</center>';
                }
            },
            { 	targets: 3, 
                render: function ( data, type, full, meta ) {
