...
function totalInvoice(){
    var jenis_produk = $('#<?= yii\bootstrap\Html::getInputId($model, "jenis_produk") ?>').val();
    var m3 = 0; var total_harga_inv = 0; var harga_invoice =0; var subtotal=0; var total_m3=0; var bayar = 0; var pph = 0;
    ...
    setTimeout(function(){
        total_ppn = Math.round( total_harga_inv * <?= Params::DEFAULT_PPN?> );
        if(jenis_produk == "Log"){
        ...
        if($("#<?= \yii\helpers\Html::getInputId($model, "kawasan_berikat") ?>").is(":checked")){
            if($("#<?= \yii\helpers\Html::getInputId($model, "ceklis_pph") ?>").is(":checked")){
                bayar = total_harga_inv - pph;
            } else {
                bayar = total_harga_inv;
            }
        } else {
            if($("#<?= \yii\helpers\Html::getInputId($model, "ceklis_pph") ?>").is(":checked")){
                bayar = total_harga_inv + total_ppn - pph;
            } else {
                bayar = total_harga_inv + total_ppn;
            }
        }
