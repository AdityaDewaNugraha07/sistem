        ...
        columnDefs: [
			...
			{	targets: 16, class:"text-align-center td-kecil",
                render: function ( data, type, full, meta ) {
                    var ret = ' - ';
                    if(full[22] == "IDR"){
                        jml = formatInteger(unformatNumber(full[20]));
                    } else {
                        jml = formatNumberForUser2Digit(unformatNumber(full[20]));
                    }
                    if(data=='PAID'){
                        ret = '<span class="label label-sm label-success" style="font-size: 10px; padding: 0px 6px;">'+data+'</span><br><a style="font-size: 10px;" onclick="infoVoucher('+full[18]+')">'+( (full[19])?full[19]:'' )+'</a><br>'+jml;
                    }else if(data=='UNPAID'){
                        ret = '<span class="label label-sm label-warning" style="font-size: 10px; padding: 0px 6px;">'+data+'</span><br><a style="font-size: 10px;" onclick="infoVoucher('+full[18]+')">'+( (full[19])?full[19]:'' )+'</a><br>'+jml;
