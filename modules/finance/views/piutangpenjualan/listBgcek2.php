    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Penerimaan Non-Tunai Kas Besar (Bilyet Giro / Cek)'); ?></h4>
            </div>
                        ...
                        <table class="table table-striped table-bordered" id="table-info">
							<thead>
								<tr>
									...
									<th><?= Yii::t('app', 'Nama Konsumen'); ?></th>
									<th><?= Yii::t('app', 'Bank'); ?></th>
									<th><?= Yii::t('app', 'No. Acct'); ?></th>
...
function getPiutangTerbayar(callback=null){
	...
	$.ajax({
		url    : '<?php echo \yii\helpers\Url::toRoute(['/finance/piutangpenjualan/SetHighlightListBegcek']); ?>',
		type   : 'POST',
		data   : {  },
...
var highlightData = {};
function dtInfo(){
	highlightData = JSON.parse($("#place-piutangterbayar").val() || "{}");
    var dt_table =  $('#table-info').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/finance/piutangpenjualan/openBgcek') ?>',data:{dt: 'table-info'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
			...
			{	targets: 1,
                ...
                render: function ( data, type, full, meta ) {
                    return "<a onclick='pickKasbesar(\""+data+"-"+full[6]+"\")' class='btn btn-xs btn-icon-only btn-default tooltips' data-original-title='Pick' style='width: 25px; height: 25px;'><i class='fa fa-plus-circle'></i></a>"+data;
                }
            },
		...
		"fnRowCallback": function(nRow, aData){
            var sisa = highlightData[aData[0]];

            if(typeof sisa !== "undefined"){
                if(sisa > 0){
                    $(nRow).css("background-color","#FFF2DA");
                }else{
                    $(nRow).css("background-color","#E8FFDA");
