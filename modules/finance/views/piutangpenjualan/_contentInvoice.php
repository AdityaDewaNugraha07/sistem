	...
	<table class="table table-striped table-bordered table-hover" id="table-nota">
		<thead>
			<tr style="background-color: #F1F4F7; ">
				...
				<th style="text-align: center; width: 160px;"><?= Yii::t('app', 'No. Invoice'); ?></th>
				<th style="text-align: center; line-height: 1"><?= Yii::t('app', 'Tanggal<br>Invoice'); ?></th>
		...
		<tbody>
			...
			foreach($models as $i => $model){
				$sql = "SELECT * FROM t_piutang_penjualan WHERE bill_reff = '".$model->nomor."' AND cancel_transaksi_id IS NULL";
			...
			?>
			<tr style="cursor: pointer;" onclick="selectRow(this,null,true); setHighlight(this,true);">
				<td style="text-align: center;"><?= $i+1; ?></td>
				<td>
                    <a onclick="infoInvoice('<?= $model->invoice_id ?>')" style="font-size: 1.1rem;"><?= $model->nomor ?></a>
					<?= \yii\bootstrap\Html::activeHiddenInput($model, "invoice_id") ?>
					<?= \yii\bootstrap\Html::activeHiddenInput($model, "nomor") ?>
				</td>
				<td style="text-align: center;"><?= app\components\DeltaFormatter::formatDateTimeForUser2($model->tanggal) ?></td>
				<td>
					<span class="pull-left"><?= $matauang; ?></span>
					<span class="pull-right"><?= \app\components\DeltaFormatter::formatNumberForUserFloat($model->total_bayar) ?></span>
				</td>
				<td>
					<span class="pull-left"><?= $matauang; ?></span>
					<span class="pull-right"><?= \app\components\DeltaFormatter::formatNumberForUserFloat($terbayar) ?></span>
				</td>
				<td>
					<span class="pull-left"><?= $matauang; ?></span>
                    <?php
//                    echo "<pre>";
//                    print_r($model->total_bayar);
//                    echo "<pre>";
//                    print_r($terbayar);
//                    exit;
                    ?>
					<span class="pull-right"><?= \app\components\DeltaFormatter::formatNumberForUserFloat($model->total_bayar-$terbayar) ?></span>
				</td>
		...
		<tfoot>
			<tr style="background-color: #F1F4F7">
				...
				<td style="text-align: right; vertical-align: middle; font-weight: 600">
					<span class="pull-left"><?= $matauang; ?></span>
					<span class="pull-right"><?= app\components\DeltaFormatter::formatNumberForUserFloat($jmltagihan) ?></span>
				</td>
				<td style="text-align: right; vertical-align: middle; font-weight: 600">
					<span class="pull-left"><?= $matauang; ?></span>
					<span class="pull-right"><?= app\components\DeltaFormatter::formatNumberForUserFloat($jmlterbayar) ?></span>
				</td>
				<td style="text-align: right; vertical-align: middle; font-weight: 600">
					<span class="pull-left"><?= $matauang; ?></span>
					<span class="pull-right"><?= app\components\DeltaFormatter::formatNumberForUserFloat($jmlsisatagihan) ?></span>
				</td>
				<td></td>
			</tr>
		</tfoot>
