    ...
    var dt_table =  $('#table-laporan').dataTable({
        ...
        columnDefs: [
			...
			{	
				...
				render: function ( data, type, full, meta ) {
					var ret="<a onclick=\"infoTBP('"+full[0]+"')\">"+full[3]+"</a>";
					return ret;
                }
			},
			{ 	targets: 3, class:'td-kecil', 
                render: function ( data, type, full, meta ) {
					var ret = '-';
					if(full[4]){
						ret="<a onclick=\"infoSPL('"+full[12]+"')\">"+full[4]+"</a>";
					}else{
						ret="<a onclick=\"infoSPO('"+full[13]+"')\">"+full[5]+"</a>";
					}
					return ret;
                }
            },
			...
			{	targets: 6, class: 'text-align-right td-kecil', 
				render: function ( data, type, full, meta ) { 
					var ret = formatNumberForUser(full[19]);
					return ret;
                }
			},
			...
			{	targets: 8, class: 'text-align-right td-kecil', 
				render: function ( data, type, full, meta ) { 
					var ret = formatNumberForUser(full[21]);
					return ret;
                }
			},
			...
			{	targets: 12, class: 'text-align-center td-kecil',
                render: function ( data, type, full, meta ) { 
					...
					if(full[11]){
						...
						if(full[10]){
							ret="<a onclick=\"infoVoucher('"+full[14]+"')\">"+full[10]+"</a>";
						}
						if(full[9]){
							ret="<a onclick=\"infoKasKecil('"+full[9]+"','"+full[2]+"')\">"+full[9]+"</a>";
						}
					}
					return ret;
                }
