...
$this->title = 'Laporan Hutang Pembelian Bahan Baku';
		...
		<div class="portlet light bordered">
			...
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject hijau bold"><?= Yii::t('app', 'Rekap Hutang Suplier Bahan Baku'); ?><span id="periode-label" class="font-blue-soft"></span></span>
				</div>
				<div class="tools">
				...
				<div class="place-total">
					<table class="table table-striped table-bordered table-hover" style="width: 100%;">
						<tr style="background-color: #D7E1EC">
							<td colspan="2" style="text-align: right"><b>Total Page &nbsp; </b></td>
						...
						<tr style="background-color: #D7E1EC">
							<td colspan="2" style="text-align: right"><b>Total Semua &nbsp; </b></td>
							<td style="width:190px; font-weight: 600;" class='text-align-center'>
...
function dtLaporan(){
    ...
    var dt_table =  $('#table-laporan').dataTable({
		...
		pageLength: 100,
        ajax: { 
			url: '<?= \yii\helpers\Url::toRoute('/finance/laporan/TagihanSuplierBB') ?>',
			data:{
				dt: 'table-laporan',
        ...
        columnDefs: [
			...
			{ 	targets: 2, 
				render: function(data, type, full, meta){
					return "<span class='tooltips' data-original-title='Klik untuk lihat detail tagihan'><a onclick='showTBP("+full[1]+")' style='color:#333;' onMouseOver='this.style.color=\"#4B77BE\"' onMouseOut='this.style.color=\"#333\"'>"+
                                                        full[2] 
                                                +"</a> &nbsp; </span>";
				}
			},
		...
		"fnDrawCallback": function( oSettings ) {
			...
			var totaltagihan = api.column( 4 ).data().reduce( function (a, b) { return intVal(a) + intVal(b); }, 0 );
			var totallunas = api.column( 5 ).data().reduce( function (a, b) { return intVal(a) + intVal(b); }, 0 );
			var totalhutang = api.column( 6 ).data().reduce( function (a, b) { return intVal(a) + intVal(b); }, 0 );
...
function showTBP(suplier_id){
	openModal('<?= \yii\helpers\Url::toRoute(['/finance/laporan/tagihanSuplierBB']) ?>?show=tagihan&suplier_id='+suplier_id,'modal-show-tbp','75%');
}
function infoTBP(id){
...
function infoTagihan(id){
	var url = '<?= \yii\helpers\Url::toRoute(['/finance/openvoucher/infoOpenvoucher','id'=>'']) ?>'+id;
	var modal_id = 'modal-info-tbp';
	$(".modals-place-2").load(url, function() {
		$("#"+modal_id).modal('show');
		$("#"+modal_id).on('hidden.bs.modal', function (e) {
