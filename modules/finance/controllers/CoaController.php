...
class CoaController extends DeltaBaseController
{
	...
	public function actionSource(){
		...
		$sql = "SELECT acct_tipenya as text, min(acct_no) AS id FROM m_acct_rekening WHERE active = TRUE GROUP BY text ORDER BY id ASC";
		$roots = Yii::$app->db->createCommand($sql)->queryAll();
		foreach($roots as $i => $root){
			$data[$i] = $root;
			$data[$i]['id'] = $data[$i]['id'].'-root';
			$sql2 = "SELECT * FROM m_acct_rekening 
					WHERE acct_tipenya = '".$root['text']."' AND acct_no ILIKE '%000%' AND active = TRUE AND acct_flag = FALSE 
			...
			foreach($childrens as $ii => $child){
				...
				$sql3 = "SELECT * FROM m_acct_rekening 
						WHERE acct_tipenya = '".$root['text']."' AND active = TRUE 
