...
function dtMaster(){
    var dt_table =  $('#table-dokumen').dataTable({
        ajax: { url: '<?= isset($url)?$url:\yii\helpers\Url::toRoute('/qms/dokumen/masterOnModal'); ?>',data:{dt: 'table-dokumen'} },
        order: [
            [2, 'asc']
        ],
        columnDefs: [
            ...
            {	targets: 5, 
                ...
                render: function ( data, type, full, meta ) {
                    return "<center><a onclick='pick(\""+full[0]+"\",\""+full[1]+"\")' class='btn btn-xs btn-icon-only btn-default' data-original-title='Pick' style='width: 25px; height: 25px;'><i class='fa fa-plus-circle'></i></a><center>";
                }
            },
        ],
