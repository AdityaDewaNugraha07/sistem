                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    ...
                                    <span class="caption-subject hijau bold"><?= Yii::t('app', 'Master Dokumen'); ?></span>
                                </div>
                                <div class="tools">
...
<?php $this->registerJs(" dtMaster(); setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Dokumen'))."')", yii\web\View::POS_READY); ?>
<script>
function dtMaster(){ 
    var dt_table =  $('#table-master').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/qms/dokumen/index') ?>',data:{dt: 'table-master'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
...
function create(){
	openModal('<?= \yii\helpers\Url::toRoute('/qms/dokumen/create') ?>','modal-master-create', '70%');
}
function info(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/qms/dokumen/info','id'=>'']) ?>'+id,'modal-master-info');
}
function printout(caraPrint){
    var search = $('.form-control.input-sm.input-small.input-inline').val();
	window.open("<?= yii\helpers\Url::toRoute('/qms/dokumen/dokumenPrint') ?>?"+$('#form-search-laporan').serialize()+"&caraprint="+caraPrint+"&search="+search,"",'location=_new, width=1200px, scrollbars=yes');
}
</script> 