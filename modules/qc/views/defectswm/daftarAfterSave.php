    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Daftar Defect Sawmill Yang Telah Dibuat'); ?></h4>
            </div>
...
function dtAfterSave(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/qc/defectswm/daftarAfterSave') ?>',data:{dt: 'modal-aftersave'} },
        order: [],
        columnDefs: [
...
function lihatDetail(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/qc/defectswm/index','defect_swm_id'=>'']); ?>'+id);
}

function edit(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/qc/defectswm/index','defect_swm_id'=>'']); ?>'+id+'&edit=1');
}
