...
$this->title = 'Laporan Losstime Sawmill';
		...
		<div class="portlet light bordered">
			...
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject hijau bold"><?= Yii::t('app', 'Laporan Losstime Sawmill '); ?><span id="periode-label" class="font-blue-soft"></span></span>
				</div>
				<div class="tools">
...
function dtLaporan(){
    var dt_table =  $('#table-laporan').dataTable({
		pageLength: 100,
        ajax: { 
			url: '<?= \yii\helpers\Url::toRoute('/qc/laporan/losstimeswm') ?>',
			data:{
				dt: 'table-laporan',
        ...
        columnDefs: [
			...
			{	targets: 9, class: 'text-align-center td-kecil',
				render: function ( data, type, full, meta ) {
					var start = new Date(full[7]);
					var end = new Date(full[8]);
					var selisih = end - start; // selisih dalam ms (milisecon)
        			var selisih_menit = Math.floor(selisih / 60000); // conv ke menit
					return formatNumberForUser(selisih_menit);
                }
			},
			{	targets: 10, class: 'td-kecil',
				render: function ( data, type, full, meta ) {
					return full[9]?full[9]:'<center>-</center>';
                }
			},
        ],
...
function printout(caraPrint){
	window.open("<?= yii\helpers\Url::toRoute('/qc/laporan/losstimeswmPrint') ?>?"+$('#form-search-laporan').serialize()+"&caraprint="+caraPrint,"",'location=_new, width=1200px, scrollbars=yes');
}
</script>