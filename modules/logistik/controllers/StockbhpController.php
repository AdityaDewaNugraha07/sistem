	...
	public function actionGetStockActivity(){
		if(\Yii::$app->request->isAjax){
			...
			if(!empty($bhp_id)){
				...
				$modPersediaan = \app\models\HPersediaanBhp::find()->where(['bhp_id'=>$bhp_id])->andWhere("tgl_transaksi BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."' ")->orderBy(['created_at'=>SORT_ASC])->all();
				$saldoawal = \app\models\HPersediaanBhp::find()->select("sum(qty_in)-sum(qty_out) as total_qty")->where(['bhp_id'=>$bhp_id])->andWhere("tgl_transaksi < '{$tgl_awal}'")->one()->total_qty;
				$data['html'] .= $this->renderPartial('_stockActivity',['modPersediaan'=>$modPersediaan,'bhp_id'=>$bhp_id,'modBhp'=>$modBhp,'saldoawal'=>$saldoawal, 'tgl_awal'=>$tgl_awal, 'tgl_akhir'=>$tgl_akhir]);
