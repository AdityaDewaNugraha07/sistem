...
class SupplierController extends DeltaBaseController
{
    ...
    public function actionIndex()
    {
        ...
        if (\Yii::$app->request->get('dt') === 'table-laporan') {
            if ((\Yii::$app->request->get('laporan_params')) !== null) {
                ...
                $model->suplier_ket = $form_params['MSuplier']['suplier_ket'];
            }
    ...
    public function actionCreate()
    {
        if (\Yii::$app->request->isAjax) {
            $model = new MSuplier();
            $model->active = true;
            if ((Yii::$app->user->identity->user_group_id === Params::USER_GROUP_ID_KANIT_LOG_SENGON) || (Yii::$app->user->identity->user_group_id === Params::USER_GROUP_ID_STAFF_LOG_SENGON)) {
                $model->type = "LS";
            } elseif ((Yii::$app->user->identity->user_group_id === Params::USER_GROUP_ID_KANIT_LOG_ALAM) || (Yii::$app->user->identity->user_group_id === Params::USER_GROUP_ID_STAFF_LOG_ALAM)) {
                $model->type = "LA";
            }
            if (Yii::$app->request->post('MSuplier')) {
                ...
                try {
                    ...
                    if ($model->validate()) {
                        ...
                        $data['message_validate'] = ActiveForm::validate($model);
                    }
                    if ($success_1) {
                        $transaction->commit();
                        $data['status'] = true;
