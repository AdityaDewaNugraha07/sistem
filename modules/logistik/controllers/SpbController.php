...
//				exit;
    ...
    public function actionDaftarSpb(){
        if(\Yii::$app->request->isAjax){
			...
			if(\Yii::$app->request->get('dt')=='table-aftersave'){
				...
				$param['column'] = [ $param['table'].'.spb_id',
									'spb_kode',
									'spb_tanggal', 
									'pegawaidiminta.pegawai_nama AS diminta', 
	...
	public function actionDaftarBpbblmterima(){
        if(\Yii::$app->request->isAjax){
			$departement_id = Yii::$app->request->get('dept_id');
			if(\Yii::$app->request->get('dt')=='table-aftersave'){
				$param['table']= \app\models\TBpb::tableName();
				$param['pk']= $param['table'].'.'.\app\models\TBpb::primaryKey()[0];
				$param['column'] = [ $param['table'].'.bpb_id',
									'bpb_kode',
									'bpb_status',
									'created_at',
									];
				$param['where'] = "departement_id = ".$departement_id. " and bpb_status = 'BELUM DITERIMA' ";
				return \yii\helpers\Json::encode(\app\components\SSP::complex( $param ));
			}
			return $this->renderAjax('daftarBpbblmterima',['departement_id'=>$departement_id]);
        }
    }
	...
	function actionTerimaBarang($id){
		if(\Yii::$app->request->isAjax){
			...
			$pesan = Yii::t('app', 'Apakan anda yakin akan melakukan penerimaan barang tanpa set plan?');
			$data = [];
			if( Yii::$app->request->post('deleteRecord')){
			...
			return $this->renderAjax('@views/apps/partial/_deleteRecordConfirmTerimaBarang',[
				'id'=>$id,
				'pesan'=>$pesan,
	...
	public function actionInfoApproval($spb_id){
		if(\Yii::$app->request->isAjax){
			$model = \app\models\TSpb::findOne($spb_id);
			$modDetail = \app\models\TSpbDetail::find()->where(['spb_id'=>$spb_id])->all();
			$data = [];
			if( Yii::$app->request->get('refresh')){
				$data = $model;
				return $this->asJson($data);
			}
			return $this->renderAjax('infoApproval',['model'=>$model,'modDetail'=>$modDetail]);
		}
	}
	...
	function actionTerimaBarangModal($bpb_id){
		if(\Yii::$app->request->isAjax){
			$modTerimaBhpSub = new \app\models\TTerimaBhpSub();
			$modPersediaanBhpSub = new \app\models\HPersediaanBhpSub();
			$model = \app\models\TBpb::findOne($bpb_id);
			$model->bpb_tgl_diterima = (!empty($model->bpb_tgl_diterima)?\app\components\DeltaFormatter::formatDateTimeForUser2($model->bpb_tgl_diterima):" - ");
			$model->bpb_diterima = (!empty($model->bpb_diterima)?$model->bpbDiterima->pegawai_nama:" - ");
			$modDetail = \app\models\TBpbDetail::find()->where(['bpb_id'=>$model->bpb_id])->all();
			$detail = \app\models\TBpbDetail::findOne(['bpb_id'=>$model->bpb_id]);
	
			if(Yii::$app->request->post('TTerimaBhpSub')){
				$transaction = Yii::$app->db->beginTransaction();
				try {
					$success_1 = false; // t_terima_bhp_sub
					$success_2 = false; // t_bpb
					$success_3 = false; // h_persediaan_bhp_sub
					// if($modTerimaBhpSub->load(Yii::$app->request->post(), '')){
						// $modTerimaBhpSub->save();
						// print_r($modTerimaBhpSub->errors);
						// exit;
						$kode = \app\components\DeltaGenerator::kodeBudgeting();
						foreach($_POST['TTerimaBhpSub'] as $i => $terimabhp){
							$modTerimaBhpSub = new \app\models\TTerimaBhpSub();
							$modTerimaBhpSub->attributes = $terimabhp;
							$modTerimaBhpSub->kode = $kode;
							$modTerimaBhpSub->departement_id = Yii::$app->user->identity->pegawai->departement_id;
							$modTerimaBhpSub->status_approval = \app\models\TApproval::STATUS_NOT_CONFIRMATED;
							$modTerimaBhpSub->tanggal = date('Y-m-d');
							if($modTerimaBhpSub->validate()){
								if($modTerimaBhpSub->save()){
									$success_1 = true;
							
									$modPersediaanBhpSub = new \app\models\HPersediaanBhpSub();
									$modPersediaanBhpSub->attributes 		= $modTerimaBhpSub->attributes;
									$modPersediaanBhpSub->bhp_id 			= $modTerimaBhpSub->bhp_id;
									$modPersediaanBhpSub->waktu_transaksi 	= date('Y-m-d H:i:s');
									$modPersediaanBhpSub->reff_no 			= $modTerimaBhpSub->kode;
									$modPersediaanBhpSub->reff_detail_id 	= $modTerimaBhpSub->terima_bhp_sub_id;
									$modPersediaanBhpSub->tgl_transaksi		= date('Y-m-d');
									$modPersediaanBhpSub->qty_in			= $modTerimaBhpSub->qty;
									if($modPersediaanBhpSub->validate()){
										if($modPersediaanBhpSub->save()){
											$success_3 = true;
										}
									} else {
										$success_3 = false;
						...
						if(!empty($model)){
							$model->bpb_status = "SUDAH DITERIMA";
							$model->bpb_tgl_diterima = \app\components\DeltaFormatter::formatDateTimeForUser2(date('d/m/Y'));
							$model->bpb_diterima = Yii::$app->user->identity->pegawai_id;
							if($model->validate()){
								if($model->save()){
									$success_2 = true;
						...
						if ($success_1 && $success_2 && $success_3 ) {
							$transaction->commit();
							// return $this->redirect(['index','success'=>1,'spb_id'=>$model->spb_id]); // ini masih menampilkan error 302
							$data['status'] = true;
		...
		return $this->renderAjax('terimaBarang',['model'=>$model,'modDetail'=>$modDetail, 'modTerimaBhpSub'=>$modTerimaBhpSub]);
	}	
	
	public function actionAddItemRincian($bpb_id){
        if(\Yii::$app->request->isAjax){
            $modTerimaBhpSub = new \app\models\TTerimaBhpSub();
			$modPersediaanBhpSub = new \app\models\HPersediaanBhpSub();
			$model = \app\models\TBpb::findOne($bpb_id);
			$detail = \app\models\TBpbDetail::findOne(['bpb_id'=>$model->bpb_id]);
			$query = "
					SELECT t_bpb_detail.bhp_id, t_bpb_detail.bpbd_id, bhp_nm, bpbd_jml FROM t_bpb_detail 
					JOIN m_brg_bhp ON m_brg_bhp.bhp_id = t_bpb_detail.bhp_id
					WHERE bpb_id = $bpb_id
					ORDER BY t_bpb_detail.bpbd_id ASC
					;
				";
			$mod = Yii::$app->db->createCommand($query)->queryAll();
			if(count($mod)>0){
				$arraymap = \yii\helpers\ArrayHelper::map($mod, 'bpbd_id', function ($m) {
					return $m['bhp_nm'] . ' (qty terpenuhi: ' . $m['bpbd_jml'] . ')';
				});
				foreach($mod as $i => $val){
					$bhp_nm = $val['bhp_nm'].' (qty terpenuhi: '.$val['bpbd_jml'].')';
					$data[] = ['id'=>$val['bpbd_id'], 'text'=>$bhp_nm];
				}
			}
            $data['item'] = $this->renderPartial('_addItemRincian',['bpb_id'=>$bpb_id, 'arraymap'=>$arraymap,'modTerimaBhpSub'=>$modTerimaBhpSub, 'modPersediaanBhpSub'=>$modPersediaanBhpSub, 'model'=>$model, 'detail'=>$detail]);
            return $this->asJson($data);
        }
    }


	function actionSetItemRincian(){
		if(\Yii::$app->request->isAjax){
            $bpbd_id = Yii::$app->request->post('bpbd_id');
			$detail = \app\models\TBpbDetail::findOne(['bpbd_id'=>$bpbd_id]);
            if(!empty($bpbd_id)){
                $data = \app\models\MBrgBhp::findOne($detail->bhp_id);
            }else{
                $data = [];
            }
            return $this->asJson($data);
	...
	function actionSetBhp($bpb_id){
		if(\Yii::$app->request->isAjax){
            $bpbd_id = Yii::$app->request->post('bpbd_id');
            if(!empty($bpb_id)){
                $data = \app\models\TBpbDetail::findOne(['bpbd_id'=>$bpbd_id, 'bpb_id'=>$bpb_id]);
            }else{
                $data = [];
            }
            return $this->asJson($data);
