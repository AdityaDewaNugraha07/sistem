            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
						...
						<?php echo $form->field($model, 'deskripsi')->textarea(['style'=>'font-size:1.2rem', 'rows'=>5])->label("Keterangan Retur"); ?>
                    </div>
					<div class="col-md-6">
						...
						<?php echo $form->field($model, 'ppn_nominal')->textInput(['class'=>'form-control float','readonly'=>'readonly', 'value'=>\app\components\DeltaFormatter::formatNumberForUser($model->ppn_nominal)])->label("PPn"); ?>
						<?php echo yii\bootstrap\Html::activeHiddenInput($model, 'ppn_nominal',['readonly'=>'readonly']); ?>
...
function setTotalKembali(){
	...
	var ppn_nominal = $('#modal-transaksi').find('input[name*="[ppn_nominal]"]').val();
	var qty_retur = $('#modal-transaksi').find('input[name*="[qty]"]').val();
	harga_retur = (unformatNumber(harga_terima)-unformatNumber(potongan));
	total_terima = (harga_retur + unformatNumber(ppn_nominal)) * qty_retur;
