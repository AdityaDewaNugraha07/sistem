...
<tr style="">
    ...
    <td style="vertical-align: middle; text-align: center;">
		...
		if(!empty($modelDetail->terima_bhpd_id)){
			...
			if(!empty($mod)) {
            ?>
				xxx <a onclick="infoReturBHP(<?= $mod['retur_bhp_id'] ?>);" class="blue-steel" style="font-size: 1rem"><?= $mod['kode']; ?></a>
            <?php 
            } else { 
                if ($model->cancel_transaksi_id > 0 || $model->cancel_transaksi_id != NULL || $model->cancel_transaksi_id != null) {
                ?>
