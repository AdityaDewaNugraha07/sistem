                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <div class="col-md-7"><?= DeltaFormatter::formatDateTimeForUser2($model->spb_tanggal); ?></div>
                        </div>
                        <div class="form-group col-md-12">
                        ...
                        <?php if($tampilAlasan && !empty($model->reason_ditolak)): ?>
                            <div class="form-group col-md-12">
                                <label class="col-md-5 control-label"><strong><?= Yii::t('app', 'Alasan ditolak'); ?></strong></label>
                                <div class="col-md-7"><?php $ditolak = Json::decode($model->reason_ditolak); echo $ditolak['alasan_ditolak'] ?>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group col-md-12">
                            <label class="col-md-5 control-label"><strong>Alasan</strong></label>
                            <div class="col-md-7"  style="margin-top:7px;">
                                <?php 
                                    if(!empty($model->reason_approval)) {
                                        foreach(Json::decode($model->reason_approval) as $res) {
                                            if($res['status'] === TApproval::STATUS_REJECTED) {
                                                echo $res['reason'];
                                            }
                                        }
                                    }else {
							...
							<table class="table table-striped table-bordered table-advance table-hover" id="table-detail">
								...
								<tbody>
									...
									<tr>
										...
										<td><?php echo (!empty($detail->spbd_tgl_dipakai)? DeltaFormatter::formatDateTimeForUser2($detail->spbd_tgl_dipakai):'<center>-</center>'); ?></td>
...
//                                      } 
                                ?>
                                <?php // if($model->spb_diminta == \app\components\Params::DEFAULT_PEGAWAI_ID_AGUNG_ENGINERING){
//					echo '<span class="pull-left">'.yii\helpers\Html::a(Yii::t('app', '<i class="fa fa-print"></i>&nbsp; Print '),'javascript:void(0)',['class'=>'btn blue btn-outline','onclick'=>'printSpb('.$model->spb_id.')']).' &nbsp; </span>';
//                                      } 
                                ?>     
                                <?php echo '<span class="pull-left">'.yii\helpers\Html::a(Yii::t('app', '<i class="fa fa-print"></i>&nbsp; Print '),'javascript:void(0)',['class'=>'btn blue btn-outline','onclick'=>'printSpb('.$model->spb_id.')']).' &nbsp; </span>'; ?>        
                ...
                <?php
                if(($model->approve_status != app\models\TApproval::STATUS_NOT_CONFIRMATED) && ($model->approve_status != app\models\TApproval::STATUS_REJECTED) && (!in_array($model->spb_status, ['TERPENUHI', 'DITOLAK']))){
                    $jam_skrg = date("H:i:s");
                    if($jam_skrg < "15:30:00"){ //  maksimal SPP input jam 15.30 sore
                        echo Html::a(Yii::t('app', '<i class="fa fa-external-link"></i>&nbsp; Proses ke SPP '),'javascript:void(0)',['class'=>'btn blue-hoki btn-outline','onclick'=>'lanjutSpp('.$model->departement_id.','.$model->spb_id.')']);
                    }else{
                        echo Html::a(Yii::t('app', '<i class="fa fa-external-link"></i>&nbsp; Proses ke SPP '),'javascript:void(0)',['class'=>'btn grey btn-outline tooltips','data-original-title'=>'Waktu Proses SPP Maksimal 15:30 WIB']);
                    }
                    echo Html::a(Yii::t('app', '<i class="fa fa-external-link"></i>&nbsp; Proses ke BPB '),'javascript:void(0)',['class'=>'btn green-meadow btn-outline','onclick'=>'lanjutBpb('.$model->departement_id.','.$model->spb_id.')']);
                }

                if(!in_array($model->approve_status, [TApproval::STATUS_NOT_CONFIRMATED, TApproval::STATUS_REJECTED]) && $model->spb_status == 'BELUM DIPROSES'){
                    if($model->mutation_req !== TRUE){
                        echo Html::a(Yii::t('app', '<i class="fa fa-external-link"></i>&nbsp; Ajukan Mutasi '),'javascript:void(0)',['class'=>'btn blue-ebonyclay btn-outline','onclick'=>'permintaanMutasi('.$model->spb_id.')']);
                    }
                    echo Html::a(Yii::t('app', '<i class="fa fa-edit"></i>&nbsp; Edit SPB '),'javascript:void(0)',['class'=>'btn yellow btn-outline','onclick'=>'editSpb('.$model->spb_id.')']);
                    echo Html::a(Yii::t('app', '<i class="fa fa-remove"></i>&nbsp; Tolak SPB '),'javascript:void(0)',['class'=>'btn red-intense btn-outline','onclick'=>'tolakSpb('.$model->spb_id.')']);
                }
                ?>
            </div>
        </div>
...
function lihatSpb(spb_id){
    window.location.replace('<?= Url::toRoute(['/logistik/spb/index','spb_id'=>'']); ?>'+spb_id);
}
function editSpb(spb_id){
    window.location.replace('<?= Url::toRoute(['/logistik/spb/index','edit'=>true,'spb_id'=>'']); ?>'+spb_id);
}
function tolakSpb(spb_id){
    $(".modals-place-2").load('<?= Url::toRoute(['/logistik/penerimaanspb/tolakSpb','id'=>'']) ?>'+spb_id, function() {
    ...
    //$.ajax({
    //    url    : '<?php//= Url::toRoute(['/logistik/penerimaanspb/tolakSpb']) ?>//',
    //    type   : 'POST',
    //    data   : {spb_id:spb_id},
...
function lanjutBpb(dept_id,spb_id){
    window.open("<?= Url::toRoute('/logistik/bpb/index') ?>?loadjs[dept_id]="+dept_id+"&loadjs[spb_id]="+spb_id,"_blank");
}
function lanjutSpp(dept_id,spb_id){
    window.open("<?= Url::toRoute('/logistik/spp/index') ?>?loadjs[dept_id]="+dept_id+"&loadjs[spb_id]="+spb_id,"_blank");
}
function permintaanMutasi(spb_id){
    openModal('<?= Url::toRoute(['/logistik/penerimaanspb/permintaanMutasi','id'=>'']); ?>'+spb_id,'modal-global-confirm');
}
