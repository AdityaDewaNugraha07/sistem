...
class PengajuanpembelianlogController extends DeltaBaseController
{
	...
	public function actionIndex(){
		...
		if(isset($_GET['pengajuan_pembelianlog_id'])){
			...
			$model->total_volume = \app\components\DeltaFormatter::formatNumberForUserFloat($model->total_volume,4);
        ...
        if( Yii::$app->request->post('TPengajuanPembelianlog')){
            ...
            try {
				...
				if(!isset($_GET['edit'])){
                    ...
                    $model->kode_cardpad = \app\components\DeltaGenerator::kodeCardPad();
				}else{
		...
		if(\Yii::$app->request->isAjax){
            ...
            if(count($modMap)>0){
                foreach($modMap as $i => $map){
                    ...
                    $data['html_pmr'] .= $this->renderPartial('_itemPermintaan',['modPmr'=>$modPmr,'i'=>$i,'edit'=>$edit]);
                }
            }
	...
	public function actionSetKontrak($log_kontrak_id){
		if(\Yii::$app->request->isAjax){
            ...
            if(!empty($log_kontrak_id)){
                ...
                // ambil data dari t_pmr yang masuk dalam 
                // cari hasil_orientasi_id nya dulu dul
                $hasil_orientasi_id = $model->hasil_orientasi_id;
                
                // trus cari gradernya
                $graders = \app\models\TDkg::find()->where(['hasil_orientasi_id'=>$hasil_orientasi_id])->all();
                foreach ($graders as $grader => $t_dkg) {
                    $dkg_id = $t_dkg->dkg_id;
                    $pmrs = \app\models\MapDkgPmrPengajuanPembelianlog::find()->where(['dkg_id'=>$dkg_id])->all();
                    $data['html'] = "";
                    $data['pmr_id'] = "";
                    foreach ($pmrs as $pmr => $modMap) {
                        $pmr_id = $modMap->pmr_id;
                        $modPmr = \app\models\TPmr::find()->where(['pmr_id'=>$pmr_id])->one();
                        $total_m3 = \app\models\TPmrDetail::find()->select("SUM(qty_m3) AS total_m3")->where("pmr_id = ".$modPmr->pmr_id)->one();
                        $modPmr->total_m3 = $total_m3->total_m3;
                        $data['html'] .= $this->renderPartial('_itemPermintaan',['modPmr'=>$modPmr]);
                        $data['pmr_id'] .= $pmr_id;                                
                    }
                }
			}
