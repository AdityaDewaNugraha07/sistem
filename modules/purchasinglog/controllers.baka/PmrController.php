...
class PmrController extends DeltaBaseController
{
    ...
    public function actionInfoStock(){
		if(\Yii::$app->request->isAjax){
            $jenis_log = \Yii::$app->request->get('jenis_log');
			return $this->renderAjax('_infoStock',['jenis_log'=>$jenis_log]);
        }
    }

    public function actionInfoStockLS(){
		$model = new \app\models\HPersediaanLog();
		$model->tgl_transaksi = date('d/m/Y');
        $data = [];
        $data['html'] = '';
        if(!empty($model->tgl_transaksi)){
            $data['html'] = $this->renderPartial('../../../ppic/views/stocksengon/rekapContent',['model'=>$model, 'afkir'=>0]);
        }
        return $this->asJson($data);
    }
   
    public function actionInfoStockLJ(){
		$model = new \app\models\HPersediaanLog();
		$model->tgl_transaksi = date('d/m/Y');
        $data = [];
        $data['html'] = '';
        if(!empty($model->tgl_transaksi)){
            $data['html'] = $this->renderPartial('../../../ppic/views/stocksengon/rekapContentJabon',['model'=>$model, 'afkir'=>0]);
        }
        $data['xxx'] = 'xxx';
        return $this->asJson($data);
    }

    public function actionInfoTracking() {
        $jenis_log = Yii::$app->request->get('jenis_log');
        if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='modal-daftarSpmLogTracking'){
                ...
                $param['where'] = "t_spk_shipping.status = 'APPROVED' and (t_spk_shipping.status_jenis IS NULL or t_spk_shipping.status_jenis = 0) ";
                $param['order'] = "ORDER BY t_spk_shipping_tracking DESC";
                $param['limit'] = "1";*/
                
                $param['table']= \app\models\TSpkShipping::tableName();
                $param['pk']= $param['table'].'.'.\app\models\TSpkShipping::primaryKey()[0];
				$param['column'] = [$param['table'].'.spk_shipping_id',         //0
                                    $param['table'].'.kode',                    //1
                                    $param['table'].'.etd',                     //2
                                    $param['table'].'.eta_logpond',             //3
                                    $param['table'].'.eta',                     //4
                                    $param['table'].'.lokasi_muat',             //5
                                    $param['table'].'.estimasi_total_batang',   //6
                                    $param['table'].'.status_jenis',            //7
									];
                $param['where'] = "t_spk_shipping.status = 'APPROVED' and (t_spk_shipping.status_jenis IS NULL or t_spk_shipping.status_jenis = 0)";
                
				return \yii\helpers\Json::encode(\app\components\SSP::complex( $param ));
			}
			return $this->renderAjax('_infoTracking',['jenis_log'=>$jenis_log]);
        }
    }
}
