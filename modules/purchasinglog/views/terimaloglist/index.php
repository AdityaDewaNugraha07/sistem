                            ...
                            <div class="portlet-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
										...
										<?= $form->field($model, 'nomor_kontrak')->textarea(['disabled'=>true]); ?>
										<?= $form->field($model, 'kode_bajg')->textarea(); ?>
                                        ...
                                        <?= $form->field($model, 'area_pembelian')->inline(true)->radioList(['Jawa'=>'Jawa','Luar Jawa'=>'Luar Jawa'],['onchange'=>'setDiameter()'])->label('Area Pembelian'); ?>
                                    </div>
                                </div>
                                <br><br><hr>
...
function hitungRata(obj){
    if( $(obj).parents('tr').find('input[name*="[diameter_ujung]"]').length!=0 ){ // jika 2 diameter
        ...
        if( $("input:radio[name*='[area_pembelian]']:checked").val() == "Jawa" ){ // Jawa, D rata = ROUND(diameter_rata)
            var diameter_rata = Math.round(( (ujung) + (pangkal) )/2);
        } else { // Luar Jawa, D rata = ROUNDDOWN(diameter_rata)
            var diameter_rata = Math.floor(( (ujung) + (pangkal) )/2);
        }

        if((ujung) && (pangkal)){
            $(obj).parents('tr').find('input[name*="[diameter_rata]"]').val( diameter_rata );
        }else{
        ...
        if( $("input:radio[name*='[area_pembelian]']:checked").val() == "Jawa" ){ // Jawa, D rata = ROUND(diameter_rata)
            var diameter_rata = Math.round(( (ujung1) + (ujung2) + (pangkal1) + (pangkal2) )/4);
        } else { // Luar Jawa, D rata = ROUNDDOWN(diameter_rata)
            var diameter_rata = Math.floor(( (ujung1) + (ujung2) + (pangkal1) + (pangkal2) )/4);
        }
    ...
    // setdropdown range diameter
    if(diameter_rata > 0){
        var dropdown = $(obj).parents('tr').find('select[name*="[volume_range]"]');
        dropdown.find('option').each(function() {
            var val = $(this).val();
            if (val == '70-up') {
                if (diameter_rata >= 70) {
                    dropdown.val(val);
                    $(obj).parents('tr').find('input[name*="[volume_range]"]').val( val );
                    return false;
                }
            } else {
                if(val.includes('-')) {
                    ...
                    var min = dia[0];
                    var max = dia[1];
                    if(diameter_rata >= min && diameter_rata <= max){
                        dropdown.val(val);
                        $(obj).parents('tr').find('input[name*="[volume_range]"]').val( val );
                        return false;
                    }
    ...
    if( $("input:radio[name*='[area_pembelian]']:checked").val()){
        hitungVolume(obj);
    } else {
        cisAlert('Mohon pilih area pembelian!');
    }
}
...
function afterSave(id){
    ...
    // $('input[name*="[area_pembelian]"]').prop("disabled", false);
    <?php if(isset($_GET['edit'])){ ?>
...
function edit(ele){
    ...
    $(ele).parents('tr').find("input[name*='[diameter_rata]']").attr('disabled','');
    $(ele).parents('tr').find("select[name*='[volume_range]']").attr('disabled','');
    $(ele).parents('tr').find("input[name*='[volume_value]']").attr('disabled','');
    $(ele).parents('tr').find(".input2_diameter_rata").removeAttr('disabled');
    $(ele).parents('tr').find(".input2_volume_value").removeAttr('disabled');
    console.log();
}
...
function hitungVolume(obj, parent = 'tr'){
    ...
    // PEMBELIAN JAWA
    if( $("input:radio[name*='[area_pembelian]']:checked").val() == "Jawa" ){
        ...
        var zzz = (0.7854 * (panjang - (cacat_panjang / 100)) * ((ratarata - cacat_gb/100) * (ratarata - (cacat_gb)) * 1) / 10000) - (pGrowong);
    ...
    // PEMBELIAN LUAR JAWA
        ...
        var Vol = Math.floor( zzz * 100) / 100;
    }
    console.log( Vol );   
    $(obj).parents(parent).find('input[name*="[volume_value]"]').val(Vol); 
}
</script>