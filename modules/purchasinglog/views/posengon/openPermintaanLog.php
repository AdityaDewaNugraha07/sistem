    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Pilih Permintaan Pembelian Log Sengon/Log Jabon'); ?></h4>
            </div>
...
function dtAfterSave(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/purchasinglog/posengon/openpermintaanlog'); ?>',data:{dt: 'modal-aftersave'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
			...
			{ 	targets: 1, 
                render: function ( data, type, full, meta ) {
                    var asd = full[12];
                    if(asd=="<?= app\models\TApproval::STATUS_APPROVED ?>" && (full[15]=="OPEN") && (!full[14]) ){
