    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Detail Keputusan Pembelian Log Alam'); ?></h4>
            </div>
...
function dtAfterSave(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/purchasinglog/spmlog/openDetailKeputusanPembelianlog') ?>',data:{dt: 'modal-aftersave'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
            ...
            { 	targets: 1, 
                render: function ( data, type, full, meta ) {
                    var ret =  "<a onclick='pickKeputusanPembelianLog(\""+full[0]+"\")' class='btn btn-xs btn-icon-only btn-default tooltips' data-original-title='Pick' style='width: 25px; height: 25px;'><i class='fa fa-plus-circle'></i></a>"+data;
                    return ret;
                }
            },
			{ 	targets: 2, 
