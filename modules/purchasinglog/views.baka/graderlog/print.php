		...
		<td colspan="3" style="padding: 0px;">
			<table style="width: 100%" id="table-detail">
				<tr style="border-bottom: solid 1px #000; border-top: solid 1px #000;">
					...
					<th style="padding: 10px; border: solid 1px #000;"><?= Yii::t('app', 'Type'); ?></th>
					<th style="padding: 10px; border: solid 1px #000;"><?= Yii::t('app', 'Status'); ?></th>
				</tr>
				
				<?php 
				foreach ($model as $key) {
					$graderlog_kode = $key['graderlog_kode'];
					$graderlog_nm = $key['graderlog_nm'];
					$graderlog_almt = $key['graderlog_almt'];
					$graderlog_phone = $key['graderlog_phone'];
					$type = $key['type'];
					$active = $key['active'];
					$active == 1 ? $status = 'Active' : $status = 'Non-Active'
				?>
				<tr>
					<td style="padding: 3px; border: solid 1px #000;"><?= $graderlog_nm; ?></td>
					<td style="padding: 3px; border: solid 1px #000;"><?= $graderlog_kode; ?></td>
					<td style="padding: 3px; border: solid 1px #000;"><?= $graderlog_almt; ?></td>
					<td style="padding: 3px; border: solid 1px #000;"><?= $graderlog_phone; ?></td>
					<td style="padding: 3px; border: solid 1px #000;"><?= $type; ?></td>
