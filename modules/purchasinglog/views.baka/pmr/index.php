                            ...
                            <div class="portlet-body">
                                <div class="row">
                                    <div class="col-md-5">
                                        ...
                                        <?= $form->field($model, 'jenis_log')->dropDownList(["LA"=>"LOG ALAM","LS"=>"LOG SENGON","LJ"=>"LOG JABON"],['class'=>'form-control',"prompt"=>'','onchange'=>'setHeader(); setDetail();']); ?>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <?php //<?= $form->field($model, 'approver_5')->dropDownList([],['class'=>'form-control','prompt'=>'','disabled'=>true]); ?>
                                    </div>
                                </div>
                                <br><br><hr>
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <span><?php echo \yii\helpers\Html::button( Yii::t('app', '<i class="fa fa-search"></i> Info Tracking'),['id'=>'btn-info-tracking','class'=>'btn blue btn-outline ciptana-spin-btn pull-right td-kecil', 'style'=>'margin-left: 10px;','onclick'=>'infoTracking();']); ?><span>
                                        <span><?php echo \yii\helpers\Html::button( Yii::t('app', '<i class="fa fa-search"></i> Info Stok'),['id'=>'btn-info-stock','class'=>'btn blue btn-outline ciptana-spin-btn pull-right td-kecil','onclick'=>'infoStock();']); ?></span>
                                    </div>
                                </div>
...
<?php $this->registerJs(" 
    ...
    setDetail();
    $('#btn-info-stock').hide();
    $('#btn-info-tracking').hide();
    $pagemode
", yii\web\View::POS_READY); ?>
<script>

function detail_permintaan(jenis_log) {
    console.log('tes');
}
...
function setDetail(){
    var jenis_log = $("#<?= \yii\helpers\Html::getInputId($model, "jenis_log") ?>").val();
    if (jenis_log == "LA") {
        $('#btn-info-stock').html('<i class=\'fa fa-search\'></i> Info Stock Log Alam');
        $('#btn-info-tracking').html('<i class=\'fa fa-search\'></i> Info Tracking Log Alam');
    } else if (jenis_log == "LS") {
        $('#btn-info-stock').html('<i class=\'fa fa-search\'></i> Info Stock Log Sengon');
        $('#btn-info-tracking').hide();
    } else if (jenis_log == "LJ"){
        $('#btn-info-stock').html('<i class=\'fa fa-search\'></i> Info Stock Log Jabon');
        $('#btn-info-tracking').hide();
    }
    ...
    <?php
    if (!empty($_GET['pmr_id'])) {
    ?>
    var pmr_id = '<?php echo $_GET['pmr_id'];?>';
    <?php
    } else {
    ?>
    var pmr_id = 0;
    ...
    $.ajax({
        ...
        type   : 'POST',
        data   : {jenis_log:jenis_log, success:success, pmr_id:pmr_id},
        success: function (data){
            if(data.html){
...
function infoStock() {
    var jenis_log = $("#<?= \yii\helpers\Html::getInputId($model, "jenis_log") ?>").val();
    if (jenis_log == "LA") {
        openModal('<?= \yii\helpers\Url::toRoute(['/purchasinglog/pmr/infoStock','jenis_log'=>'LA']) ?>','modal-infoStock','90%');
    } else if (jenis_log == "LS") {
        openModal('<?= \yii\helpers\Url::toRoute(['/purchasinglog/pmr/infoStock','jenis_log'=>'LS']) ?>','modal-infoStock','90%');
    } else if (jenis_log == "LJ"){
        openModal('<?= \yii\helpers\Url::toRoute(['/purchasinglog/pmr/infoStock','jenis_log'=>'LJ']) ?>','modal-infoStock','90%');
    }
}

function infoTracking() {
    var jenis_log = $("#<?= \yii\helpers\Html::getInputId($model, "jenis_log") ?>").val();
    console.log(jenis_log);
    openModal('<?= \yii\helpers\Url::toRoute(['/purchasinglog/pmr/infoTracking','jenis_log'=>'LA']) ?>','modal-infoTracking','90%');
}
