			...
			<table style="width: 100%; " border="0" id="table-detail">
				...
				foreach($modDetail as $i => $detail){
					...
					<?php }
						...
						// $trtotal = $trtotal+1;
						if($model->jenis_produk == "JasaKD" || $model->jenis_produk == "JasaGesek" || $model->jenis_produk == "JasaMoulding") {
							$namaProdukDitampilkan = false;
							foreach($modPaletKD as $iii => $paletKD) {?>
								<tr style='page-break-inside: avoid;'>
									<td style='padding: 2px 5px; border-right: 1px solid black;'>
										<?php 
										if (!$namaProdukDitampilkan) {
											echo $detail->produkJasa->nama;
										}
										?>
									</td>
									<td style='padding: 2px 5px; border-right: solid 1px #000;'>
										<?php echo " - ".$paletKD['t']." ".$paletKD['t_satuan']." x ".$paletKD['l']." ".$paletKD['l_satuan']." x ".$paletKD['p']." ".$paletKD['p_satuan'];?>
									</td>
									<td style='padding: 2px 5px; border-right: solid 1px #000;'>
										<span style='float: right;'>
											<?php echo $paletKD['qty_kecil']." (".$paletKD['satuan_kecil'].")";  ?>
										</span>
									</td>
									<td style='padding: 2px 5px; border-right: solid 1px #000;'>
										<span style='float: right;'>
											<?php echo number_format($paletKD['kubikasi'],4); ?>
										</span>
									</td>
									<td style='padding: 2px 5px;'>
										<?php 
						...
						<?php }
							...
							<tr>
								<td style='padding: 2px 5px; border-right: 1px solid black;'>
									<?php 
									// jika produk dengan grade mengandung kata fsc 100 % maka tampilkan nama ilimiah sesuai dengan jenis log
									$modProduk = \app\models\MBrgProduk::findOne(['produk_id' => $detail->produk->produk_id]);
									if ($modProduk && stripos($modProduk->grade, 'FSC 100') !== false) {
										$modJeniskayu = \app\models\MJenisKayu::findOne(['jenis_produk' => $modProduk->produk_group, 'nama' => $modProduk->jenis_kayu]);
										...
										echo $produkNama . "<br><em>" . htmlspecialchars($modJeniskayu['othername']) . "</em>";
									} else {
								...
								<td style='padding: 2px 5px; border-right: solid 1px #000;'>
									<?php echo "<span>".$detail->produk->produk_dimensi."</span>"; ?>
								</td>
								<td style='padding: 2px 5px; border-right: solid 1px #000;'>
									<?php 
				...
				<tr style="height: 0.4cm;  border-right: solid 1px transparent; ">
					...
					<td rowspan="3" style="width: 8cm; text-align: center; vertical-align: bottom;">
						...
						if($model->jenis_dokumen == "Nota Perusahaan"){
							echo '<img src="'.\Yii::$app->view->theme->baseUrl.'/cis/img/sertifikat_tanpa_CARB.png" alt="" class="logo-default" style="width: 100%; margin-bottom: -15px;"> 	&nbsp;'; 
							// echo '<img src="'.\Yii::$app->view->theme->baseUrl.'/cis/img/sertifikatplusFSC.jpg" alt="" class="logo-default" style="width: 100%; margin-bottom: -15px;"> 	&nbsp;'; 
						}
						?>
					</td>
