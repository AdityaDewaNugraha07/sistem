...
$this->title = 'Laporan Dokumen Penjualan';
		...
		<div class="portlet light bordered">
			...
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject hijau bold"><?= Yii::t('app', 'Laporan Dokumen Penjualan '); ?><span id="periode-label" class="font-blue-soft"></span></span>
				</div>
				<div class="tools">
...
<?php $this->registerJs("
    formconfig();
    setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Lap. Dok. Penjualan KO'))."');
...
function dtLaporan(){
    var dt_table =  $('#table-laporan').dataTable({
		pageLength: 100,
        ajax: { 
			url: '<?= \yii\helpers\Url::toRoute('/tuk/laporan/dokumenPenjualan') ?>',
			data:{
				dt: 'table-laporan',
...
function printout(caraPrint){
	window.open("<?= yii\helpers\Url::toRoute('/tuk/laporan/dokumenPenjualanPrint') ?>?"+$('#form-search-laporan').serialize()+"&caraprint="+caraPrint,"",'location=_new, width=1200px, scrollbars=yes');
}
