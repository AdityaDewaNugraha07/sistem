    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Riwayat Dokumen Penjualan'); ?></h4>
            </div>
...
function dtAfterSave(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/tuk/dokumenpenjualankb/daftarAfterSave') ?>',data:{dt: 'modal-aftersave'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
			...
			{ 	targets: 4, class: "td-kecil",
                render: function ( data, type, full, meta ) {
					var sleep = full[13];
					return '<center>'+data+'<br>'+sleep+'</center>';
                }
            },
...
function lihatDetail(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/tuk/dokumenpenjualankb/index','dokumen_penjualan_id'=>'']); ?>'+id);
}
function edit(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/tuk/dokumenpenjualankb/index','dokumen_penjualan_id'=>'']); ?>'+id+'&edit=1');
}
</script>