...
class DokumenpenjualankbController extends DeltaBaseController
{
...
//				exit;
	...
	public function actionOpenSPMTernota(){
		if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='table-spm'){
				...
				$param['where']=$param['table'].".cancel_transaksi_id IS NULL AND t_nota_penjualan.spm_ko_id NOT IN( SELECT spm_ko_id FROM t_dokumen_penjualan ) AND ".$param['table'].".jenis_produk = 'Log' AND op_ko_id != 999999 ";
	...
	function actionGetItems(){
		if(\Yii::$app->request->isAjax){
            ...
            $modSpm = \app\models\TSpmKo::findOne(['spm_ko_id'=>$modNota->spm_ko_id]);
			$modDetail = [];
            $data = [];
            ...
            if(count($modNotaDetail)>0){
                foreach($modNotaDetail as $i => $notadetail){
					...
					// $modSpmLog = \app\models\TSpmLog::findOne(['reff_no'=>$modSpm->kode, 'volume'=>$notadetail->kubikasi]);
					$modSpmLog = \app\models\TSpmLog::findOne($notadetail->spm_log_id);
					...
					$data['html'] .= $this->renderPartial('_item',['modDetail'=>$modDetail,'i'=>$i,'modNota'=>$modNota,'modSPMDetail'=>$modSPMDetail, 'modSpm'=>$modSpm, 'modSpmLog'=>$modSpmLog, 'modKayu'=>$modKayu]);
                }
            }
            return $this->asJson($data);
        }
		...
		if(\Yii::$app->request->isAjax){
            ...
            if(count($modDokPenjualan)>0){
                foreach($modDokPenjualan as $i => $dokjual){
					...
					// $modSpmLog = \app\models\TSpmLog::findOne(['reff_no'=>$modSpm->kode, 'volume'=>$dokjual->kubikasi]);
					$modSpmLog = \app\models\TSpmLog::findOne($dokjual->spm_log_id);
					$modKayu = \app\models\MKayu::findOne($modSpmLog->kayu_id);
                    $data['html'] .= $this->renderPartial('_item',['modDetail'=>$modDetail,'i'=>$i,'modNota'=>$modNota,'modSPMDetail'=>$modSPMDetail,'modSpm'=>$modSpm, 'modSpmLog'=>$modSpmLog, 'modKayu'=>$modKayu]);
                }
            }
		...
		if(\Yii::$app->request->isAjax){
			...
			if(!empty($spm_ko_id)){
				...
				if(!empty($modOp)){
					...
					$data['op']['terima_logalam_id'] = ($modOp->terima_logalam_id);
				}
			}
			return $this->asJson($data);
		}
