...
//					print_r($success_4);
//					exit;
                    if ($success_1 && $success_3 && $success_4) { // && $success_2
                        $transaction->commit();
                        $data['status'] = true;
    ...
    public function actionDeleteHasilProduksi(){
		if(\Yii::$app->request->isAjax){
			...
			$modProduksi = \app\models\TProduksi::findOne(['nomor_produksi'=>$model->nomor_produksi]);
                        $modKirimGudangDetail = \app\models\TKirimGudangDetail::findOne(['nomor_produksi'=>$model->nomor_produksi]);
            if( Yii::$app->request->post('deleteRecord')){
                $transaction = \Yii::$app->db->beginTransaction();
                try {
                    ...
                    $success_4 = false; // t_kirim_gudang_detail
                    ...
                    if(!empty(\app\models\TKirimGudangDetail::findOne(['nomor_produksi'=>$model->nomor_produksi]))){
                        if($modKirimGudangDetail->delete() && $validatingtransaction){
                            $success_4 = true;
                        }
                    }
