                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    ...
                                    <span class="caption-subject hijau bold"><?= Yii::t('app', 'Master Departement'); ?></span>
                                </div>
                                <div class="tools">
...
function dtMaster(){
    var dt_table =  $('#table-master').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/sysadmin/departement/index') ?>',data:{dt: 'table-master'} },
        order: [
            [0, 'DESC']
        ],
        columnDefs: [
...
function create(){
	openModal('<?= \yii\helpers\Url::toRoute('/sysadmin/departement/create') ?>','modal-master-create');
}
function info(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/sysadmin/departement/info','id'=>'']) ?>'+id,'modal-master-info');
}
function printout(caraprint){
	window.open("<?= yii\helpers\Url::toRoute('/sysadmin/departement/printout') ?>?id="+$('#showing_id').val()+"&caraprint="+caraprint,"",'location=_new, width=1200px, scrollbars=yes');
}
</script>