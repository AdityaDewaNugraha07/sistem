...
<div class="modal fade" id="modal-user-reset" tabindex="-1" role="basic" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="btn btn-icon-only btn-default fa fa-close" data-dismiss="modal" aria-hidden="true" style="float: right; padding-top: 2px; padding-bottom: 2px;"></button>
                <h4 class="modal-title"><?= Yii::t('app', 'Reset Password User'); ?></h4>
            </div>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
                        <?= $form->field($model, 'user_group_id')->dropDownList(\app\models\MUserGroup::getOptionList(),['class'=>'form-control','prompt'=>'', 'disabled' => true]) ?>
                        <?= $form->field($model, 'username')->textInput(['readonly'=>'readonly']); ?>
                        <?= $form->field($model, 'authKey')->textInput(['readonly'=>'readonly']); ?>
                        <?= $form->field($model, 'password')->textInput(['readonly'=>'readonly'])->label('Reset Password');?>
                    </div>
                    <div class="col-md-6">
                        ...
                        <?= yii\bootstrap\Html::activeHiddenInput($modUserProfile, 'avatar',['readonly'=>'readonly']); ?>
                        <?= \yii\bootstrap\Html::activeHiddenInput($modUserProfile, 'fullname', ['readonly'=>'readonly']) ?>
                        <?= $form->field($modUserProfile, 'email')->textInput(['readonly'=>'readonly']) ?>
                        <?= $form->field($model, 'pegawai_id')->dropDownList(\app\models\MPegawai::getOptionList(),['class'=>'form-control select2','prompt'=>'', 'disabled' => true]); ?>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <?php echo \yii\helpers\Html::button( Yii::t('app', 'Save'),['class'=>'btn hijau btn-outline ciptana-spin-btn',
                    'onclick'=>'submitformajax(this,"$(\'#modal-user-reset\').modal(\'hide\'); $(\'#table-user\').dataTable().fnClearTable();")'])?>
            </div>
