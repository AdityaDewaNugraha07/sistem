        ...
        <div class="portlet light bordered">
            <div class="portlet-body">
                <ul class="nav nav-tabs pull-right">
                    <li class="active">
                        <a href="<?= yii\helpers\Url::toRoute("/sysadmin/catatan/index"); ?>"> <?= Yii::t('app', $this->title); ?> </a>
                    </li>
                    <li class="">
                        <a href="<?= yii\helpers\Url::toRoute("/sysadmin/catatan/create"); ?>"> <?= Yii::t('app', 'Tambah '.$this->title); ?> </a>
                    </li>
                    <li class="">
...
function dtCatatan(){
    var dt_table =  $('#table-catatan').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/sysadmin/catatan/index') ?>',data:{dt: 'table-catatan'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
...
function create(){
    window.location = '<?= \yii\helpers\Url::toRoute('/sysadmin/catatan/create') ?>'; 
}

// fungsi tombol info
function info(id){
    openModal('<?= \yii\helpers\Url::toRoute(['/sysadmin/catatan/info','id'=>'']) ?>'+id,'modal-catatan-info');
}
