                            ...
                            <div class="portlet-body">
                                <table class="table table-striped table-bordered table-hover" id="table-harga">
                                    <thead>
                                        <tr>
                                            <th></th>
                                            <th><?= Yii::t('app', 'Kelompok Limbah') ?></th>
...
<?php $this->registerJs(" dtHarga(); setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Harga'))."');", yii\web\View::POS_READY); ?>
<script>
function dtHarga(){
    var dt_table =  $('#table-harga').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/sysadmin/hargalimbah/index') ?>',data:{dt: 'table-harga'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
...
function create(){
	openModal('<?= \yii\helpers\Url::toRoute('/sysadmin/hargalimbah/create') ?>','modal-harga-create');
}
function info(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/sysadmin/hargalimbah/info','id'=>'']) ?>'+id,'modal-harga-info');
}
</script>