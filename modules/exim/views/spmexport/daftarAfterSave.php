    ...
    var dt_table =  $('#table-aftersave').dataTable({
        ...
        columnDefs: [
			...
			{	targets: 13, class:"td-kecil text-center",
				render: function ( data, type, full, meta ) {
                    let ret = "";
                    if(full[12] && full[12] === "<?= TSpmKo::REALISASI ?>"){
                        ret = "<span class='label label-sm label-success'> "+full[12]+" </span>";
					}else if(full[11]){
						ret = "<span class='label label-sm label-danger'>ABORTED</span>";
					}else if (full[14] === 'PROCESS') {
                        ret = "<span class='label label-sm label-danger' style='font-size: 10px'>Menunggu Penghapusan</span>";
                    }
					return ret;
				}
			},
			{	targets: 14, class:"td-kecil",
				...
				render: function ( data, type, full ) {
                    let display = "";
                    if(full[11] || (full[12] && full[12] === "<?= TSpmKo::REALISASI ?>") || full[14] === 'PROCESS'){
						display =  'visibility: hidden;';
					}
                    return `<div style="text-align: center;">
                                <a style="${display}" class="btn btn-xs btn-outline blue-hoki tooltips" data-original-title="Update" onclick="edit(${full[0]})">
                                    <i class="fa fa-edit"></i>
                                </a>
                                <a style="margin-left: -5px;" class="btn btn-xs btn-outline dark tooltips" data-original-title="Lihat" onclick="lihatDetail(${full[0]})">
                                    <i class="fa fa-eye"></i>
                                </a>
                            </div>`;
				}
