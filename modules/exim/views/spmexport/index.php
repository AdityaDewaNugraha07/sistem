...
<!--						<a class="btn dark btn-sm btn-outline" href="<?php // echo yii\helpers\Url::toRoute("/marketing/spm/scanSpm") ?>"><i class="icon-frame"></i> <?php // echo Yii::t('app', 'Scan Pemuatan'); ?></a>-->
						<a class="btn blue btn-sm btn-outline" onclick="daftarAfterSave()"><i
                                    class="fa fa-list"></i> <?= Yii::t('app', 'SPM Yang Telah Dibuat') ?></a>
					</span>
                </div>
                <div class="row">
                    <div class="col-md-12">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <?php if (isset($_GET['spm_ko_id'])) { ?>
                                            <?= Html::activeHiddenInput($model, "packinglist_id") ?>
                                            <?= $form->field($model, 'nomor')->textInput()->label("Packinglist") ?>
                                            <?php
                                                if(!empty($model->packinglist_id)){
                                                    $Packinglist_id = strstr($model->packinglist_id, '-', true);
                                                    $modPackignlist = \app\models\TPackinglist::findOne($Packinglist_id);
                                                    $modOpExport = \app\models\TOpExport::findOne($modPackignlist->op_export_id);
                                                    $nomorKontrak = $modOpExport->nomor_kontrak ;
                                                }
                                            ?>
                                        <?php } else { ?>
                                                ...
                                                <div class="col-md-7">
                                                        ...
                                                        try {
                                                            $sql = "
                                                                ...
                                                                FROM 
                                                                    t_spm_ko 
                                                                WHERE 
                                                                    jenis_penjualan = 'export' 
                                                                  AND cancel_transaksi_id IS NULL 
                                                                ORDER BY spm_ko_id DESC
                                                            ";
                                                            $modAllSPM = Yii::$app->db->createCommand($sql)->queryAll();
                                                            $disabled = [];
                                                            foreach ($modAllSPM as $izxs => $illdis) {
                                                                $disabled[$illdis['packinglist_id'] . '-' . $illdis['container_no']] = ['disabled' => true];
                                                            }
                                                            echo Html::activeDropDownList($model, 'packinglist_id', TPackinglistContainer::getOptionListSPM(), [
                                                    ...
                                                    <span class="input-group-btn" style="width: 20%">
														<a  class="btn btn-icon-only btn-default tooltips"
                                                            onclick="openPackinglist();" data-original-title="Daftar OP"
                                        ...
                                        <?php } ?>
                                        <?= Html::activeHiddenInput($model, 'cust_id') ?>
                                        ...
                                        <div class="form-group" id="place-buyer">
                                            ...
                                            <div class="col-md-7" style="padding-bottom: 5px;">
                                                <?= Html::textarea('nomor_kontrak', $nomorKontrak, ['class' => 'form-control', 'style' => 'width:100%', 'disabled' => 'disabled']) ?>
                                            </div>
                                        </div>
                                        <div class="form-group" id="place-buyer">
                                            <label class="col-md-5 control-label"><?= Yii::t('app', 'Shipment To') ?></label>
                                            <div class="col-md-7" style="padding-bottom: 5px;">
                                                <?= Html::activeTextarea($model, 'shipment_to', ['class' => 'form-control', 'style' => 'width:100%', 'disabled' => 'disabled']) ?>
                                            </div>
                                        </div>
                                        <div class="form-group" id="place-applicant" style="display: none;">
                                            <label class="col-md-5 control-label"><?= Yii::t('app', 'Applicant') ?></label>
                                            <div class="col-md-7" style="padding-bottom: 5px;">
                                                <?= Html::activeTextarea($model, 'applicant', ['class' => 'form-control', 'style' => 'width:100%', 'disabled' => 'disabled']) ?>
                                            </div>
                                        </div>
                                        <div class="form-group" id="place-notify_party" style="display: none;">
                                            ...
                                            <div class="col-md-7" style="padding-bottom: 5px;">
                                                <?= Html::activeTextarea($model, 'notify_party', ['class' => 'form-control', 'style' => 'width:100%', 'disabled' => 'disabled']) ?>
                                            </div>
                                        </div>
                                        ...
                                        <?php if (isset($_GET['spm_ko_id'])): ?>
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-7">
                                                    ...
                                                    $label = '';
                                                    $options = [];
                                                    $manipulasi = TPengajuanManipulasi::findOne(['reff_no' => $model->kode, 'status' => 'PROCESS']);
                                                    if($manipulasi !== null) {
                                                        $approver = TApproval::findAll(['reff_no' => $model->kode, 'parameter1' => $manipulasi->kode]);
                                                        if(count($approver) > 0) {
                                                            foreach ($approver as $approve) {
                                                                if($model->status === TSpmKo::REALISASI) {
                                                                    if($approve->level === 4 && $approve->status === TApproval::STATUS_NOT_CONFIRMATED) {
                                                                        $ada_pengajuan_hapus = true;
                                                                    }
                                                                }else if($approve->level === 2 && $approve->status === TApproval::STATUS_NOT_CONFIRMATED) {
                                                                    $ada_pengajuan_hapus = true;
                                                                }
                                                            }
                                                        }
                                                    ...
                                                    if($ada_pengajuan_hapus) {
                                                        $label = '<i class="fa fa-trash-o"></i> Menunggu penghapusan...';
                                                        $options = [
                                                            'class' => 'btn btn-default',
                                                            'disabled' => 'disabled'
                                                        ];
                                                    }else{
                                                        $label = '<i class="fa fa-trash-o"></i> HAPUS SPM EXPORT';
                                                        $options = [
                                                            'class' => 'btn btn-danger',
                                                            'onclick' => "hapusSPMExport('$model->spm_ko_id')"
                                                        ];
                                                    }

                                                    echo Html::button($label, $options);
                                                    ?>
                                                </div>
                                            </div>
                                        <?php endif ?>
                                    ...
                                    <div class="col-md-5">
                                        ...
                                        <?= $form->field($model, 'tanggal', [
                                            'template' => '{label}<div class="col-md-7"><div class="input-group input-medium date date-picker bs-datetime">{input} <span class="input-group-addon">
																	<button class="btn default" type="button" style="margin-left: 0;"><i class="fa fa-calendar"></i></button></span></div> 
                                        ...
                                        <?= $form->field($model, 'disetujui')->dropDownList(MPegawai::getOptionListByDept(Params::DEPARTEMENT_ID_MARKETING), ['class' => 'form-control select2', 'prompt' => ''])->label('Disetujui Oleh') ?>
                                        <?= $form->field($model, 'tanggal_rencanamuat', [
                                            'template' => '{label}<div class="col-md-7"><div class="input-group input-medium date date-picker bs-datetime">{input} <span class="input-group-addon">
																	<button class="btn default" type="button" style="margin-left: 0;"><i class="fa fa-calendar"></i></button></span></div> 
																	{error}</div>'])->textInput(['readonly' => 'readonly']) ?>
                                        <?php if ($model->status === TSpmKo::REALISASI) { ?>
                                            <div class="form-group" style="margin-bottom: 5px;">
                                            ...
                                            <?= $form->field($model, 'waktu_selesaimuat', [
                                                'template' => '{label}<div class="col-md-7"><div class="input-group input-medium date form_datetime bs-datetime">{input} <span class="input-group-addon">
																	<button class="btn default" type="button" style="margin-left: 0;"><i class="fa fa-calendar"></i></button></span></div> 
																	{error}</div>'])->textInput(['readonly' => 'readonly']) ?>
                                            <?= $form->field($model, 'diperiksa')->dropDownList(MPegawai::getOptionListWithDeptName(), ['class' => 'form-control select2', 'prompt' => ''])->label('Diperiksa Oleh') ?>
                                            <?= $form->field($model, 'diperiksa_security')->dropDownList(MPegawai::getOptionListWithDeptName(), ['class' => 'form-control select2', 'prompt' => ''])->label('Diperiksa Security') ?>
                                            <?= $form->field($model, 'dikeluarkan')->dropDownList(MPegawai::getOptionListWithDeptName(), ['class' => 'form-control select2', 'prompt' => ''])->label('Dikeluarkan Oleh') ?>
                                        <?php } else { ?>
                                            <?php if (isset($_GET['edit'])) { ?>
                                                <div class="form-group field-tspmko-status" style="margin-bottom: 5px;">
                                                    <label class="col-md-5 control-label"
                                                           for="tspmko-status">Status</label>
                                                    <div class="col-md-7">
                                                        <?php if(!$ada_pengajuan_hapus) : ?>
                                                        <select id="tspmko-status" class="form-control"
                                                                name="TSpmKo[status]" style="width:200px;"
                                                                onchange="setStatus();">
                                                            <option value="" style="background-color: #FBE88C"
                                                                    selected="">Belum Realisasi
                                                            </option>
                                                            <option value="<?= TSpmKo::REALISASI ?>"
                                                                    style="background-color: #95EBA3">Sudah Realisasi
                                                            </option>
                                                        </select>
                                                        <?php else: ?>
                                                        <h4 style="background-color: #FBE88C;text-align: center"><b>BELUM REALISASI</b></h4>
                                                        <?php endif ?>
                                                    </div>
                                                </div>
                                                <div id="place-waktuselesaimuat" style="display: none;">
                                                    <?= $form->field($model, 'waktu_selesaimuat', [
                                                        'template' => '{label}<div class="col-md-7"><div class="input-group input-medium date form_datetime bs-datetime">{input} <span class="input-group-addon">
																		<button class="btn default" type="button" style="margin-left: 0;"><i class="fa fa-calendar"></i></button></span></div> 
																		{error}</div>'])->textInput(['readonly' => 'readonly']) ?>
                                                    <?= $form->field($model, 'diperiksa')->dropDownList(MPegawai::getOptionListMarketing(), ['class' => 'form-control select2', 'prompt' => ''])->label('Diperiksa Oleh') ?>
                                                    <?= $form->field($model, 'diperiksa_security')->dropDownList(MPegawai::getOptionListCheckerSecurity(), ['class' => 'form-control select2', 'prompt' => ''])->label('Diperiksa Security') ?>
                                                    <?= $form->field($model, 'dikeluarkan')->dropDownList(MPegawai::getOptionListMarketing(), ['class' => 'form-control select2', 'prompt' => ''])->label('Dikeluarkan Oleh') ?>
                                                </div>
                                            <?php } else if (isset($_GET['spm_ko_id'])) { ?>
                                                <div class="form-group" style="margin-bottom: 5px;">
                                                    ...
                                                    <div class="col-md-7 text-align-center">
                                                        ...
                                                        <?php if(!$ada_pengajuan_hapus): ?>
                                                        <h4><a class="btn btn-xs btn-outline blue-hoki tooltips"
                                                               data-original-title="Update SPM"
                                                               onclick="edit(<?= $model->spm_ko_id ?>)">
                                        ...
                                        <div class="col-md-12">
                                            <div class="table-scrollable">
                                                <table class="table table-striped table-bordered table-advance table-hover table-laporan"
                                                       style="width: 90%" id="table-detail-produklist">
                                                    <thead>
                                                    <tr>
                                                        <th rowspan="2"
                                                        ...
                                                        <th rowspan="2"
                                                            style="width: 150px; font-size: 1.3rem; line-height: 0.9; padding: 5px;"><?= Yii::t('app', 'Kode Produk') ?></th>
                                                        <th rowspan="2"
                                                        ...
                                                        <th rowspan="2"
                                                            style="width: 100px; line-height: 0.9; font-size: 1.1rem;"><?= Yii::t('app', 'Scanned By') ?></th>
                                                    </tr>
                                                    <tr>
                                            ...
                                            <table class="table table-striped table-bordered table-advance table-hover"
                                                ...
                                                <tr>
                                                    <td colspan="7" style="display: none;" id="place-addProduk">
                                                        <a class="btn btn-xs blue-hoki" onclick="addProduk();"
                                                           style="margin-top: 10px;"><i
                                                                    class="fa fa-plus"></i> <?= Yii::t('app', 'Tambah Produk') ?>
                                                        </a>
                                                    </td>
                                                </tr>
                                                </tfoot>
...
<script>
    function openPackinglist() {
        const url = '<?= Url::toRoute(['/exim/spmexport/openPackinglist']) ?>';
    ...
    function setOP() {
        const packinglist_id = $('#<?= Html::getInputId($model, "packinglist_id") ?>').val();
        $.ajax({
            url: '<?= Url::toRoute(['/exim/spmexport/setOP']) ?>',
            type: 'POST',
    ...
    function getItems(packinglist_id) {
        $.ajax({
            url: '<?= Url::toRoute(['/exim/spmexport/getItems']); ?>',
            type: 'POST',
            data: {packinglist_id: packinglist_id},
            success: function (data) {
                if (data.html) {
    ...
    function listRandom(ele) {
        ...
        var url = '<?= Url::toRoute(['/exim/spmexport/listRandom']); ?>?produk_id=' + produk_id + '&post=' + packinglist_id;
    ...
    function save() {
        ...
        $("#<?= Html::getInputId($model, "waktu_selesaimuat") ?>").parents(".form-group").removeClass("has-error");
        $("#<?= Html::getInputId($model, "diperiksa") ?>").parents(".form-group").removeClass("has-error");
        $("#<?= Html::getInputId($model, "diperiksa_security") ?>").parents(".form-group").removeClass("has-error");
        $("#<?= Html::getInputId($model, "dikeluarkan") ?>").parents(".form-group").removeClass("has-error");
        $("#<?= Html::getInputId($model, "disetujui") ?>").parents(".form-group").removeClass("has-error");
        $("#<?= Html::getInputId($model, "waktu_selesaimuat") ?>").parents(".form-group").addClass("has-success");
        $("#<?= Html::getInputId($model, "diperiksa") ?>").parents(".form-group").addClass("has-success");
        $("#<?= Html::getInputId($model, "diperiksa_security") ?>").parents(".form-group").addClass("has-success");
        $("#<?= Html::getInputId($model, "dikeluarkan") ?>").parents(".form-group").addClass("has-success");
        $("#<?= Html::getInputId($model, "disetujui") ?>").parents(".form-group").addClass("has-success");
    ...
    function validatingDetail($form) {
        var has_error = 0;
        var status = $("#<?= Html::getInputId($model, "status") ?>").val();
        var diperiksa = $("#<?= Html::getInputId($model, "diperiksa") ?>").val();
        var diperiksa_security = $("#<?= Html::getInputId($model, "diperiksa_security") ?>").val();
        var dikeluarkan = $("#<?= Html::getInputId($model, "dikeluarkan") ?>").val();
        var disetujui = $("#<?= Html::getInputId($model, "disetujui") ?>").val();
        var final_destination = $("#<?= Html::getInputId($model, "final_destination") ?>").val();
        if (status == "<?= app\models\TSpmKo::REALISASI ?>") {
            if (!$("#<?= Html::getInputId($model, "waktu_selesaimuat") ?>").val()) {
                $("#<?= Html::getInputId($model, "waktu_selesaimuat") ?>").parents(".form-group").addClass("has-error");
                $("#<?= Html::getInputId($model, "waktu_selesaimuat") ?>").parents(".form-group").removeClass("has-success");
                has_error = has_error + 1;
            }
            if (!diperiksa) {
                $("#<?= Html::getInputId($model, "diperiksa") ?>").parents(".form-group").addClass("has-error");
                $("#<?= Html::getInputId($model, "diperiksa") ?>").parents(".form-group").removeClass("has-success");
                has_error = has_error + 1;
            }
            if (!diperiksa_security) {
                $("#<?= Html::getInputId($model, "diperiksa_security") ?>").parents(".form-group").addClass("has-error");
                $("#<?= Html::getInputId($model, "diperiksa_security") ?>").parents(".form-group").removeClass("has-success");
                has_error = has_error + 1;
            }
            if (!dikeluarkan) {
                $("#<?= Html::getInputId($model, "dikeluarkan") ?>").parents(".form-group").addClass("has-error");
                $("#<?= Html::getInputId($model, "dikeluarkan") ?>").parents(".form-group").removeClass("has-success");
                has_error = has_error + 1;
            }
            if (!disetujui) {
                $("#<?= Html::getInputId($model, "disetujui") ?>").parents(".form-group").addClass("has-error");
                $("#<?= Html::getInputId($model, "disetujui") ?>").parents(".form-group").removeClass("has-success");
                has_error = has_error + 1;
            }
        ...
        if (!$("#<?= Html::getInputId($model, "final_destination") ?>").val()) {
            $("#<?= Html::getInputId($model, "final_destination") ?>").parents(".form-group").addClass("has-error");
            $("#<?= Html::getInputId($model, "final_destination") ?>").parents(".form-group").removeClass("has-success");
            has_error = has_error + 1;
        }
    ...
    function afterSave(id) {
        ...
        <?php } ?>
        <?php if(isset($_GET['edit'])){ ?>
        $('#<?= Html::getInputId($model, "tanggal_kirim") ?>').prop("disabled", false);
        $('#<?= Html::getInputId($model, 'tanggal_kirim') ?>').siblings('.input-group-addon').find('button').prop('disabled', false);
        $('#<?= Html::getInputId($model, 'tanggal_rencanamuat') ?>').siblings('.input-group-addon').find('button').prop('disabled', false);
        $('#<?= Html::getInputId($model, "kendaraan_nopol") ?>').prop("disabled", false);
        $('#<?= Html::getInputId($model, "kendaraan_supir") ?>').prop("disabled", false);
        $('#<?= Html::getInputId($model, "status") ?>').prop("disabled", false);
        $('#<?= Html::getInputId($model, "waktu_selesaimuat") ?>').prop("disabled", false);
        $('#<?= Html::getInputId($model, 'waktu_selesaimuat') ?>').siblings('.input-group-addon').find('button').prop('disabled', false);
        $('#<?= Html::getInputId($model, "diperiksa") ?>').prop("disabled", false);
        $('#<?= Html::getInputId($model, "diperiksa_security") ?>').prop("disabled", false);
        $('#<?= Html::getInputId($model, "dikeluarkan") ?>').prop("disabled", false);
        $('#<?= Html::getInputId($model, "disetujui") ?>').prop("disabled", false);
        $('#<?= Html::getInputId($model, "container_kode") ?>').prop("disabled", false);
        $('#<?= Html::getInputId($model, "seal_no") ?>').prop("disabled", false);
...
//					fillSpmRealisasi();
    ...
    function getCurrentProdukList() {
        ...
        var kode_spm = $("#<?= Html::getInputId($model, "kode") ?>").val();
        var status = "<?= (!empty($model->status)) ? $model->status : "" ?>";
        $.ajax({
            url: '<?= Url::toRoute(['/exim/spmexport/getCurrentProdukList']); ?>',
            type: 'POST',
    ...
    function stockAvailable(ele) {
        ...
        var url = '<?= Url::toRoute(['/gudang/availablestockproduk/produkListOnModal', 'tr_seq' => '']); ?>' + tr_seq + '&jns_produk=' + jns_produk + '&notin=' + notin;
        $(".modals-place-3-min").load(url, function () {
    ...
    function daftarAfterSave() {
        openModal('<?= Url::toRoute(['/exim/spmexport/daftarAfterSave']) ?>', 'modal-aftersave', '95%');
    }

    function edit(id) {
        window.location.replace('<?= Url::toRoute(['/exim/spmexport/index', 'spm_ko_id' => '']); ?>' + id + '&edit=1');
    }
...
//    result = (Math.round( result * 10000 ) / 10000 ).toString(); // karena mengambil nilai asli
    ...
    function hapusSPMExport(id) {
        openModal('<?= Url::toRoute(['/exim/spmexport/hapusSPMExport'])?>?id=' + id, 'modal-transaksi');
    }
