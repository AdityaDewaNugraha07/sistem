				...
				<?php
					if($model->bundle_partition == true){
						...
						if($model->jenis_produk == "Plywood" || $model->jenis_produk == "Lamineboard" || $model->jenis_produk == "Platform"){
							...
							$colspan_footer += 1;
						}else if($model->jenis_produk == "FingerJointLamineBoard" || $model->jenis_produk == "FingerJointStick" || $model->jenis_produk == "Flooring"){
							$column = '<th rowspan="2" style="width: 100px; ">Wood<br>Type</th>';
							$colspan_footer += 1;
						}
	...
	//									$produkorder .= "/".$detail['kondisi_kayu'];
									...
									foreach($params_length as $vv => $length){
										if(count($params_length)>1){
											...
											// echo "<td style='border-right: solid 1px #000; text-align: center; padding: 3px;'>".($hmm['pcs'])."</td>";
											echo "<td style='border-right: solid 1px #000; text-align: center; padding: 3px;'>".($hmmPcs)."</td>";
											$sub_pcs += $hmm['pcs']; $sub_volume += $hmm['volume'];
									...
									if($sub_volume > 0){
										$subVolume = number_format($sub_volume,4) ;
									}else{
										$subVolume = "";
									}
									...
									echo		"<td style='border-right: solid 1px #000; text-align: right; padding: 3px; padding-right: 5px;'>".($subPcs)."</td>";
									echo		"<td style='text-align: right; padding: 3px; padding-right: 5px;'>".($subVolume)."</td>";
									echo "</tr>";
...
//								if(!empty($modAlias)){
								...
								echo '<tr style="border: solid 1px #000; border-right: solid 1px transparent; font-weight:600" class="text-align-center">';
									if($total_partition > 1){
										...
										echo "<td style='border-right: solid 1px #000; text-align:right; padding: 2px; padding-right: 5px;'><b>".$subtotal_pcs."</b></td>";
										echo "<td style='text-align:right; padding: 2px; padding-right: 5px;'><b>".number_format($subtotal_volume,4)."</b></td>";
								echo '</tr>';
//								}
							}
						}
						?>
								<tr style="">
									...
									<td style="border-right: solid 1px #000; text-align: right; padding: 3px; padding-right: 5px;"><b><?= $total_pcs ?></b></td>
									<td style="text-align: right; padding: 3px; padding-right: 5px;"><b><?= number_format($total_volume,4) ?></b></td>
								</tr>
						<?php }else{ ?>
						<tr style="height: 0.5cm; border-bottom: solid 1px #000;">
							...
							<?php if($model->jenis_produk == "Plywood" || $model->jenis_produk == "Lamineboard" || $model->jenis_produk == "Platform"){ ?>
								<td rowspan="2" style="padding: 7px 5px; border-right: solid 1px #000; vertical-align: middle;"><b><center>Wood Type</center></b></td>
								<td rowspan="2" style="padding: 7px 5px; border-right: solid 1px #000; vertical-align: middle;"><b><center>Glue</center></b></td>
							<?php } ?>
							<?php if($model->jenis_produk == "FingerJointLamineBoard" || $model->jenis_produk == "FingerJointStick" || $model->jenis_produk == "Flooring"){ ?>
								<td rowspan="2" style="padding: 7px 5px; border-right: solid 1px #000; vertical-align: middle;"><b><center>Wood Type</center></b></td>
							<?php } ?>
							<td colspan="3" style="border-right: solid 1px #000;"><b><center>Size</center></b></td>
							<td rowspan="2" style="width: 1cm; padding: 7px 5px; border-right: solid 1px #000; vertical-align: middle;"><b><center>Crate</center></b></td>
							<td rowspan="2" style="width: 1cm; padding: 7px 5px; border-right: solid 1px #000; vertical-align: middle;"><b><center>Pcs</center></b></td>
							<td colspan="3" style=""><b><center>Total</center></b></td>
						</tr>
						<tr style="border-bottom: solid 1px #000;">
							<td style="width: 2.3cm; border-right: solid 1px #000;"><center><b>Thickness</b> (<?= $modDetail[0]->thick_unit ?>)</center></td>
							<td style="width: 2.1cm; border-right: solid 1px #000;"><center><b>Width</b> (<?= $modDetail[0]->width_unit ?>)</center></td>
							<td style="width: 2.1cm; border-right: solid 1px #000;"><center><b>Length</b> (<?= $modDetail[0]->length_unit ?>)</center></td>
							<td style="width: 1.5cm; border-right: solid 1px #000;"><center><b>Pcs</b></center></td>
							<td style="width: 1.5cm; "><center><b>M<sup>3</sup></b></center></td>
						</tr>
						<?php
						// echo"<pre>";
						$groupingContain = true;                                           
						...
						if(!empty($modAlias) ){ 
								if($model->jenis_produk == "Moulding"){
									...
									$modAlias2 = Yii::$app->db->createCommand("SELECT * FROM t_alias WHERE reff_no = '{$model->kode}' AND alias_name='jenis_kayu' ORDER BY alias_id ASC")->queryAll();
									$groupings = $modAlias2;
								}                                                    
						}else{
						...
						foreach($groupings as $igroup => $grouping){
                            if(!empty($modAlias) ){ //&& ($model->jenis_produk == "Moulding")
								...
								if($groupingContain =='true'){ //kondisikan jika tampilkan digrouping direkap
									$sql = "select  
													partition_kode,
													grade,jenis_kayu,glue,profil_kayu,
													thick,thick_unit,
													width,width_unit,
													length,length_unit,pcs ,
													sum(ROUND(volume::numeric,4)) as volume ,
													string_agg(bundles_no::text, ', ' order by bundles_no asc) as bundles_no
											from t_packinglist_container
											where packinglist_id = ".$model->packinglist_id." and container_no = ".$container['container_no']."
											group by partition_kode,grade,jenis_kayu,glue,profil_kayu,width,width_unit,length,length_unit,thick,thick_unit,pcs ";
									$modDetails = \Yii::$app->db->createCommand($sql)->queryAll();
								}else{
                                	$modDetails = \app\models\TPackinglistContainer::find()
							...
							// echo"</pre>";
							// exit;
							if($groupingContain =='true'){
								
								if(!empty($modAlias)){
									echo '<tr style="border: solid 1px #000; border-right: solid 1px transparent; font-weight:600" class="text-align-center">';
									echo	'<td colspan=8>'.(($approval->status == \app\models\TApproval::STATUS_APPROVED)?$grouping['value_alias']:$grouping['value_original']).'</td>';
									echo '</tr>';
								}
								// echo"<pre>";
								// print_r($groupings);
								// echo"</pre>";
								// exit;
								$subtotal_bundles = 0; $subtotal_pcs = 0; $subtotal_volume=0;$grade = ""; $glue = ""; $profil = "";
								foreach($modDetails as $i => $detail){
									if(($i+1) != $subtotal_bundles){
										$subtotal_bundles += 1;
									}										
									$perbdls = substr_count($detail['bundles_no'],",")+1;
									// echo"<pre>";
									// print_r($detail['profil_kayu']);
									// echo"</pre>";
									// exit;
									$subtotalpcs = $detail['pcs'] * $perbdls;
									$subtotal_pcs += $detail['pcs'];
									$subtotal_volume += number_format($detail['volume'],4);
									$total_pcs += $detail['pcs'] * $perbdls; // 
									$total_volume += number_format($detail['volume'],4);//$total_bundles = $detail->bundles_no;                                                               
									...
									if(!empty($detail['jenis_kayu'])){
										$aliasjenis_kayu = app\models\TAlias::findOne(['reff_no'=>$model->kode,'alias_name'=>'jenis_kayu','value_original'=>$detail['jenis_kayu']]);
										if(!empty($aliasgrade)&&($approval->status == \app\models\TApproval::STATUS_APPROVED)){
											$jenis_kayu = $aliasjenis_kayu->value_alias;
										}else{
									...
									if(!empty($detail['profil_kayu'])){
										$aliasprofil = app\models\TAlias::findOne(['reff_no'=>$model->kode,'alias_name'=>'profil_kayu','value_original'=>$detail['profil_kayu']]);
                                        if(!empty($aliasprofil)&&($approval->status == \app\models\TApproval::STATUS_APPROVED)){
                                            $profil = $aliasprofil->value_alias;
                                        }else{
                                            $profil = $detail['profil_kayu'];
                                        }
                                    }
								?>	
									<tr style="border: solid 1px #000; border-right: solid 1px transparent;">
									<td style="border-right: solid 1px #000; text-align: center; padding: 2px;width: 2cm"><?= $detail['bundles_no'] ?></td>
									<td style="border-right: solid 1px #000; text-align: center; padding: 2px;"><?= $grade ?></td>
									<?php if($model->jenis_produk == "Plywood" || $model->jenis_produk == "Lamineboard" || $model->jenis_produk == "Platform"){ ?>
									<td style="border-right: solid 1px #000; text-align: center; padding: 2px;"><?= $jenis_kayu ?> </td>
									...
									<td style="border-right: solid 1px #000; text-align: center; padding: 2px;"><?= $perbdls ?></td>
									<td style="border-right: solid 1px #000; text-align: center; padding: 2px;"><?= $detail['pcs'] ?></td>
									<td style="border-right: solid 1px #000; text-align: right; padding: 2px; padding-right: 5px;"><?= $subtotalpcs ?></td>
									<td style="text-align: right; padding: 2px; padding-right: 5px;"><?= ($_GET['caraprint'] == "EXCEL")? app\components\DeltaFormatter::formatNumberForUserFloat($detail['volume']) : number_format($detail['volume'],4); ?></td>
								</tr>
								<?php if((($i+1) == count($modDetails)) && (!empty($modAlias))){ ?>
									<tr style="border-top: solid 1px #000; background-color: #F1F4F7;">
										...
										<td class="text-align-center" style="padding: 1px; padding-right: 5px; border-right: solid 1px #000; text-align: right;"><b><?= \app\components\DeltaFormatter::formatNumberForUserFloat($subtotal_pcs) ?></b></td>
										<td class="text-align-center" style="padding: 1px; padding-right: 5px; text-align: right;"><b><?= ($_GET['caraprint'] == "EXCEL")? app\components\DeltaFormatter::formatNumberForUserFloat($subtotal_volume) : number_format($subtotal_volume,4); ?></b></td>
									</tr>
								<?php }									
