							...
							<thead>
								<tr>
									...
									<th><?= Yii::t('app', 'Packinglist No.'); ?></th>
                                                                        <th><?= Yii::t('app', 'Packinglist Kode'); ?></th>
								</tr>
							</thead>
						</table>
						</div>
...
function dtMaster(){
    var dt_table =  $('#table-spm').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/exim/invoice/openPackinglist') ?>',data:{dt: 'table-spm', status_inv :'<?= $status_inv ?>'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
			{ 	targets: 0, 
                render: function ( data, type, full, meta ) {
					return "<a onclick='pick(\""+full[0]+"\",\""+full[5]+"\")' class='btn btn-xs btn-icon-only btn-default' data-original-title='Pick' style='width: 25px; height: 25px;'><i class='fa fa-plus-circle'></i></a>";
                }
            },
        ],
		"autoWidth" : false,
