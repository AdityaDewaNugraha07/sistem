		...
		<td colspan="3" style="padding: 0px; vertical-align:top;">
			<table style="width: 100%;" id="table-detail">
				<thead>
					...
					<tr>
						<td colspan="10" style="border-bottom: solid 1px transparent;border-top: solid 1px;">
							<table style="width: 100%;" border="0">
								<tr style="">
					...
					<tr>
						<td colspan="10" style="width: 50%; height: 7cm; vertical-align: top; padding: 5px 10px; background-color: #F1F4F7; border-right: solid 1px transparent;">
				...
				<tbody>
					...
					if (count($modDetails) > 0) {
						foreach ($modDetails as $ii => $detail) {
							...
							$grade = "";
							$glue = "";
							$aliasglue = app\models\TAlias::findOne(['reff_no' => $modPackinglist->kode, 'alias_name' => 'glue', 'value_original' => $detail->produk->glue]);
							if (!empty($aliasglue) && ($approval->status == \app\models\TApproval::STATUS_APPROVED)) {
								$glue = $aliasglue->value_alias;
							} else {
								$glue = $detail->produk->glue;
							}
							$aliasgrade = app\models\TAlias::findOne(['reff_no' => $modPackinglist->kode, 'alias_name' => 'grade', 'value_original' => $detail->produk->grade]);
							if (!empty($aliasgrade) && ($approval->status == \app\models\TApproval::STATUS_APPROVED)) {
								$grade = $aliasgrade->value_alias;
							} else {
								$grade = $detail->produk->grade;
							}
				...
				<tfoots>
					...
					?>
						<tr style="border-top: solid 1px #000;">
							...
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td class="text-align-right" style="padding: 3px; text-align: right; font-weight: bold;"><span class="pull-left" style="padding-left:2px;"><?php echo $model->mata_uang; ?></span><?php echo number_format($total_biaya_tambahan, 2); ?></td>
						</tr>
					<?php
					} else {
					...
					<tr style="border-top: solid 1px #000; border-bottom: solid 1px transparent;">
						...
						<?php if (!empty($model->freight_preview)) { ?>
							...
							<td style="font-size: 1.1rem; padding: 3px; text-align: right; padding-bottom: 0px; ">
								<b class="pull-left" style="padding-left:2px;"><?= $model->mata_uang ?></b>
								<b class="pull-right"><?= (!empty($model->freight) ? \app\components\DeltaFormatter::formatNumberForUserFloat($model->freight, 2) : 0);  ?></b>
							</td>
						<?php } ?>
					</tr>
				</tfoots>
