                                ...
                                <div class="row">
									...
									<div class="col-md-6">
										...
										<div class="form-group">
											...
											<div class="col-md-8" style="padding-bottom: 5px;">
												<span class="input-group-btn" style="width: 78%">
										...
										<div class="form-group">
											<label class="col-md-4 control-label"><?= Yii::t('app', 'Freight'); ?></label>
											<div class="col-md-8" style="padding-bottom: 5px;">
												<span class="input-group-btn" style="width: 60%">
													<?= \yii\bootstrap\Html::activeTextInput($model, 'freight', ['class'=>'form-control float','style'=>'width:100%','placeholder'=>'USD']) ?>
												</span>
												<span class="input-group-btn" style="width: 10%">
                                                    <?= $form->field($model, 'freight_preview',['template' => '<div class="mt-checkbox-list col-md-7" style="margin-left: 15px; margin-right: 20px;"><label class="mt-checkbox mt-checkbox-outline">{input} {error}</label> </div>',])->checkbox([],false)->label(Yii::t('app', 'Piutang Aktif')); ?>
												</span>
												<span class="input-group-addon" style="width: 10%; width: 10%;  padding: 0; border: 0; background-color:transparent;">
                                                    Priveiw Print
												</span>
										...
										<div class="table-scrollable">
											<table class="table table-striped table-bordered table-advance table-hover" style="width: 90%" id="table-detail">
                                                ...
                                                <tr>
													...
													<tr>
														<td></td>
														<td class="text-align-right" style="padding-top: 15px;">Biaya Tambahan</td>
														<td colspan="6" class="text-align-right"><?= \yii\bootstrap\Html::activeTextInput($model, 'keterangan_biaya_tambahan', ['class'=>'form-control text-align-left','style'=>'padding: 2px; padding-left: 5px; width:100%','placeholder'=>'Input Keterangan Biaya Tambahan']) ?></td>
														<td>
															<?= \yii\bootstrap\Html::activeTextInput($model, 'total_biaya_tambahan', ['class'=>'form-control text-align-right float','style'=>'padding: 2px; width:100%','placeholder'=>'']) ?>
														</td>
													</tr>
													<tr>
...
function setDifferentPrice(){
	...
	$.ajax({
		url    : '<?= Url::toRoute(['/exim/invoice/updateDiffSizeDiffPrice']); ?>',
		type   : 'POST',
...
function setGroupingQty(){
	...
	$.ajax({
		url    : '<?= Url::toRoute(['/exim/invoice/updateGroupingQty']); ?>',
		type   : 'POST',
...
function setParent(){
    ...
    $("#<?= yii\bootstrap\Html::getInputId($model, "keterangan_biaya_tambahan") ?>").val('');
    $("#<?= yii\bootstrap\Html::getInputId($model, "total_biaya_tambahan") ?>").val(0);
	$.ajax({
        url    : '<?= Url::toRoute(['/exim/invoice/setParent']) ?>',
        type   : 'POST',
        ...
        success: function (data) {
			...
			if(data){
				...
				$("#<?= yii\bootstrap\Html::getInputId($model, "freight") ?>").val(data.freight);
...
function openPackinglist(){
	...
	var url = '<?= Url::toRoute(['/exim/invoice/openPackinglist','status_inv'=>'']); ?>'+status_inv;
...
function getItems(packinglist_id){
    $.ajax({
		url    : '<?= Url::toRoute(['/exim/invoice/getItems']); ?>',
		type   : 'POST',
		data   : {packinglist_id:packinglist_id},
		success: function (data) {
			if(data.html){
...
function subTotal(){
	...
	$("#table-detail tbody tr").each(function(i,v){
        ...
        $.ajax({
            url    : '<?= Url::toRoute(['/exim/invoice/setSubtotal']); ?>',
            type   : 'POST',
            data   : {tgl:tgl,kubikasi_display:kubikasi_display,harga:harga},
            success: function (data) {
                var subtotal = formatNumberForUser2Digit(data);
                var ppn = 0;
                ...
                $(tr).find('input[name*="[subtotal_display]"]').val( formatNumberForUser(formatNumberForUser2Digit(subtotal)) );
...
function total_potongan() {
	$("#tinvoice-total_potongan").change(function() {
        ...
        var total_biaya_tambahan = unformatNumber($("#<?= yii\bootstrap\Html::getInputId($model, "total_biaya_tambahan") ?>").val());
        ...
        var total_bayar = (total_harga + total_ppn + total_biaya_tambahan) - total_potongan;
        ...
        $("#<?= yii\bootstrap\Html::getInputId($model, "total_biaya_tambahan") ?>").val( formatNumberForUser( formatNumberFixed2(total_biaya_tambahan)) );
        $("#<?= yii\bootstrap\Html::getInputId($model, "total_bayar") ?>").val( formatNumberForUser( formatNumberFixed2(total_bayar) ) );	
	});
}

function total_biaya_tambahan() {
	$("#tinvoice-total_biaya_tambahan").change(function() {
        var total_palet = 0;
        var total_pcs = 0;
        var total_kubikasi = 0;
        var total_harga = 0;
...
function total(){
	...
	var total_potongan = unformatNumber($('#tinvoice-total_potongan').val());
	var total_biaya_tambahan = unformatNumber($('#tinvoice-total_biaya_tambahan').val());
	var total_bayar = (total_harga + total_ppn + total_biaya_tambahan) - total_potongan;
	$("#<?= yii\bootstrap\Html::getInputId($model, "total_palet") ?>").val( formatNumberForUser(total_palet) );
	$("#<?= yii\bootstrap\Html::getInputId($model, "total_pcs") ?>").val( formatNumberForUser(total_pcs) );
	$("#<?= yii\bootstrap\Html::getInputId($model, "total_kubikasi") ?>").val( formatNumberForUser(total_kubikasi) );
	...
	$("#<?= yii\bootstrap\Html::getInputId($model, "total_biaya_tambahan") ?>").val( formatNumberForUser( formatNumberFixed2(total_biaya_tambahan)) );
	$("#<?= yii\bootstrap\Html::getInputId($model, "total_bayar") ?>").val( formatNumberForUser( formatNumberFixed2(total_bayar) ) );
	$("#<?= yii\bootstrap\Html::getInputId($model, "total_harga") ?>").val( formatNumberForUser( formatNumberFixed2(total_harga) ) );    
}
...
function validatingDetail($form){
	...
	var biaya_tambahan = unformatNumber( $("#<?= \yii\bootstrap\Html::getInputId($model, "biaya_tambahan") ?>").val() );
...
function afterSave(id,edit){
	getItemsById(id,edit,function(){
		...
		<?php if(isset($_GET['edit'])){ ?>
			...
			$('input[name*="freight_preview"]').prop("disabled", false);
			...
			$('#<?= \yii\bootstrap\Html::getInputId($model, "freight") ?>').prop("disabled", false);
			...
			$("#table-detail > tbody > tr").find("input[name*='[keterangan]']").prop("disabled", false);
			...
			$('#<?= \yii\bootstrap\Html::getInputId($model, "total_biaya_tambahan") ?>').prop("disabled", false);
			$('#<?= \yii\bootstrap\Html::getInputId($model, "keterangan_biaya_tambahan") ?>').prop("disabled", false);            
...
function updateTotalBayar(id){
	...
	$.ajax({
		url    : '<?= Url::toRoute(['/exim/invoice/updateTotalBayar']); ?>',
		type   : 'POST',
...
function getItemsById(id,edit=null,callback=null){
	...
	var grouping_qty = $("#<?= \yii\helpers\Html::getInputId($model, "grouping_qty") ?>").is(":checked");
    $.ajax({
		url    : '<?= Url::toRoute(['/exim/invoice/getItemsById']); ?>',
		type   : 'POST',
...
function setDropdownPackinglist(){
	...
	$.ajax({
		url    : '<?= Url::toRoute(['/exim/invoice/setDropdownPackinglist']); ?>',
		type   : 'POST',
...
function daftarAfterSave(){
    openModal('<?= Url::toRoute(['/exim/invoice/daftarAfterSave']) ?>','modal-aftersave','90%');
}

function edit(id){
    window.location.replace('<?= Url::toRoute(['/exim/invoice/index','invoice_id'=>'']); ?>'+id+'&edit=1');
}
...
function printoutpeb(caraprint){
	var id = $('#<?= \yii\bootstrap\Html::getInputId($model, "invoice_id"); ?>').val();
	openModal('<?= Url::toRoute('/exim/invoice/konfirmsipeb') ?>?invoice_id='+id,'modal-konfirmsi','90%');
}
</script>