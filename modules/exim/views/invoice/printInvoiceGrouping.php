		...
		<td style="width: 50%; height: 8cm; vertical-align: top; padding: 5px 10px; background-color: #F1F4F7; border-right: solid 1px transparent;">
			<table style="width: 100%;">
				...
				<!--				<tr>
					...
					<td style="width: 0.5cm; vertical-align: top; text-align: center;"><b>:</b></td>
					<td style="vertical-align: top;"><?php // echo strtoupper((!empty($modPackinglist->harvesting_area)?$modPackinglist->harvesting_area:"<i>TBA</i>"));
														?></td>
				</tr>-->
			</table>
		</td>
			...
			<table style="width: 100%" id="table-detail">
				<thead>
					<tr style="border-bottom: solid 1px #000;">
						<td style="width:  0.5cm; padding: 5px; border-right: solid 1px #000; vertical-align: middle;"><b>
								<center>No</center>
							</b></td>
						<td style="width: 4.5cm; padding: 5px; border-right: solid 1px #000; vertical-align: middle;"><b>
								<center>Product Name</center>
							</b></td>
						<?php if ($model->jenis_produk == "Plywood" || $model->jenis_produk == "Lamineboard" || $model->jenis_produk == "Platform") {
							$colspan_footer = "6"; ?>
							<td style="width: 1.2cm; padding: 5px; border-right: solid 1px #000; vertical-align: middle; line-height: 1"><b>
									<center>Glue</center>
								</b></td>
						<?php } else {
							$colspan_footer = "5";
						} ?>
						<td style="width: 1.5cm; padding: 5px; border-right: solid 1px #000; vertical-align: middle; line-height: 1"><b>
								<center>Grade</center>
							</b></td>
						<td style="width: 4cm; padding: 5px; border-right: solid 1px #000; vertical-align: middle; line-height: 1"><b>
								<center>Dimension</center>
							</b></td>
						<td style="width: 2cm; padding: 5px; border-right: solid 1px #000; line-height: 1; vertical-align: middle;"><b>
								<center>Unit Price<br>(<?= $model->mata_uang ?>/M<sup>3</sup>)</center>
							</b></td>
						<td style="width: 1.2cm; padding: 5px; border-right: solid 1px #000; vertical-align: middle;"><b>
								<center><?= $satuanbesar ?></center>
							</b></td>
						<td style="width: 1.2cm; padding: 5px; border-right: solid 1px #000; vertical-align: middle;"><b>
								<center>Pcs</center>
							</b></td>
						<td style="width: 1.4cm; padding: 5px; border-right: solid 1px #000; vertical-align: middle;"><b>
								<center>M<sup>3</sup></center>
							</b></td>
						<td style="padding: 5px; vertical-align: middle;"><b>
					...
					if (count($modDetails) > 0) {
						foreach ($modDetails as $ii => $detail) {
							$modInvoiceDetail = \app\models\TInvoiceDetail::find()
								...
								->where("invoice_id = {$model->invoice_id} AND keterangan = '{$detail->keterangan}' AND grade='{$detail->grade}' AND harga_jual = {$detail->harga_jual} ")
								->one();
							$modInvoiceDetail->qty_besar = $detail->qty_besar;
							$modInvoiceDetail->qty_kecil = $detail->qty_kecil;
							//							$modInvoiceDetail->kubikasi_display = $detail->kubikasi_grouping;
							$modInvoiceDetail->kubikasi_display = $detail->kubikasi_display;
							$modInvoiceDetail->harga_jual = $detail->harga_jual;
							$modInvoiceDetail->subtotal = $detail->subtotal;
							$modInvoiceDetail->subtotal_display = $detail->subtotal_display;

							$tebal = "";
							$lebar = "";
							$panjang = "";
							$modtebal = Yii::$app->db->createCommand("SELECT produk_t FROM t_invoice_detail
																		JOIN m_brg_produk ON t_invoice_detail.produk_id = m_brg_produk.produk_id 
																		WHERE invoice_id = {$model->invoice_id} AND keterangan = '{$detail->keterangan}' 
																			AND grade='{$detail->grade}' AND harga_jual = {$detail->harga_jual} 
																		GROUP BY 1")->queryAll();
							if (count($modtebal) > 0) {
								if (count($modtebal) > 1) {
							...
							$modlebar = Yii::$app->db->createCommand("SELECT produk_l FROM t_invoice_detail
																		JOIN m_brg_produk ON t_invoice_detail.produk_id = m_brg_produk.produk_id 
																		WHERE invoice_id = {$model->invoice_id} AND keterangan = '{$detail->keterangan}' 
																			AND grade='{$detail->grade}' AND harga_jual = {$detail->harga_jual} 
																		GROUP BY 1")->queryAll();
							...
							$modpanjang = Yii::$app->db->createCommand("SELECT produk_p FROM t_invoice_detail
																		JOIN m_brg_produk ON t_invoice_detail.produk_id = m_brg_produk.produk_id 
																		WHERE invoice_id = {$model->invoice_id} AND keterangan = '{$detail->keterangan}' 
																			AND grade='{$detail->grade}' AND harga_jual = {$detail->harga_jual} 
																		GROUP BY 1")->queryAll();
							...
							if (count($modpanjang) > 0) {
								...
								if (!$norandom) {
									$sql_p = "SELECT t_packinglist_container.length AS produk_p FROM t_invoice_detail 
                                                ...
                                                WHERE t_invoice.invoice_id = {$model->invoice_id} AND t_packinglist_container.packinglist_id = {$modPackinglist->packinglist_id} 
                                                        AND t_invoice_detail.keterangan = '{$detail->keterangan}' 
                                                AND grade='{$detail->grade}' AND harga_jual = {$detail->harga_jual} 
                                                GROUP BY 1";
								...
								if ($model->invoice_id == '303' && $detail->harga_jual == '950') { // special case (00272 th.2019)
							...
							$approval = \app\models\TApproval::findOne(['reff_no' => $modPackinglist->kode, 'parameter1' => 'ALIAS PACKINGLIST']);
							$grade = "";
							$glue = "";
							$aliasglue = app\models\TAlias::findOne(['reff_no' => $modPackinglist->kode, 'alias_name' => 'glue', 'value_original' => $modInvoiceDetail->produk->glue]);
							if (!empty($aliasglue) && ($approval->status == \app\models\TApproval::STATUS_APPROVED)) {
								$glue = $aliasglue->value_alias;
							} else {
								$glue = $modInvoiceDetail->produk->glue;
							}
							$aliasgrade = app\models\TAlias::findOne(['reff_no' => $modPackinglist->kode, 'alias_name' => 'grade', 'value_original' => $modInvoiceDetail->produk->grade]);
							if (!empty($aliasgrade) && ($approval->status == \app\models\TApproval::STATUS_APPROVED)) {
								$grade = $aliasgrade->value_alias;
							} else {
								$grade = $modInvoiceDetail->produk->grade;
							}
							...
							echo	"<td style='text-align:left; padding: 2px; border-right: solid 1px #000; vertical-align: top;'>" .
								(!empty($detail->keterangan) ? $detail->keterangan : $modInvoiceDetail->produk->produk_nama)
								. "</td>";
							if ($model->jenis_produk == "Plywood" || $model->jenis_produk == "Lamineboard" || $model->jenis_produk == "Platform") {
							...
							echo	"<td style='padding: 2px; border-right: solid 1px #000; vertical-align: top;'>" .
								...
								"<span class='pull-right'>" . number_format($modInvoiceDetail->harga_jual, 2) . "</span>"
								. "</td>";
							echo	"<td style='text-align:center; padding: 2px; border-right: solid 1px #000; vertical-align: top;'>" .
								number_format($modInvoiceDetail->qty_besar)
								. "</td>";
							echo	"<td style='text-align:right; padding: 2px; border-right: solid 1px #000; vertical-align: top;'>" .
								number_format($modInvoiceDetail->qty_kecil)
								. "</td>";
							echo	"<td style='text-align:right; padding: 2px; border-right: solid 1px #000; vertical-align: top;'>" .
								number_format($modInvoiceDetail->kubikasi_display, 4)
								. "</td>";
							//							$xcvxcv = floor( ($modInvoiceDetail->harga_jual * $modInvoiceDetail->kubikasi_display) * 1000) / 1000;
							$xcvxcv = $modInvoiceDetail->harga_jual * $modInvoiceDetail->kubikasi_display;
							//                            echo "<i>" . $xcvxcv . "</i><br>";
							//                            GRAND TOTAL DI PRINT DAN WEB BEDA
							//                            PER TANGGAL 2022-06-28 TIDAK DI BULATKAN MENDAPATKAN HASIL YANG SESUAI
							//                            SEMENTARA TIDAK PAKAI KEBIJAKAN
							//                            $total += $xcvxcv;
							...
							$total_palet += $modInvoiceDetail->qty_besar;
							$total_pcs += $modInvoiceDetail->qty_kecil;
							$total_m3 += $modInvoiceDetail->kubikasi_display;
							$total += $xcvxcv;
						}
					}
					?>
				...
				<tfoot>
					<?php
					if ($model->total_potongan > 0 || $model->total_biaya_tambahan > 0) {
					?>
						<tr style="border-top: solid 1px #000;">
							<td colspan="<?= $colspan_footer ?>" class="text-align-right" style="padding: 3px; text-align: right; border-right: solid 1px #000; font-weight: bold;">TOTAL</td>
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td class="text-align-right" style="padding: 3px; text-align: right; font-weight: bold;"><span class="pull-left" style="padding-left:2px;"><?php echo $model->mata_uang; ?></span><?php echo number_format($total, 2); ?></td>
						</tr>
					<?php
					}
					?>
					...
					$total_potongan = $model->total_potongan;
					if ($total_potongan > 0) {
					?>
						<tr style="border-top: solid 1px #000;">
							<td colspan="<?= $colspan_footer ?>" class="text-align-right" style="padding: 3px; text-align: right; border-right: solid 1px #000; font-weight: bold;"><?php echo $keterangan_potongan; ?></td>
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td colspan="2" class="text-align-right" style="padding: 3px; text-align: right; font-weight: bold;"><span class="pull-left" style="padding-left:2px;"><?php echo $model->mata_uang; ?></span><?php echo number_format($total_potongan, 2); ?></td>
						</tr>
					<?php
					} else {
						$total_potongan = 0;
					...
					$total_biaya_tambahan = $model->total_biaya_tambahan;
					if ($total_biaya_tambahan > 0) {
					?>
						<tr style="border-top: solid 1px #000;">
							<td colspan="<?= $colspan_footer ?>" class="text-align-right" style="padding: 3px; text-align: right; border-right: solid 1px #000; font-weight: bold;"><?php echo $keterangan_biaya_tambahan; ?></td>
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td class="text-align-right" style="padding: 3px; text-align: center; border-right: solid 1px #000;">&nbsp;</td>
							<td colspan="2" class="text-align-right" style="padding: 3px; text-align: right; font-weight: bold;"><span class="pull-left" style="padding-left:2px;"><?php echo $model->mata_uang; ?></span><?php echo number_format($total_biaya_tambahan, 2); ?></td>
						</tr>
					<?php
					} else {
					...
					$grand_total = $total - $total_potongan + $total_biaya_tambahan;
					?>
					<tr style="border-top: solid 1px #000;">
						<td colspan="<?= $colspan_footer ?>" style="padding: 3px; text-align: right; border-right: solid 1px #000;"><b>GRAND TOTAL</b> &nbsp; </td>
						...
						<td style="padding: 3px; text-align: right;">
							...
							<b class="pull-right"><?= number_format($grand_total, 2) ?></b>
						</td>
					</tr>
					<tr style="border-top: solid 1px #000;">
						<td colspan="8" style="font-size: 1.1rem; padding: 3px; text-align: left;"><i>
								<b>Say : </b>
								<?= \app\components\DeltaFormatter::formatNumberTerbilangDollar(\app\components\DeltaFormatter::formatNumberForUserFloat($grand_total, 2)) ?>
							</i> </td>
					</tr>
					<tr style="border-top: solid 1px #000; border-bottom: solid 1px transparent;">
						...
						<?php if (!empty($model->fob_preview)) { ?>
							...
							<td style="font-size: 1.1rem; padding: 3px; text-align: right; padding-bottom: 0px; ">
								<b class="pull-left" style="padding-left:2px;"><?= $model->mata_uang ?></b>
								<b class="pull-right"><?= \app\components\DeltaFormatter::formatNumberForUserFloat($model->fob, 2) ?></b>
							</td>
						<?php } ?>
					</tr>
					<tr style="border-top: solid 1px #000; border-bottom: solid 1px transparent;">
						<td colspan="<?= $colspan_footer + 2; ?>" style="border-bottom: solid 1px transparent; font-size: 1.1rem; padding: 3px; padding-bottom: 0px; padding-left: 10px; text-align: left;"></td>
						<?php if (!empty($model->freight_preview)) { ?>
							<td style="font-size: 1.1rem; padding: 3px; text-align: right; padding-bottom: 0px; "><b>Freight : </b></td>
							<td style="font-size: 1.1rem; padding: 3px; text-align: right; padding-bottom: 0px; ">
								<b class="pull-left" style="padding-left:2px;"><?= $model->mata_uang ?></b>
								<b class="pull-right"><?= \app\components\DeltaFormatter::formatNumberForUserFloat($model->freight, 2) ?></b>
							</td>
						<?php } ?>
					</tr>
				</tfoot>
	...
	<tr style="border: solid 1px #000; border-top: solid 1px transparent; width:<?= $tablewidth ?>cm; bottom: 10px;">
		<td colspan="3" style="">
			<table style="width: 100%; font-size: 1.1rem;" border="0">
				...
				?>
				<tr>
					<td style="height: <?= (0.5 * $blankspace) ?>cm">&nbsp;</td>
				</tr>
				<tr>
...
<table style="width: 20cm; margin: 10px;" border="0">
	<tr>
		<td style="font-size: 0.9rem; border: solid 1px transparent; vertical-align: top;text-align:right;">
			<span class="pull-right nomor-dokumen-qms" style="font-size: 0.8rem;">CWM-FK-EXM-04-0</span>
		</td>
	</tr>
</table>
