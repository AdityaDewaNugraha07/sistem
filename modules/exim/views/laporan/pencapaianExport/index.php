...
$this->title = 'Target Pencapaian Export';
...
<!-- BEGIN PAGE TITLE-->
<h1 class="page-title"> <?php echo Yii::t('app', 'Target Pencapaian Export'); ?></h1>
<!-- END PAGE TITLE -->
<!-- END PAGE HEADER -->
...
<?php $this->registerJs(" 
    ...
    setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Target Pencapaian Export'))."');
...
function getRekap(){
    ...
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/exim/laporan/PencapaianExport/']); ?>',
        type   : 'GET',
...
function printout(caraPrint){
	window.open("<?= yii\helpers\Url::toRoute('/exim/laporan/pencapaianExport/printRekap') ?>?"+$('#form-search-laporan').serialize()+"&caraprint="+caraPrint,"",'location=_new, width=1200px, scrollbars=yes');
}
