        ...
        columnDefs: [
			...
			{	targets: 1, class: 'text-align-center td-kecil3',
				render: function ( data, type, full, meta ) {
					var invoice_id = full[0];
                                        return '<a titile='+invoice_id+' title="Klick untuk menampilkan Info" onclick="info('+invoice_id+');">'+data+'</a> ';
                                }
			},
			...
			{	targets: 3, class: 'text-align-right td-kecil3',
				render: function ( data, type, full, meta ) {
                    ...
                    // if( tgl > tgl_kebijakan ){
                        // console.table({data: parseFloat(data), tambahan: parseFloat(full[24]), potongan: parseFloat(full[25])})
                        // return formatNumberFixed2(data);
                        // return data;
                    // }else{
                    //     let grand_total = parseFloat(data) + parseFloat(full[24]) - parseFloat(full[25]);
                    //     return formatNumberRoundUp2Digit(grand_total);
                    // }
                    // END KEBIJAKAN
					return formatNumberForUser2Digit(parseFloat(data));
...
function info(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/exim/laporan/info','id'=>'']) ?>'+id,'modal-master-info','90%'," $('#table-master').dataTable().fnClearTable(); ");
}
</script>