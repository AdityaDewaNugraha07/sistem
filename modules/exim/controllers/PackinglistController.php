...
//                                  '(select status from t_approval where t_approval.reff_no=t_packinglist.kode and t_packinglist.mengetahui = t_approval.assigned_to order by approval_id desc limit 1) as status',
                ...
                $param['where'] = $param['table'] . ".cancel_transaksi_id IS NULL 
                                AND t_packinglist.status = 'PROFORMA' 
                                AND t_packinglist.status_approval = '" . TApproval::STATUS_APPROVED . "'";
                return \yii\helpers\Json::encode(\app\components\SSP::complex($param));//AND t_approval.status = '". \app\models\TApproval::STATUS_APPROVED."'
            }
...
//                    '(select status from t_approval where t_approval.reff_no=t_packinglist.kode and t_packinglist.mengetahui = t_approval.assigned_to order by approval_id desc limit 1) as status',
                ...
                $param['where'] = $param['table'] . ".cancel_transaksi_id IS NULL 
                                AND t_packinglist.status = 'FINAL' 
                                AND t_packinglist.status_approval = '" . TApproval::STATUS_APPROVED . "'";
                return \yii\helpers\Json::encode(\app\components\SSP::complex($param));//AND t_approval.status = '". \app\models\TApproval::STATUS_APPROVED."'
            }
        ...
        if (\Yii::$app->request->isAjax) {
            ...
            if (!empty($packinglist_id)) {
                ...
                if (!empty($modPackinglist)) {
                    ...
                    if (count($modAlias) > 0) {
                        $modApproval = TApproval::findOne(['reff_no' => $modPackinglist->kode, 'parameter1' => 'ALIAS PACKINGLIST']);
                        $status = "";
                        $reason = '';
                        if (!empty($modApproval)) {
                            if ($modApproval->status == TApproval::STATUS_APPROVED) {
                            ...
                            } else if ($modApproval->status == TApproval::STATUS_NOT_CONFIRMATED) {
                            ...
                            } else if ($modApproval->status == TApproval::STATUS_REJECTED) {
    ...
    public function actionPrint()
    {
        ...
        if (!empty($_GET['id'])) {
            ...
            $modOpEx = \app\models\TOpExport::findOne($model->op_export_id);
            $modApproval = TApproval::findOne(['reff_no' => $model->kode, 'parameter1' => 'ALIAS PACKINGLIST']);
            if (!empty($modApproval)) {
                if ($modApproval->status == TApproval::STATUS_NOT_CONFIRMATED) {
                    echo "<h3>Alias Belum di Konfirmasi oleh Approver</h3>";
                    exit;
                }
            }
    ...
    public function actionPrintlampiran()
    {
        $this->layout = '@views/layouts/metronic/print';
        if (!empty($_GET['id'])) {
            $model = \app\models\TPackinglist::findOne($_GET['id']);
            ...
            $caraprint = Yii::$app->request->get('caraprint');
            if ($model->status == "PROFORMA") {
                $paramprint['judul'] = Yii::t('app', 'SUMMARY PROFORMA PACKING LIST');
            } else {
                $paramprint['judul'] = Yii::t('app', 'SUMMARY PACKING LIST');
            }
            $paramprint['judul2'] = $model->jenis_produk;
            if ($caraprint == 'PRINT') {
                return $this->render('printPackinglistLampiran', ['model' => $model, 'modOpEx' => $modOpEx, 'paramprint' => $paramprint]);
            } else if ($caraprint == 'PDF') {
                $pdf = Yii::$app->pdf;
                $pdf->options = ['title' => $paramprint['judul']];
                $pdf->filename = $paramprint['judul'] . '.pdf';
                $pdf->methods['SetHeader'] = ['Generated By: ' . Yii::$app->user->getIdentity()->userProfile->fullname . '||Generated At: ' . date('d/m/Y H:i:s')];
                $pdf->content = $this->render('printPackinglistLampiran', ['model' => $model, 'modOpEx' => $modOpEx, 'paramprint' => $paramprint]);
                return $pdf->render();
            } else if ($caraprint == 'EXCEL') {
                return $this->render('printPackinglistLampiran', ['model' => $model, 'modOpEx' => $modOpEx, 'paramprint' => $paramprint]);
            }
        ...
        if (\Yii::$app->request->isAjax) {
            ...
            if (Yii::$app->request->post('TAlias')) {
                ...
                try {
                    ...
                    $modelApproval = TApproval::find()->where(['reff_no' => $modPackinglist->kode, 'parameter1' => 'ALIAS PACKINGLIST'])->all();
                    if (count($modelApproval) > 0) { // edit mode
                        if (TApproval::deleteAll(['reff_no' => $modPackinglist->kode, 'parameter1' => 'ALIAS PACKINGLIST'])) {
...
//					exit;
    ...
    public function actionLoadAlias()
    {
        if (\Yii::$app->request->isAjax) {
            ...
            $model = new \app\models\TAlias();
            $modPackinglist = \app\models\TPackinglist::findOne($packinglist_id);
            if ($modPackinglist->jenis_produk == "Moulding" || $modPackinglist->jenis_produk == "FingerJointLamineBoard" || $modPackinglist->jenis_produk == "FingerJointStick" || $modPackinglist->jenis_produk == "Flooring") {
