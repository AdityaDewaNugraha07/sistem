...
class PenerbitblController extends DeltaBaseController
{
    ...
    public function actionPenerbitBLPrint(){
        $this->layout = '@views/layouts/metronic/print';

        $search = Yii::$app->request->get('search');
        if ($search != "") {
            $andWhere = " and (kode ilike '%".$search."%' or nama ilike '%".$search."%' or alamat ilike '%".$search."%' )";
        } else {
            $andWhere = '';
        }
        
        $sql = "select * from m_penerbit_bl where 1=1 ".$andWhere." order by penerbit_bl_id desc";
