...
.watermark {
        ...
        font-size: 7rem;
        color: rgba(255, 0, 0, 0.1); /* Ubah warna sesuai kebutuhan */
        z-index: 1; /* Pastikan watermark berada di belakang konten */
        pointer-events: none; /* Agar tidak mengganggu interaksi */
}
.modal-body {
	position: relative; /* Tambahkan ini agar watermark berfungsi */
}
</style>
<div class="modal fade" id="modal-gkk" tabindex="-1" role="basic" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="btn btn-icon-only btn-default fa fa-close" data-dismiss="modal" aria-hidden="true" style="float: right; padding-top: 2px; padding-bottom: 2px;"></button>
                <h4 class="modal-title"><?= Yii::t('app', 'Detail GKK'); ?></h4>
            </div>
            <div class="modal-body">
				<?=($model->cancel_transaksi_id >0) ? '<div class="watermark">ABORTED</div>': ''; ?>
                <div class="row">
                    <div class="col-md-12">
						<table style="width: 19cm; margin: 10px; height: 10cm;" border="1">
							<tr style="height: 2cm;">
								<td colspan="3">
									<table style="width: 100%; height: 1cm;" border="0">
										<tr style="">
											<td style="text-align: left; vertical-align: top; padding: 10px; width: 5.5cm; height: 1.5cm; border-bottom: solid 1px transparent; border-right: solid 1px transparent;">
											...
											<td style="width: 6cm; height: 1.4cm; vertical-align: top; padding: 10px;">
												<table>
													...
													<tr>
														<td style="width:2cm; vertical-align: top;">TBP Reff</td>
														<td style="vertical-align: top;">: &nbsp;</td>
														<td>
															<?php
																if(!empty($model->tbp_reff)){
																	$model->tbp_reff = explode(',', $model->tbp_reff);
																	$model->tbp_reff = implode('<br>', $model->tbp_reff);
																	echo $model->tbp_reff;
																}
															?>
														</td>
							...
							<tr  style="height: 4cm; vertical-align: top;">
								<td colspan="3">
										...
										<tr>
											<td style="height: 20px; vertical-align: middle; border-left: solid 1px transparent;">
												...
												if(!empty($model->penerima)){
													echo "<span style='font-size:1rem'><b>".$model->penerima."</b></span>";
												}
												?>
											</td>
			...
			?>
                <?= yii\helpers\Html::button(Yii::t('app', 'Print'),['class'=>'btn blue-steel ciptana-spin-btn btn-outline','onclick'=>'printGKK('.$model->gkk_id.')']); ?>
            <?php
				}
			?>
			</div>
