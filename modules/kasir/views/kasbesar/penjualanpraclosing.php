    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Rincian Laporan Penjualan Kas Kecil Sebelum Closing ').app\components\DeltaFormatter::formatDateTimeForUser(substr($models[0]->tanggal, 0,10) ); ?></h4>
            </div>
							...
							<table class="table table-striped table-bordered table-hover" id="table-list" style="width: 100%;">
								...
								<tbody>
									<tr>
										...
										<td class="td-kecil" style="background-color: #daffaa"> &nbsp; </td>
										<td class="td-kecil text-align-right" style="background-color: #daffaa; font-weight: bold; "><?= app\components\DeltaFormatter::formatNumberForUserFloat(\app\models\HSaldoKasbesar::getSaldoAwal($models[0]->tanggal) ); ?></td>
									</tr>
									<?php 
									$totalkredit = 0; $totaldebit = 0; $debit = 0; $kredit = 0; $saldoakhir = 0;
									foreach($models as $i => $model){ 
									
									$kodeAwal = \app\components\DeltaGenerator::kodeKasBesar($models[0]->tanggal);
									$pref = substr($kodeAwal, 0,6);
									$seq = substr($kodeAwal, 6,2);
									...
									$saldoawal = \app\models\HSaldoKasbesar::getSaldoAwal($models[0]->tanggal);
            ...
            <div class="modal-footer text-align-center" style="padding-top: 10px;">
                <?php // echo yii\helpers\Html::button(Yii::t('app', 'Print'),['class'=>'btn blue-steel ciptana-spin-btn btn-outline','onclick'=>'printout("PRINT","'.$models[0]->tanggal.'")']); ?>
                <?php // echo yii\helpers\Html::button(Yii::t('app', 'Excel'),['class'=>'btn green-seagreen ciptana-spin-btn btn-outline','onclick'=>'printout("EXCEL","'.$models[0]->tanggal.'")']); ?>
                <?php // echo yii\helpers\Html::button(Yii::t('app', 'PDF'),['class'=>'btn red-flamingo ciptana-spin-btn btn-outline','onclick'=>'printout("PDF","'.$models[0]->tanggal.'")']); ?>
            </div>
        </div>
        <!-- /.modal-content -->
    </div>
