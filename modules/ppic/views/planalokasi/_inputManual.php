...
function kirim() {
    ...
    $('#loader').show();
    $.ajax({
        url: '<?= Url::toRoute(['/ppic/planalokasi/inputManuals']); ?>',
        type: 'POST',
        data: {
            clause: $('#clause').val(),
            keyword: $('#keyword').val(),
...
function showDetail(datas) {
    var no = datas.match(/No\s*:\s*(\d+)/);
    var no_barcode = no[1];
    var jenis_alokasi = '<?= $jenis_alokasi; ?>';
    $.ajax({
        url: '<?= Url::toRoute(['/ppic/planalokasi/showDetail']); ?>',
        type: 'POST',
        data: {
        ...
        success: function(data) {
            if (data) {
                if (data['msg'] == "Data ok") {
                    var url = '<?= \yii\helpers\Url::toRoute(['/ppic/planalokasi/review','no_barcode'=>'']) ?>'+no_barcode+'&jenis_alokasi='+jenis_alokasi;
                    $(".modals-place-2").load(url, function() {
                        $("#modal-review .modal-dialog").css('width','70%');
