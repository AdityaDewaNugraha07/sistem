...
$this->title = 'Scan Terima Log Hasil Potong';
...
<?php $this->registerJs(" 
	formconfig();
	reading();
	setMenuActive('" . json_encode(app\models\MMenu::getMenuByCurrentURL('Scan Penerimaan Log dari Hasil Potong')) . "');
...
function pick(datas) {
    ...
    $.ajax({
        url: '<?= \yii\helpers\Url::toRoute(['/ppic/scanterimaloghasilpotong/showDetail']); ?>',
        type: 'POST',
        data: {
        ...
        success: function(data) {
            if (data) {
                if (data['msg'] == "Data ok") {
                    openModal('<?= \yii\helpers\Url::toRoute(['/ppic/scanterimaloghasilpotong/review','no_barcode'=>'']) ?>'+no_barcode+'&id='+data.pemotongan_log_detail_potong_id,'modal-review','95%');
...
function inputManual() {
    openModal('<?= \yii\helpers\Url::toRoute(['/ppic/scanterimaloghasilpotong/inputManual']) ?>', 'modal-inputManual', '60%');
}

function daftarScanned() {
    openModal('<?= \yii\helpers\Url::toRoute(['/ppic/scanterimaloghasilpotong/daftarScanned']) ?>', 'modal-scanned', '95%');
}
</script>