    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header text-left">
                <b>INPUT PENERIMAAN LOG HASIL POTONG</b>
            </div>
...
function kirim() {
    ...
    $('#loader').show();
    $.ajax({
        url: '<?= Url::toRoute(['/ppic/scanterimaloghasilpotong/inputManuals']); ?>',
        type: 'POST',
        data: {
            clause: $('#clause').val(),
            keyword: $('#keyword').val(),
        ...
        success: function(res) {
            ...
            if(!Array.isArray(res.datas)) {
                showDetail(res.datas, res.no_barcode);
            }else {
...
function showDetail(datas, no_barcode) {
    var no_barcode = no_barcode;

    $.ajax({
        url: '<?= Url::toRoute(['/ppic/scanterimaloghasilpotong/showDetail']); ?>',
        type: 'POST',
        data: {
        ...
        success: function(data) {
            if (data) {
                if (data['msg'] == "Data ok") {
                    var url = "<?= yii\helpers\Url::toRoute("/ppic/scanterimaloghasilpotong/review") ?>?no_barcode="+no_barcode+"&id="+data.pemotongan_log_detail_potong_id;
                    ...
                    var url = "<?= yii\helpers\Url::toRoute("/ppic/scanterimaloghasilpotong/view") ?>?no_barcode="+no_barcode+"&id="+data.pemotongan_log_detail_potong_id;
                    $(".modals-place-3").load(url, function() {
                        $("#modal-review .modal-dialog").css('width','95%');
                        $("#modal-review").modal('show');
                        $("#modal-review").on('hidden.bs.modal', function () {});
...
function multipleResult(datas){
    ...
    datas.forEach(function(v, i) {
        html += `<tr>
                    ...
                    <td class="text-center td-kecil">${v.no_lap_baru}</td>
                    <td class="text-center td-kecil">${v.no_barcode_baru}<input type="hidden" id="no_barcode" value="${v.no_barcode_baru}"></td>
                    <td class="text-center td-kecil">
                        <button onclick="showDetail('ID : ${v.pemotongan_log_detail_potong_id}\\nNo : ${v.no_barcode_baru}', '${v.no_barcode_baru}')" class="btn btn-xs btn-info">
                            <i class="fa fa-eye"></i>
                        </button>
                    </td>
                </tr>`
