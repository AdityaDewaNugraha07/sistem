    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Daftar Brakedown Yang Telah Dibuat'); ?></h4>
            </div>
                        ...
                        <table class="table table-striped table-bordered table-hover table-laporan" id="table-aftersave">
							<thead>
								<tr>
									...
									<th><?= Yii::t('app', 'Kode SPK Sawmill'); ?></th>
...
function dtAfterSave(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/ppic/brakedown/daftarAfterSave') ?>',data:{dt: 'modal-aftersave'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
            ...
            { 	targets: 7, class:'text-align-center td-kecil',
                render: function ( data, type, full, meta ) {
                    ...
                    if(full[7]=="<?= \app\models\TCancelTransaksi::STATUS_ABORTED; ?>" || today !== full[4]) { // hanya dibuka di hari yg sama saja
...
function lihatDetail(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/ppic/brakedown/index','brakedown_id'=>'']); ?>'+id);
}

function edit(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/ppic/brakedown/index','brakedown_id'=>'']); ?>'+id+'&edit=1');
}
