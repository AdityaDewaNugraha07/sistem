                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    ...
                                    <span class="caption-subject hijau bold"><?= Yii::t('app', 'Master Grade'); ?></span>
                                </div>
                                <div class="tools">
                            ...
                            <div class="portlet-body">
                                <table class="table table-striped table-bordered table-hover" id="table-grade">
                                    <thead>
                                        <tr>
                                            <th></th>
...
<script>
function dtGrade(){
    var dt_table =  $('#table-grade').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/ppic/grade/index') ?>',data:{dt: 'table-grade'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
...
function create(){
	openModal('<?= \yii\helpers\Url::toRoute('/ppic/grade/create') ?>','modal-grade-create');
}
function info(id){
	openModal('<?= \yii\helpers\Url::toRoute(['/ppic/grade/info','id'=>'']) ?>'+id,'modal-grade-info');
}
...
function updateStatus(id){
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/grade/updateStatus','id'=>'']); ?>'+id,
        type   : 'POST',
        data   : {},
        success: function (data) {
			if(data){
                $('#modal-grade-info').modal('hide');
				$('#table-grade').dataTable().fnClearTable();
			}
        },
