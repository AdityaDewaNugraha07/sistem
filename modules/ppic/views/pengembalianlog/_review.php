<div class="modal fade zzz" id="modal-review" tabindex="-1" role="basic" aria-hidden="true">
    <div class="modal-dialog modal-sm" style="margin: 120px auto !important;">
        <div class="modal-content">
            <div class="modal-header text-left"><b>INPUT PENGEMBALIAN LOG</b></div>
            <div class="modal-body text-center" style="max-height: none !important;">
                ...
                <table style="margin-left: 25px;">
                    ...
                    <tr>
                        <td>Volume</td>
                        <td> : </td>
                        <td style="padding-left: 10px;" class="vol"><?php echo $modLog->fisik_volume; ?> m<sup>3</sup></td>
                    </tr>
                    <!-- TAMBAH FSC -->
                    <tr>
                        <td>Status FSC</td>
                    ...
                    <!-- eo FSC -->
                    <tr>
                        <td style="vertical-align: top;">Alasan Pengembalian</td>
                        <td style="vertical-align: top;"> : </td>
                        <td style="padding-left: 10px;"><textarea id="alasan" name="alasan" class="form-control" style="width: 100%;"></textarea></td>
                    </tr>
                    <tr>
                        <td colspan="3">
                            <hr>
...
<script>
    ...
    function addLog() {
        ...
        var no_barcode = '<?php echo $modLog->no_barcode; ?>';
        var kayu_id = $('#kayu_id').val();
        var alasan = $('#alasan').val();
        $.ajax({
            url: '<?= \yii\helpers\Url::toRoute(['/ppic/pengembalianlog/addLog']); ?>',
            type: 'POST',
            data: {
            ...
            success: function(data) {
                ...
                if(data.html){
                    ...
                    if($('#modal-inputManual')){
                        $('#modal-inputManual').modal('hide');
                    }
                } else {
                    $('#table-detail tbody').html('<tr><td colspan="15" style="text-align: center;"><i>Data tidak ditemukan</i></td></tr>');
                }
            },
    ...
    function validatingDetail(){
        ...
        if(alasan == '' || !alasan){
            $('#alasan').addClass('error-tb-detail');
		    has_error = has_error + 1;
        } else {
            $('#alasan').removeClass('error-tb-detail');
        }
