...
<div class="portlet light bordered">
    <div class="portlet-title">
        <div class="caption">
            ...
            <span class="caption-subject hijau bold"><?= Yii::t('app', 'Output Setting') ?></span>
        </div>
        <div class="tools">
            ...
            <a href="javascript:void(0);" class="btn btn-default btn-sm btn-show-history" style="font-family: FontAwesome; height: unset" onclick="openModal('<?= Url::toRoute('/ppic/monitoringsetting/modalhistory') ?>?type=OUTPUT','modal-history', '80%');"><i class="fa fa-undo"></i>
                Riwayat Output
            </a>
        </div>
    </div>
    <div class="portlet-body">
        ...
        <div class="row margin-top-20">
            <div class="col-md-12 col-sm-12 col-lg-12">
                <button type="button" class="btn btn-sm yellow-mint margin-bottom-10" id="btn-add-item" onclick="addItem()">
                    <i class="fa fa-plus"></i> Tambah Item
                </button>
                <table class="table table-bordered table-hover table-desktop" id="table-output-setting-detail-desktop">
                    <thead>
                        <tr>
                ...
                <table class="table table-bordered table-hover table-mobile" id="table-output-setting-detail-mobile" style="width: 100%;">
                    <thead>
                        <tr class="cis-mobile">
...
<script>
    function init() {
        ...
        const detail = new mtrg.Detail('setting');
        const storage = mtrg.storage('output-setting-details');
        storage.clear();

        detail.renderDetail();
        $('#form-output-setting').on('beforeSubmit', function() {
            if (storage.get().length > 0) {
                ...
                $.post("<?= Url::toRoute('/ppic/monitoringsetting/output') ?>", data)
        ...
        function showData(id) {
            $.ajax({
                url: '<?= Url::toRoute('/ppic/monitoringsetting/show') ?>?id=' + id,
                success: function({
                    master,
                    details
                }) {
    ...
    function addItem() {
        const jenis_kayu = $('#<?= Html::getInputId($model, 'jenis_kayu')?>').val();
        openModal('<?= Url::toRoute('/ppic/monitoringsetting/modaloutputdetail') ?>?jenis_kayu='+jenis_kayu, 'modal-output-detail', null);
    }
</script>