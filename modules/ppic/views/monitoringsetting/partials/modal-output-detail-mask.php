    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Output Detail Setting') ?></h4>
            </div>
...
<script>
    const jenis_kayu = '<?= $jenis_kayu ?>';
    const grades = JSON.parse('<?= Json::encode(MDefaultValue::getOptionList('mtrg-grade-setting-output'))?>');
    // if(jenis_kayu === 'Sengon') {
    //     delete(grades['M Ply']);
    // }else {
    //     delete(grades['M Ply Manual']);
    //     delete(grades['M Ply PPC']);
    //     delete(grades['M Ply Plytech']);
    // }
    const mask = mtrg
        .$('.place-detail')
        .setLabel('Grades')
    ...
    mtrg.$('#form-output-detail').on('beforeSubmit', function () {
        $.ajax({
            ...
            success: function (result) {
                try {
                    mtrg.storage('output-setting-details').insert(result);
                    $('#modal-output-detail').modal('toggle');
                    mtrg.detail('setting').renderDetail();
                }catch (e) {
                    alert(e.message);
                }
            }
