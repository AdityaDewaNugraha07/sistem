...
function dtProduk(){
    var dt_table =  $('#table-palet').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/ppic/pengajuanrepacking/paletditerima') ?>',data:{dt: 'table-palet'} },
        order: [
//            [0, 'desc']
        ],
        columnDefs: [
            ...
            {	targets: 4, class:"td-kecil", 
                render: function ( data, type, full, meta ) {
					return "<a onclick='pickPalet(\""+full[3]+"\")' class='btn btn-xs btn-icon-only btn-default' data-original-title='Pick' style='width: 25px; height: 25px;'><i class='fa fa-plus-circle'></i></a>"+data;
                }
            },
            {	targets: 5, class:"td-kecil", 
                render: function ( data, type, full, meta ) {
					return data+ ' <b>' +full[10]+ '<b>';
                }
            },
