				...
				render: function ( data, type, full, meta ) {
					...
					if(full[10]){
                                            ...
                                            var ret =  '<center>\n\
                                                                ...
                                                                <a class="btn btn-xs btn-outline dark tooltips" data-original-title="Lihat" onclick="lihatDetail('+full[0]+')"><i class="fa fa-eye"></i></a>\n\
                                                               \n\<a style="'+display+' margin-left: -5px;" class="btn btn-xs btn-outline red-flamingo tooltips <?= (Yii::$app->user->identity->user_group_id == \app\components\Params::USER_GROUP_ID_SUPER_USER)?"":"hidden"; ?>" data-original-title="Delete" onclick="deleteHasilRepacking('+full[0]+')"><i class="fa fa-trash-o"></i></a>\n\
                                                        </center>';
                                        }
//					var ret =  '<center>\n\
//                                                            <a style="margin-right: 0px; '+display+'" class="btn btn-xs btn-outline blue-hoki tooltips" data-original-title="Print QRCode" onclick="printKartuBarang('+full[0]+')"><i class="fa fa-print"></i></a>\n\
...
function deleteHasilRepacking(id){
	var url = "<?= yii\helpers\Url::toRoute("/ppic/pengajuanrepacking/deleteHasilRepacking") ?>?id="+id+"&tableid=table-aftersave";
