			...
			<table style="width: 100%" id="table-detail">
				...
				$row = 0;
					foreach($modDetail as $i => $detail){
						$total_pcs += $detail->qty_besar;
                        $m_brg_produk = \app\models\MBrgProduk::findOne($detail->produk_id);
						//$total_m3 += $detail->qty_m3; ?>
                        <?php
                            if($model->keperluan == 'Penanganan Barang Retur'){
                                $qty_besar = $detail->qty_kecil;
                                $m3 = $detail->kubikasi;
                                $modRetur = \app\models\TReturProdukDetail::findOne($detail->retur_produk_detail_id);
                                $no_produksi = $modRetur->nomor_produksi . ' - ';
                            } else {
                                $sql_x = "select produk_id, ".
                                        ...
                                        "   ";
                                $xxx = Yii::$app->db->createCommand($sql_x)->queryOne();
                                $palet = $xxx['palet'];
                                $qty_kecil = $xxx['qty_kecil'];
                                $kubikasi = $xxx['kubikasi'];
                                $qty_besar = $detail->qty_besar;
                                $m3 = ($kubikasi / $palet) * $detail->qty_besar;
                                $no_produksi = '';
                            }
                        ?>
                        <?php $row = $row+1; ?>
                        <tr style="border-bottom: 1px solid black">
                            ...
                            <td class="td-kecil2" style="border-right: 1px solid black; text-align: left;">
                                <b><?php
                                echo $no_produksi;
                                echo $m_brg_produk->produk_nama;
                                ?><b><br>
                                <?php //$detail->nomor_produksi; ?></b>
                            </td>
                            <td class="td-kecil2" style="border-right: 1px solid black;"><?= $detail->produk->produk_dimensi; ?></td>
                            <td class="td-kecil2" style="border-right: 1px solid black; text-align: center;"><?= \app\components\DeltaFormatter::formatNumberForUserFloat($qty_besar); ?></td>
                            <td class="td-kecil2" style="border-right: 1px solid black; text-align: right;"><?= $m3; ?> </td>
