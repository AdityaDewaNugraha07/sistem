											...
											<table class="table table-striped table-bordered table-advance table-hover table-laporan" style="width: 100%; border: 1px solid #A0A5A9;" id="table-master">
												<thead>
													<tr>
														...
														<th style="width: 120px; font-size: 1.3rem; line-height: 0.9; padding: 5px;"><?= Yii::t('app', 'Permintaan'); ?></th>
...
<?php $this->registerJs(" 
	formconfig();
	reading();
	setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Scan Terima Mutasi'))."');
    getItemsScanned();
", yii\web\View::POS_READY); ?>
<script>
function getItemsScanned(){
    var dt_table =  $('#table-master').dataTable({
        ajax: { url: '<?= yii\helpers\Url::toRoute("/ppic/scanterimamutasi/getItemsScanned") ?>',data:{dt: 'table-master'} },
//		order: [
//            [1, 'desc'],
//        ],
        "pageLength": 10,
        columnDefs: [
            ...
            { 	targets: 1, class:"text-align-center td-kecil", 
                render: function ( data, type, full, meta ) {
					return "<a onclick='infoPalet(\""+data+"\",\""+full[8]+"\")'>"+data+"</a>";
                }
            },
            ...
            { 	targets: 3, class:"text-align-center fontsize-0-9", 
                render: function ( data, type, full, meta ) {
					return data;
                }
            },
            ...
            { 	targets: 6, class:"text-align-center td-kecil fontsize-0-9", 
                render: function ( data, type, full, meta ) {
...
function pick(prod_number){
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/scanterimamutasi/showDetail']); ?>',
        type   : 'POST',
        data   : {prod_number:prod_number},
        success: function (data) {
			if(data){
                if (data['msg'] == "Data ok") {
                    openModal('<?= \yii\helpers\Url::toRoute(['/ppic/scanterimamutasi/review','nomor_produksi'=>'']) ?>'+prod_number,'modal-review','300px');
                } else {
                    cisAlert(data['msg']);
                }
            } else {
...
/**function pick(prod_number){
	...
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/ppic/scanterimamutasi/saveNomorProduksi']); ?>',
        type   : 'POST',
        data   : {prod_number:prod_number},
        success: function (data) {
			if(data.msg){
...
function hapusItem(id){
    openModal('<?= \yii\helpers\Url::toRoute(['/ppic/scanterimamutasi/deleteNomorProduksi','id'=>'']) ?>'+id,'modal-delete-record',null);
}
