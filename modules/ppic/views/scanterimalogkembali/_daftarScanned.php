    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Daftar Scanned Log dari Pengembalian Log'); ?></h4>
            </div>
...
function dtScanned(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/ppic/scanterimalogkembali/daftarScanned') ?>',data:{dt: 'modal-scanned'} },
        order: [
            [0, 'desc']
        ],
        autoWidth:false,
            ...
            {	targets: 11, class:"text-align-center td-kecil",
                render: function ( data, type, full, meta ) {
                    ...
                    var ret = '<center>\n\
                                    ...
                                    <a class="btn btn-xs btn-outline dark" id="print-btn-this" onclick="window.open(\'<?= Url::to(["/ppic/scanterimalogkembali/print", "id" => "" ]) ?>' + full[11]+'&no_barcode='+ full[2] + '&caraprint=PRINT\', \'Print Barcode\', \'width=1200\')"><i class="fa fa-qrcode"></i></a>\n\
...
function lihatDetail(id){
    openModal('<?= \yii\helpers\Url::toRoute(['/ppic/scanterimalogkembali/lihatDetail','id'=>'']) ?>'+id,'modal-madul','95%');
}

function confirmHapusDetail(id){
    openModal('<?= \yii\helpers\Url::toRoute(['/ppic/scanterimalogkembali/confirmHapusDetail','id'=>'']) ?>'+id,'modal-confirm','250px');
}
</script>