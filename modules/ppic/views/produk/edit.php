        ...
        <div class="modal-content">
                                ...
                                <div class="col-md-8">
                                    <div class="fileinput fileinput-new" data-provides="fileinput">
                                        <div class="fileinput-new thumbnail">
                                            <img src="'.(!empty($model->produk_gbr)?
                                            \yii\helpers\Url::base().'/uploads/gud/produk/'.$model->produk_gbr :
            ...
            <div class="modal-footer">
                <?php echo \yii\helpers\Html::button( Yii::t('app', 'Save'),['class'=>'btn hijau btn-outline ciptana-spin-btn',
                    'onclick'=>'submitformajax(this,"$(\'#modal-produk-edit\').modal(\'hide\'); $(\'#table-produk\').dataTable().fnClearTable();")'])?>
            </div>
...
function setDropdownJenisKayu(callback=null){
	...
	$.ajax({
		...
		success: function (data) {
			...
			$("#<?= \yii\bootstrap\Html::getInputId($model, "jenis_kayu") ?>").val("<?= $model->jenis_kayu ?>");
...
function setDropdownGrade(callback=null){
	...
	$.ajax({
		...
		success: function (data) {
			...
			$("#<?= \yii\bootstrap\Html::getInputId($model, "grade") ?>").val("<?= $model->grade ?>");
			if(callback){ callback(); }
		},
		error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
	});
...
function setDropdownWarnaKayu(callback=null){
	...
	$.ajax({
		...
		success: function (data) {
			...
			$("#<?= \yii\bootstrap\Html::getInputId($model, "warna_kayu") ?>").val("<?= $model->warna_kayu ?>");
			if(callback){ callback(); }
		},
		error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
	});
...
function setDropdownGlue(callback=null){
	...
	$.ajax({
		...
		success: function (data) {
			...
			$("#<?= \yii\bootstrap\Html::getInputId($model, "glue") ?>").val("<?= $model->glue ?>");
			if(callback){ callback(); }
		},
		error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
	});
...
function setDropdownProfilKayu(callback=null){
	...
	$.ajax({
		...
		success: function (data) {
			...
			$("#<?= \yii\bootstrap\Html::getInputId($model, "profil_kayu") ?>").val("<?= $model->profil_kayu ?>");
			if(callback){ callback(); }
		},
		error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
	});
...
function setDropdownKondisiKayu(callback=null){
	...
	$.ajax({
		...
		success: function (data) {
			...
			$("#<?= \yii\bootstrap\Html::getInputId($model, "kondisi_kayu") ?>").val("<?= $model->kondisi_kayu ?>");
			if(callback){ callback(); }
		},
		error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
	});
