		...
		<td colspan="3" style="padding: 0px;">
			<table style="width: 100%" id="table-detail">
				<tr style="border-bottom: solid 1px #000; border-top: solid 1px #000;">
					<th style="padding: 10px; border: solid 1px #000;"><?= Yii::t('app', 'Jenis Produk') ?></th>
					<th style="padding: 10px; border: solid 1px #000;"><?= Yii::t('app', 'Kode Produk') ?></th>
					<th style="padding: 10px; border: solid 1px #000;"><?= Yii::t('app', 'Nama Produk') ?></th>
					<th style="padding: 10px; border: solid 1px #000;"><?= Yii::t('app', 'Dimensi') ?></th>
				</tr>
				
				<?php 
				foreach ($model as $key) {
					$produk_group = $key['produk_group'];
					$produk_kode = $key['produk_kode'];
					$produk_nama = $key['produk_nama'];
					$produk_dimensi = $key['produk_dimensi'];
				?>
				<tr>
					<td style="padding: 3px; border: solid 1px #000;"><?= $produk_group; ?></td>
					<td style="padding: 3px; border: solid 1px #000;"><?= $produk_kode; ?></td>
					<td style="padding: 3px; border: solid 1px #000;"><?= $produk_nama; ?></td>
					<td style="padding: 3px; border: solid 1px #000;"><?= $produk_dimensi; ?></td>
				</tr>
				<?php } ?>
			</table>
		</td>
