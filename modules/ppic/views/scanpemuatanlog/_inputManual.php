                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-lg-12">
                        <form action="" class="form-inline">
                            <input type="hidden" id="spm_ko_id" value="<?= $spm_ko_id; ?>">
                            <?php 
...
<script>
function kirim() {
    var spm_ko_id = document.querySelector('#spm_ko_id').value;
    $('#multiple-result').hide();
    $('#loader').show();
    $.ajax({
        url: '<?= Url::toRoute(['/ppic/scanpemuatanlog/inputManuals']); ?>',
        type: 'POST',
        data: {
            clause: $('#clause').val(),
            keyword: $('#keyword').val(),
...
function showDetail(datas, no_barcode) {
    ...
    console.log('id '+no_barcode);
    $.ajax({
        url: '<?= Url::toRoute(['/ppic/scanpemuatanlog/showDetailLog']); ?>',
        type: 'POST',
        data: {
            datas: datas,
            spm_ko_id: spm_ko_id,
        ...
        success: function(data) {
            if (data) {
                if (data['msg'] == "Data ok") {
                    modalReview('<?= Url::toRoute(['/ppic/scanpemuatanlog/reviewLog','no_barcode'=>'']) ?>' + data['no_barcode']+'&spm_ko_id='+spm_ko_id);
                } else if (data['msg'] == "Data sudah ada" || data['msg'] == "Data log alam untuk dijual") {
                    modalReview('<?= Url::toRoute(['/ppic/scanpemuatanlog/view','persediaan_log_id'=>'']) ?>' + data['persediaan_log_id']);
                } else {
                    cisAlert(data['msg']);
                }
            }
...
function multipleResult(datas){
    ...
    datas.forEach(function(v, i) {
        html += `<tr>
                    ...
                    <td class="text-center td-kecil">${v.no_barcode}<input type="hidden" id="no_barcode" value="${v.no_barcode}"></td>
                    <td class="text-center td-kecil">
                        <button onclick="showDetail('ID : ${v.persediaan_log_id}\\nNo : ${v.no_barcode}', '${v.no_barcode}')" class="btn btn-xs btn-info">
                            <i class="fa fa-eye"></i>
                        </button>
                    </td>
                </tr>`
