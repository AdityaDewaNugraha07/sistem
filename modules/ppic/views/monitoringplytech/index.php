...
$this->title = 'Plytech';
?>
<h1 class="page-title"> <?php echo Yii::t('app', 'Plytech'); ?></h1>
<div class="row">
    <div class="col-md-12">
                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    ...
                                    <span class="caption-subject hijau bold"><?= Yii::t('app', 'Output Plytech') ?></span>
                                </div>
                                <div class="tools">
                                    ...
                                    <a href="javascript:void(0);" 
                                        ...
                                        onclick="openModal('<?= Url::toRoute('/ppic/monitoringplytech/modalhistory') ?>?type=OUTPUT', 'modal-history', '80%')"
                                ...
                                <div class="row margin-top-20">
                                    <div class="col-md-12 col-sm-12 col-lg-12">
                                        <button type="button" 
                                            ...
                                            id="btn-add-item" 
                                            onclick="openModal('<?= Url::toRoute('/ppic/monitoringplytech/insertoutputdetail') ?>', 'modal-output-detail', null)"
                                            >
                                            <i class="fa fa-plus"></i> Tambah Item
                                        </button>
                                        <table class="table table-bordered table-hover table-desktop"
                                               id="table-output-plytech-detail-desktop">
                                            <thead>
                                            <tr>
                                        ...
                                        <table class="table table-bordered table-hover table-mobile"
                                               id="table-output-plytech-detail-mobile"
                                               style="width: 100%;">
                                            <thead>
                                            <tr class="cis-mobile">
...
<script>
    function init() {
        ...
        const detail = new mtrg.Detail('plytech');
        const storage = mtrg.storage('output-plytech-details');
        storage.clear();

        detail.renderDetail();

        $('#form-output-plytech').on('beforeSubmit', function() {
            if (storage.get().length > 0) {
                ...
                $.post("<?= Url::toRoute('/ppic/monitoringplytech/index') ?>", data)
        ...
        function showData(id) {
            $('.modal').modal('toggle');
            $.ajax({
                url: '<?= Url::toRoute('/ppic/monitoringplytech/show') ?>?id=' + id,
                success: function({
                    master,
                    details
                }) {
                    ...
                    storage.set(details.map((d, i) => ({
                        ...d,
                        id: i + 1,
                        grade: JSON.parse(d.grade).Patching
                    })));
                    ...
                    if (master['status_approval'] === '<?= TApproval::STATUS_REJECTED ?>') {
                        $('.btn-koreksi').show();
                        $('#form-output-plytech').append(`<input type="hidden" name="TMtrgInOut[mtrg_in_out_id]" value="${master['mtrg_in_out_id']}"/>`);
                    }
                }
            });
        }
