...
$this->title = 'Drying';
?>
<h1 class="page-title"> <?php echo Yii::t('app', 'Drying'); ?></h1>
<div class="row">
    <div class="col-md-12">
                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    ...
                                    <span class="caption-subject hijau bold"><?= Yii::t('app', 'Output Drying') ?></span>
                                </div>
                                <div class="tools">
                                    ...
                                    <a href="javascript:void(0);" 
                                        ...
                                        style="font-family: FontAwesome; height: unset" 
                                        onclick="openModal('<?= Url::toRoute('/ppic/monitoringdrying/modalhistoryoutput') ?>?type=OUTPUT', 'modal-history-output', '80%')"
                            ...
                            <div class="portlet-body">
                                ...
                                <div class="row margin-top-20">
                                    <div class="col-md-12 col-sm-12 col-lg-12">
                                        <button type="button" 
                                            class="btn btn-sm yellow-mint margin-bottom-10" 
                                            id="btn-add-item" 
                                            onclick="openModal('<?= Url::toRoute('/ppic/monitoringdrying/modalOutputDetail') ?>', 'modal-output-detail', null)"
                                            >
                                            <i class="fa fa-plus"></i> Tambah Item
                                        </button>
                                        <table class="table table-bordered table-hover table-desktop" id="table-output-drying-detail-desktop">
                                            <thead>
                                                <tr>
                                        ...
                                        <table class="table table-bordered table-hover table-mobile" id="table-output-drying-detail-mobile" style="width: 100%;">
                                            <thead>
                                                <tr class="cis-mobile">
                                        ...
                                        <button class="btn btn-outline green-jungle btn-koreksi" type="button" style="display: none"><i class="fa fa-pencil-square-o"></i> Koreksi
                                        </button>
                                    </div>
                                </div>
                            </div>
...
<?php
$this->registerCssFile('@web/themes/metronic/global/plugins/loader/loader-github.css');
...
<script>
    function init() {
        ...
        const detail = new mtrg.Detail('drying');
        const storage = mtrg.storage('output-drying-details');
        storage.clear();

        detail.renderDetail();

        $('#form-output-drying').on('beforeSubmit', function() {
            if (storage.get().length > 0) {
                ...
                $.post("<?= Url::toRoute('/ppic/monitoringdrying/index') ?>", data)
        ...
        function showData(id) {
            $('.modal').modal('toggle');
            $.ajax({
                url: '<?= Url::toRoute('/ppic/monitoringdrying/show') ?>?id=' + id,
                success: function({
                    master,
                    details
                }) {
                    ...
                    if (master['status_approval'] === '<?= TApproval::STATUS_REJECTED ?>') {
                        $('.btn-koreksi').show();
                        $('#form-output-drying').append(`<input type="hidden" name="TMtrgInOut[mtrg_in_out_id]" value="${master['mtrg_in_out_id']}"/>`);
                    }
                }
            });
        }
