...
<div class="portlet light bordered">
    <div class="portlet-title">
        <div class="caption">
            ...
            <span class="caption-subject hijau bold"><?= Yii::t('app', 'Output Rotary') ?></span>
        </div>
        <div class="tools">
            <a href="javascript:void(0);" class="fullscreen btn btn-default btn-sm"
               style="height: auto; top: 0;color: unset; font-size: 12px"> Full Screen</a>
            <a href="javascript:void(0);" class="btn btn-default btn-sm" style="font-family: FontAwesome; height: unset"
               onclick="showHistory()"><i class="fa fa-undo"></i> Riwayat Output</a>
        </div>
    </div>
    <div class="portlet-body">
        ...
        <div class="row margin-top-20">
            <div class="col-md-12 col-sm-12 col-lg-12">
                <button type="button" class="btn btn-sm yellow-mint margin-bottom-10" onclick="addItem()"
                        id="btn-add-item"><i
                            class="fa fa-plus"></i> Tambah Item
                </button>
                <table class="table table-bordered table-hover table-desktop" id="table-output-rotary-detail-desktop">
                    <thead>
                    <tr>
                ...
                <table class="table table-bordered table-hover table-mobile" id="table-output-rotary-detail-mobile"
                       style="width: 100%;">
                    <thead>
                    <tr class="cis-mobile">
...
<script>
    ...
    function localMain() {
        localStorage.removeItem('output-rotary-details');
        $('#jam').timepicker({
            minuteStep: 1,
            showMeridian: false,
            showSeconds: true,
        ...
        generateDetail();
        $('#form-output-rotary').on('beforeSubmit', function () {
            if('<?= date('Y-m-d H:i:s') <= date('Y-m-d') . ' 09:00:00' && date('Y-m-d H:i:s') >= date('Y-m-d') . ' 07:00:00'?>' === '1') {
                if(!confirm('Data akan di masukan pada setup hari sebelumnya. \nApakah anda yakin ingin melanjutkan?')) {
                    return false;
                }
            }
            const localData = localStorage.getItem('output-rotary-details');
            if (localData !== null && JSON.parse(localData).length > 0) {
                ...
                $.post("<?= Url::toRoute('/ppic/monitoringrotary/output')?>", data)
        ...
        $('button[type=reset').on('click', function () {
            ...
            $('#<?= Html::getInputId($model, 'tanggal_kupas')?>').attr('disabled', false);
            $('#<?= Html::getInputId($model, 'tanggal_produksi')?>').attr('disabled', false);
            ...
            $('#closing').attr('checked', false).attr('disabled', false);
        })
    }

    function addItem() {
        openModal('<?= Url::toRoute("/ppic/monitoringrotary/modalOutputDetail")?>', 'modal-output-detail', null)
    }
    ...
    function generateDetail() {
        $('#table-output-rotary-detail-desktop .data-details').empty();
        $('#table-output-rotary-detail-mobile .data-details').empty();
        const details = localStorage.getItem('output-rotary-details');
        if (details !== null && JSON.parse(details).length > 0) {
            JSON.parse(details).forEach(function (row) {
                $('#table-output-rotary-detail-desktop .data-details').append(`
                    <tr>
                        <td class="text-center">${row['id']}</td>
                        <td>${row['unit']}</td>
                        <td>${row['grade']}</td>
                        <td>${row['tebal']}</td>
                        <td>${getKeySize(row['size'])}</td>
                        <td>${row['pcs']}</td>
                        <td>${row['volume']}</td>
                ...
                `);
                $('#table-output-rotary-detail-mobile .data-details').append(`
                    <tr>
                        ...
                        <td class="text-center" style="vertical-align: middle">
                            <button type="button" class="btn btn-warning btn-sm btn-edit margin-bottom-10" onclick="editRow(${row['id']})"><i class="fa fa-pencil"></i></button>
            ...
            $('#table-output-rotary-detail-desktop').append('<tr><td colspan="9" style="text-align: center; font-style: italic">Tidak ada item</td></tr>');
            $('#table-output-rotary-detail-mobile').append('<tr><td colspan="3" style="text-align: center; font-style: italic">Tidak ada item</td></tr>');
        }
    }
    ...
    function showHistory() {
        openModal('<?= Url::toRoute('/ppic/monitoringrotary/modalhistoryoutput')?>', 'modal-history-output', '95%');
    }

    function showData(id) {
        $.ajax({
            url: '<?= Url::toRoute('/ppic/monitoringrotary/show')?>?id=' + id,
            success: fillInput
        });
    }
    ...
    function fillInput(data) {
        ...
        const localData = details.map((row, index) => ({
            ...
            'pcs': row.pcs,
            'volume': row.volume,
            'mtrg_in_out_detail_id': row.mtrg_in_out_detail_id
        }));
        const approval = JSON.parse(reason_approval);
        localStorage.removeItem('output-rotary-details');
        localStorage.setItem('output-rotary-details', JSON.stringify(localData));

        const lkode = $('#<?= Html::getInputId($model, 'kode')?>');
        const lkupas = $('#<?= Html::getInputId($model, 'tanggal_kupas')?>');
        const lproduksi = $('#<?= Html::getInputId($model, 'tanggal_produksi')?>');
        ...
        lkupas.val(tanggal_kupas).attr('disabled', true);
        lproduksi.val(tanggal_produksi).attr('disabled', true);
        ...
        const tgl = tanggal_produksi.split(" ")[0];
        const time = tanggal_produksi.split(" ")[1];
        const date = new Date(tgl);
        $('#jam').timepicker('setTime', time).attr('disabled', true);
        $('#tanggal').val(`${date.getDate() < 10 ? '0' + date.getDate() : date.getDate()}/${date.getMonth() < 9 ? '0' + (date.getMonth() + 1) : (date.getMonth() + 1)}/${date.getFullYear()}`);
        ...
        approval.forEach(function (approver) {
            ...
            const text = approver.tanggal_approve ? `(${formatDateForUser2(approver.tanggal_approve)} : ${approver.reason})` : '';
            elemen.after(`
        ...
        if(status_approval === '<?= TApproval::STATUS_REJECTED?>') {
            $('.btn-koreksi').show();
            $('#form-output-rotary').append(`<input type="hidden" name="TMtrgInOut[mtrg_in_out_id]" value="${mtrg_in_out_id}"/>`);
        }
    }

    function editRow(id) {
        const storage = JSON.parse(localStorage.getItem('output-rotary-details'));
        const data = storage.filter(row => row.id === id)[0];
        openModal('<?= Url::toRoute("/ppic/monitoringrotary/updateOutputDetail")?>?' + $.param(data), 'modal-output-detail', null)
    }
