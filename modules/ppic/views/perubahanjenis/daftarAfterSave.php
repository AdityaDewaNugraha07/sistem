    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Daftar Pengajuan Yang Telah Dibuat'); ?></h4>
            </div>
...
function dtAfterSave(){
    var dt_table =  $('#table-aftersave').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute('/ppic/perubahanjenis/daftarAfterSave') ?>',data:{dt: 'modal-aftersave'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
            ...
            { 	targets: 6, class:'text-align-center td-kecil',
                render: function ( data, type, full, meta ) {
					var display = ""; 
                    // if((full[4]=="<?= app\models\TApproval::STATUS_REJECTED ?>") || (full[4]=="<?= \app\models\TCancelTransaksi::STATUS_ABORTED; ?>") ) {
                    if((full[4] !=="Not Confirmed") ) {
...
function lihatDetail(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/ppic/perubahanjenis/index','log_rubahjenis_id'=>'']); ?>'+id);
}

function edit(id){
    window.location.replace('<?= \yii\helpers\Url::toRoute(['/ppic/perubahanjenis/index','log_rubahjenis_id'=>'']); ?>'+id+'&edit=1');
}
