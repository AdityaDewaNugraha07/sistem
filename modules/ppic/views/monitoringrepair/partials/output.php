...
<div class="portlet light bordered">
    <div class="portlet-title">
        <div class="caption">
            ...
            <span class="caption-subject hijau bold"><?= Yii::t('app', 'Output Repair') ?></span>
        </div>
        <div class="tools">
            ...
            <a href="javascript:void(0);" class="btn btn-default btn-sm btn-show-history" style="font-family: FontAwesome; height: unset" onclick="openModal('<?= Url::toRoute('/ppic/monitoringrepair/modalhistory') ?>?type=OUTPUT','modal-history', '80%');"><i class="fa fa-undo"></i>
                Riwayat Output
            </a>
        </div>
    </div>
    <div class="portlet-body">
        ...
        <div class="row margin-top-20">
            <div class="col-md-12 col-sm-12 col-lg-12">
                <button type="button" class="btn btn-sm yellow-mint margin-bottom-10" id="btn-add-item" onclick="openModal('<?= Url::toRoute('/ppic/monitoringrepair/modaloutputdetail') ?>', 'modal-output-detail', null);">
                    <i class="fa fa-plus"></i> Tambah Item
                </button>
                <table class="table table-bordered table-hover table-desktop" id="table-output-repair-detail-desktop">
                    <thead>
                        <tr>
                ...
                <table class="table table-bordered table-hover table-mobile" id="table-output-repair-detail-mobile" style="width: 100%;">
                    <thead>
                        <tr class="cis-mobile">
...
<script>
    function init() {
        ...
        const detail = new mtrg.Detail('repair');
        const storage = mtrg.storage('output-repair-details');
        storage.clear();

        detail.renderDetail();
        $('#form-output-repair').on('beforeSubmit', function() {
            if (storage.get().length > 0) {
                ...
                $.post("<?= Url::toRoute('/ppic/monitoringrepair/output') ?>", data)
        ...
        function showData(id) {
            $.ajax({
                url: '<?= Url::toRoute('/ppic/monitoringrepair/show') ?>?id=' + id,
                success: function({
                    master,
                    details
                }) {
