    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Input Detail Repair') ?></h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-sm-12 col-md-12 col-lg-12">
                        <?php
                        echo $form->field($modDetail, 'unit')->dropDownList(MDefaultValue::getOptionList('mtrg-unit-repair'), ['prompt' => '-- Pilih Unit --']);
...
<script>
    const grades = JSON.parse('<?= Json::encode(MDefaultValue::getOptionList('mtrg-grade-repair-output'))?>');
    delete grades['Output'];
    const mask = mtrg
        .$('.place-detail')
        .setLabel('Grades')
        .setGrades(grades)
    ...
    mtrg.$('#form-input-detail').on('beforeSubmit', function () {
        $.ajax({
            url: '<?= Url::toRoute('/ppic/monitoringrepair/extractdetail')?>',
            type: 'POST',
            data: $(this).serialize(),
            success: function (result) {
                try {
                    mtrg.storage('input-repair-details').insert(result);
                    $('#modal-input-detail').modal('toggle');
                    mtrg.detail('repair', 'input').renderDetail();
                }catch (e) {
                    alert(e.message);
                }
            }
