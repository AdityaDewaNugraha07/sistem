    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= Yii::t('app', 'Tambahkan Pengajuan Produk Baru'); ?></h4>
            </div>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
                        <?= $form->field($modDetail, 'produk_group')->dropDownList(\app\models\MDefaultValue::getOptionListProduk('jenis-produk'),['class'=>'form-control','onchange'=>'setJenisProduk();']) ?>
            ...
            <div class="modal-footer">
                <?php echo \yii\helpers\Html::button( Yii::t('app', 'Tambah Data'),['class'=>'btn hijau btn-outline ciptana-spin-btn',
                    'onclick'=>'submitData()'
                    ]);
                ?>
            </div>
...
<script>
$('#tpengajuanmasterprodukdetail-produk_gbr').change(function() {
    var file = $('#tpengajuanmasterprodukdetail-produk_gbr')[0].files[0].name;
    $('#label_nama_file').text(file);
    console.log('klik');
});

$('#remove_file').click(function() {
    $('#label_nama_file').text('No file selected');
});

function submitData() {
    var form = $('#form-produk-create')[0];
    var data = new FormData(form); 

    $.ajax({
        url: '<?= \yii\helpers\Url::toRoute(['/ppic/pengajuanmasterproduk/create']); ?>', 
        type: 'POST',
        data: data,
        processData: false, 
        contentType: false,
        success: function(response) {
            if (response.status) {
                cekKode(response.data.produk_kode, function(exist) {
                    ...
                    } else if (kodeExist(response.data.produk_kode)) {
                        cisAlert('Produk dengan Kode "' + response.data.produk_kode + '" sudah ditambahkan.');
                    } else {
                        appendRow(response.data);
                        $('#modal-produk-create').modal('hide');
                    }
                });
            } else {
                        ...
                        setDropdownProfilKayu(function(){
                            setDropdownKondisiKayu(function(){
                                setKodeNamaProduk(function(){
                                    ...
                                    $("#<?= yii\bootstrap\Html::getInputId($modDetail, "produk_t") ?>").val("0");
                                    $("#<?= yii\bootstrap\Html::getInputId($modDetail, "produk_l") ?>").val("0");
                                    $("#<?= yii\bootstrap\Html::getInputId($modDetail, "produk_p") ?>").val("0");
                                    
                                    if(jenis_produk == 'Sawntimber'){
                                        $('#<?= \yii\bootstrap\Html::getInputId($modDetail, 'produk_p_satuan') ?>').val("cm");
                                        $('#<?= \yii\bootstrap\Html::getInputId($modDetail, 'produk_l_satuan') ?>').val("cm");
                                        $('#<?= \yii\bootstrap\Html::getInputId($modDetail, 'produk_t_satuan') ?>').val("cm");
                                    }else if(jenis_produk == 'Moulding'){
                                        $('#<?= \yii\bootstrap\Html::getInputId($modDetail, 'produk_p_satuan') ?>').val("feet");
                                        $('#<?= \yii\bootstrap\Html::getInputId($modDetail, 'produk_l_satuan') ?>').val("mm");
                                        $('#<?= \yii\bootstrap\Html::getInputId($modDetail, 'produk_t_satuan') ?>').val("mm");
                                    }else{
                                        $('#<?= \yii\bootstrap\Html::getInputId($modDetail, 'produk_p_satuan') ?>').val("mm");
                                        $('#<?= \yii\bootstrap\Html::getInputId($modDetail, 'produk_l_satuan') ?>').val("mm");
                                        $('#<?= \yii\bootstrap\Html::getInputId($modDetail, 'produk_t_satuan') ?>').val("mm");
                                    }
                                    setDimensi();
