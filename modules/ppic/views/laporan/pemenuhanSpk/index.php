...
$this->title = 'Monitoring Pemenuhan SPK Sawmill';
		...
		<div class="portlet light bordered">
			...
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject hijau bold"><?= Yii::t('app', 'Monitoring Pemenuhan SPK Sawmill '); ?></span>
				</div>
				<div class="tools">
...
function dtLaporan(){
    var dt_table =  $('#table-laporan').dataTable({
		pageLength: 100,
        ajax: { 
			url: '<?= \yii\helpers\Url::toRoute('/ppic/laporan/pemenuhanSpk') ?>',
			data:{
				dt: 'table-laporan',
        ...
        columnDefs: [
            ...
            {	targets: 4, class: 'text-align-center td-kecil',
				render: function ( data, type, full, meta ) {
					return full[5]?full[5]:0;
                }
			},
            {	targets: 5, class: 'text-align-right td-kecil',
				render: function ( data, type, full, meta ) {
					var kubikasi = full[2] * full[3] * full[4] * full[5] / 1000000; //cm3 convert to m3
...
function printout(caraPrint){
	window.open("<?= yii\helpers\Url::toRoute('/ppic/laporan/pemenuhanSpkPrint') ?>?"+$('#form-search-laporan').serialize()+"&caraprint="+caraPrint,"",'location=_new, width=1200px, scrollbars=yes');
}
</script>