...
<script>
    function init() {
        const table = $('#table-monitoring-rekap').DataTable({
            ajax: {
                url: '<?= Url::toRoute('/ppic/laporan/rekapmonitoringrotary') ?>',
                type: 'POST',
                data: function(d) {
            ...
            columnDefs: [{
                ...
                }, 
                {
                    ...
                    render: (data, type, row, meta) => {
                        let jam = Math.floor(parseInt(data) / 60);
    ...
    function printout(caraPrint) {
        window.open("<?= Url::toRoute('/ppic/laporan/rekapmonitoringrotaryprint') ?>?caraprint=" + caraPrint + "&" + $('#filter-rekap').serialize(), "",
