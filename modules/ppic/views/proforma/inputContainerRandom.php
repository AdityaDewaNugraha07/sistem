...
if($tipe=='new'){
	$container_no = \Yii::$app->db->createCommand("SELECT MAX(container_no) FROM t_packinglist_container WHERE packinglist_id = ".$packinglist_id)->queryOne()['max'];
	$container_no = (!empty($container_no)?$container_no:0 )+1;
...
?>
<div class="modal fade" id="modal-inputrandom" tabindex="-1" role="basic" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="btn btn-icon-only btn-default fa fa-close" data-dismiss="modal" aria-hidden="true" style="float: right; padding-top: 2px; padding-bottom: 2px;"></button>
                <h4 class="modal-title"><?= Yii::t('app', 'Input Produk Random'); ?></h4>
            </div>
								...
								<thead>
									...
									<tr style="background-color: #E3E7EA">
										<td colspan="<?= $colspan_header ?>" style="vertical-align: middle;">
											<b>Container Seq. : 
												<?= yii\bootstrap\Html::textInput("container_no",(!empty($container_no)?$container_no:""),['class'=>'form-control','style'=>'width:40px; text-align:center; font-weight:600; display:inline; padding:2px; height: 26px; color:#3E66A6']) ?>
											</b> &nbsp; &nbsp; &nbsp; &nbsp; 
										...
										for($i=0;$i<$total_row;$i++){
											...
											<tr>
												...
												<td style="padding: 2px; width: 200px;">
													<?= \yii\bootstrap\Html::activeTextInput($model, '['.$i.']volume_display',['class'=>'form-control float','disabled'=>'disabled','style'=>'text-align:right; height: 26px; padding:2px; font-size:1.2rem;','value'=>0]) ?>
												</td>
											</tr><?php
										}
									}
								...
								<tfoot style="background-color: #E3E7EA">
									<tr>
										...
										<td colspan="<?= $colspan_footer ?>" style="text-align: left; vertical-align: middle; font-size: 1.2rem;" > 
											Gross Weight (kg) : <?= yii\bootstrap\Html::textInput("gross_weight",(!empty($gross_weight)?$gross_weight:0),['class'=>'form-control float','onblur'=>'setContainerDetails();','style'=>'width:80px; text-align:right; font-weight:600; display:inline; padding:2px; height: 26px; color:#3E66A6']) ?> &nbsp; &nbsp; &nbsp;
											Nett Weight (kg) : <?= yii\bootstrap\Html::textInput("nett_weight",(!empty($nett_weight)?$nett_weight:0),['class'=>'form-control float','onblur'=>'setContainerDetails();','style'=>'width:80px; text-align:right; font-weight:600; display:inline; padding:2px; height: 26px; color:#3E66A6']) ?>
										</td>
            ...
            <div class="modal-footer">
                <?php echo \yii\helpers\Html::button( Yii::t('app', 'Set Container'),['class'=>'btn hijau btn-outline ciptana-spin-btn',
//                    'onclick'=>'set(this);'
                    'onclick'=>'setContainer(this);'
                    ]);
                        ?>
            </div>
...
if($tipe=='new'){
    ...
    $pagemode = "formconfig(); setValueRandom();";
}
$this->registerJs("
    $pagemode
...
function totalRandom(ele){
	var qty = 0; var m3 = 0; var total_pcs = 0; var total_vol = 0;
	$(ele).parents('table').find('tbody > tr').each(function(){
		...
		$(this).find('.qtyperuk').each(function(){ // random
			...
			sub_qty += unformatNumber( qty );
			sub_m3 += unformatNumber( m3 );
		});
		if($(this).find('.qtyperuk').length > 0){
			var sub_m3_string = (Math.round( sub_m3 * 10000 ) / 10000 ).toString();
			$(tr).find('input[name*="[pcs]"]').val( formatNumberForUser(sub_qty) );
			console.log(sub_m3_string);
			$(tr).find('input[name*="[volume_display]"]').val( formatNumberForUser(sub_m3_string) );
		}
		total_pcs += unformatNumber( $(tr).find('input[name*="[pcs]"]').val() );
		total_vol += unformatNumber( $(tr).find('input[name*="[volume_display]"]').val() );
	});
	var nett_weight = formatNumberFixed4(total_vol) * 850;
	var gross_weight = (formatNumberFixed4(total_vol) * 850) + 400;
	$(ele).parents('table').find('input[name*="tot_pcs"]').val( total_pcs );
	$(ele).parents('table').find('input[name*="tot_vol"]').val( formatNumberFixed4(total_vol) );
	$(ele).parents('table').find('input[name*="gross_weight"]').val( formatNumberFixed2(gross_weight) );
	$(ele).parents('table').find('input[name*="nett_weight"]').val( formatNumberFixed2(nett_weight) );
}
...
function setValueRandom(packinglist_id){
	var packinglist_id = '<?= $packinglist_id ?>';
	var container_no = '<?= $container_no ?>';
	var elm = $("#form-inputrandom").find(".table-contrainer");
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/ppic/proforma/setValueRandom']); ?>',
		type   : 'POST',
		data   : {packinglist_id:packinglist_id,container_no:container_no},
				...
				$(data).each(function(){
					...
					var container_no = $(elm).find("input[name='container_no']").val();
						if(container_no == db['container_no']){
							...
							$(elm).find("input[name='lot_code']").val( db['lot_code'] );
