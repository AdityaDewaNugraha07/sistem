	...
	public function actionIndex(){
		...
		if( Yii::$app->request->post('TSpl')){
            ...
            try {
				...
				$success_4 = true;	// && update tmp_spp_spo_spl_tbp
                    ...
                    if($model->save()){
                        ...
                        if( (isset($_POST['TSplDetail'])) && (count($_POST['TSplDetail'])>0) ){
                            foreach($_POST['TSplDetail'] as $i => $detail){
								...
								$mapTmpReffno = \app\models\TmpSppSpoSplTbp::findOne(['sppd_id'=>$detail['sppd_id']]);
								$sq1 = "SELECT 
	...
	public function actionDaftarSpl(){
        if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='table-spl'){
				...
				$param['column'] = ['spl_id','spl_kode',['col_name'=>'spl_tanggal','formatter'=>'formatDateForUser2'],'pegawai_nama','cancel_transaksi_id'];
