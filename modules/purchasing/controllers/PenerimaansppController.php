...
//		$model->tgl_awal = date('d/m/Y',strtotime("-1 day"));
    ...
    public function actionSppberes(){
		$model = new \app\models\TSppDetail();
		$model->tgl_awal = date('d/m/Y');
		$model->tgl_akhir = date('d/m/Y');
		$model->status = ['COMPLETE'];
		return $this->render('sppberes',['model'=>$model]);
    }
...
//					$data['html'] .= $this->renderPartial('_item',['mods'=>$mods,'detail'=>$detail,'i'=>$i]);
					...
					if(!empty($detail['bhp_id'])){
                        ...
                        // itung retur sek jon
                        // map jupuk terima_bhpd_id jon 
                        $map_spp_detail_reff = \app\models\MapSppDetailReff::findOne(["sppd_id"=>$detail['sppd_id']]);
                            $terima_bhpd_id = $map_spp_detail_reff['terima_bhpd_id'];
                        
                        // retur jupuk qt jon
                        $retur_bhp = \app\models\TReturBhp::findOne(["terima_bhpd_id"=>$terima_bhpd_id]);
                            $qty_retur = $retur_bhp['qty'];
                        $data['items'][$i]['qty_retur'] = $qty_retur;
						
                        $data['items'][$i]['status_spp_detail'] = $modSppDetail->StatusSppDetail;
						$data['items'][$i]['html_suplier'] = "";
						// eo iturg retur sek jon
        ...
        if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='table-laporan'){
				...
				$param['column'] = ['terima_bhpd_id',
									...
									'terimabhpd_harga',
									"CASE 
									    WHEN t_terima_bhp.ppn_nominal > 0 THEN
                                            CASE 
												WHEN tglterima >= '2025-01-02' THEN ( terimabhpd_qty * terimabhpd_harga * 0.11 )
                                                WHEN tglterima >= '2022-04-01' THEN ( terimabhpd_qty * terimabhpd_harga * 0.11 ) 
                                                ELSE ( terimabhpd_qty * terimabhpd_harga * 0.1 ) 
                                            END 
                                        ELSE 0
                                    END AS ppn",
									't_terima_bhp.spo_id',
									"CASE
		                                WHEN t_terima_bhp.ppn_nominal > 0 THEN
	                                        CASE
												WHEN tglterima >= '2025-01-02' THEN ( terimabhpd_qty * terimabhpd_harga ) + ( terimabhpd_qty * terimabhpd_harga * 0.11 ) + COALESCE(pph_peritem, 0)
			                                    WHEN tglterima >= '2022-04-01' THEN ( terimabhpd_qty * terimabhpd_harga ) + ( terimabhpd_qty * terimabhpd_harga * 0.11 ) + COALESCE(pph_peritem, 0)
			                                    ELSE ( terimabhpd_qty * terimabhpd_harga ) + ( terimabhpd_qty * terimabhpd_harga * 0.1 ) + COALESCE(pph_peritem, 0)
                                            END 
                                        ELSE ( terimabhpd_qty * terimabhpd_harga ) + COALESCE(pph_peritem, 0)
	                                END AS total",
...
//				exit;
			...
			try {
				...
				if($model->validate()){ 
					if(!empty($model->file1)){ 
						...
						$data['message']    = Yii::t('app', 'Wajib Upload FOTO Penawaran');
					}
					if($model->save()){
						$success_1 = true;
					}
	...
	public function actionImage($id){
        if(\Yii::$app->request->isAjax){
                $attch = \app\models\TPenawaranBhp::findOne($id);
