						...
						<div class="col-md-6">
							<?= $form->field($model, 'tgl_transaksi',[
								'template'=>'{label}<div class="col-md-8"><div class="input-group input-medium date date-picker">{input} <span class="input-group-btn">
											 ...
											 {error}</div>'])->textInput(['readonly'=>'readonly'])->label(Yii::t('app', 'Per Tanggal')); ?>
							<?= $form->field($model, 'bhp_nm')->textInput()->label(Yii::t('app', 'Nama BHP')); ?>
						</div>
						<div class="col-md-5">
							<?= $form->field($model, 'bhp_group')->dropDownList(\app\models\MDefaultValue::getOptionList('group-bahan-pembantu'),['prompt'=>'All'])->label(Yii::t('app', 'Kelompok BHP')); ?>
							<?= $form->field($model, 'available',['template' => '{label}<div class="mt-checkbox-list col-md-7"><label class="mt-checkbox mt-checkbox-outline">{input} {error}</label> </div>',])
									->checkbox([],false)->label(Yii::t('app', 'Available Stock Only')); ?>
						</div>
					</div>
