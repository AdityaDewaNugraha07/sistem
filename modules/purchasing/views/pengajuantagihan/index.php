                    ...
                    <div class="col-md-12">
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    <span class="caption-subject bold"><h4><?= Yii::t('app', 'Pengajuan Tagihan Nota Transfer Supplier Bahan Pembantu'); ?></h4></span>
                                </div>
                                <div class="tools">
                            ...
                            <div class="portlet-body">
                                ...
                                <div class="row" id="mode-cari" style="display: none;">
                                    ...
                                    <div class="col-md-5">
										<?= $form->field($model, 'kode_spo')->textInput(['placeholder'=>'Ketik Kode SPO'])->label("Kode SPO"); ?>
										<?= $form->field($model, 'nomor_nota')->textInput(['placeholder'=>'Ketik No. Nota'])->label("No. Nota"); ?>
										<?= $form->field($model, 'nomor_kuitansi')->textInput(['placeholder'=>'Ketik No. Kuitansi'])->label("No. Kuitansi"); ?>
										<?= $form->field($model, 'no_fakturpajak')->textInput(['placeholder'=>'Ketik No. Faktur Pajak'])->label("No. Faktur Pajak"); ?>
                                    </div>
									<div class="col-md-1">
											...
											<table class="table table-striped table-bordered table-advance table-hover" id="table-detail" style="width: 100%;">
												<thead>
													<tr class="nodrag">
														...
														<th style="width: 100px;"><?= Yii::t('app', 'Kode PO'); ?></th>
														...
														<th style="width: 80px;"><?= Yii::t('app', 'Lunas'); ?></th>
...
function getItems(){
	...
	var kode_spo = $('#<?= yii\bootstrap\Html::getInputId($model, 'kode_spo') ?>').val();
	var nomor_nota = $('#<?= yii\bootstrap\Html::getInputId($model, 'nomor_nota') ?>').val();
	var nomor_kuitansi = $('#<?= yii\bootstrap\Html::getInputId($model, 'nomor_kuitansi') ?>').val();
	var no_fakturpajak = $('#<?= yii\bootstrap\Html::getInputId($model, 'no_fakturpajak') ?>').val();
	$.ajax({
		url    : '<?php echo \yii\helpers\Url::toRoute(['/purchasing/pengajuantagihan/getItems']); ?>',
		type   : 'POST',
		data   : {tgl:tgl,mode:mode,tgl_awal:tgl_awal,tgl_akhir:tgl_akhir,suplier:suplier,kode_tbp:kode_tbp,kode_spo:kode_spo,nomor_nota:nomor_nota,nomor_kuitansi:nomor_kuitansi, no_fakturpajak:no_fakturpajak},
...
function pickPanelTBP(){
	openModal('<?= \yii\helpers\Url::toRoute(['/purchasing/pengajuantagihan/pickPanelTBP']) ?>','modal-tbp','75%');
}
function pickingTBP(){
	var picked = $('#select_data').val();
	var eleid = $('#eleid').val();
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/purchasing/pengajuantagihan/pickTBP']); ?>',
		type   : 'POST',
		data   : {picked:picked},
		success: function (data) {
			if(data.html){
...
function save(ele){
    ...
    if(formrequiredvalidate($form)){
        ...
        if(validatingDetail(ele)){
			...
			$.ajax({
				url    : '<?php echo \yii\helpers\Url::toRoute(['/purchasing/pengajuantagihan/index']); ?>',
				type   : 'POST',
...
function setStatus(ele,value=null,ket){
	if(value){
		if(value=="DIAJUKAN"){
			value = "<label style='font-size:1rem;' class='label label-warning tooltips' title='Menunggu Konfirmasi Finance'>"+value+"</label>";
...
function deleteItem(ele){
	var pengajuan_tagihan_id = $(ele).parents("tr").find("input[name*='[pengajuan_tagihan_id]']").val();
    openModal('<?= \yii\helpers\Url::toRoute(['/purchasing/pengajuantagihan/delete','id'=>''])?>'+pengajuan_tagihan_id,'modal-delete-record');
}
...
function updateBerkas(pengajuan_tagihan_id){
	openModal('<?= \yii\helpers\Url::toRoute(['/finance/tagihanbhp/updateBerkas','pengajuan_tagihan_id'=>''])?>'+pengajuan_tagihan_id,'modal-transaksi',null,'getItems();');
}
