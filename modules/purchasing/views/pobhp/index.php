                            ...
                            <div class="portlet-body">
                                <div class="row">
                                    <div class="col-md-6">
											...
											<div class="form-group">
												...
												<div class="col-md-7" style="padding-bottom: 5px;">
													<span class="input-group-btn" style="width: 90%">
														<?= Html::activeTextInput($model, 'spo_kode', ['class'=>'form-control','style'=>'width:100%']) ?>
													</span>
										...
										<?php
										//if ( isset($_GET['spo_id']) || $model->cancel_transaksi_id == NULL || $terimabhp_kode == NULL || $terimabhp_kode == NULL ) {
										if ( isset($_GET['spo_id']) && $model->cancel_transaksi_id == NULL && $terimabhp_kode == NULL ) {
										?>
											<div class="form-group">
                                    ...
                                    <div class="col-md-6">
										...
										<?php if(isset($_GET['spo_id'])){ ?>
											...
											<div class="form-group">
												...
												<div class="col-md-7" style="margin-top:7px; height: 30px;">
													<?php
													if(count($model)>0){
														if($model->approve_status == \app\models\TApproval::STATUS_APPROVED){
															echo '<span class="label label-sm label-success"> '.$model->approve_status .' At: '.$model->updated_at.' </span>';
														}else if($model->approve_status == \app\models\TApproval::STATUS_REJECTED){
															echo '<span class="label label-sm label-danger"> '.$model->approve_status  .' At: '.$model->updated_at.' </span>';
														}else{
											...
											<div class="form-group">
												...
												<div class="col-md-7" style="margin-top:7px; height: 30px;">
													<?= $model->reason_approval ? \yii\helpers\Json::decode($model->reason_approval, true)[0]['reason'] : '' ?>
												</div>
											</div>
											<?php endif;?>
										<?php } ?>
										...
										<?= \yii\helpers\Html::activeHiddenInput($model, "berkas") ?>
										...
										<div class="form-group">
											<div class="form-group">
												<div class="col-md-5 pull-right">
													<?php echo Html::activeDropDownList($model, 'mata_uang', \app\models\MDefaultValue::getOptionList('mata-uang'),['class'=>'form-control','style'=>'font-size: 1.3rem; padding: 3px; height: 27px;','onchange'=>'setMataUang()']) ?>
												</div>
                                            ...
                                            <table class="table table-striped table-bordered table-advance table-hover" id="table-detail">
                                                <thead>
                                                    <tr>
														...
														<th rowspan="2" style="width: 60px; vertical-align: middle; text-align: center;"><?= Yii::t('app', 'Status<br>Garansi'); ?></th>
												...
												<tfoot>
													...
													<tr>
														<td colspan="8" style="text-align: right; font-weight: 500; ">&nbsp; PPh</td>
														<td style="padding-left: 8px; padding-right: 8px;">
															<?= \yii\helpers\Html::activeTextInput($model, 'spo_pph_nominal', ['class'=>'form-control float', 'onkeyup'=>'hitungpph()','style'=>'padding:3px;']); ?>
														</td>
													</tr>
													<tr>
														<td colspan="8" style="text-align: right; font-weight: 500; vertical-align: middle;">
															<?= Yii::t('app', 'PPN'); ?>
														</td>
														<td style="padding-left: 8px; padding-right: 8px; padding: 3px 8px; text-align: right;">
															<?php echo yii\bootstrap\Html::activeTextInput($model,'spo_ppn_nominal',['class'=>'form-control money-format','style'=>'width:100%; padding:3px;', 'disabled' => 'disabled', 'onkeyup' => 'setSubtotal()']); ?>
														</td>
														<td colspan="2"></td>
													</tr>
													<tr>
														<td colspan="8" style="text-align: right; vertical-align: middle;">&nbsp;<span> <?= Yii::t('app', 'Total Bayar'); ?> </span></td>
	...
	$.ajax({
		...
		success: function (data) {
			...
			if(data.penawaran){
				...
				$('#<?= \yii\helpers\Html::getInputId($model, "berkas") ?>').val(data.berkas);
				// var tests = JSON.parse(data.penawaran);
				// console.log(tests);
				// console.log(data.penawaran);
				// console.log(data.berkas);
				// console.log(data.penawaran_bhp_id);
			}else{
				$('#<?= \yii\helpers\Html::getInputId($model, "berkas") ?>').val('false');
			}
			cekberkasPenawaran();
...
function cekberkasPenawaran(){
	var jumlah_item = $('#table-detail tbody tr').length;	
	var filepenawaran = 0;
	for(let i = 0; i < jumlah_item; i++){
		var penawaran = unformatNumber($('#TSpoDetail_'+i+'_cekfilepenawaran').val());
		filepenawaran += penawaran ;
		// console.log('looop '+i);
		// console.log('jmlPenawaran '+penawaran);			
	}
	// console.log('file penawaran ' +filepenawaran);
	if (filepenawaran == 0 ){
		$('#<?= \yii\helpers\Html::getInputId($model, "berkas") ?>').val('true'); //console.log('true');
	}else{
		$('#<?= \yii\helpers\Html::getInputId($model, "berkas") ?>').val('false'); //console.log('false');
	}
}
...
function hitungppn(){
	...
	$('#table-detail tbody tr').each(function(index){
		...
		var harga = unformatNumber($(this).find('input[name*="[spod_harga]"]').val());
		if($('#<?= yii\bootstrap\Html::getInputId($model, 'spo_is_ppn') ?>').is(':checked')){
			harga = harga / ( 1 + <?= Params::DEFAULT_PPN?>);
            console.log(harga)
			...
			ppn += unformatNumber(qty * harga) * <?= Params::DEFAULT_PPN?>;
...
function checkPenawaran(){
	var xstatusberkas = $("#tspo-berkas").val();
	// console.log(xstatusberkas);
	if(xstatusberkas == 'true'){
		return true;
	}else{
...
function setMataUang(){
	var selected = $("#<?= Html::getInputId($model, "mata_uang") ?>").val();
	$.ajax({
...
function ppn_checked() {
    $('#<?= Html::getInputId($model, 'spo_is_ppn')?>').on('change', function() {
        if(this.checked) {
            $('#<?= Html::getInputId($model, 'spo_ppn_nominal')?>').attr('disabled', false);
        }else {
            $('#<?= Html::getInputId($model, 'spo_ppn_nominal')?>').attr('disabled', true);
        }
    })
}
</script>