...
<?php	
	if(!empty($model->spo_id)){
		$mod = \app\models\TSpo::findOne($model->spo_id);
	}else{
		$mod = \app\models\TSppDetail::findOne($model->sppd_id);
	}	
?>
<style>
tr:hover .button {
...
function dtPenawaran(){
    var dt_table =  $('#table-penawaran').dataTable({
		...
		rowCallback: function (row, data) {
            // Kondisi untuk memberi gaya khusus pada baris
            if (data[3] === "<?= $mod->suplier->suplier_nm ?>") {
                $(row).css('background-color', '#fceeb1'); // Warna sesuai $mark
            }
        }
    });
}
