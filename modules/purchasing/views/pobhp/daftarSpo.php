    ...
    var dt_table =  $('#table-spo').dataTable({
        ...
        columnDefs: [
			...
			{	targets: 7, 
				class:'td-kecil',
				createdCell :  function (td, cellData, rowData, row, col) {
					$(td).attr('style', 'font-size:1rem; text-align: center; width: 10px;'); 
				},				
				render: function ( data, type, full, meta ) {
					...
					if(full[8]){
						...
						if (full[12] == '') {
							...
							var ret = '<span class="label label-sm label-danger text-center" style="text-decoration: line-through; text-align: center;">'+full[7]+'</span>';
						}
					}else{
						if(data){
							//var ret = '<a onclick="infoTBP(\''+full[6]+'\')" >'+data+'</a>';
							if (full[12] == '') {
								var ret = '';
							} else {
								var ret = '<center><a onclick="infoTBP(\''+full[6]+'\')" >'+data+'</a></center>';
							}
						}
					}
                    return ret;
			...
			{	targets: 8, 
                ...
                render: function ( data, type, full, meta ) {
                    var ret = data;
                    if(full[8]){
                        var ret = '<span class="label label-sm label-danger text-center"><?= \app\models\TCancelTransaksi::STATUS_ABORTED ?></span>';
                    }else{
                        if(full[05]== 'Not Confirmed'){                            
                            var ret = '<span style="font-size: 10px; text-align: right;"><b>Assigne to :</b> '+full[9]+'</span><br>\n\
								<span style="font-size: 10px; text-align: right;"><b>Confirm by :</b> '+full[10]+'</span>';
                        }else{
                            var ret = '<span style="font-size: 10px; text-align: right;"><b>Assigne to :</b> '+full[9]+'</span><br>\n\
								<span style="font-size: 10px; text-align: right;"><b>Confirm by :</b> '+full[10]+'</span><br>\n\
                                                                    <span style="font-size: 10px; text-align: right;"><b>At :</b> '+full[13]+'</span>';
                        }    
                    }
                    return ret;
                },
				createdCell :  function (td, cellData, rowData, row, col) {
					$(td).attr('style', 'font-size:1rem; text-align: center;'); 
				},
            },			
            {	targets: 9,
				class:'td-kecil',
