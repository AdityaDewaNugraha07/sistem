...
<div class="modal fade" id="modal-produklist" tabindex="-1" role="basic" aria-hidden="true">
...
function dtProduk(){
    var dt_table =  $('#table-produk').dataTable({
        ajax: { url: '<?= \yii\helpers\Url::toRoute(['/gudang/availablestockproduk/produklist','produk_id'=>'']) ?><?= $produk_id ?>&satuan_kecil=<?= $satuan_kecil ?>&gudang_nm=<?= $gudang_nm ?>',data:{dt: 'table-produk'} },
        order: [
            [0, 'desc']
        ],
        columnDefs: [
			...
			{	targets: 10,
				...
				render: function ( data, type, full, meta ) {
                    ...
                    return formatNumberFixed4(full[10]);
                }
			},
			{ 	targets: 11, class: "text-align-right", searchable: false, orderable: false,
                render: function ( data, type, full, meta ) {
					if (full[11] > 0) {
						var info = '<center><a class=\"btn btn-xs blue-hoki btn-outline tooltips\" href=\"javascript:void(0)\" onclick=\"info(\''+full[3]+'\')\"><i class="fa fa-info-circle"></i></a></center>';
					} else {
						var info = '';
					}
					return info;
