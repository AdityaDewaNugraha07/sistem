...
$this->title = 'Laporan Penerimaan Retur';
...
function dtLaporan(){
    var dt_table =  $('#table-laporan').dataTable({
		pageLength: 100,
        ajax: { 
			url: '<?= \yii\helpers\Url::toRoute('/gudang/laporan/penerimaanretur') ?>',
			data:{
				dt: 'table-laporan',
        ...
        columnDefs: [
			{ 	targets: 0, orderable: false, width: '5%', class: 'td-kecils',
                render: function ( data, type, full, meta ) {
					return '<center>'+(meta.row+1)+'</center>';
                }
            },
			{ 	targets: 1, class: 'td-kecils', 
                render: function ( data, type, full, meta ) {
			...
			{ 	targets: 2, class: 'td-kecils text-align-center',
				render: function ( data, type, full, meta ) {
					return full[3];
                }
			},
            { 	targets: 3, class: 'td-kecils text-align-center',
				render: function ( data, type, full, meta ) {
					return full[4];
                }
			},
            { 	targets: 4, class: 'td-kecils',
				render: function ( data, type, full, meta ) {
					return full[5];
                }
			},
            { 	targets: 5, class: 'td-kecils',
				render: function ( data, type, full, meta ) {
					return full[6];
                }
			},
            { 	targets: 6, class: 'td-kecils text-align-right',
				render: function ( data, type, full, meta ) {
					return formatNumberForUser(full[7]);
                }
			},
            { 	targets: 7, class: 'td-kecils text-align-right',
				render: function ( data, type, full, meta ) {
					return full[8];
                }
			},
            { 	targets: 8, class: 'td-kecils',
				render: function ( data, type, full, meta ) {
					return full[9];
                }
			},
            { 	targets: 9, class: 'td-kecils text-align-center',
				render: function ( data, type, full, meta ) {
					return full[10]?full[10]:'-';
                }
			},
            { 	targets: 10, class: 'td-kecils text-align-center',
				render: function ( data, type, full, meta ) {
					return full[11]?full[11]:'-';
                }
			},
            { 	targets: 11, class: 'td-kecils text-align-center',
				render: function ( data, type, full, meta ) {
					return full[12]?full[12]:'-';
                }
			},
            { 	targets: 12, class: 'td-kecils text-align-center',
				render: function ( data, type, full, meta ) {
					return full[13]?full[13]:'BELUM DITERIMA';
                }
			},
			{ 	targets: 13, class: 'td-kecils text-align-center',
				render: function ( data, type, full, meta ) {
					return full[14];
                }
			},
...
function printout(caraPrint){
	window.open("<?= yii\helpers\Url::toRoute('/gudang/laporan/penerimaanreturPrint') ?>?"+$('#form-search-laporan').serialize()+"&caraprint="+caraPrint,"",'location=_new, width=1200px, scrollbars=yes');
}
...
function setFilterByProdukGroup(callback=null){
	...
	$("#<?= yii\bootstrap\Html::getInputId($model, "grade") ?>").val('');
