...
$this->title = 'Laporan Permintaan Penanganan Barang Retur Sudah Dikirim Gudang Belum Diterima PPIC';
...
function dtLaporan(){
    var dt_table =  $('#table-laporan').dataTable({
		pageLength: 100,
        ajax: { 
			url: '<?= \yii\helpers\Url::toRoute('/gudang/laporan/permintaanreturblmditerima') ?>',
			data:{
				dt: 'table-laporan',
...
function printout(caraPrint){
	window.open("<?= yii\helpers\Url::toRoute('/gudang/laporan/permintaanreturblmditerimaPrint') ?>?"+$('#form-search-laporan').serialize()+"&caraprint="+caraPrint,"",'location=_new, width=1200px, scrollbars=yes');
}
