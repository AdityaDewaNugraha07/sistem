...
<!-- BEGIN PAGE TITLE-->
<h1 class="page-title"> <?php echo Yii::t('app', 'Rekap Stock Produk'); ?></h1>
<!-- END PAGE TITLE -->
<!-- END PAGE HEADER -->
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <div class="col-md-8">
                                                <?= $form->field($model, 'produk_group')->dropDownList(['Plywood'=>'Plywood','Lamineboard'=>'Lamineboard','Platform'=>'Platform','Sawntimber'=>'Sawntimber','Moulding'=>'Moulding','Veneer'=>'Veneer'],['prompt'=>''])->label("Produk"); ?>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-5">
...
<?php $this->registerJs(" 
    ...
    setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Rekap Stock Produk'))."');
...
function getRekap(){
    ...
    $.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/gudang/laporan/RekapStockProduk/']); ?>',
        type   : 'GET',
...
function printout(caraPrint){
	window.open("<?= yii\helpers\Url::toRoute('/gudang/laporan/RekapStockProdukPrint/') ?>?"+$('#form-search-laporan').serialize()+"&caraprint="+caraPrint,"",'location=_new, width=1200px, scrollbars=yes');
}

function changePertanggalLabel(){
	if($('#<?= yii\bootstrap\Html::getInputId($model, 'produk_group');?>').val()){
		$('#periode-label').html("Periode "+$('#<?= yii\bootstrap\Html::getInputId($model, 'produk_group');?>').val());
	}else{
		$('#periode-label').html("");
	}
}
