                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                ...
                                <span class="pull-right">
									<a class="btn dark btn-sm btn-outline" href="<?= yii\helpers\Url::toRoute("/gudang/mutasigudang/transaksiCepat") ?>"><i class="icon-speedometer"></i> <?= Yii::t('app', 'Transaksi Cepat'); ?></a>
                            ...
                            <div class="portlet-body">
                                <div class="row">
                                    <div class="col-md-5">
										...
										<?php }else{ ?>
											<div class="form-group" style="margin-bottom: 5px;">
												...
												<div class="col-md-7">
													...
													<span class="input-group-btn" style="width: 50%">
														<a class="btn btn-icon-only btn-default tooltips" onclick="availableProduk();" data-original-title="Cari Ketersedian Produk" style="margin-left: 3px; border-radius: 4px;"><i class="fa fa-list"></i></a>
										...
										<?= $form->field($model, 'produk_dimensi')->textInput(['disabled'=>'disabled'])->label("Dimensi"); ?>
									</div>
									<div class="col-md-6">
										...
										<?= $form->field($modProduk, 'produk_qty_satuan_kecil')->textInput(['readonly'=>'readonly','class'=>'form-control input-small'])->label("Qty Per Palet"); ?>
								...
								<div class="row">
                                    <div class="col-md-5">
										...
										if(!isset($_GET['mutasi_gudang_id'])){
											...
											<div class="form-group">
												<label class="col-md-5 control-label"><?= Yii::t('app', 'Kode Mutasi'); ?></label>
												<div class="col-md-7" style="padding-bottom: 5px;">
													<span class="input-group-btn" style="width: 90%">
										...
										<?php if(isset($_GET['mutasi_gudang_id'])){ ?>
											<div class="form-group">
												<label class="col-md-5 control-label"><?= Yii::t('app', 'Gudang Tujuan'); ?></label>
												<div class="col-md-7" style="padding-bottom: 5px;">
													<span class="input-group-btn">
														<?= \yii\bootstrap\Html::activeTextInput($model, 'gudang_tujuan_display', ['class'=>'form-control','style'=>'width:100%']) ?>
													</span>
												</div>
											</div>
										<?php }else{ ?>
											<?= $form->field($model, 'gudang_tujuan')->dropDownList( [] ,['class'=>'form-control select2','prompt'=>'']); ?>
										<?php } ?>
                                    </div>
                                    <div class="col-md-6">
...
$pagemode = "";
if(isset($_GET['mutasi_gudang_id'])){
    $pagemode = "afterSave(".$_GET['mutasi_gudang_id'].");";
}
?>
<?php $this->registerJs(" 
    $pagemode
	formconfig();
	$('select[name*=\"[gudang_id]\"]').select2({
		allowClear: !0,
	...
	$('select[name*=\"[persediaan_produk_id]\"]').select2({
		...
		ajax: {
			url: '".\yii\helpers\Url::toRoute('/gudang/penerimaanko/findAvailableProduk')."',
			dataType: 'json',
			delay: 250,
	...
	$('select[name*=\"[persediaan_produk_id]\"]').siblings('.select2-container').css('width','175px');
		...
		$.ajax({
			...
			success: function (data) {
				...
				if(data.model){
					...
					$('#<?= yii\bootstrap\Html::getInputId($model, "gudang_asal") ?>').val(data.terima.gudang_id);
					$('#<?= yii\bootstrap\Html::getInputId($model, "gudang_asal_display") ?>').val(data.gudang_asal_display);
					$('#<?= yii\bootstrap\Html::getInputId($modProduk, "produk_id") ?>').val(data.model.produk_id);
					$('#<?= yii\bootstrap\Html::getInputId($modProduk, "produk_satuan_besar") ?>').val(data.model.produk_satuan_besar);
					$('#<?= yii\bootstrap\Html::getInputId($modProduk, "produk_qty_satuan_besar") ?>').val("1");
					$('#<?= yii\bootstrap\Html::getInputId($modProduk, "produk_qty_satuan_kecil") ?>').val(data.terima.qty_kecil);
					$('#<?= yii\bootstrap\Html::getInputId($modProduk, "produk_satuan_kecil") ?>').val(data.terima.qty_kecil_satuan);
					$('#<?= yii\bootstrap\Html::getInputId($modProduk, "kapasitas_kubikasi") ?>').val(data.terima.qty_m3);
					
				}
				setDropdownGudangtujuan();
			},
...
function showTablePenerimaanKhusus(){
	if( $("input:radio[name*='[jenis_penerimaan]']:checked").val() == "Biasa" ){
		...
		<?php if(isset($_GET['mutasi_gudang_id'])){ ?>
			getItemsByPk("<?= $_GET['mutasi_gudang_id']; ?>");
		<?php } ?>
	}
}

function setDropdownProduk(){
	setDropdown('<?= \yii\helpers\Url::toRoute(['/func/setDropdownProduk']); ?>','<?= \yii\bootstrap\Html::getInputId($model, 'produk_id') ?>',null,'setProduk();');
}
...
function daftarAfterSave(){
    openModal('<?= \yii\helpers\Url::toRoute(['/gudang/mutasigudang/daftarAfterSave']) ?>','modal-aftersave','90%');
}
function availableProduk(){
	openModal('<?= \yii\helpers\Url::toRoute(['/gudang/availablestockproduk/produklist']); ?>','modal-produklist','90%');
}
...
function scan(){
	var url = '<?= \yii\helpers\Url::toRoute(['/gudang/mutasigudang/scanMutasi']) ?>';
...
function setDropdownGudangtujuan(){
    $('#<?= \yii\bootstrap\Html::getInputId($model, 'gudang_tujuan') ?>').addClass('animation-loading');
    var gudang_asal = $('#<?= \yii\bootstrap\Html::getInputId($model, 'gudang_asal') ?>').val();
	$.ajax({
		url    : '<?= \yii\helpers\Url::toRoute(['/gudang/mutasigudang/setDropdownGudangtujuan']); ?>',
		type   : 'POST',
		data   : {gudang_asal:gudang_asal},
		success: function (data) {
			$('#<?= \yii\bootstrap\Html::getInputId($model, 'gudang_tujuan') ?>').append(data.html).trigger('change');
            $('#<?= \yii\bootstrap\Html::getInputId($model, 'gudang_tujuan') ?>').removeClass('animation-loading');
		},
		error: function (jqXHR) { getdefaultajaxerrorresponse(jqXHR); },
	});
}
