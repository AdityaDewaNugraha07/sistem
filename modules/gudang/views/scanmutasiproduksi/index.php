								...
								<div class="row">
                                    <div class="col-md-12">
										<div class="table-scrollable">
											<table class="table table-striped table-bordered table-advance table-hover table-laporan" style="width: 90%; border: 1px solid #A0A5A9; display: none;" id="table-detail-infoproduk">
												<thead>
													<tr>
														...
														<th style="width: 70px; font-size: 1.3rem; line-height: 0.9; padding: 5px;"><?= Yii::t('app', 'Qty'); ?></th>
														<th style="width: 70px; font-size: 1.3rem; line-height: 0.9; padding: 5px;"><?= Yii::t('app', 'M<sup>3</sup>'); ?></th>
													</tr>
												</thead>
												<tbody>
...
<?php $this->registerJs(" 
	...
	reading();
	setMenuActive('".json_encode(app\models\MMenu::getMenuByCurrentURL('Scan Mutasi Produksi'))."');
	setScanner();
...
function getItemsScanned(id){
    $.ajax({
		...
		success: function (data) {
			...
			if(data.html2){
				$("#table-detail-infoproduk").css('display', '');
				$('#table-detail-infoproduk tbody').html(data.html2);
			}
...
function pick(nomor_produksi){
	...
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/gudang/scanmutasiproduksi/showDetail']); ?>',
        type   : 'POST',
        data   : {pengajuan_repacking_id:pengajuan_repacking_id, nomor_produksi:nomor_produksi},
        success: function (data) {            
			if(data){
                if (data['msg'] == "Data ok") {
                    openModal('<?= \yii\helpers\Url::toRoute(['/gudang/scanmutasiproduksi/review','pengajuan_repacking_id'=>'']) ?>'+pengajuan_repacking_id+'&nomor_produksi='+nomor_produksi,'modal-review','300px');
                } else {
                    //openModal('<?= \yii\helpers\Url::toRoute(['/gudang/scanmutasiproduksi/view','nomor_produksi'=>'']) ?>'+datas+'&gudang_id='+gudang_id,'modal-review','300px');
                    cisAlert(data['msg']);
                }
            } else {
                cisAlert('y');
