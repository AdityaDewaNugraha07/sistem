...
<table style="width: 13.5cm; margin: 10px; height: 9cm;" border="1">
	...
	<tr>
		...
		<td style="width: 2.5cm; vertical-align: middle; padding: 3px;">: &nbsp; 
			<?= preg_replace('/(FSC100)/i', '<strong>$1</strong>', $model->produk->grade) ?>
		</td>
	</tr>
	...
	<?php }else if($model->produk->produk_group == "Veneer" && $model->produk->jenis_kayu != ""){ ?>
		<tr style="">
			<td colspan="2" style="height: 10px; line-height: 1px; border-right: solid 1px transparent; border-top: solid 1px #000;"> </td>
		</tr>
		<tr>
			...
			<td style="vertical-align: middle; padding: 3px;">: &nbsp; <?= $model->produk->jenis_kayu; ?></td>
			<td style="vertical-align: middle; padding: 3px 0px 3px 10px; border-right: solid 1px transparent;" ><b>Condition</td>
			<td style="vertical-align: middle; padding: 3px;">: &nbsp; <b><?= $model->produk->warna_kayu; ?></b></td>
		</tr>
	<?php } ?>
	<tr>
		...
		<td colspan="2" rowspan="5" style="width: 4.5cm; vertical-align: top; padding: 20px 10px 10px; border-bottom: solid 1px transparent; text-align: center;">
            ...
            <?php if($model->produk->jenis_kayu == ""){ ?>
				<br><span style="font-size: 1.5rem;" class='text-bold'><?= $model->produk->warna_kayu; ?></span>
			<?php }	?>
            <!-- <br> -->
            <!-- <span style="font-size: 1.2rem;"><?= preg_match('/FSC\s?100%/i', $model->produk->grade) ? '<b>FSC 100%</b>' : '' ?></span> -->
			<!-- <span style="font-size: 1.5rem;" class='text-bold'><?= $model->produk->warna_kayu; ?></span> -->
		</td>
	</tr>
	<tr>
                        ...
                        <table style="width: 100%; height: 100%;">
                            <tr>
                                ...
                                <td style="width: 33%; text-align: center; vertical-align: middle; font-size: 3rem"><b>
                                    ...
                                    $jenis_produk = $model->produk->produk_group;
                                    $grade = $model->produk->grade;
                                    if(!empty($modHasilProduksi)){
                                        if ($jenis_produk == "Plywood" && $grade == "D") {
                                            echo "<span class='text-left'>P</span>";
                                            echo "<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>";
                                            echo "<span class='text-right'>L</span>";
                                        } else {
                                            echo "P";
                                        }
                                    }else if(!empty($modHasilRepacking)){
										if(strpos($modHasilRepacking->hasil_dari, "Penanganan Barang Retur") !== false){
											echo "RTR";
										} else {
											echo "R";
										}
