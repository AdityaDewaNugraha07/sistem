...
function pick(datas){
	...
	var gudang_id = $('#<?= \yii\bootstrap\Html::getInputId($model, "gudang_id") ?>').val();
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/gudang/penerimaanko/showDetail']); ?>',
        type   : 'POST',
        ...
        success: function (data) {            
			if(data){
                if (data['msg'] == "Data ok") {
                    ...
                    openModal('<?= \yii\helpers\Url::toRoute(['/gudang/penerimaanko/review','nomor_produksi'=>'']) ?>'+datas+'&gudang_id='+gudang_id,'modal-review','300px');
                } else {
                    //var pengajuan_repacking_id = data['pengajuan_repacking_id'];
