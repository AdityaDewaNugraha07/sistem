...
function getItemsScanned(){
    var dt_table =  $('#table-master').dataTable({
        ajax: { url: '<?= yii\helpers\Url::toRoute("/gudang/penerimaanko/getItemsScannedRepacking") ?>',data:{dt: 'table-master'} },
//		order: [
//            [1, 'desc'],
//        ],
        "pageLength": 10,
...
function pick(datas){
	...
	var gudang_id = $('#<?= \yii\bootstrap\Html::getInputId($model, "gudang_id") ?>').val();
	$.ajax({
        url    : '<?= \yii\helpers\Url::toRoute(['/gudang/penerimaanko/showDetailRepacking']); ?>',
        type   : 'POST',
        data   : {gudang_id:gudang_id, datas:datas},
        success: function (data) {            
			if(data){
                if (data['msg'] == "Data ok") {
                    var pengajuan_repacking_id = data['pengajuan_repacking_id'];
                    openModal('<?= \yii\helpers\Url::toRoute(['/gudang/penerimaanko/reviewRepacking','nomor_produksi'=>'']) ?>'+datas+'&gudang_id='+gudang_id,'modal-review','300px');
                } else {
                    //var pengajuan_repacking_id = data['pengajuan_repacking_id'];
                    //openModal('<?= \yii\helpers\Url::toRoute(['/gudang/penerimaanko/view','nomor_produksi'=>'']) ?>'+datas+'&gudang_id='+gudang_id,'modal-review','300px');
                    cisAlert(data['msg']);
                }
            }
        },
