		...
		<td colspan="3" style="padding: 0px;">
			<table style="width: 100%" id="table-detail">
				<tr style="border-bottom: solid 1px #000; border-top: solid 1px #000;">
					<th style="padding: 10px; border: solid 1px #000;"><?= Yii::t('app', 'Gudang Kode') ?></th>
					<th style="padding: 10px; border: solid 1px #000;"><?= Yii::t('app', 'Gudang Nama') ?></th>
					<th style="padding: 10px; border: solid 1px #000;"><?= Yii::t('app', 'Gudang Keterangan') ?></th>
					<th style="padding: 10px; border: solid 1px #000;"><?= Yii::t('app', 'Active') ?></th>
				</tr>
				
				<?php 
				foreach ($model as $key) {
					$gudang_kode = $key['gudang_kode'];
					$gudang_nm = $key['gudang_nm'];
					$gudang_ket = $key['gudang_ket'];
					$active = $key['active'];
					$active == 1 ? $status = "Active" : $status = "Non-Active";
				?>
				<tr>
					<td style="padding: 3px; border: solid 1px #000;"><?= $gudang_kode; ?></td>
					<td style="padding: 3px; border: solid 1px #000;"><?= $gudang_nm; ?></td>
					<td style="padding: 3px; border: solid 1px #000;"><?= $gudang_ket; ?></td>
