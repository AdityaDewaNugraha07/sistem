        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
						...
						<div class="form-group col-md-12">
                            ...
                            <div class="col-md-7" style="margin-top: 5px;">
								<strong>
									<?php
									$modNota = \app\models\TNotaPenjualan::findOne($model->nota_penjualan_id);
									echo $modNota->jenis_produk;
									?>
								</strong>
							</div>
                        </div>
					...
					<div class="col-md-6">
						<?php // echo $form->field($model, 'waktu_terima',[
...
//											{error}</div>'])->textInput(['readonly'=>'readonly']); ?>
						<?= $form->field($model, 'waktu_terima')->textInput(['disabled'=>'disabled']); ?>
						<?= $form->field($model, 'kendaraan_nopol')->textInput(); ?>
						<?= $form->field($model, 'kendaraan_supir')->textInput(); ?>
						<?= $form->field($model, 'petugas_penerima')->dropDownList(\app\models\MPegawai::getOptionListMarketing(),['class'=>'form-control select2','prompt'=>''])->label('Diterima Oleh'); ?>
						<?= $form->field($model, 'diperiksa_security')->dropDownList(\app\models\MPegawai::getOptionListCheckerSecurityOnly(),['class'=>'form-control select2','prompt'=>''])->label('Diperiksa Security'); ?>
						<?= $form->field($model, 'gudang_id')->dropDownList(\app\models\MGudang::getOptionList(),['class'=>'form-control select2','prompt'=>''])->label('Gudang Penerimaan'); ?>
					</div>
                </div>
				<br>
				<div class="row">
            ...
            <div class="modal-footer text-align-center">
				<?php echo \yii\helpers\Html::button( Yii::t('app', 'Terima'),['class'=>'btn hijau btn-outline ciptana-spin-btn',
                    'onclick'=>'save(this)'
                    ]); //"$(\'#close-btn-modal\').removeAttr(\'disabled\'); $(\'#close-btn-modal\').trigger(\'click\'); $(\'#table-laporan\').dataTable().fnClearTable();"
				?>
            </div>
...
function save(){
	...
	$("#<?= \yii\bootstrap\Html::getInputId($model, "gudang_id") ?>").parents(".form-group").removeClass("has-error");
    if(formrequiredvalidate($form)){ 
		...
		if(validatingDetail()){
            submitformajax($form, "$(\'#modal-transaksi\').modal(\'hide\'); $(\'#table-laporan\').dataTable().fnClearTable();");
        }
    }
    return false;
}
...
function validatingDetail($form){
	...
	if(!gudang_id){
		$("#<?= \yii\bootstrap\Html::getInputId($model, "gudang_id") ?>").parents(".form-group").addClass("has-error");
		$("#<?= \yii\bootstrap\Html::getInputId($model, "gudang_id") ?>").parents(".form-group").removeClass("has-success");
		has_error = has_error + 1;
	}
	if(has_error === 0){
        return true;
