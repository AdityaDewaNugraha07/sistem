...
class PenerimaankoController extends DeltaBaseController
{
	...
	function actionGetProduk(){
		if(\Yii::$app->request->isAjax){
			...
			if(!empty($persediaan_produk_id)){
                ...
                $modPenerimaan = \app\models\TTerimaKo::findOne(['nomor_produksi'=>$modPersediaan->nomor_produksi]);
    ...
    public function actionPengajuancetakulang(){
		if(\Yii::$app->request->isAjax){
            ...
            $model->approver1 = \app\components\Params::DEFAULT_PEGAWAI_ID_ILHAM;
	...
	public function actionFindAvailableProduk(){
		if(\Yii::$app->request->isAjax){
			...
			if(!empty($term)){
				...
				// 	WHERE h_persediaan_produk.nomor_produksi ilike '%{$term}%'
				// 	GROUP BY  1,2            
                ...
                //         $sql = "select persediaan_produk_id from h_persediaan_produk where nomor_produksi = '".$val['nomor_produksi']."' and produk_id = '".$val['produk_id']."' ";
                //         $modPersediaan = Yii::$app->db->createCommand($sql)->queryOne();
				// 		$data[] = ['id'=>$modPersediaan['persediaan_produk_id'], 'text'=>$val['nomor_produksi']];
				// 	}
				// }
                $data[] = ['id'=>'kosong', 'text'=>'Fitur Pencarian ini tidak dapat digunakan, silahkan menggunakan fitur yang lain (gunakan fitur disamping kanan kolom ini)'];
			}
            return $this->asJson($data);
        }
	}
    ...
    public function actionRiwayatpenerimaanPrint(){
		...
		$paramprint['judul'] = Yii::t('app', 'Laporan Riwayat Penerimaan Kayu Olahan');
    ...
    function actionGetItemsScanned(){
		if(\Yii::$app->request->isAjax){
            if(\Yii::$app->request->get('dt')=='table-master'){
                ...
                $param['where'] = "t_terima_ko.tanggal::date >= CURRENT_DATE - INTERVAL '2 days' AND t_terima_ko.tanggal::date < CURRENT_DATE + INTERVAL '1 day'";
    ...
    public function actionReview(){
        if(\Yii::$app->request->isAjax){
            ...
            $modBrgProduk = \app\models\MBrgProduk::findOne(['produk_id'=>$modHasilProduksi->produk_id]);
            $modKirimGudangDetail = \app\models\TKirimGudangDetail::findOne(['nomor_produksi'=>$nomor_produksi]);
            return $this->renderAjax('_review',['gudang_id'=>$gudang_id, 'modHasilProduksi'=>$modHasilProduksi, 'modBrgProduk'=>$modBrgProduk, 'modKirimGudangDetail'=>$modKirimGudangDetail]);
        }
    }
    ...
    public function actionShowDetail(){
		if(\Yii::$app->request->isAjax){
            ...
            $data['msg'] = "";
			$data['status'] = false;
			$nomor_produksi = \Yii::$app->request->post('datas');
            ...
            if(!empty($modHasilProduksi)){
                ...
                $data['status'] = false;
                $data['msg'] = "Produk tidak ditemukan! ".$nomor_produksi;
            }
		}
        return $this->asJson($data);
	}
    ...
    public function actionRejectReasonNomorProduksi() {
        $id = $_GET['id'];
        $modKirimGudangDetail = \app\models\TKirimGudangDetail::findOne(['kirim_gudang_detail_id' => $id]);
        return $this->renderAjax('_rejectReason', ['modKirimGudangDetail'=>$modKirimGudangDetail]);
    }

    public function actionSimpanlahSusahAmat() {
		...
		if(\Yii::$app->request->isAjax){
			if( Yii::$app->request->post('kirim_gudang_detail_id') != "" && Yii::$app->request->post('reject_reason') != "" ){
				$transaction = \Yii::$app->db->beginTransaction();
				try {
					$success_1 = false;
					$success_2 = false;
                    
                    $by = Yii::$app->user->identity->pegawai_id;

                    $jfson = ['status'=>"REJECTED",
                                'by'=> $by,
                                'at'=>date('Y-m-d H:i:s'),
                                'reason'=>$_POST['reject_reason']
                                ];
                    $reject_reason = \yii\helpers\Json::encode($jfson);
                    $sqlUpdate =  "update t_kirim_gudang_detail
                                    set status = 'REJECTED'
                                    , reject_reason = '[".$reject_reason."]'
                                    where kirim_gudang_detail_id = ".$kirim_gudang_detail_id." ";
                    $success_1 = Yii::$app->db->createCommand($sqlUpdate)->execute();

                    $modelApproval = new \app\models\TApproval();
                    $modelApproval->assigned_to = 4410;
                    $modelApproval->reff_no = "RPKO".$modKirimGudangDetail->nomor_produksi;
                    $modelApproval->tanggal_berkas = date("Y-m-d");
                    $modelApproval->level = 1;
                    $modelApproval->status = \app\models\TApproval::STATUS_NOT_CONFIRMATED;
                    $modelApproval->active = true;
                    $modelApproval->created_at = date("Y-m-d H:i:s");
                    $modelApproval->created_by = $by;
                    $modelApproval->updated_at = date("Y-m-d H:i:s");
                    $modelApproval->updated_by = $by;
    ...
    public function actionReviewRepacking(){
        if(\Yii::$app->request->isAjax){
            $gudang_id = $_GET['gudang_id'];
            $nomor_produksi = $_GET['nomor_produksi'];
            ...
            return $this->renderAjax('_reviewRepacking',['gudang_id'=>$gudang_id, 
                                                            'modBrgProduk'=>$modBrgProduk,
                                                            'modProduksi'=>$modProduksi, 
                                                            'modHasilRepacking'=>$modHasilRepacking]);
        }
    }

    public function actionShowDetailRepacking(){
		if(\Yii::$app->request->isAjax){
            $data = [];
            $data['msg'] = "";
			$data['status'] = false;
			...
			$gudang_id = \Yii::$app->request->post('gudang_id');
            $modProduksi = \app\models\TProduksi::findOne(['nomor_produksi'=>$nomor_produksi]);
            $modHasilRepacking = \app\models\THasilRepacking::findOne(['produk_id'=>$modProduksi->produk_id]);
            $modTerima = \app\models\TTerimaKo::findOne(['nomor_produksi'=>$nomor_produksi]);
		...
		if(\Yii::$app->request->isAjax){
            ...
            try {
                        ...
                        if(empty($modTerima)){
                            ...
                            if($model->validate()){
                                if($model->save()){
                                    ...
                                    $modPersediaan = new \app\models\HPersediaanProduk();
                                    $modPersediaan->attributes = $model->attributes;
                                    $modPersediaan->nomor_produksi = $nomor_produksi;
                                    $modPersediaan->tgl_transaksi = $model->tanggal;
                                    $modPersediaan->reff_no = $model->kode;
                                    $modPersediaan->in_qty_palet = $model->qty_palet;
                                    $modPersediaan->in_qty_kecil = $model->qty_kecil;
                ...
                if ($success_1 && $success_2 && $success_3) {
                    ...
                    Yii::$app->session->setFlash('error', 'Data gagal disimpan! <br>suckses1 = '.$success_1."<br>suckses2 = ".$success_2."<br>suckses3 = ".$success_3);
                }
            } catch (\yii\db\Exception $ex) {
                $transaction->rollback();
                $data['message'] = $ex;
