		...
		if(\Yii::$app->request->isAjax){
			if(\Yii::$app->request->get('dt')=='modal-aftersave'){
				...
				$param['join']= ['JOIN m_gudang ON m_gudang.gudang_id = '.$param['table'].'.gudang_tujuan 
								  ...
								  JOIN t_terima_ko ON t_terima_ko.nomor_produksi = '.$param['table'].'.nomor_produksi'];
			...
			if($cek['palet'] > 0){
				if(!empty($modTerima)){
					...
					if(!empty($modPersediaan)){
						$modProdukKeluar = \app\models\TProdukKeluar::findOne(['nomor_produksi'=>$modPersediaan->nomor_produksi]);
						if(empty($modProdukKeluar)){
							$data = $modPersediaan->attributes;
						} else {
							$data['msg'] = "Palet sedang dalam proses dikeluarkan";
						}
					}
				}
			}else{
