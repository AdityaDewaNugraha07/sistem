...
/**
 * This is the model class for table "m_profil_kayu".
 *
 * @property integer $profil_kayu_id
 ...
 */
class MProfilKayu extends \app\models\DeltaBaseActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    {
        return [
                'profil_kayu_id' => Yii::t('app', 'Profil Kayu'),
	...
	public static function getOptionListNama($jenis_produk=null)
    {
		if(!empty($jenis_produk)){
			$res = self::find()->where(['active'=>true,'jenis_produk'=>$jenis_produk])->orderBy('profil_kayu_id ASC')->all();
		}else{
			$res = self::find()->where(['active'=>true])->orderBy('profil_kayu_id ASC')->all();
		}
