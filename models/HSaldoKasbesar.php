...
/**
 * This is the model class for table "h_saldo_kasbesar".
 *
 * @property integer $saldo_kasbesar_id
 ...
 */
class HSaldoKasbesar extends \app\models\DeltaBaseActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public function rules()
    {
        return [
            ...
            [['kode', 'status', 'reff_no'], 'string', 'max' => 50],
        ];
    }
    ...
    {
        return [
                'saldo_kasbesar_id' => Yii::t('app', 'Saldo Kasbesar'),
                'kode' => Yii::t('app', 'Kode'),
	...
	public static function getSaldoAkhir($tgl=null) {
		if(!empty($tgl)){
			...
			$sql = "SELECT sum(debit)-sum(kredit) AS saldo FROM h_saldo_kasbesar WHERE tanggal <= '".$tgl."'" ;
		}else{
			$sql = "SELECT sum(debit)-sum(kredit) AS saldo FROM h_saldo_kasbesar";
		}
		$mod = \Yii::$app->db->createCommand($sql)->queryOne();
		return ($mod['saldo'])?$mod['saldo']:0;
	}
}
